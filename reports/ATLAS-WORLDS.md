...
- **`readnum`** (3240 строк; recompute, whole_only)  
  **THE UNREAD NUMBER** — the four places a number hides (05.09, the largest gate of the silence atlas: numbers-unread, 179 rows of 415 refusals).
- **`personforms`** (3240 строк; recompute, whole_only)  
...
- **`machine`** (2232 строк; recompute, whole_only, unsigned_only)  
...
- **`nomention`** (2160 строк; recompute, whole_only)  
  **WHAT WAS NEVER SAID** — the fourth gate of the silence atlas (06.09, d5's measure: unknown-entity, the question names a thing the story never carried).
- **`selfmodelforms`** (2097 строк; recompute, whole_only)  
...
- **`share`** (996 строк; recompute, whole_only)  
  shares said in WORDS: «two thirds», «три четверти», the share of a quantity with its ground («12 divided by 3 is 4, and 4 times 2 is 8»), the share in comparison («half as many»), mixed numbers («one and a half»), and shares that add to one whole; only whol…
- **`rem`** (966 строк; recompute, whole_only)  
  **DIVISION WITH A REMAINDER** — THE SUM ADDS UP AND THE DIVISION IS NOT FINISHED. Measured 11.09 across the 237 show worlds: «остаток меньше делителя», «remainder is less than» — NONE; «dividend = divisor × quotient + remainder» — NONE. Yet the corpus does …
...
- **`translate`** (792 строк; recompute, whole_only)  
  **TRANSLATION BETWEEN THE NINE LANGUAGES** — «как будет «кот» по-английски? cat.», «how do you say thank you in french? merci.», each language into each.
...
- **`lever`** (726 строк; recompute, whole_only)  
  **THE LEVER** — two DIFFERENT products that are equal, and inverse proportion as a computation.
- **`pronouns`** (720 строк; recompute, exact_only)  
  **the pronoun genus (e9's order 03.09, SVAMP band: the pronoun market buys a word as a pronoun from pages of three sentences «X V1 N. <pron> V2 M. X V3 K» with K = N ± M, and without this genus it bought names standing second): pages of three sentences in o…
- **`mapscale`** (720 строк; recompute, whole_only)  
  **THE MAP SCALE** — THE GREATER THE NUMBER IN THE SCALE, THE SMALLER THE MAP. Measured 11.09 across the 245 show worlds: «масштаб карты», «scale of the map», the writing «1 : 1000» — NONE. The word «масштаб» occurs 43 times and all of it in PROSE: in Dahl (…
- **`cmpshare`** (706 строк; recompute, whole_only)  
...
- **`signadd`** (576 строк; recompute, whole_only)  
  **SIGNED ADDITION** — TO SUBTRACT A NEGATIVE IS TO ADD. Measured 11.09 across the 234 show worlds: «(−a) + (−b)» — NOT ONE occurrence.
- **`reduce`** (570 строк; recompute, whole_only)  
  **REDUCING A FRACTION** — a common factor leaves both parts, and the fraction stays the same.
- **`point`** (564 строк; recompute, whole_only)  
  **THE POINT ON A PLANE** — A PAIR OF NUMBERS IN WHICH THE ORDER MEANS SOMETHING. Measured 11.09 across the 248 show worlds: «точка (3, 2)», «point (3, 2)», «right and up» — NONE. The writing «(a, b)» occurs 4141 times, and all of it in the worlds of vectors…
- **`share_langs`** (560 строк; recompute, unsigned_only)  
  **SHARES IN EIGHT LANGUAGES (the owner's word: every language in surplus; the share world says «two thirds of 12 is 8» in en/ru): the half, a third, two thirds, a quarter, three quarters, a fifth, two fifths of a number on de/fr/es/it/pt/nl/pl/tr, statement…
...
- **`verbbridge`** (480 строк; same_object, whole_only)  
  **THE BRIDGE OF THE ASKED FORM** — a verb asked in its OWN interrogative form (07.09).
- **`stats_langs`** (480 строк; recompute, unsigned_only)  
  **THE MEAN AND THE PERCENT IN EIGHT LANGUAGES (the owner's word: every language in surplus; the average and percent worlds say them in en/ru): statement and question answered by the statement (М-153) with a ledger the court recomputes** — «der Durchschnitt …
- **`physics_langs`** (480 строк; recompute, unsigned_only)  
  **SPEED AND PRESSURE IN EIGHT LANGUAGES (the owner's word: every language in surplus; the physics worlds say speed = distance ÷ time and pressure = force ÷ area in en/ru): statement and question answered by the statement (М-153) with the unit names in their…
- **`pctchange`** (480 строк; recompute, whole_only)  
  **CHANGE BY A PERCENT** — THE PERCENT IS TAKEN FROM WHAT STANDS NOW. Measured 11.09 across the 243 show worlds: «increased by N %» — NONE; «decreased by N %» — NONE; «by what percent» — NONE. The world of percent shows ONLY «N % of a number» (120 lines), an…
- **`dateforms`** (480 строк; recompute, whole_only, exact_only)  
  **THE DATE** — the carry whose base is NOT the same twice (06.09).
- **`compare_langs`** (480 строк; recompute, unsigned_only)  
...
- **`remainders`** (328 строк; recompute, whole_only)  
...
- **`estim`** (228 строк; recompute, whole_only)  
  **THE ESTIMATE** — round BEFORE counting, and catch the gross error by it.
- **`numphrase`** (225 строк; recompute, whole_only)  
...
- **`eng_proof`** (204 строк; recompute, whole_only, unsigned_only)  
...
- **`prog`** (202 строк; recompute, whole_only, unsigned_only)  
...
- **`dyck`** (162 строк; recompute, whole_only, unsigned_only)  
  **THE ROW THAT MUST BE CLOSED (14.09, rung two of the ladder, asked for by holon-f9 for BBH's `dyck_languages`). The corpus showed brackets only as a SIGN INSIDE A FORMULA; a row that must be CLOSED it never showed once. A BRACKET IN A FORMULA IS A SIGN BES…
- **`bound`** (144 строк; recompute, whole_only)  
  **THE BOUND OF A MAGNITUDE** — not every valid subtraction is a possible action.
- **`boolexpr`** (136 строк; recompute, whole_only)  
...
- **`deacc`** (280 строк; declared, declared_table_only)  
  **НЕМЕЦКИЙ ВИНИТЕЛЬНЫЙ ПРИ ГЛАГОЛЕ** — СВОД ОТВЕРГАЛ «ich sehe der Zettel» И 923 СТРОКАМИ НЕ СКАЗАЛ, ПОЧЕМУ. Замер 13.09 ПО ПЛАСТУ НЕМЕЦКОГО (8 274 строки), и шёл он мерой, а не глазом: для всякого необъяснённого отказа с причиной «agreement» считались стро…
- **`romgender`** (262 строк; declared, declared_table_only)  
  **РОМАНСКИЙ РОД** — ЧЕТЫРЕ ПЛАСТА ОТВЕРГАЮТ АРТИКЛЬ ЧУЖОГО РОДА И НИ ОДИН НЕ ГОВОРИТ, ЧТО РОД СКАЗАН АРТИКЛЕМ. Замер 13.09 мерой, а не глазом: для всякого необъяснённого отказа с причиной «agreement» сочтены родственные его замене строки, и четыре романских…
...
- **`huobj`** (260 строк; declared, declared_table_only)  
  **ВЕНГЕРСКОЕ ОБЪЕКТНОЕ СПРЯЖЕНИЕ** — СВОД ОТВЕРГАЛ «én látok azt» И НЕ СКАЗАЛ, ЧТО ГЛАГОЛ ПОМНИТ ПРЕДМЕТ. Замер 13.09 ПО ПЛАСТУ ВЕНГЕРСКОГО (2 240 строк): пар «неопределённое / определённое» 48 (восемь глаголов на шесть лиц), по 10 строк каждая — «látok val…
- **`viclass`** (250 строк; declared, declared_table_only)  
  **ВЬЕТНАМСКОЕ СЧЁТНОЕ** — СВОД ОТВЕРГАЛ «hai con bàn» И НЕ СКАЗАЛ ПОЧЕМУ. Замер 13.09 ПО ПЛАСТУ ВЬЕТНАМСКОГО (2 935 строк): пар «счётное + имя» 1 116 при двадцати различных (con mèo 71, cái bàn 67, tờ báo 66, cuốn sách 65, chiếc xe 60…); отказов «¬ ba quả m…
- **`jaclass`** (222 строк; declared, declared_table_only)  
  **ЯПОНСКОЕ СЧЁТНОЕ** — СВОД ОТВЕРГАЛ «三台の本» И НЕ СКАЗАЛ, ЧТО 本 СЛУЖИТ В ДВУХ РОЛЯХ. Замер 13.09 ПО ПЛАСТУ ЯПОНСКОГО (2 890 строк): сочетаний «число + счётное» 366; 冊 при 本 (книга) 206 строк, 枚 при 紙 (бумага) 11, 台 при 車 (машина) 11, 本 при ペン (ручка), 傘 (зон…
- **`zhclass`** (210 строк; declared, declared_table_only)  
...
- **`deplur`** (156 строк; declared, declared_table_only)  
  **НЕМЕЦКОЕ МНОЖЕСТВЕННОЕ** — СВОД ПИСАЛ «Bücher» И «Stifte» ТЫСЯЧАМИ СТРОК И НИ РАЗУ НЕ СКАЗАЛ, ЧТО ОДНОГО ПРАВИЛА У НЕГО НЕТ. Замер 12.09 ПО СВОДУ, ПОРОЗНЬ: «Bücher» 1 567 строк, «Stifte» 820, «Blumen» 802, «Münzen» 791, «Tassen» 762, «Eier» 740, «Bälle» 7…
- **`trharm`** (154 строк; declared, declared_table_only)  
  **ТУРЕЦКАЯ ГАРМОНИЯ** — СВОД ПИСАЛ «-lar» И «-ler» ВОСЕМЬСОТ РАЗ И НЕ СКАЗАЛ, ЧЕМ ОНИ РАЗНЯТСЯ. Замер 12.09 ПО ПЛАСТУ ТУРЕЦКОГО (3 425 строк): «-lar» 563 вхождения, «-ler» 302 — ОДИН И ТОТ ЖЕ суффикс в двух обличьях; сочетаний «число + имя» 2 311, из них с …
- **`koparticle`** (150 строк; declared, declared_table_only)  
  **КОРЕЙСКАЯ ЧАСТИЦА** — СВОД ОТВЕРГАЛ «책가» И НЕ СКАЗАЛ, ЧТО РЕШАЕТ ПОСЛЕДНИЙ ЗВУК. Замер 13.09 ПО ПЛАСТУ КОРЕЙСКОГО (2 215 строк): частица 은 стои́т 540 раз, 는 610, 이 98, 가 46, 을 41, 를 44; отказы «¬ 책가 있어요», «¬ 나무이 있어요», «¬ 저는 책를 봐요» — есть; строк, объясняющ…
- **`fipart`** (150 строк; declared, declared_table_only)  
  **ФИНСКИЙ ПАРТИТИВ** — СВОД ОТВЕРГАЛ «kaksi talo» И НЕ СКАЗАЛ, ЧТО ПОСЛЕ ЧИСЛА СТОИ́Т ЕДИНСТВЕННОЕ. Замер 13.09 ПО ПЛАСТУ ФИНСКОГО (2 995 строк): talo 40 / taloa 38 / talot 24; kukka 38 / kukkaa 34 / kukat 24; kirja 39 / kirjaa 34 / kirjat 24; kauppa 33 / k…
...
- **`serestar`** (124 строк; declared, declared_table_only)  
...
- **`angle`** (122 строк; recompute, whole_only, exact_only)  
  **СУММА УГЛОВ** — СТО ВОСЕМЬДЕСЯТ У ВСЯКОГО ТРЕУГОЛЬНИКА, КАКОВ БЫ ОН НИ БЫЛ. Замер 12.09 по всему своду в 39 МБ: «сумма углов» — НОЛЬ вхождений; свод знает слово «треугольник» (452 строки) и «прямой угол» (9), считает площади и периметры — и ни разу не гов…
- **`tzone`** (120 строк; recompute, whole_only, exact_only)  
  **ЧАСОВОЙ ПОЯС** — ЧАС ЕСТЬ СВОЙСТВО МЕСТА, А НЕ МГНОВЕНИЯ. Замер 12.09 по всему своду в 39 МБ: «часовой пояс» — НОЛЬ вхождений; свод знает часы и сложение по кругу двадцати четырёх — и ни разу не говорит, что одно и то же мгновение зовётся в разных местах …
...
- **`soundarticle`** (116 строк; declared, declared_table_only)  
...
- **`decase`** (110 строк; declared, declared_table_only)  
  **НЕМЕЦКИЙ ПРЕДЛОГ ДВУХ ПАДЕЖЕЙ** — СВОД ПИСАЛ «im» 4 775 РАЗ И НЕ СКАЗАЛ, ЧТО ЭТО ВЫБОР. Замер 12.09 ПО СВОДУ, ПОРОЗНЬ: «im» (= in dem, дательный) 4 775 строк, «am» (= an dem) 1 721, «auf dem / auf das» 1 000, «ins» (= in das, винительный) 28, «ans» 13; ст…
- **`overlap`** (106 строк; recompute, whole_only, exact_only)  
...
- **`pctbase`** (104 строк; recompute, whole_only, exact_only)  
...
- **`homo`** (102 строк; recompute, whole_only, exact_only)  
...
- **`cond`** (102 строк; recompute, whole_only, exact_only)  
  **УСЛОВНАЯ ВЕРОЯТНОСТЬ** — ЗНАНИЕ СУЖАЕТ МНОЖЕСТВО ИСХОДОВ, А НЕ МЕНЯЕТ ПРЕДМЕТА. Замер 12.09 по всему своду: «условная вероятность» — НОЛЬ вхождений; свод считает вероятность 446 строками, знает независимость во времени («монета не помнит») и знает, что со…
...
- **`elision`** (96 строк; declared, declared_table_only)  
...
- **`numline`** (94 строк; recompute, whole_only, exact_only)  
...
- **`direct`** (90 строк; recompute, whole_only, exact_only)  
  **ПРЯМАЯ РЕЧЬ** — ЗНАК ПРИ НЕЙ СТАВИТСЯ ПРАВИЛОМ, А НЕ НА СЛУХ. Замер 12.09 по всему своду в 39 МБ: «прямая речь» — НОЛЬ вхождений, «слова автора» — ноль, «— сказал» — ноль; свод несёт мир диалога и мир сцены, где люди говорят, — но ни разу не показывает, П…
- **`leap`** (86 строк; recompute, whole_only, exact_only)  
  **ВИСОКОСНЫЙ ГОД** — ПРАВИЛО, ВНУТРИ КОТОРОГО ДВА ИСКЛЮЧЕНИЯ, И ВТОРОЕ ОТМЕНЯЕТ ПЕРВОЕ. Замер 12.09 по всему своду в 39 МБ: «високосн» — НОЛЬ вхождений; свод знает названия месяцев и их порядок, знает перенос через месяц и считает дни единицей — и ни разу н…
...
- **`swclass`** (80 строк; declared, declared_table_only)  
  **КЛАСС В СУАХИЛИ** — СВОД ОТВЕРГАЛ «kitabu mmoja» ПЯТНАДЦАТЬЮ СТРОКАМИ И НЕ СКАЗАЛ ПОЧЕМУ. Замер 12.09 ПО ПЛАСТУ СУАХИЛИ (5 820 строк): «ninaona kitabu kimoja» и подобных — сотни строк; отказов «¬ ninaona kitabu mmoja», «¬ ninaona mtu kimoja», «¬ ninaona v…
...
- **`chancetrap`** (68 строк; recompute, whole_only, exact_only)  
...
- **`fence`** (66 строк; recompute, whole_only, exact_only)  
...
- **`lang_fi`** (2995 строк; declared, tier1_unverified, whole_only)  
  finnish: FIFTEEN CASES said by ENDING, not by preposition, and CONSONANT GRADATION that changes the STEM (kukka → kukan) rather than the suffix (tier 1: structure green, forms agree with the pack's own rules)
...
- **`logic_sets`** (296 строк; recompute, finite_explicit_only)  
...
- **`speech`** (1934 строк; declared, recompute, whole_only)  
...
- **`formulas`** (1162 строк; recompute, whole_only)  
...
- **`dataformat`** (800 строк; same_object, flat_only)  
...
- **`behavior`** (1635 строк; declared, whole_only)  
...
- **`letterwork`** (96 строк; recompute, whole_only)  
  **DEEDS UPON LETTERS** — A WORD AS A ROW, NOT ONLY AS A MEANING (14.09, bought by a zero of the probe).
