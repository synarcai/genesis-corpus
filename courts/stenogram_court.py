...
ШАГ = re.compile(rf"^(?:step|шаг) (\d+) (?:on grid|на сетке) ({Г}): (?:action|действие) «([^»]+)» → (?:response|отклик): "
...
def судить(строка, слой=None):
    ...
    м = ШАГ.match(с)
    if м:
        номер, вход, действие, сетка, отказ = (int(м.group(1)), м.group(2), м.group(3),
                                              м.group(4), м.group(5))
        состояние = S.разобрать(вход)
        if состояние is None:
            return True, False
        # СТРОКА СУДИМА САМА ПО СЕБЕ по своему входу; слой, если он открыт,
        # сверяет ещё и НЕПРЕРЫВНОСТЬ: вход шага равен отклику прошлого и
        # номер идёт по порядку — разрыв цепи есть ложь о форме.
        if слой.состояние is not None and not слой.обрыв:
            if номер != слой.ждём or S.записать(слой.состояние) != вход:
                слой.обрыв = True
                return True, False
            слой.ждём += 1
        новое, причина = исполнить(состояние, действие)
        ...
        return True, S.записать(новое) == сетка
    м = КОНЕЦ.match(с)
    if м:
