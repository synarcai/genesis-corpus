...
def _образцы():
    ...
    for язык in F.ЯЗЫКИ:
        ...
        имя_альт = "(" + "|".join(re.escape(и) for и in
                                  sorted(все_имена, key=lambda с: (-len(с), с))) + ")"
...
def _образцы_лица():
    """Образцы первого лица: те же дыры, но ИМЕНИ В РАМКЕ НЕТ.

    ЧТО ЗДЕСЬ СУДИТСЯ СВЕРХ АРИФМЕТИКИ. Страница утверждает о себе: «имени моего здесь нет».
    Утверждение это ПРОВЕРЯЕМО — суд смотрит, что ни одно объявленное имя языка в строке не
    стои́т, — и потому оно закон, а не заверение.

        УТВЕРЖДЕНИЕ ОБ ОТСУТСТВИИ ПРОВЕРЯЕТСЯ ТАК ЖЕ, КАК УТВЕРЖДЕНИЕ О НАЛИЧИИ, И СТОИТ
        РОВНО СТОЛЬКО ЖЕ: страница, назвавшая имя и сказавшая, что имени нет, лжёт о себе.
    """
    вон = []
    for язык in F.ЯЗЫКИ:
        формы = sorted({N.вещь(язык, в, n) for в in F.ВЕЩИ[язык]
                        for a, b in F.ПАРЫ for n in (a, b, a - b)},
                       key=lambda с: (-len(с), с))
        альт = "(?:" + "|".join(re.escape(ф) for ф in формы) + ")"
        рамка = F.РАМКИ_ЛИЦА[язык]
        куски, конец = [], 0
        for м in _ДЫРА.finditer(рамка):
            куски.append(re.escape(рамка[конец:м.start()]))
            имя = м.group(1)
        ...
        все_имена = F.ИМЕНА[язык][0] + F.ИМЕНА[язык][1]
        if язык == "ru":
            все_имена = все_имена + tuple(F.ИМЕНА_РОДИТЕЛЬНЫЙ[и] for и in все_имена)
        вон.append((язык, frozenset(все_имена), re.compile("^" + "".join(куски) + "$")))
    return tuple(вон)


ОБРАЗЦЫ_ЛИЦА = _образцы_лица()
...
def _судить(строка):
    ...
    for язык, форма, свои_имена, образец in ОБРАЗЦЫ:
        ...
        вещи = F.ВЕЩИ[язык]
        for кусок, n in zip(_слова_вещей(с, F.РАМКИ[язык][форма]), (a, b, c)):
            if кусок is not None and not any(N.вещь(язык, в, n) == кусок for в in вещи):
                return True, False
        return True, True
    for язык, имена, образец in ОБРАЗЦЫ_ЛИЦА:
        м = образец.match(с)
        if not м:
            continue
        ...
        if c != a - b or c < 2:
            return True, False
        вещи = F.ВЕЩИ[язык]
        for кусок, n in zip(_слова_вещей(с, F.РАМКИ_ЛИЦА[язык]), (a, b, c)):
            if кусок is not None and not any(N.вещь(язык, в, n) == кусок for в in вещи):
                return True, False
        # ИМЕНИ НЕТ — И ЭТО ПРОВЕРЕНО, А НЕ ПРИНЯТО НА ВЕРУ. Проверка эта СТРАХОВОЧНАЯ:
        # ныне имя не пролезет и в образец, ибо все дыры рамки суть числа да слова вещей.
        # Но рамка — вещь правимая, и день, когда в ней явится свободное место, не должен
        # стать днём, когда утверждение «имени моего здесь нет» перестанет проверяться.
        if any(re.search(r"(?<![^\W\d_])" + re.escape(и) + r"(?![^\W\d_])", с)
               for и in имена):
            return True, False
        return True, True
    return False, False
...
def _слова_вещей(с, рамка):
    """Слова вещей в порядке дыр рамки — вынимаются повторным разбором.

    РАМКА ПОДАЁТСЯ, А НЕ ВЫЧИСЛЯЕТСЯ ПО (ЯЗЫК, ФОРМА): рядов рамок в доме стало два —
    третье лицо с именем и первое без имени, — и разбор, знающий лишь первый, молча
    пропустил бы счётную форму второго. Так и вышло на подсадке: «у меня 7 шара» суд
    назвал истиной, ибо форм вещи в этой ветви не смотрел вовсе.

        ВЕТВЬ, ДОБАВЛЕННАЯ БЕЗ ВСЕХ ПРОВЕРОК СОСЕДНЕЙ, ЕСТЬ ДЫРА РОВНО В ТОМ, ЧЕГО В НЕЙ
        НЕ ПОВТОРИЛИ.
    """
