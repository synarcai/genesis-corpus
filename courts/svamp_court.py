...
def main():
    ...
    if пойманы != len(подсадки):
        ...
        return 1
    # ПАДЕЖ ПРИ ПРЕДЛОГЕ: СЧЁТНАЯ ФОРМА НЕ ЕСТЬ ПАДЕЖНАЯ.
    #
    # Свод три дня нёс 24 строки, ложные о языке, и ни один из 157 судов их не читал: «коробку
    # с 12 мелков» вместо «мелками», «pudełko z 12 kredek» вместо «kredkami», «eine Schachtel
    # mit 12 Buntstifte» вместо «Buntstiften». Суд согласования сверял СЧЁТ и видел верное —
    # «мелков» есть верная счётная форма при двенадцати; падеж не был ничей.
    #
    # Рубеж строится ЖИВОЙ СТРАНИЦЕЙ: суд берёт страницу дома, подменяет падежную форму счётной
    # — той самой, какую дом дал бы этому числу, — и требует, чтобы подменённая НЕ БЫЛА НАЗВАНА
    # ИСТИННОЙ. Признак, отличающий падеж от счёта, здесь же и проверяется: падежная форма при
    # двенадцати и при пяти ОДНА, счётная — разная, и подмена потому видна.
    падежные = []
    for язык, товары in F.ПРИ_ПРЕДЛОГЕ.items():
        for ключ, форма in товары.items():
            честная = F.страница_акта(язык, "коробка", 0, 1, 12, 5)
            if _судить(честная) != (True, True):
                падежные.append(("честная не истинна", язык, честная))
                continue
            for число in (12, 5):
                счётная = F._товар_форма(язык, ключ, число)
                if счётная == форма:
                    continue
                битая = честная.replace(f"{число} {форма}", f"{число} {счётная}", 1)
                if битая != честная and _судить(битая) == (True, True):
                    падежные.append(("счётная принята за падежную", язык, битая))
    if падежные:
        for почему, язык, стр in падежные[:6]:
            print(f"  ПАДЕЖ ПРИ ПРЕДЛОГЕ {язык} — {почему}: {стр[:110]}")
        print(f"ФОРМЫ SVAMP FAIL: падежных рубежей нарушено {len(падежные)}")
        return 1
