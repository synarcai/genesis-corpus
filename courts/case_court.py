...
import rugram  # noqa: E402 — множественное ВЫВОДИТСЯ правилом, а не берётся списком
...
# РУССКАЯ СТОРОНА ЧИСЛОВОЙ РАМКИ СУДИТСЯ ТЕМ ЖЕ ЗАКОНОМ (15.09): суд ВЫВОДИТ множественное
# правилом русской основы (`rugram.именительный_мн`) и сверяет с названным. Список форм тут
# не нужен и был бы вреден: сверка со списком зеленеет от совпадения с самой собою.
#
#     ДВЕ СТОРОНЫ ОДНОЙ РАМКИ СУДЯТСЯ ДВУМЯ ПРАВИЛАМИ ДВУХ ЯЗЫКОВ, А НЕ ОДНИМ СПИСКОМ НА ОБЕ.
МНОЖЕСТВЕННОЕ_РУ = re.compile(
    r"^множественное число слова ([а-яё]+) — ([а-яё]+)\.$")
ЕДИНСТВЕННОЕ_РУ = re.compile(
    r"^единственное число слова ([а-яё]+) — ([а-яё]+)\.$")
ВОПРОС_ЧИСЛА_РУ = re.compile(
    r"^какое (множественное|единственное) число у слова ([а-яё]+)\? (.+)$")
# ИМЕНА РУССКОЙ СТОРОНЫ — ТЕ ЖЕ, ЧТО У ДОМА ЕДИНИЦ: единственное слово берётся из его
# объявления, и строка о слове, какого дом не объявлял, судом не читается.
_ОБЪЯВЛЕННЫЕ_РУ = frozenset(
    ф[1][0] for ф in units.ФОРМЫ.values() if ф and ф[1])
...
def _судить(строка):
    ...
    if m:
        ...
        return True, (_объявлено(один)
                      and units.мн_правилом(один) == много)
    m = ВОПРОС_ЧИСЛА_РУ.match(с)
    if m:
        какое, спрошено, ответ = m.groups()
        образец = (МНОЖЕСТВЕННОЕ_РУ if какое == "множественное"
                   else ЕДИНСТВЕННОЕ_РУ)
        свой = образец.match(ответ)
        if свой is None:
            return False, True
        судимо, истинно = судить(ответ)
        return судимо, bool(истинно and свой.group(1) == спрошено)
    m = МНОЖЕСТВЕННОЕ_РУ.match(с)
    if m:
        один, много = m.groups()
        return True, (один in _ОБЪЯВЛЕННЫЕ_РУ
                      and rugram.именительный_мн(один) == много)
    m = ЕДИНСТВЕННОЕ_РУ.match(с)
    if m:
        много, один = m.groups()
        # ЕДИНСТВЕННОЕ ОБЯЗАНО БЫТЬ ОБЪЯВЛЕННЫМ СЛОВОМ, А НЕ ЛЮБЫМ, ЧЬЁ ПРАВИЛО ДАЁТ
        # НАЗВАННОЕ МНОЖЕСТВЕННОЕ: правило проверяет ПЕРЕХОД, а не существование.
        return True, (один in _ОБЪЯВЛЕННЫЕ_РУ
                      and rugram.именительный_мн(один) == много)
    m = ПАДЕЖОМ.match(с)
    if m:
...
# мира («mathfound» — 1006 строк) — судимы ВСЕ до одной. Значит показы суда суть весь мир, и
...
ЗАМКНУТЫЕ_МИРЫ = frozenset({"mathfound"})
