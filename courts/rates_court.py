...
# ЧИТАТЕЛЬ ПОШЁЛ ЗА ОБЪЯВЛЕНИЕМ (13.09): ДЕЯТЕЛИ переехали в дом.
import ratesforms as _Г  # noqa: E402
ДЕЯТЕЛИ_EN = {д[0] for д in _Г.ДЕЯТЕЛИ}
# И РУССКИЕ ТОЖЕ — ОБЪЯВЛЕНЫ ТАМ ЖЕ, А ЧИТАЛИСЬ ТОЛЬКО АНГЛИЙСКИЕ (09.09). Шрам выше записан
# об английской ветви и об английской же вылечен: бралось `д[0]`. Русская ветвь брала деятеля
# каким дали, и подсадка «водитель» → «водител» проходила ИСТИНОЙ.
#
#     ЛЕЧЕНИЕ, ЗАПИСАННОЕ ОДНОЙ ВЕТВЬЮ, НЕ ЛЕЧИТ ВТОРОЙ. Объявление лежало рядом — в том же
#     кортеже, соседним полем, — и не было прочтено.
ДЕЯТЕЛИ_RU = {д[1] for д in _Г.ДЕЯТЕЛИ}
_ЕД = {}
_ИМЯ = {}
...
def _судить(строка):
    ...
    m = СТАВКА_RU.match(с)
    if m:
        кто, ставка, форма, _когда = m.groups()
        if кто not in ДЕЯТЕЛИ_RU:
            return False, True      # необъявленный деятель — чужая рамка
        имя = _ру_имя(форма)
