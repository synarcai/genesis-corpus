...
ФОРМЫ = [
    ...
    # ЕДИНСТВЕННОСТЬ СЕРЕДИНЫ — ВЕРДИКТ С ОСНОВАНИЕМ (М-147): «да» у нечётного
    # числа элементов, «нет» у чётного; суд считает элементы сам.
    (rf"^(yes|no): {Ч} has (\d+) items, and (\d+) is (odd|even)$",
     lambda сл, a, n, n2, ч: len(числа(a)) == int(n) == int(n2) and ((сл == "yes") == (int(n) % 2 == 1)) and (ч == ("odd" if int(n) % 2 else "even"))),
    (rf"^(да|нет): у {Ч} элементов (\d+), а это (нечётное|чётное) число$",
     lambda сл, a, n, ч: len(числа(a)) == int(n) and ((сл == "да") == (int(n) % 2 == 1)) and (ч == ("нечётное" if int(n) % 2 else "чётное"))),
]
...
# мира («statistics» — 1176 строк) — судимы ВСЕ до одной. Значит показы суда суть весь мир, и
...
ЗАМКНУТЫЕ_МИРЫ = frozenset({"statistics"})
