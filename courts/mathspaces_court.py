...
# СЕМЕЙСТВО ЕСТЬ РОД (М-146): формы одной рамки — утверждение, отрицание,
# вопрос через дом пары — один род на язык; роды названы признаком формы.
РОДЫ_ПРОСТРАНСТВ = (
    ("расстояние", r"distance|расстояние"), ("середина", r"midpoint|середина"),
    ("сдвиг", r"shifted|сдвига"), ("отражение", r"reflected|отражения"),
    ("поворот", r"rotated|поворота"), ("степень", r"degree|степень"),
    ("счёт графа", r"vertices|edges|вершин|р[её]б"), ("путь", r"path|пут"),
    ("связность", r"connected|связен"), ("цикл", r"cycle|цикл"),
    ("линия Фано", r"one line|одной линии|not lie on line|не лежат на линии"),
    ("точка Фано", r"one point|одной точке"), ("пучок Фано", r"lies on exactly|лежит ровно"),
    ("транспонирование", r"transposed|транспонированная"), ("сумма матриц", r"\\\] \\\+ \\\["),
    ("матрица на вектор", r"·"), ("след", r"trace|след"), ("определитель", r"determinant|определитель"),
    ("строка и столбец", r"row|column|строка|столбец"),
)


def _семейства():
    вон = {имя: [] for имя, _ in РОДЫ_ПРОСТРАНСТВ}
    for о, п in ОБРАЗЦЫ:
        имя = next((имя for имя, признак in РОДЫ_ПРОСТРАНСТВ if re.search(признак, о)), None)
        assert имя is not None, о[:60]
        вон[имя].append((о, п))
    return tuple((имя, формы) for имя, формы in вон.items() if формы)


СЕМЕЙСТВА_СУДА = _семейства()
...
# мира («mathspaces» — 814 строк) — судимы ВСЕ до одной. Значит показы суда суть весь мир, и
...
ЗАМКНУТЫЕ_МИРЫ = frozenset({"mathspaces"})
