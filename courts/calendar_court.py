...
# ОБЪЯВЛЕНИЕ ПЕРЕЕХАЛО В ДОМ, И ЧИТАТЕЛЬ ПОШЁЛ ЗА НИМ (13.09). Суд берёт таблицы
# РАЗБОРОМ ИСХОДНИКА, а не ввозом, — и потому привязан к ФАЙЛУ, а не к имени.
#
#     ПЕРЕЕЗД ЗАМЫКАНИЯ ОБЯЗАН ПЕРЕВЕЗТИ И ЧИТАТЕЛЕЙ ОБЪЯВЛЕНИЯ, А НЕ ТОЛЬКО ЕГО САМО.
#     Кузница осталась на месте, таблицы ушли в дом — и суд, читавший кузницу, нашёл
#     пустоту и объявил ложью ВСЕ страницы мира. Ворота закрылись, и это спасло свод.
ГЕНЕРАТОР = КОРЕНЬ / "tools/calendarforms.py"
...
# СОГЛАСОВАНИЕ ЧИТАЕТ СВОЙ СУД, А НЕ СОСЕД (08.09). Образец взял форму дырой («days?»),
# и потому ОБЯЗАН её проверить: дыра без проверки не слепа — она БЛАГОСЛОВЛЯЕТ, и «1 days after
# monday» прошло бы здесь истиной, держась лишь на суде английского числа (М-131). Тот же довод,
# каким лечили дом мер на языках.
_ФОРМА_ДНЯ = re.compile(r"\b(\d+) (days?) after\b")


def _день_по_счёту(с):
    """Всякое «N day(s) after» в строке согласовано со своим числом."""
    return all(слово == ("day" if int(n) == 1 else "days")
               for n, слово in _ФОРМА_ДНЯ.findall(с))
...
def _судить(строка):
    ...
    if m:
        ...
        return True, ((откуда[0] + k) % откуда[1] == куда[0]
                      and _день_по_счёту(с) and _леджер_круга(г[3:], откуда, k, куда))
    ...
    if m:
        ...
        if (откуда is None or куда is None or спрошен != повтор
            ...
            return False, True
        return True, ((откуда[0] + k) % откуда[1] == куда[0]
                      and _день_по_счёту(с) and _леджер_круга(г[5:], откуда, k, куда))
    m = СОСЕД.match(с)
    if m:
        г = [x for x in m.groups() if x is not None]
...
# мира («calendar» — 846 строк) — судимы ВСЕ до одной. Значит показы суда суть весь мир, и
...
ЗАМКНУТЫЕ_МИРЫ = frozenset({"calendar"})
