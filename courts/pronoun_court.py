...
import rugram  # noqa: E402 — счётная форма вещи меняется числом
...
# -*- coding: utf-8 -*-
# РУССКИЕ СТОРОНЫ ЧЕТЫРЁХ РОДОВ (15.09). Суд читает их ТЕМИ ЖЕ разборами рамки, какими
# читает две первые: имя лица, глагольная пара и знак берутся `_рамка_ru`, а нового тут —
# лишь хвост вопроса, держание и сравнение двух актов.
ХВОСТ_RU = r"(осталось|теперь|в итоге|всего)"
ДЕРЖИТ_RU = r"(держит|держат|хранит|хранят)"
ДО_RU = r"(сначала|поначалу|вначале)"


def _ru_до(м):
    """Состояние ДО актов: ответ повторяет первую рамку и ПЕРВОЕ число."""
    лица, v1, хвост = _ru_разбор(м)
    n, вещь1, мест, v2, m, из_них, вещь2, _до, a2, b2, повтор, n2, вещь3 = хвост
    n, m, n2 = int(n), int(m), int(n2)
    if None in лица or not _лица_ru(лица, мест):
        return False
    if _рамка_ru(v1, v2, из_них, len(лица), мест) is None:
        return False
    # ПОВТОР ОБЯЗАН БЫТЬ ТЕМ ЖЕ ЗАЧИНОМ И ТЕМ ЖЕ ЧИСЛОМ: ответ о состоянии до актов есть
    # первое утверждение задачи, сказанное заново, — и число в нём ПЕРВОЕ, а не итог.
    return n2 == n and вещь1 == вещь3 and повтор.strip() == м.group(0).split(".")[0].strip()[:len(повтор.strip())]


def _ru_хвост(м):
    """Хвост вопроса: «осталось», «теперь», «всего», «в итоге» — и счёт сходится."""
    лица, v1, хвост = _ru_разбор(м)
    n, вещь1, мест, v2, m, из_них, вещь2, хв, a2, b2, on, зн, om, k = хвост
    n, m, k, on, om = int(n), int(m), int(k), int(on), int(om)
    if None in лица or not _лица_ru(лица, мест):
        return False
    if [РОД_П[x] for x in лица] != [x for x in (a2, b2) if x]:
    ...
    if знак is None:
        return False
    # ХВОСТ «ОСТАЛОСЬ» ПРИНАДЛЕЖИТ УБЫЛИ, «ВСЕГО» — ПРИБЫЛИ: хвост, сказанный не о своём
    # знаке, есть ложь о деле, а не о слове.
    if (хв == "осталось" and знак > 0) or (хв == "всего" and знак < 0):
        return False
    return (on, om) == (n, m) and зн == ("+" if знак > 0 else "−") and k == n + знак * m >= 0


def _ru_держание(м):
    """Другое держание: «держит», «хранит» — и число его есть итог задачи."""
    лица, v1, хвост = _ru_разбор(м)
    n, вещь1, мест, v2, m, из_них, имя, держит, k, вещь2 = хвост
    n, m, k = int(n), int(m), int(k)
    if None in лица or not _лица_ru(лица, мест):
        return False
    if " и ".join(лица) != имя:
    ...
    if знак is None:
        return False
    # ЧИСЛО ДЕРЖАНИЯ СОГЛАСОВАНО С ЧИСЛОМ ЛИЦ: «держат» при двоих, «держит» при одном.
    мн = держит.endswith("ат") or держит.endswith("ят")
    return мн == (len(лица) == 2) and k == n + знак * m >= 0


def _ru_держание_вопрос(м):
    лица, v1, хвост = _ru_разбор(м)
    n, вещь1, мест, v2, m, из_них, вещь2, держит, имя, on, зн, om, k = хвост
    n, m, k, on, om = int(n), int(m), int(k), int(on), int(om)
    if None in лица or not _лица_ru(лица, мест) or " и ".join(лица) != имя:
        return False
    знак = _рамка_ru(v1, v2, из_них, len(лица), мест)
    if знак is None:
        return False
    мн = держит.endswith("ат") or держит.endswith("ят")
    return (мн == (len(лица) == 2) and (on, om) == (n, m)
            and зн == ("+" if знак > 0 else "−") and k == n + знак * m >= 0)


# ДВА АКТА ОДНОГО НОСИТЕЛЯ, ПО-РУССКИ: пары глаголов объявлены здесь же, и суд сверяет
# РОД формы с родом имени — «Аня сделал» не пройдёт, хотя число сойдётся.
СРАВНЕНИЯ_RU_СУД = {("испёк", "съел"), ("испекла", "съела"),
                    ("купил", "продал"), ("купила", "продала"),
                    ("сделал", "продал"), ("сделала", "продала")}


def _одна_вещь(*формы):
    """Одна ли вещь названа тремя счётными формами.

    СЧЁТНАЯ ФОРМА МЕНЯЕТСЯ ЧИСЛОМ, И СЛИЧАТЬ НАДО КЛЮЧ, А НЕ ПОВЕРХНОСТЬ (15.09): «26
    булочек», «21 булочка» и «булочек» суть одно слово в трёх формах, и суд, сравнивший их
    строками, назвал бы честную страницу ложью — за то, что русский склоняет.
    """
...
def _ru_два_акта(м):
    имя1, сд, a, вещь1, имя2, пр, b, вещь2, имя3, сд2, d, вещь3, пр2, a2, b2, d2 = м.groups()
    a, b, d, a2, b2, d2 = (int(x) for x in (a, b, d, a2, b2, d2))
    if not (имя1 == имя2 == имя3) or not _одна_вещь(вещь1, вещь2, вещь3):
        return False
    if (сд, пр) not in СРАВНЕНИЯ_RU_СУД or сд2 != сд or пр2 != пр:
        return False
    род = _род_имени(имя1)
    if род is None:
        return False
    женская = сд.endswith("а") or сд.endswith("ла")
    if женская != (род == "f"):
        return False
    return (a, b, d) == (a2, b2, d2) and d == a - b > 0
...
ОБРАЗЦЫ = (
    ...
    (rf"^{ПЕРВОЕ} {Ч} {СЛ}\. {ВТОРОЕ}\. сколько {СЛ} было у {ИМЯ}(?: и {ИМЯ})? {ДО_RU}\? (.+?) {Ч} {СЛ}\.$", _ru_до),
    (rf"^{ПЕРВОЕ} {Ч} {СЛ}\. {ВТОРОЕ}\. сколько {СЛ} {ХВОСТ_RU} у {ИМЯ}(?: и {ИМЯ})?\? {Ч} ([+−]) {Ч} = {Ч}\.$", _ru_хвост),
    (rf"^{ПЕРВОЕ} {Ч} {СЛ}\. {ВТОРОЕ}\. ([А-ЯЁ][а-яё]+(?: и [А-ЯЁ][а-яё]+)?) {ДЕРЖИТ_RU} {Ч} {СЛ}\.$", _ru_держание),
    (rf"^{ПЕРВОЕ} {Ч} {СЛ}\. {ВТОРОЕ}\. сколько {СЛ} {ДЕРЖИТ_RU} ([А-ЯЁ][а-яё]+(?: и [А-ЯЁ][а-яё]+)?)\? {Ч} ([+−]) {Ч} = {Ч}\.$", _ru_держание_вопрос),
    # «ЧЕМ» ВЕДЁТ ГЛАГОЛ, А НЕ ИМЯ, И ЭТО РАЗЛИЧАЕТ ДВА РАЗНЫХ РОДА (15.09). «Аня сделала на
    # 6 монет больше, чем продала» есть ДВА АКТА ОДНОГО носителя; «Коля нашёл на 2 монеты
    # больше, чем Петя» есть полоса ДВУХ носителей, и дом её — `gsmtaskforms`. Образец,
    # бравший после «чем» что угодно, читал чужую страницу своей рамкой и звал её ложью.
    #
    #     СУД, ЧИТАЮЩИЙ ЧУЖУЮ СТРАНИЦУ СВОЕЙ РАМКОЙ, НЕ ОШИБАЕТСЯ В СЧЁТЕ — ОН ОШИБАЕТСЯ В
    #     ТОМ, ЧЬЮ СТРАНИЦУ ЧИТАЕТ, и вердикт его тем вернее выглядит, чем он ложнее.
    (rf"^{ИМЯ} (\S+) {Ч} {СЛ}\. {ИМЯ} (\S+) {Ч} {СЛ}\. {ИМЯ} (\S+) на {Ч} {СЛ} больше, чем ([а-яё]+): {Ч} − {Ч} = {Ч}\.$", _ru_два_акта),
    (rf"^{ИМЯ} (\S+) {Ч} {СЛ}\. {ИМЯ} (\S+) {Ч} {СЛ}\. на сколько {СЛ} больше (он|она) ([а-яё]+), чем ([а-яё]+)\? {Ч} − {Ч} = {Ч}\.$", _ru_два_акта_вопрос),
)
...
def _судить(строка):
    ...
    с = строка.strip()
    # СТРАЖ ПРОПУСКАЕТ ЧУЖОЕ БЫСТРО, И ПОТОМУ ОБЯЗАН ЗНАТЬ ВСЕ СВОИ ФОРМЫ (15.09). Русское
    # сравнение двух актов — «Аня сделала 23 монеты… Аня сделала на 6 монет больше, чем
    # продала» — местоимения не несёт ВОВСЕ: русский опускает подлежащее второго глагола, и
    # страж, искавший местоимение, отправлял честную страницу в немоту.
    if not re.search(r"\b(he|she|they|он|она|они)\b", с) and "больше, чем" not in с:
        return False, False
...
# мира («pronouns» — 480 строк) — судимы ВСЕ до одной. Значит показы суда суть весь мир, и
...
ЗАМКНУТЫЕ_МИРЫ = frozenset({"pronouns"})
