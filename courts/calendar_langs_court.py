#!/usr/bin/env python3
"""[THE WEEK IN EIGHT LANGUAGES COURT] — the cycle is counted, the day
forms and the count form of «day» are the house's."""
import pathlib
import sys
...
# THE WORLD IS CLOSED: every honest line of «calendar_langs» is a shape of this court
# (measured 04.09), so a line of it this court does not recognise is a lie.
ЗАМКНУТЫЕ_МИРЫ = frozenset({"calendar_langs"})
...
def main():
    ...
    for путь in worlds(kind="shows"):
        if путь.name != "genesis_calendar_langs.txt":
            continue
    ...
    print(f"НЕДЕЛЯ НА ЯЗЫКАХ {поза}: {итог['ложных']} ложных из {итог['судимых']} судимых, несудимых {итог['несудимых']}")
