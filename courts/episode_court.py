...
# РУССКАЯ СТОРОНА ДЕЛЕЖА (15.09): «17 яблок на 5 детей: каждому по 3 яблока, лишних 2». Суд
# пересчитывает делёж САМ и сверх того проверяет ВСЕ ЧЕТЫРЕ счётные формы по закону пакета —
# вещь при общем числе, носителя при числе носителей и вещь при частном.
#
#     СТРАНИЦА, ГДЕ СЧЁТ ВЕРЕН, А ФОРМА НЕТ, УЧИТ НЕВЕРНОМУ РУССКОМУ ПРИ ВЕРНОЙ АРИФМЕТИКЕ.
ДЕЛЁЖ_РУ = re.compile(
    r"^(\d+) ([а-яё]+) на (\d+) ([а-яё]+): каждому по (\d+) ([а-яё]+), лишних (\d+)\.$")
...
# ДВЕ ОСТАВШИЕСЯ РУССКИЕ ПОВЕРХНОСТИ ДОЛИ (15.09): полярность вопроса — «сколько ушло» — и
# целое, собранное из двух счётов. Суд пересчитывает обе тем же действием, каким пересчитывает
# английских близнецов; полярность при этом сверяется отдельно, ибо ответ у неё ДРУГОЙ.
УШЕДШЕЕ_RU = re.compile(
    r"^у \w+ было (\d+) (\S+)\. (половина|треть|четверть|пятая часть) "
    r"(\S+) ушла\. сколько \4 ушло\? ушло (\d+) (\S+)(?::.*)?\.$")
ЦЕЛОЕ_ИЗ_ДВУХ_RU = re.compile(
    r"^у \w+ было (\d+) (\S+), а потом ещё (\d+) (\S+)\. "
    r"(половина|треть|четверть|пятая часть) (\S+) ушла\. "
    r"сколько \6 осталось\? осталось (\d+) (\S+)(?::.*)?\.$")
...
def _судить(строка, слой=None):
    ...
    m = ДЕЛЁЖ_РУ.match(с)
    if m:
        a, вещь1, b, кто, q, вещь2, r = m.groups()
        a, b, q, r = int(a), int(b), int(q), int(r)
        if b < 1:
            return True, False
        # СЛОВО, ИЗВЕСТНОЕ ПАКЕТУ В ДРУГОЙ ФОРМЕ, ЕСТЬ ЛОЖЬ, А НЕ НЕМОТА (15.09). «17 яблоки»
        # не счётная форма — но «яблоки» пакет ЗНАЕТ: это именительное множественное,
        # выведенное его же правилом. Молчать о нём значило бы звать чистотой слепоту:
        # неверная форма ЗНАКОМОГО слова есть ровно та порча, ради которой суд и стои́т.
        ключ_в = rugram.ПО_ФОРМЕ.get(вещь1) or rugram.ПО_МНОЖЕСТВЕННОМУ.get(вещь1)
        ключ_к = rugram.ПО_ФОРМЕ.get(кто) or rugram.ПО_МНОЖЕСТВЕННОМУ.get(кто)
        ключ_в2 = rugram.ПО_ФОРМЕ.get(вещь2) or rugram.ПО_МНОЖЕСТВЕННОМУ.get(вещь2)
        if ключ_в is None or ключ_к is None or ключ_в2 != ключ_в:
            return False, True                  # слово вне объявления пакета — суд молчит
        формы = (вещь1 == rugram.форма(ключ_в, a)
                 and кто == rugram.форма(ключ_к, b)
                 and вещь2 == rugram.форма(ключ_в, q))
        return True, формы and divmod(a, b) == (q, r)
    m = ЗА_ДНИ.match(с)
    if m:
    ...
    if m:
        ...
        if зн is None:
            return False, True
        return True, всего % зн == 0 and всего - всего // зн == осталось
    m = УШЕДШЕЕ_RU.match(с)
    if m:
        всего, зн, ушло = int(m.group(1)), знаменатель(m.group(3)), int(m.group(5))
        if зн is None:
            return False, True
        # ПОЛЯРНОСТЬ ВОПРОСА ПРОВЕРЯЕТСЯ ЧИСЛОМ: «сколько ушло» требует ДОЛИ, а не остатка,
        # и страница, ответившая остатком, есть ложь при верной арифметике.
        return True, всего % зн == 0 and всего // зн == ушло
    m = ЦЕЛОЕ_ИЗ_ДВУХ_RU.match(с)
    if m:
        a, b = int(m.group(1)), int(m.group(3))
        зн, осталось = знаменатель(m.group(5)), int(m.group(7))
        if зн is None:
            return False, True
        всего = a + b
        return True, всего % зн == 0 and всего - всего // зн == осталось
...
def main():
    ...
    print(f"ЭПИЗОДНАЯ АЛГЕБРА {поза}: {ложных} ложных из {судимых} судимых "
          f"({len(пути)} файлов)"
