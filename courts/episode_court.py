...
def впитать_русские_ставки(путь):
    ...
    with путь.open(encoding="utf-8", errors="replace") as поток:
        for строка in поток:
            m = СТАВКА_ВМЕСТ_RU.match(строка.strip().lower())
            if not m:
                continue
...
def боевые_рамки(строка):
    ...
    if m:
        # СУДИТЬ СЛОВО ПО ЧУЖОМУ ЯЗЫКУ — ХУЖЕ, ЧЕМ МОЛЧАТЬ (08.09; тот же закон, каким
        # седьмого числа лечили суд счёта на девяти языках). Голый факт — четыре слова с
        # числом — есть очертание, какое носят и чужие языки: нидерландское «ik tel 2 meter»
        # ВЕРНО (мера при числе там не меняется), а суд, читающий его английским согласованием,
        # ждал «2 meters» и звал истинную строку ложью. Пять таких строк дал мир языка в час,
        # когда счёт числом пришёл к нидерландскому.
        # ПРИЗНАК ВЗЯТ У́ЖЕ, ЧЕМ У СОСЕДА, И ЭТО НАРОЧНО. Суд памяти молчит обо ВСЯКОЙ
        # строке, чужой английскому знаку, — и платит за это молчанием о коротких строках
        # без служебных слов («ann has 2 meter»: «meter» есть нидерландский знак, и строка
        # выходит ничьей). Здесь довольно НАЗВАННОГО чужого языка: строка, чей язык знак
        # называет прямо и это не английский, суду не подсудна; строка без имени языка
        # судится по-прежнему, и «2 meter» в ней остаётся ложью.
...
def впитать_ставки(путь):
    ...
    with путь.open(encoding="utf-8", errors="replace") as поток:
        for строка in поток:
            m = ПРИХОДЯТ.match(строка.strip().lower())
            if m:
...
def _судить(строка, слой=None):
    # СУД ЧИТАЕТ СТРОКУ СТРОЧНЫМИ — ЦЕЛИКОМ, А НЕ МЕСТАМИ (05.09, суд родов: 8 972
    # строки шести английских миров историй остались без хозяина счёта после того,
    # как волна имён стала писать «Felix» вместо «felix» — рамки суда сравнивали
    # сырую строку со строчными словарями имён и молчали). Письмо имени — дело
    # пакета; суд судит форму и число, и регистр ему не предмет.
    строка = строка.strip().lower()
    с = строка
...
# МИР ЗАМКНУТ, И ЭТО ПРОВЕРЕНО ДЕЛОМ (06.09, перепись закрываемости): палата,
# суженная до одного этого суда, судила КАЖДУЮ строку мира «items» (2357 строк) —
# судимы ВСЕ до одной. Значит молчание суда о строке этого мира есть не
# воздержание, а форма, которой мир не писал; обёртка не может сработать ни на
# одной сегодняшней строке, и это доказательство, а не выборка.
#
# ОБЁРТКА ВЗЯТА СО СЛОЕМ: слой этого суда несёт ДЕЯТЕЛЕЙ мира, и обычное
# `замкнуть` съело бы его, зовя `судить_(строка)` без слоя. Имя мира едет в том
# же слое отдельным полем — `actors.Слой` наследует `closedworld.Слой`.
ЗАМКНУТЫЕ_МИРЫ = frozenset({"items"})
