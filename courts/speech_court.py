...
# ССЫЛКА ЭТА МЕРТВА И НАЗВАНА МЁРТВОЙ (13.09): имя объявлено и НИ РАЗУ не зовётся в теле
# суда. Мёртвая ссылка на файл, чьё содержимое переехало, есть ложь, которую однажды
# прочтут как правду, — оттого она переведена за объявлением, а не оставлена гнить.
#
#     ССЫЛКА, КОТОРОЙ НИКТО НЕ ПОЛЬЗУЕТСЯ, НЕ ПАДАЕТ ПРИ ПЕРЕЕЗДЕ И ОТТОГО ЛЖЁТ ДОЛЬШЕ
#     ВСЕХ ПРОЧИХ.
ГЕНЕРАТОР = КОРЕНЬ / "tools/speechforms.py"
...
# ТРИ ПРАВИЛА УСТУПКИ, ПРИБАВЛЕННЫЕ 13.09, И КАЖДОЕ ПЕРЕСЧИТЫВАЕТСЯ ЗАНОВО.
#
# Род «уступка» показывал ЧЕТЫРЕ страницы против семисот двадцати у соседа по дому — ниже
# закона массы (девять), то есть ОБЕЩАНИЕ, ДАННОЕ И НЕ ИСПОЛНЕННОЕ. Страниц прибавлено до
# шестнадцати, и суд научен пересчитывать каждое новое правило.
#
#     СТРАНИЦА ПИШЕТСЯ НЕ ПРЕЖДЕ, ЧЕМ СУД НАУЧИТСЯ ЕЁ ПЕРЕСЧИТЫВАТЬ. Ворота закрылись на
#     двадцати четырёх строках, и это правильный порядок: сперва мера, потом показ.
ЧЁТНЫЕ_ОБЫЧНО = re.compile(
    r"^(?:чётные числа обычно составны: из (\d+) чётных до (\d+) простое ровно одно"
    r"|even numbers are usually composite: of (\d+) even numbers below (\d+), "
    r"exactly one is prime)\.$")
ХОТЯ_ЧЁТНО = re.compile(
    r"^(?:хотя (\d+) чётно, \1 простое"
    r"|although (\d+) is even, \2 is prime)\.$")
НЕ_СОСЕДИ = re.compile(
    r"^(?:простые числа обычно не стоят рядом: среди (\d+) простых до (\d+) "
    r"соседних пар (\d+)"
    r"|primes are usually not adjacent: among (\d+) primes below (\d+) "
    r"there is (\d+) adjacent pair)\.$")
ХОТЯ_СОСЕДИ = re.compile(
    r"^(?:хотя (\d+) и (\d+) простые, они соседи"
    r"|although (\d+) and (\d+) are prime, they are adjacent)\.$")
ЧЁТНО_ДЕЛИТЕЛЕЙ = re.compile(
    r"^(?:у числа обычно чётное число делителей: до (\d+) таких чисел (\d+), "
    r"а с нечётным — (\d+)"
    r"|a number usually has an even count of divisors: below (\d+), "
    r"(\d+) numbers do and (\d+) do not)\.$")
ХОТЯ_ДЕЛИТЕЛЕЙ = re.compile(
    r"^(?:хотя (\d+) есть число, делителей у \1 три: ([\d, ]+)"
    # ОБРАТНАЯ ССЫЛКА СЧИТАЕТ ВСЕ СКОБКИ ОБРАЗЦА, А НЕ СКОБКИ СВОЕЙ ВЕТВИ: в русской ветви
    # две группы, и первая английская есть ТРЕТЬЯ. Первая редакция писала здесь `\2` и
    # ссылалась на русскую — образец не ловил английскую строку вовсе.
    #
    #     ВЕТВЬ ОБРАЗЦА НЕ ИМЕЕТ СВОЕГО СЧЁТА СКОБОК; ССЫЛКА ВНУТРИ НЕЁ СЧИТАЕТ ОТ НАЧАЛА.
    r"|although (\d+) is a number, \3 has three divisors: ([\d, ]+))\.$")
...
def связь(строка):
    ...
    m = ЧЁТНЫЕ_ОБЫЧНО.match(с)
    if m:
        чётных, верх = [int(x) for x in m.groups() if x is not None]
        свои = [n for n in range(2, верх + 1) if n % 2 == 0]
        прост = set(_простые_до(верх))
        # «РОВНО ОДНО» ПРОВЕРЯЕТСЯ СЧЁТОМ, А НЕ ВЕРОЙ В СЛОВО.
        return len(свои) == чётных and sum(1 for n in свои if n in прост) == 1
    m = ХОТЯ_ЧЁТНО.match(с)
    if m:
        n = int(next(г for г in m.groups() if г))
        # ИСКЛЮЧЕНИЕ ОБЯЗАНО БЫТЬ ЕДИНСТВЕННЫМ: n чётно, просто, и прочие чётные составны.
        прост = set(_простые_до(60))
        прочие = [x for x in range(2, 61) if x % 2 == 0 and x != n]
        return n % 2 == 0 and n in прост and not any(x in прост for x in прочие)
    m = НЕ_СОСЕДИ.match(с)
    if m:
        сколько, верх, пар = [int(x) for x in m.groups() if x is not None]
        простые = _простые_до(верх)
        соседей = sum(1 for a, b in zip(простые, простые[1:]) if b - a == 1)
        return len(простые) == сколько and соседей == пар
    m = ХОТЯ_СОСЕДИ.match(с)
    if m:
        a, b = [int(x) for x in m.groups() if x is not None][:2]
        прост = set(_простые_до(60))
        соседи = [(x, y) for x, y in zip(sorted(прост), sorted(прост)[1:]) if y - x == 1]
        return a in прост and b in прост and b - a == 1 and соседи == [(a, b)]
    m = ЧЁТНО_ДЕЛИТЕЛЕЙ.match(с)
    if m:
        верх, чётных, нечётных = [int(x) for x in m.groups() if x is not None]
        счёт = [sum(1 for d in range(1, n + 1) if n % d == 0) for n in range(1, верх + 1)]
        return (sum(1 for k in счёт if k % 2 == 0) == чётных
                and sum(1 for k in счёт if k % 2) == нечётных)
    m = ХОТЯ_ДЕЛИТЕЛЕЙ.match(с)
    if m:
        г = [x for x in m.groups() if x is not None]
        n, ряд = int(г[0]), [int(x) for x in г[1].replace(",", " ").split()]
        делители = [d for d in range(1, n + 1) if n % d == 0]
        return делители == ряд and len(делители) % 2 == 1
    m = ХОТЯ_БЫ.match(с)
    if m:
        г = [x for x in m.groups() if x is not None]
