...
def _объявленные_лица():
    ...
    """
    # ЛИЦА ЧИТАЮТСЯ ИМПОРТОМ ПОРОЖДАЮЩЕГО (05.09): прежде суд разбирал его текст
    # по AST и брал литерал; с волны имён лица ВЫВОДЯТСЯ из пакета, литерала нет,
    # и AST-чтение молча вернуло бы пустой словарь — суд ослеп бы без вести.
    вон = {}
    import gen_genesis_speech as _Г
    for имя, род, он_она, него_неё, en, he_she in _Г.ЛИЦА:
...
АНАФОРА_EN = re.compile(
    ...
    r"\3 has (\d+) \w+ left\.$")
# СЛОВО ВЕРДИКТА — НЕОБЯЗАТЕЛЬНАЯ ПРИСТАВКА ОТВЕТА (М-147, М-146): «да: 6
# делится на 3. значит 6 кратно 3.» судится тем же ходом, что и повествование.
ЗНАЧИТ = re.compile(
    r"^(?:да: |yes: )?(?:(\d+) делится на (\d+)(?:, частное (\d+))?\. значит (\d+) (кратно \d+|чётно)"
    r"|(\d+) is divisible by (\d+)(?:, the quotient is (\d+))?\. therefore (\d+) "
...
НЕ_ДЕЛИТСЯ = re.compile(
    ...
    r"(\d+))\.$")
# ВОПРОС О СЛЕДСТВИИ — свой вопрос рода «значит»: ответ есть вывод из
# посылки, и суд требует, чтобы посылка была истинной и вывод следовал.
СЛЕДУЕТ = re.compile(
...
def _судить(строка, слой=None):
    ...
    if m:
        ...
        частное = int(г.pop(2)) if len(г) == 5 else None
        n, d, повтор, вывод_ = int(г[0]), int(г[1]), int(г[2]), г[3]
        if n != повтор or (частное is not None and (n % d or частное != n // d)):
            return True, False
        посылка = n % d == 0
...
# суженная до одного этого суда, судила КАЖДУЮ строку мира «speech» (1910 строк) —
...
ЗАМКНУТЫЕ_МИРЫ = frozenset({"speech"})
