...
НЕ_РОДЫ = ('ПОКУПКА', 'ПОКУПКА_ВОПРОС', 'ПОКУПКА_RU', 'ПОКУПКА_RU_ВОПРОС')
...
# ═══ ОБРАЗЦЫ ═══════════════════════════════════════════════════════
#
# РАССКАЗ И ВОПРОС — ОДИН ОБРАЗЕЦ, А НЕ ДВА: помощники слияния живут общим домом
# (`tools/onepattern.py`), где сказаны и закон, и его цена — пересчёт обратных ссылок.
_вместе, _г = onepattern.вместе, onepattern.захваты
...
ПОКУПКА_ОБА = _вместе(ПОКУПКА, ПОКУПКА_ВОПРОС)
ПОКУПКА_RU_ОБА = _вместе(ПОКУПКА_RU, ПОКУПКА_RU_ВОПРОС)
...
# ВОПРОС СТОИ́Т ПОСЕРЕДИНЕ, А НЕ В НАЧАЛЕ, и это тот же образец: «…положил туда 3 яблока.
# сколько ещё не хватает? не хватает ещё 5 яблок.» Приставка необязательна, шов объявлен дырой.
ВМЕЩАЕТ_RU = re.compile(
    r"^(\S+) вмещает (\d+) (\S+); (\S+) положил(а?) туда (\d+) "
    r"(\S+?)(?:; |\. сколько ещё не хватает\? )не хватает ещё (\d+) (\S+)\.$")
...
ДЕЛЁЖ_RU = re.compile(
    ...
    r"досталось по (\d+) (\S+)\.$")
# НАСТОЯЩЕЕ ВРЕМЯ И ВОПРОС ПО-РУССКИ (15.09). Английская сторона рода держала три формы,
# русская одну, и назвала это МЕРА ЯЗЫКОВОГО ПЕРЕКОСА ДОМА. Суд пересчитывает их тем же
# делом: деление без остатка и счётная форма при каждом числе.
ДЕЛЁЖ_RU_НЫНЕ = re.compile(
    r"^(\S+) делит (\d+) (\S+) между (\d+) друзьями; каждому "
    r"достаётся по (\d+) (\S+)\.$")
ДЕЛЁЖ_RU_ВОПРОС = re.compile(
    r"^(\S+) разделил(а?) (\d+) (\S+) между (\d+) друзьями\. сколько "
    r"(\S+) досталось каждому другу\? каждому досталось по (\d+) (\S+)\.$")
...
ДЕНЬГИ_RU_НЫНЕ = re.compile(
    r"^(\S+) зарабатывает (\d+) (\S+) и платит (\d+) (\S+) за "
    r"билет; \1 откладывает (\d+) (\S+)\.$")
ДЕНЬГИ_RU_ВОПРОС = re.compile(
    r"^(\S+) заработал(а?) (\d+) (\S+) и заплатил\2 (\d+) (\S+) за "
    r"билет\. сколько рублей отложил\2 \1\? \1 отложил\2 (\d+) (\S+)\.$")
...
def судить(строка):
    ...
    m = ПОКУПКА_ОБА.match(с)
    if m:
        _к, а, и1, б, и2, место, всего, з1, з2, з3 = _г(m)
        if место not in дом.МЕСТА:
            return False, False
        return True, (_сумма_покупки(а, и1, б, и2, всего)
                      and (int(з1), int(з2), int(з3)) == (int(а), int(б), int(всего)))
    m = ПОКУПКА_RU_ОБА.match(с)
    if m:
        кто, суф, а, ф1, б, ф2, п, местн, всего, фп, з1, з2, з3 = _г(m)
    ...
    m = ДЕЛЁЖ_RU_НЫНЕ.match(с)
    if m:
        _кто, всего, ф1, друзей, каждому, ф2 = m.groups()
        всего, друзей, каждому = int(всего), int(друзей), int(каждому)
        ключ = _ру_ключ(ф1)
        if ключ is None:
            return False, False
        return True, (друзей > 0 and всего == друзей * каждому
                      and _ру_пара(всего, ф1, ключ)
                      and _ру_пара(каждому, ф2, ключ))
    m = ДЕЛЁЖ_RU_ВОПРОС.match(с)
    if m:
        кто, суф, всего, ф1, друзей, ф_в, каждому, ф2 = m.groups()
        всего, друзей, каждому = int(всего), int(друзей), int(каждому)
        ключ = _ру_ключ(ф1)
        if ключ is None:
            return False, False
        # ВОПРОС СПРАШИВАЕТ О ТОЙ ЖЕ ВЕЩИ, ЧТО ДЕЛИЛИ: форма множественного родительного.
        return True, (друзей > 0 and всего == друзей * каждому
                      and _суффикс_верен(кто, суф)
                      and _ру_ключ(ф_в) == ключ
                      and _ру_пара(всего, ф1, ключ)
                      and _ру_пара(каждому, ф2, ключ))
    ...
    m = ДЕНЬГИ_RU_НЫНЕ.match(с)
    if m:
        _кто, плата, ф1, билет, ф2, осталось, ф3 = m.groups()
        плата, билет, осталось = int(плата), int(билет), int(осталось)
        return True, (плата - билет == осталось and билет > 0
                      and _ру_пара(плата, ф1, "рубль")
                      and _ру_пара(билет, ф2, "рубль")
                      and _ру_пара(осталось, ф3, "рубль"))
    m = ДЕНЬГИ_RU_ВОПРОС.match(с)
    if m:
        кто, суф, плата, ф1, билет, ф2, осталось, ф3 = m.groups()
        плата, билет, осталось = int(плата), int(билет), int(осталось)
        return True, (плата - билет == осталось and билет > 0
