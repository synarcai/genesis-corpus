...
# ═══ ОБРАЗЦЫ ═══════════════════════════════════════════════════════
#
# РАССКАЗ И ВОПРОС — ОДИН ОБРАЗЕЦ, А НЕ ДВА (08.09). Прибор ширины считает род ОБРАЗЦОМ: рассказ,
# писанный отдельным образцом, есть род БЕЗ вопросной поверхности, сколько бы вопросов о том же
# самом ни стояло рядом. Суд и так читал их вместе — «ПОКУПКА.match(с) or ПОКУПКА_ВОПРОС.match(с)»,
# один разбор на обе половины; ныне это сказано ОДНИМ образцом, и род выходит один, с вопросом.
#
...
#
# Захваты берутся БЕЗ ПУСТЫХ: ветви равны числом групп, и непойманная ветвь даёт None (приём суда
# календаря). Пустая строка группы («купил(а?)») не есть None и не теряется.
def _вместе(*образцы):
    """Один образец из рассказа и вопроса — чередованием ветвей.

    ОБРАТНАЯ ССЫЛКА НЕ ПЕРЕЖИВАЕТ СЛИЯНИЯ БЕЗ ПЕРЕСЧЁТА (08.09, поймано сличением вердиктов).
    Образцы этого суда держат имя деятеля обратной ссылкой — «\1 bought»; в чередовании номера
    групп СДВИГАЮТСЯ, и «\1» второй ветви начинает указывать на первую, которая при этом
    совпадении пуста. Семьдесят четыре строки вышли из-под суда молча.

        ЧИСЛО ГРУППЫ ЕСТЬ ЕЁ МЕСТО В ОБРАЗЦЕ, А НЕ ЕЁ ИМЯ. Сливая образцы, надо пересчитать
        всякую ссылку на сдвиг предыдущих ветвей.
    """
    куски, сдвиг = [], 0
    for о in образцы:
        текст = re.sub(r"\\(\d+)",
...
def _г(м):
    """Захваты совпадения без пустых — половины равны, и лишняя ветвь молчит."""
