#!/usr/bin/env python3
"""[REMAINDERS IN EIGHT LANGUAGES COURT] — the division is done again."""
import pathlib
import sys
...
# THE WORLD IS CLOSED: every honest line of «remainders_langs» is a shape of
# this court (built so, 03.09), so a line of it this court does not recognise is a lie.
ЗАМКНУТЫЕ_МИРЫ = frozenset({"remainders_langs"})
...
def main():
    ...
    for путь in worlds(kind="shows"):
        if путь.name != "genesis_remainders_langs.txt":
            continue
    ...
    print(f"ОСТАТКИ НА ЯЗЫКАХ {поза}: {итог['ложных']} ложных из {итог['судимых']} судимых, несудимых {итог['несудимых']}")
