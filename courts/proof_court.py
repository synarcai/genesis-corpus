...
ШАГ_RU = re.compile(
    ...
    r"(\d+) \+ (\d+) = (\d+)$")
# ИМЯ ХОДА ЕСТЬ ХВОСТ СТРАНИЦЫ, А НЕ ЕЁ ЧАСТЬ (12.09): суд читает разбор и пропускает
# объявленное имя — оно не пересчитывается, оно объявлено. Конечной точки образец не
# требует: суд снимает её сам (`rstrip(".")` ниже), и первая редакция хвоста, писавшая
# «matter\.», не подошла ни к одной странице из шестидесяти восьми.
СЛУЧАЙ_EN = re.compile(
    r"^every whole number is even or odd; (\d+) is (even|odd)"
    r"(?:\. this step is called a COMPLETE CASE SPLIT: the cases leave no "
    r"third, and so the witness settles the matter)?$")
СЛУЧАЙ_RU = re.compile(
    r"^всякое целое чётно или нечётно; (\d+) (чётно|нечётно)"
    r"(?:\. такой ход зовётся ПОЛНЫМ РАЗБОРОМ СЛУЧАЕВ: случаи не оставляют "
    r"третьего, и потому свидетель решает дело)?$")
...
# ЧЁТНОЕ НЕ-КВАДРАТ ГОВОРИТ «ДА» ТОЮ ЖЕ РАМКОЙ (holon 03.09, YN-AMBIGUOUS): все
# «да»-показы рамки прежде были квадратами, и рынок вердикта видел двух
# согласных исполнителей. Свидетель второй записи — разложение на два
# множителя, и суд считает его сам.
