#!/usr/bin/env python3
"""[MEAN AND PERCENT IN EIGHT LANGUAGES COURT] — the ledger is recomputed."""
import pathlib
import sys
...
# THE WORLD IS CLOSED: every honest line of «stats_langs» is a shape of this court
# (measured 04.09), so a line of it this court does not recognise is a lie.
ЗАМКНУТЫЕ_МИРЫ = frozenset({"stats_langs"})
...
def main():
    ...
    for путь in worlds(kind="shows"):
        if путь.name != "genesis_stats_langs.txt":
            continue
    ...
    print(f"СВОДКА НА ЯЗЫКАХ {поза}: {итог['ложных']} ложных из {итог['судимых']} судимых, несудимых {итог['несудимых']}")
