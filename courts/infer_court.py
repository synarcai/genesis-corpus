...
def _верно(форма, поля):
    ...
    if форма == "контрапозиция":
        # РАВНОСИЛЬНОСТЬ ПРОВЕРЯЕТСЯ ОБОИМИ СВИДЕТЕЛЯМИ: один обязан молчать в обеих
        # записях (не делится ни на {b}, ни на {a}), другой — говорить в обеих.
        m, n = поля["m"], поля["n"]
        return m % b != 0 and m % a != 0 and n % a == 0 and n % b == 0
    return False
...
def main():
    ...
    #
    # ПОДСАДКИ СТРОЯТСЯ ИЗ РАМОК ДОМА, А НЕ ПИШУТСЯ ПОСТОЯННЫМИ СТРОКАМИ, и это куплено
    # отказом 12.09. Пять подсадок лежали здесь дословно — «12 делится на 4. значит 12
    # делится на 5.» — и в час, когда дом дописал к своим ходам ИМЕНА («этот ход зовётся
    # УТВЕРЖДАЮЩИМ — modus ponens»), образец перестал узнавать их вовсе: суд объявил
    # «поймано 0 из 5», не поймав НИ ОДНОЙ, ибо подсадки перестали быть страницами дома.
    #
    #     ПОДСАДКА, ЗАПИСАННАЯ ПОСТОЯННОЙ СТРОКОЙ, СТАРЕЕТ ВМЕСТЕ С РАМКОЙ И ПЕРЕСТАЁТ
    #     БЫТЬ ПОДСАДКОЙ, НЕ ПЕРЕСТАВ БЫТЬ СТРОКОЙ.
    #
    # Ныне берётся ЧЕСТНАЯ страница каждого рода и в ней подменяется ПОСЛЕДНЕЕ число.
    # Такая подсадка не может устареть: она рождается из той же рамки, что и показ.
    import re as _re
    подсадки = []
    for _форма in F.ФОРМЫ:
        _честная = next((с for с, (я, ф) in F.ПОКАЗЫ.items()
                         if ф == _форма and я == "ru"), None)
        if _честная is None:
            continue
        _числа = list(_re.finditer(r"\d+", _честная))
        if not _числа:
            continue
        _m = _числа[-1]
        подсадки.append(_честная[:_m.start()] + str(int(_m.group()) + 1)
                        + _честная[_m.end():])
    if len(подсадки) < len(F.ФОРМЫ):
        print(f"СТРОГИЙ ВЫВОД ОТКАЗ: подсадок построено {len(подсадки)} "
              f"из {len(F.ФОРМЫ)} форм — рамка без числа")
        return 2
