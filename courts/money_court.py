...
# ДРОБНАЯ ЗАПИСЬ РУБЛЯ: «2,05 рубля» — целые, копейки и форма при ДРОБНОМ числе.
ДР = r"(\d+),(\d\d) ([а-яё]+)"


def _дробь(d, c, слово):
    """Верна ли счётная форма при дробной величине d,cc — по закону ПАКЕТА, а не по правилу.

    ПРАВИЛО ДРОБНОГО СЧЁТА ОБЪЯВЛЕНО В ПАКЕТЕ РУССКОГО, и суд его лишь СПРАШИВАЕТ: пакет,
    переменивший правило, переменит и вердикт — без единой правки здесь. Суд, написавший
    «после дроби всегда родительный единственного» своими руками, разошёлся бы с пакетом в
    первый же день, когда правило тронут.
    """
    return слово == rugram.форма("рубль", d + c / 100)
...
ОБРАЗЦЫ = (
    ...
    # РУССКАЯ ДЕСЯТИЧНАЯ ЗАПИСЬ ДЕНЕГ (15.09): «2,05 рубля» — запятая и родительный
    # ЕДИНСТВЕННОГО при дробном числе. Форма проверяется тем же законом пакета, каким
    # проверяется целая («_коп»), и правило дробного объявлено в самом пакете — суд его не
    # знает и знать не должен: он спрашивает пакет.
    (rf"^{ДР} \+ {ДР} = {ДР}: {Ч} \+ {Ч} = {Ч} {СЛ}\.$",
     lambda a, ac, р1, b, bc, р2, s, sc, р3, A, B, S, к: (
         A == a * 100 + ac and B == b * 100 + bc and S == A + B == s * 100 + sc
         and _дробь(a, ac, р1) and _дробь(b, bc, р2) and _дробь(s, sc, р3) and _коп(S, к))),
    (rf"^сколько будет {ДР} \+ {ДР}\? {ДР} \+ {ДР} = {ДР}: {Ч} \+ {Ч} = {Ч} {СЛ}\.$",
     lambda a, ac, р0, b, bc, р0b, a2, ac2, р1, b2, bc2, р2, s, sc, р3, A, B, S, к: (
         (a2, ac2, b2, bc2) == (a, ac, b, bc)
         and A == a * 100 + ac and B == b * 100 + bc and S == A + B == s * 100 + sc
         and _дробь(a, ac, р0) and _дробь(b, bc, р0b)
         and _дробь(a, ac, р1) and _дробь(b, bc, р2) and _дробь(s, sc, р3) and _коп(S, к))),
...
СЕМЕЙСТВА = (
    ...
    ("кратные", _Ф[9:11]),
    ("сдача", _Ф[11:15]),
    # СЕМЕЙСТВО СУММ ДЕРЖИТ ЧЕТЫРЕ ОБРАЗЦА, А НЕ ДВА (15.09): русская десятичная запись
    # добавила два, и срез, оставшийся прежним, отсёк английские — они судились бы ВНЕ
    # семейства и в замкнутом мире звались ложью.
    #
    #     СРЕЗ ПО ЧИСЛУ ЕСТЬ ССЫЛКА НА ПОРЯДОК СПИСКА, И ВСЯКАЯ ВСТАВКА ЕГО ЛОМАЕТ МОЛЧА.
    ("суммы", _Ф[15:19]),
)
