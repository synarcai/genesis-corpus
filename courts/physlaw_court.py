...
ГЕНЕРАТОР = КОРЕНЬ / "tools/physlawforms.py"
...
# ФОРМА МЕРЫ ПРИ ЧИСЛЕ ЧИТАЕТСЯ ОДИН РАЗ НА ВСЮ СТРОКУ (08.09, ловушка согласования). Образцы
# этого суда берут форму необязательным «s» — «(\d+) newtons?» — и не читают её НИ РАЗУ: «25
# newton × 6 metres = 150 joules» проходило истиной. Править каждый образец порознь значило бы
# писать проверку двадцать раз; закон же один, и он о СТРОКЕ, а не об образце.
#
#     ДЫРА БЕЗ ПРОВЕРКИ НЕ СЛЕПА — ОНА БЛАГОСЛОВЛЯЕТ. Необязательное «s» есть дыра.
...
def _меры_согласованы(строка):
    """Всякая объявленная мера при числе стои́т в форме своего счёта.

    СУДИТЬ СЛОВО ПО ЧУЖОМУ ЯЗЫКУ — ХУЖЕ, ЧЕМ МОЛЧАТЬ: нидерландское «25 newton» и немецкое
    «25 Newton» верны без «s», и закон английского счёта им не судья. Признак — названный
    знак языка, как у суда эпизодов.
    """
    ...
    for число, слово in _МЕРА_ПРИ_ЧИСЛЕ.findall(строка):
        ...
        if мн is None:
            continue
        if слово != by_count(int(число), мн):
            return False
    return True


def __судить_без_мер(строка):
