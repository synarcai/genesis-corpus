...
ФОРМЫ = [
    ...
    (r"^binary ([01]+) is (\d+) in decimal$",
     lambda x, n: int(x, 2) == int(n)),
    (r"^(\d+) in octal is ([0-7]+)$",
     lambda n, o: format(int(n), "o") == o),
    (r"^octal ([0-7]+) is (\d+) in decimal$",
     lambda x, n: int(x, 8) == int(n)),
    (r"^(\d+) in hex is ([0-9a-f]+)$",
     lambda n, h: format(int(n), "x") == h),
    (r"^hex ([0-9a-f]+) is (\d+) in decimal$",
     lambda x, n: int(x, 16) == int(n)),
    ...
    (r"^(\d+) xor (\d+) is (\d+)$",
     lambda a, b, c: (int(a) ^ int(b)) == int(c)),
    # РУССКИЕ БЛИЗНЕЦЫ ТРЁХ РАМОК (15.09): обратное чтение записи, исключающее ИЛИ и сдвиг
    # вправо сказаны ныне обоими языками, и суд пересчитывает их тем же действием, каким
    # пересчитывает английских близнецов, — а не сверяет одну строку с другою.
    (r"^двоичное ([01]+) это (\d+) в десятичной$",
     lambda x, n: int(x, 2) == int(n)),
    (r"^шестнадцатеричное ([0-9a-f]+) это (\d+) в десятичной$",
     lambda x, n: int(x, 16) == int(n)),
    (r"^(\d+) исключающее или (\d+) побитово это (\d+)$",
     lambda a, b, c: (int(a) ^ int(b)) == int(c)),
    (r"^(\d+) сдвинутое вправо на (\d+) это (\d+)$",
