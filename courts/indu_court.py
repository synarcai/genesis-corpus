...
# «.» ВНУТРИ ПЛЕЧА — ДЕСЯТИЧНАЯ ДРОБЬ («3/4 = 0.75», алгебра единиц): предложение уже
# отрезано по своей точке, и точка в плече бывает только внутри числа
...
def _равенства(предложение):
    ...
    for м in re.finditer("=", предложение):
        ...
        if _ОБРЫВОК.match(л.group().strip()):
            continue
        a, b = _вычислить(л.group().strip(" .")), _вычислить(п_.group().strip(" ."))
...
def _судить(строка):
    ...
    if not с or not _наше(с):
        return False, False
    # ВОПРОС — НЕ УТВЕРЖДЕНИЕ (М-404, досягаемость 05.09: «22 è primo? no: 22 non è
    # primo» — суд читал вопрос «22 è primo?» как заявление «22 просто» и звал ложью 37
    # честных страниц мира простых чисел на пяти языках). Спрашивающее предложение
    # не судится; его ответ — судится.
    предложения = [п.strip().rstrip(".?!") for п in re.split(r"(?<=[.?])\s+", с)
