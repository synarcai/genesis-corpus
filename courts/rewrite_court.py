    ...
    for язык in F.ЯЗЫКИ:
        for форма in F.ФОРМЫ:
            ...
            for м in _ДЫРА.finditer(рамка):
                ...
                имя = м.group(1)
                дыры.append(имя)
                куски.append(_ЗАПИСЬ if имя in ("a", "b", "c") else r"(\d+)")
                конец = м.end()
...
def main():
    ...
    подсадки = ("чем ещё можно записать 5 + 5? 3 × 5: и то и другое есть 10.",
                "чем ещё можно записать 5 + 5? 2 × 5: и то и другое есть 11.",
