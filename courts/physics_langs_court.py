#!/usr/bin/env python3
"""[SPEED AND PRESSURE IN EIGHT LANGUAGES COURT] — the quotient is
recomputed, the unit forms are the house's."""
import pathlib
import sys
...
# THE WORLD IS CLOSED: every honest line of «physics_langs» is a shape of this court
# (measured 04.09), so a line of it this court does not recognise is a lie.
ЗАМКНУТЫЕ_МИРЫ = frozenset({"physics_langs"})
...
def main():
    ...
    for путь in worlds(kind="shows"):
        if путь.name != "genesis_physics_langs.txt":
            continue
    ...
    print(f"ФИЗИКА НА ЯЗЫКАХ {поза}: {итог['ложных']} ложных из {итог['судимых']} судимых, несудимых {итог['несудимых']}")
