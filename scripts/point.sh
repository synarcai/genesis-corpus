...
if [ -f "$TRACE" ]; then
  ...
  # СЛЕД РАЗЛИЧАЕТ ДВЕ БЕДЫ, И ОБЕ КРАСНЫ, НО НАЗЫВАЮТСЯ РАЗНО (12.09): «ПАЛО» значит,
  # что суд сказал FAIL; «ОБОРВАН» — что набор не досчитан и о доброй половине приборов
  # НЕ СКАЗАНО НИЧЕГО. Прежде точка звала второе первым и печатала «пало 311 из 311» о
  # наборе, который был убит рукой на середине и не судил ни разу.
  if [ "$VERDICT" = "ОБОРВАН" ]; then
    echo "== набор: СЛЕД НЕСЁТ ОБРЫВ — набор не досчитан, о части приборов не сказано"
    FELL=$((FELL+1))
  elif [ "$VERDICT" != "ЦЕЛ" ]; then echo "== набор: СЛЕД НЕСЁТ ПАДЕНИЕ"; FELL=$((FELL+1)); fi
