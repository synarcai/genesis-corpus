...
def перепись(путь):
    """(строк, различных, копий, макс копий одного показа) по одному миру.

    РАЗРЫВА ЛИСТА ЗДЕСЬ НЕ БЫВАЕТ, И ПРОВЕРЯТЬ ЕГО НЕЧЕМ. Шов слоя (`layer.SEAM`) есть
    «\n\x0c\n», и первая редакция прибора честно пыталась его отсечь — да делала это
    ДВАЖДЫ НЕВОЗМОЖНЫМ образом:

        1. `splitlines()` сам считает разрыв листа концом строки — до тела цикла разрыв
           не доходит НИКОГДА, и от шва остаются лишь пустые строки;
        2. а проверка стояла ПОСЛЕ `strip()`, который снял бы разрыв и сам, будь он там,
           ибо разрыв листа есть пробельный знак.

    Ни одна из двух порч не меняла числа: пустые строки и так не считаются. Но признак,
    не срабатывающий ни разу, есть ЛОЖЬ О ТОМ, ЧТО ПРИБОР ДЕЛАЕТ, и отличить его от
    работающего можно было лишь пробой на заведомо известном.

        ПРОВЕРКА, КОТОРАЯ НЕ МОЖЕТ СРАБОТАТЬ, ЧИТАЕТСЯ КАК ЗАЩИТА И НЕ ЗАЩИЩАЕТ. Она
        хуже её отсутствия: отсутствие видно, мёртвая проверка — нет.

    Ныне проверка снята, а причина записана: шов отсекается ДО тела, самим разбором строк.
    """
...
def layer_seam():
    """Шов слоя — берётся у слоя, а не переписывается здесь."""
    from layer import SEAM
    return SEAM


def _проба():
    """ПРОБА НА ЗАВЕДОМО ИЗВЕСТНОМ: доказывает, что КОПИИ СЧИТАЮТСЯ и что цель кузницы
    узнаётся.

        ПЕРЕПИСЬ КОПИЙ ЕСТЬ ВЫЧИТАНИЕ ДВУХ ЧИСЕЛ, И ОБА ОНА СЧИТАЕТ САМА. Пусть счёт
        сложится не так — и копий окажется ноль при мире, повторённом трижды; пусть
        образец цели перестанет узнавать кузницу — и переписывать будет нечего.

    Мир пробы держит повтор, пустую строку и разрыв листа, и про каждое известно
    заранее, считается оно строкой или нет.
    """
    import tempfile
    беды = []
    with tempfile.TemporaryDirectory() as д:
        ф = pathlib.Path(д) / "мир.txt"
        ф.write_text("а\nб\nа\nа\n\n  \nв\n", encoding="utf-8")
        строк, разных, копий, макс = перепись(ф)
        # ШОВ СЛОЯ ЦЕЛИКОМ: «\n\x0c\n» обязан не дать НИ ОДНОЙ строки счёта.
        шов = pathlib.Path(д) / "шов.txt"
        шов.write_text("а" + layer_seam() + "б\n", encoding="utf-8")
        швом = перепись(шов)
    if (строк, разных, копий, макс) != (5, 3, 2, 3):
        беды.append(f"перепись: {(строк, разных, копий, макс)} вместо (5, 3, 2, 3)")
    if швом != (2, 2, 0, 1):
        беды.append(f"шов слоя вошёл в счёт показов: {швом} вместо (2, 2, 0, 1)")
    # ОБРАЗЕЦ ЦЕЛИ УЗНАЁТ ОБА ОБЛИКА КУЗНИЦЫ И НЕ БЕРЁТ ЧУЖОГО.
    кузня = ('ЦЕЛЬ = "datasets/genesis_а.txt"\n'
             'emit("datasets/genesis_б.txt", f)\n'
             'emit_grouped("datasets/genesis_в.txt", g)\n'
             'ОТЧЁТ = "reports/DAY.md"\n'
             'ЧУЖОЕ = "datasets/genesis_г.json"\n')
    цели = ПУТЬ.findall(кузня)
    ждём = ["datasets/genesis_а.txt", "datasets/genesis_б.txt", "datasets/genesis_в.txt"]
    if цели != ждём:
        беды.append(f"цели кузницы: {цели} вместо {ждём}")
    return беды
...
def main(argv):
    ...
    if (беды := _проба()):
        ...
        print(f"ПЕРЕПИСЬ КОПИЙ ОТКАЗ: прибор не доказал, что считает ({len(беды)} бед)")
        return 2
    корень = КОРЕНЬ
    if "--корень" in argv:
    ...
    for ц, (ген, через_слой) in sorted(все.items()):
        ...
        if через_слой and макс > LAW:
            ...
            if форма is True:
                ...
                continue
            if форма is None:
                # ПРИБОР, НЕ СУМЕВШИЙ ПРОВЕРИТЬ, ОБЯЗАН СКАЗАТЬ «НЕ ЗНАЮ», А НЕ «ВИНОВЕН»
                # (07.09). Здесь он говорил и то, и другое: приписывал «строчность не
                # выведена» И ставил мир в нарушители, роняя ход. `compare_mult`
                # МНОГОСТРОЧЕН и от закона копий освобождён — но его порождающий не
                # импортировался (падал `plural` от моей же незавершённой правки пакета),
                # и прибор объявил долгом то, чего не проверял.
                #
                #     ПОЛОМКА СОСЕДА, ПРОЧТЁННАЯ КАК ДОЛГ СВОДА, СТОИТ ЧАСА НА ПОИСК ЛЖИ,
                #     КОТОРОЙ НЕТ. Незнание прибора есть его собственный долг, и падать он
                #     обязан на нём — а не на мире, о котором смолчал.
                немых.append(ген)
                continue
    ...
    if немых:
        print(f"  НЕ ЗНАЮ (порождающий не открывает проходов, строчность не выведена): {немых}")
    # НЕЗНАНИЕ ПРИБОРА ЕСТЬ ЕГО СОБСТВЕННЫЙ ДОЛГ, И ПАДАТЬ ОН ОБЯЗАН НА НЁМ. Ход роняется
    # и при копиях сверх закона, и при невыведенной строчности — но вердикт называет,
    # что именно случилось, а не сваливает одно на другое.
    поза = "FAIL" if (len(сверх) > СВЕРХ_LAW_РУБЕЖ or немых) else "PASS"
    почему = []
    if len(сверх) > СВЕРХ_LAW_РУБЕЖ:
        почему.append(f"копий сверх LAW в {len(сверх)} мирах")
    if немых:
        почему.append(f"строчность не выведена у {len(немых)} порождающих — ПРИБОР НЕ ЗНАЕТ")
    print(f"ПЕРЕПИСЬ КОПИЙ {поза}: миров через слой {len(слой)}, сверх LAW={LAW} {len(сверх)} "
          ...
          f"(копий {sum(р[5] for р in ряд if not р[2])}) — не судятся"
          + (f"; ПАЛО ПО: {', '.join(почему)}" if почему else "") + f"; сайдкар {САЙДКАР}")
