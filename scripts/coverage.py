...
ПРОЧИЕ = ["algo", "formula", "physics", "cyber", "notation", "program",
          "statistics", "proof", "machine", "episode", "copula", "number", "sequence", "geometry", "linalg", "calendar", "speech", "rugram", "physlaw", "compsci", "case", "valence",
          "unit"]
...
def main():
    ...
    for мир in миры:
        ...
        # СУД, ЧИТАЮЩИЙ СЛОЙ, ДОЛЖЕН ПОЛУЧИТЬ СЛОЙ И ЗДЕСЬ. Суд формул
        # проходит дороги по рёбрам ВСЕГО файла, и прибор охвата,
        # звавший его построчно и без слоя, не увидел бы восьмидесяти
        # судимых строк — не оттого, что их не судят, а оттого, что
        # спросили не так, как судят.
        слои = {}
        for имя, суд in суды.items():
        ...
        with путь.open(encoding="utf-8", errors="replace") as поток:
            for строка in поток:
                ...
                видно = (
                    ...
                    or any(
                        (с.судить(строка, слой=слои[и]) if и in слои
                         else с.судить(строка))[0]
                        for и, с in суды.items())
                )
