...
def main():
    ...
    for i, (n, нов) in enumerate(точки_м):
        ...
        if i < len(точки_р):
            ...
            dр = 1000 * (новр - прежн[1]) / max(1, nр - прежн[0])
        строки.append(f"  доля {i + 1:2}: строк {n:7} — новых/1000: {dм:6.1f} | {sum(ds) / max(1, len(ds)):6.1f} | родов {dр:6.1f}")
    ...
    print(f"ПЕРЕПИСЬ ФОРМ ОТЧЁТ: строк {всего}, рамок {рамок} (масса ≥ {LAW2}: {купимых}), родов {родов} (масса ≥ {LAW2}: {купимых_р}); "
          ...
          f"насыщение: последняя доля даёт {1000 * хвост / max(1, хв_строк):.0f} новых рамок и "
