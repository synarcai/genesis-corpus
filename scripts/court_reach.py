...
def _проба():
    """ПРОБА НА ЗАВЕДОМО ИЗВЕСТНОМ: доказывает, что РАЗРЕЗ СВОДА ПОКРЫВАЕТ ЕГО ВЕСЬ.

        ЭТОТ ПРИБОР ОБЪЯВЛЯЕТ «0 ЛЖЕЙ, 0 СЛЕПЫХ» ПО ВСЕМУ СВОДУ, И ЧИСЛО ЭТО ЧЕСТНО РОВНО
        НАСТОЛЬКО, НАСКОЛЬКО ПОЛОН ЕГО ОБХОД. Ломоть, потерянный при разрезе, не роняет
        ничего: строки его просто не будут спрошены, и «0 лжей» станет правдой о меньшем
        своде, чем объявлено.

        ПРОПУЩЕННОЕ НЕ ЛЖЁТ — ОНО МОЛЧИТ, И ПОТОМУ ЕГО НЕ ВИДНО В ЧИСЛЕ ЛЖЕЙ.

    Проба режет мир из тысячи строк и требует, чтобы ломти покрыли [0, 1000) ровно один раз
    — без дыр и без нахлёста, — и чтобы нерезаный мир отдавался одним ломтем целиком.
    """
    import tempfile
    беды = []
    with tempfile.TemporaryDirectory() as д:
        ...
        мир.write_text("\n".join(f"строка {i}" for i in range(1000)) + "\n",
                       encoding="utf-8")
        мелко = panel.ломти([мир], 0.01)
        целиком = panel.ломти([мир], 0)
    if len(мелко) < 2:
        беды.append(f"мир не разрезан вовсе: ломтей {len(мелко)} — резать разучились")
    покрыто = []
    for _п, a, b, _ц in мелко:
        покрыто.append((a, 1000 if b is None else b))
    покрыто.sort()
    место = 0
    for a, b in покрыто:
        if a != место:
            беды.append(f"разрыв или нахлёст ломтей: ждали {место}, пришло {a}")
            break
        место = b
    if not беды and место != 1000:
        беды.append(f"ломти кончились на {место}, а строк 1000 — ХВОСТ МИРА НЕ СПРОШЕН")
    if len(целиком) != 1 or целиком[0][1] != 0 or целиком[0][2] not in (None, 1000):
        беды.append(f"нерезаный мир отдан не целиком: {[(a, b) for _п, a, b, _ц in целиком]}")
    return беды
...
def main(argv):
    ...
    if (беды := _проба()):
        ...
        print(f"ДОСЯГАЕМОСТЬ СУДА ОТКАЗ: полнота обхода не доказана ({len(беды)} бед)")
        return 2
