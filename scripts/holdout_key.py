...
import liarchain as LC  # noqa: E402 — цепь свидетельств длиннее кованой и на лицах, дом не ставивший
import adjorder as AO  # noqa: E402 — порядок прилагательных четвёркой и словами, дом не бравший
...
ДОМА_СЛЕДУЮЩЕГО = ДОМА_КАНДИДАТА + ("держания без глагола", "рамки сравнения", "сцена",
                                     ...
                                     "степени", "шкала", "акт",
                                     "цепь свидетельств", "порядок прилагательных")
...
def _взять(вон, язык, форма, с, порог=2, счёт=None, числом=True):
    ...
    ЧИСЛО В ВОПРОСЕ ЕСТЬ ЧАСТНЫЙ СЛУЧАЙ НОВИЗНЫ, А НЕ ЕЁ ОПРЕДЕЛЕНИЕ (16.09). Правило
    куплено домами, чьё удержание ЧИСЛОВОЕ: «сколько монет у Ани?» без числа есть одна и та
    же строка при всяком значении, и держать её незачем. Но есть дома, чьё удержание
    СТРОЕНИЕ, а не величина:

        цепь свидетельств  дом кует 2…4 звена, ключ спрашивает 5 и 6 — вопрос «говорит ли
                           Вера правду?» не несёт числа и НОВ целиком, ибо нова цепь;
        порядок прилагательных  дом кует пары и тройки, ключ спрашивает четвёрки.

        УДЕРЖАНИЕ ЕСТЬ НОВИЗНА ЗАДАЧИ, А НЕ НОВИЗНА ЧИСЛА В НЕЙ. Требовать числа от дома,
        который ничего не считает, значит требовать не новизны, а жанра.

    Новизна таких страниц держится не признаком вопроса, а УДЕРЖАННЫМ ПАРАМЕТРОМ дома — и
    проверяется тем же, чем числовая: рубрикой «0 прожитых строк» по всем мирам свода.
    """
    if not с:
        return счёт
    кус = _разрезать(с)
    if кус and (not числом or ЧИСЛО.search(кус[0])):
        вон.append((язык, форма, с))
        return (счёт or 0) + 1
    return счёт
...
# УДЕРЖАНИЕ ДВУХ ДОМОВ ВТОРОЙ СТУПЕНИ (16.09) — И ОНО СТРУКТУРНОЕ, А НЕ ЧИСЛОВОЕ.
#
#     ШКОЛА КУЁТ КОРОТКОЕ, КЛЮЧ СПРАШИВАЕТ ДЛИННОЕ — один и тот же закон, другая длина.
#
# Цепь свидетельств: дом кует 2, 3 и 4 звена (один, два и три свидетеля); ключ спрашивает
# ПЯТЬ И ШЕСТЬ. Лица берутся из объявленного ряда пакета, но ТОЛЬКО те, каких дом не ставит
# первыми, — чтобы и лицо было ново. Порядок прилагательных: дом кует пары и тройки; ключ
# спрашивает ЧЕТВЁРКИ, и слова берёт те, каких дом не взял («black», «tiny»).
#
# ОБА УДЕРЖАНИЯ ЕСТЬ ФУНКЦИЯ ДОМА, А НЕ ВТОРОЙ СПИСОК: длины и слова вычисляются из самих
# показов дома при всяком прогоне — два похожих списка разошлись бы молча в тот день, когда
# дом прибавит род.
ЦЕПЬ_ЗВЕНЬЕВ = (5, 6)
ПОРЯДОК_ДЛИНА = 4


def _свободные_лица(язык):
    """Лица объявленного ряда, каких дом не ставит первыми, — ново и лицо, и цепь."""
    занятые = {с.split()[0] for с, (я, _р) in LC.ПОКАЗЫ.items() if я == язык}
    return tuple(и for и in LC.ЛИЦА[язык] if и not in занятые)


def _цепь():
    """Chains longer than the house forges, on persons it never puts first."""
    вон = []
    for язык in LC.ЯЗЫКИ:
        свои = _свободные_лица(язык)
        if len(свои) < max(ЦЕПЬ_ЗВЕНЬЕВ):
            continue
        р = LC.РЕЧЬ[язык]
        for длина in ЦЕПЬ_ЗВЕНЬЕВ:
            for семя in range(4):
                лица = [свои[(семя + i) % len(свои)] for i in range(длина)]
                if len(set(лица)) != длина:
                    continue
                первый = семя % 2 == 0
                сказанное = [((семя >> i) & 1) == 1 for i in range(длина - 1)]
                правда = LC._истина(первый, сказанное)
                кого = длина - 1 if семя % 2 == 0 else длина // 2
                голова = (р["правдив"] if первый else р["лжив"]).format(X=лица[0]) + р["точка"]
                хвост = []
                ...
                проход = LC._разбор_прохода(язык, лица, правда, кого)
                с = " ".join([голова] + хвост + [вопрос]) + " " + ответ + р["двоеточие"] + проход + р["точка"]
                # РОД КЛЮЧА ЕСТЬ РОД ДОМА, А НЕ НОВОЕ ИМЯ: ключ спрашивает ТО ЖЕ ДЕЛО
                # другой длины. Имя рода сверяется с указателем родов, и выдуманное
                # «цепь5» было бы обещанием рода, какого дом не объявлял.
                _взять(вон, язык, "цепь", с, числом=False)
    return вон


def _порядок_слов():
    """Four adjectives at one noun — the house forges pairs and triples only.

    УДЕРЖИВАЕТСЯ ДЛИНА, А НЕ СЛОВО (16.09, прибор `scripts/key_support.py`). Первая
    редакция брала слова, каких дом не показывал вовсе («tiny», «black»), — и прибор
    опоры честно назвал это долгом: зачин «a tiny» не стои́т в своде ни разу.

        КЛЮЧ, СПРАШИВАЮЩИЙ О РОДЕ, КОТОРОГО В СВОДЕ НЕТ, МЕРИТ НЕ ЧИТАТЕЛЯ, А СВОД.
        Читатель не может знать слова, какого не видел; спросив о нём, мы измерим свою
        же дыру и назовём её его незнанием.

    Потому слова берутся ПОКАЗАННЫЕ, а ново — их ЧИСЛО: четыре при кованых двух и трёх.
    """
    вон = []
    показанные = set()
    for с in AO.ПОКАЗЫ:
        м = re.search(r"«a ([a-z ]+) (?:" + "|".join(AO.ВЕЩИ) + r")»", с)
        if м:
            показанные |= set(м.group(1).split())
    for i, вещь in enumerate(AO.ВЕЩИ):
        разряды = [р for р in AO.РАЗРЯДЫ]
        взятые = [разряды[(i + k) % len(разряды)] for k in range(ПОРЯДОК_ДЛИНА)]
        if len(set(взятые)) != ПОРЯДОК_ДЛИНА:
            continue
        тройка = sorted(взятые, key=lambda р: AO.МЕСТО[р])
        слова = []
        for р in тройка:
            свои = [с for с in AO.СЛОВА[р] if с in показанные] or list(AO.СЛОВА[р])
            слова.append(свои[i % len(свои)])
        if len(set(слова)) != ПОРЯДОК_ДЛИНА:
            continue
        верно = AO._запись(слова, вещь)
        неверно = AO._запись([слова[1], слова[0]] + слова[2:], вещь)
        # РОД «ВЫБОР» — ДВЕ ЗАПИСИ И ОДНА НАЗВАННАЯ ПАРА; число слов в записи роду не
        # принадлежит, и потому четвёрка есть тот же род при ином параметре.
        с = (f"«{верно}» or «{неверно}»? «{верно}»: {слова[0]} comes before {слова[1]}.")
        _взять(вон, "en", "выбор", с, числом=False)
    return вон
...
СОСТАВ_СЛЕД = СОСТАВ + (
    ...
    # ДВА ДОМА ВТОРОЙ СТУПЕНИ (16.09): удержание СТРУКТУРНОЕ — длина цепи и число
    # прилагательных, — а не числовое. Школа кует короткое, ключ спрашивает длинное.
    ("liarchain", _цепь), ("adjorder", _порядок_слов))
