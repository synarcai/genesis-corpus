...
# корпуса — падает лишь тогда, когда изменился ДОМ, стоявший в ключе релиза.
ДОМА_0_8_0 = ("svamp", "числовой ряд", "перекрёсток", "буквы")
ДОМА_СЛЕДУЮЩЕГО = ДОМА_0_8_0 + ("держания без глагола", "рамки сравнения")
# ЗАМОРОЗКА КЛЮЧА (05.09, holon: ключ восьмой точки есть МЕРА РЕЛИЗА 0.8.0): пока рядом лежит
...
def main(argv):
    ...
    ряд_след = ряд + _держания() + _сравнения()                      # состав следующей точки
    ...
    print(f"УДЕРЖАННЫЙ КЛЮЧ {поза}: вопросов {len(ряд)} (рубеж {ВОПРОСОВ_РУБЕЖ}), прожитых {прожитых} "
          f"(рубеж {ПРОЖИТЫХ_РУБЕЖ}); " + _числа(ряд) + f"; домов {len(ДОМА_0_8_0)}; "
