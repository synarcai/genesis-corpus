...
import enoughforms as EN  # noqa: E402 — решение о кошельке вне таблицы случаев — единица-отношение
...
def main(argv):
    ...
    ряд_след = (ряд + _держания() + _сравнения() + _сцена()          # состав следующей точки
                ...
                + _часы() + _мера() + _дата() + _час_словом() + _скорость()
                + _округление() + _порядок() + _квантор() + _римское() + _хватит())
