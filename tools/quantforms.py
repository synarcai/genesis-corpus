...
def _судить_образцом(строка):
    ...
    с = строка.strip()
    # ВЫХОД ПО НАБОРУ СНЯТ (21.09) — он и прикрывал 598 страниц из 648, которые закон
    # звал ложью, судя их числами ЧУЖОЙ рамки. НАБОР ГОВОРИТ, ЧТО ДОМ СТРАНИЦУ
    # НАПИСАЛ, А НЕ ЧТО ОНА ВЕРНА. Замер после уплаты: закон судит все 648 верно.
    # СПРАШИВАЮТСЯ ВСЕ СОВПАВШИЕ ОБРАЗЦЫ, А НЕ ПЕРВЫЙ (21.09). Образец здесь строится на
    # КАЖДЫЙ набор чисел — потому что форма цвета гнётся по счёту, — но сами числа стоя́т в
    # рамке ДЫРАМИ, и всякая страница совпадает с доброй дюжиной образцов сразу. Обход же
    # брал первый совпавший и судил страницу (5, 3, 2) числами рамки (3, 3, 0): 598 из 648
    # своих страниц закон звал ложью, и молчал об этом лишь потому, что выход по набору
    # стоял впереди него.
    #
    #     ОБРАЗЕЦ, СОВПАВШИЙ ПЕРВЫМ, НЕ ЕСТЬ ОБРАЗЕЦ ВЕРНЫЙ, КОГДА ОБРАЗЦЫ РАЗЛИЧАЮТСЯ НЕ
    #     РАМКОЙ, А ЧИСЛАМИ, КОТОРЫЕ РАМКА ПРЯЧЕТ В ДЫРЫ. Суд обязан спросить ВСЕХ
    #     совпавших и назвать страницу ложью лишь тогда, когда СВОЕЙ её не признал никто.
    #
    # Строгости это не убавляет: страница с чужой формой цвета не совпадёт с образцом
    # своего числа, а с образцом чужого совпадёт и будет отвергнута по числам — то есть
    # отвергнута всеми.
    совпал = False
