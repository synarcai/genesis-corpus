#!/usr/bin/env python3
"""ДОМ ПРОТИВОПОЛОЖНОСТЕЙ — отношение, которого во всём своде не было ни одного.

Собеседник знает не только что вещь ЕСТЬ (дом определений) и сколько её (дом
природы), но и чему она ПРОТИВОПОЛОЖНА. Это первое в корпусе отношение
СЛОВА К СЛОВУ, а не слова к вещи, и потому оно ставится особо.

ЗАКОН, КОТОРЫМ ЭТО СУДИТСЯ, — СИММЕТРИЯ. Противоположность есть отношение
взаимное: если большой противоположен маленькому, то маленький противоположен
большому, и обе стороны стоят в показах ОБЕ. Дом проверяет себя этим же
законом: пара, объявленная в одну сторону, обязана дать показ в обе, и суд
ловит подмену второй стороны так же, как подмену первой.

ФОРМА ЦИТАТНАЯ, И ЭТО РЕШЕНИЕ, А НЕ УДОБСТВО. «Что противоположно БОЛЬШОМУ?»
требует дательного падежа, «противоположность большого» — родительного, и
всякий язык потребовал бы двух-трёх форм на слово. Цитатная рамка («что
противоположно слову большой?») берёт слово в словарной форме, и дом обходится
ОДНОЙ формой на слово в каждом языке — то есть объявляет ровно то, что знает,
и не выдумывает падежей, которых не проверял.

    python3 tools/oppositeforms.py    # самопроверка с мутантами
"""
import re

# пары объявлены в словарной форме каждого языка
ПАРЫ = {
    "ru": (("большой", "маленький"), ("длинный", "короткий"), ("высокий", "низкий"),
           ("тёплый", "холодный"), ("быстрый", "медленный"), ("светлый", "тёмный"),
           ("новый", "старый"), ("полный", "пустой"), ("лёгкий", "тяжёлый"),
           ("сухой", "мокрый"), ("громкий", "тихий"), ("сильный", "слабый")),
    "en": (("big", "small"), ("long", "short"), ("high", "low"),
           ("warm", "cold"), ("fast", "slow"), ("light", "dark"),
           ("new", "old"), ("full", "empty"), ("easy", "hard"),
           ("dry", "wet"), ("loud", "quiet"), ("strong", "weak")),
    "de": (("groß", "klein"), ("lang", "kurz"), ("hoch", "niedrig"),
           ("warm", "kalt"), ("schnell", "langsam"), ("hell", "dunkel"),
           ("neu", "alt"), ("voll", "leer"), ("leicht", "schwer"),
           ("trocken", "nass"), ("laut", "leise"), ("stark", "schwach")),
    "fr": (("grand", "petit"), ("long", "court"), ("haut", "bas"),
           ("chaud", "froid"), ("rapide", "lent"), ("clair", "sombre"),
           ("neuf", "vieux"), ("plein", "vide"), ("facile", "difficile"),
           ("sec", "mouillé"), ("fort", "faible"), ("propre", "sale")),
    "es": (("grande", "pequeño"), ("largo", "corto"), ("alto", "bajo"),
           ("caliente", "frío"), ("rápido", "lento"), ("claro", "oscuro"),
           ("nuevo", "viejo"), ("lleno", "vacío"), ("fácil", "difícil"),
           ("seco", "mojado"), ("fuerte", "débil"), ("limpio", "sucio")),
    "it": (("grande", "piccolo"), ("lungo", "corto"), ("alto", "basso"),
           ("caldo", "freddo"), ("veloce", "lento"), ("chiaro", "scuro"),
           ("nuovo", "vecchio"), ("pieno", "vuoto"), ("facile", "difficile"),
           ("asciutto", "bagnato"), ("forte", "debole"), ("pulito", "sporco")),
    "pt": (("grande", "pequeno"), ("longo", "curto"), ("alto", "baixo"),
           ("quente", "frio"), ("rápido", "lento"), ("claro", "escuro"),
           ("novo", "velho"), ("cheio", "vazio"), ("fácil", "difícil"),
           ("seco", "molhado"), ("forte", "fraco"), ("limpo", "sujo")),
    "nl": (("groot", "klein"), ("lang", "kort"), ("hoog", "laag"),
           ("warm", "koud"), ("snel", "langzaam"), ("licht", "donker"),
           ("nieuw", "oud"), ("vol", "leeg"), ("makkelijk", "moeilijk"),
           ("droog", "nat"), ("luid", "stil"), ("sterk", "zwak")),
    "pl": (("duży", "mały"), ("długi", "krótki"), ("wysoki", "niski"),
           ("ciepły", "zimny"), ("szybki", "wolny"), ("jasny", "ciemny"),
           ("nowy", "stary"), ("pełny", "pusty"), ("łatwy", "trudny"),
           ("suchy", "mokry"), ("głośny", "cichy"), ("silny", "słaby")),
}
РАМКИ = {
    "ru": ("{a} и {b} — противоположности.", "что противоположно слову {a}? {b}.", "какое слово противоположно слову «{a}»? {b}."),
    "en": ("{a} and {b} are opposites.", "what is the opposite of {a}? {b}.", 'what is the opposite of "{a}"? {b}.'),
    "de": ("{a} und {b} sind Gegensätze.", "was ist das Gegenteil von {a}? {b}.", "was ist das Gegenteil von „{a}“? {b}."),
    "fr": ("{a} et {b} sont des contraires.", "quel est le contraire de {a} ? {b}.", "quel est le contraire de « {a} » ? {b}."),
    "es": ("{a} y {b} son contrarios.", "¿cuál es el contrario de {a}? {b}.", "¿cuál es el contrario de «{a}»? {b}."),
    "it": ("{a} e {b} sono contrari.", "qual è il contrario di {a}? {b}.", "qual è il contrario di «{a}»? {b}."),
    "pt": ("{a} e {b} são contrários.", "qual é o contrário de {a}? {b}.", "qual é o contrário de «{a}»? {b}."),
    "nl": ("{a} en {b} zijn tegenstellingen.", "wat is het tegenovergestelde van {a}? {b}.", 'wat is het tegenovergestelde van "{a}"? {b}.'),
    "pl": ("{a} i {b} to przeciwieństwa.", "jakie jest przeciwieństwo słowa {a}? {b}.", "jakie jest przeciwieństwo słowa „{a}”? {b}."),
}
# ВЫВОД ИЗ ОТНОШЕНИЯ — то, чего в доме не было: отношение стояло НАЗВАННЫМ, но
# из него ничего не следовало. Пара противоположна ⇒ обе стороны вместе не
# держатся, и это первый в корпусе вывод, взятый не из числа и не из класса, а
# из отношения СЛОВА К СЛОВУ. Основание стоит ПЕРВЫМ, следствие — после вопроса
# (М-263: ответ, не называющий основания, есть круг).
#
# ФОРМА ЦИТАТНАЯ ПО ТОЙ ЖЕ ПРИЧИНЕ, ЧТО И ВЕСЬ ДОМ, и причина тут строже. «Может
# ли что-то быть большим и маленьким?» потребовало бы творительного падежа в
# русском, среднего рода в согласовании и носителя, к которому прилагательное
# приложимо (дом бывает полным и пустым, но не быстрым и медленным). Рамка «X и
# Y не бывают вместе» говорит о СЛОВАХ в словарной форме — и потому годится для
# всех двенадцати пар без единого исключения и без единого выдуманного падежа.
ВЫВОД = {
    "ru": ("что из этого следует?", "{a} и {b} не бывают вместе."),
    "en": ("what follows from this?", "{a} and {b} do not go together."),
    "de": ("was folgt daraus?", "{a} und {b} kommen nicht zusammen vor."),
    "fr": ("qu'est-ce qui en découle ?", "{a} et {b} ne vont pas ensemble."),
    "es": ("¿qué se sigue de esto?", "{a} y {b} no van juntos."),
    "it": ("che cosa ne segue?", "{a} e {b} non stanno insieme."),
    "pt": ("o que se segue disso?", "{a} e {b} não andam juntos."),
    "nl": ("wat volgt hieruit?", "{a} en {b} gaan niet samen."),
    "pl": ("co z tego wynika?", "{a} i {b} nie idą razem."),
}
ЯЗЫКИ = ПАРЫ
ФОРМЫ = ("утв", "утв_воп", "воп", "воп_кав", "воп_обратно", "вывод")

for _яз, _пары in ПАРЫ.items():
    assert _яз in РАМКИ, _яз
    assert len(_пары) == len(ПАРЫ["ru"]), (_яз, len(_пары))
    _слова = [с for п in _пары for с in п]
    assert len(_слова) == len(set(_слова)), (_яз, "слово стоит в двух парах")


def показ(язык, форма, i):
    утв, воп, кав = РАМКИ[язык]
    a, b = ПАРЫ[язык][i % len(ПАРЫ[язык])]
    if форма == "воп_кав":
        # СЛОВО В КАВЫЧКАХ ЯЗЫКА (третья полоса беседы, 05.09): человек, называя
        # слово, берёт его в кавычки — «какое слово противоположно слову
        # «большой»?» — и кавычки у каждого языка свои («», "", „“, « »).
        return кав.format(a=a, b=b)
    if форма == "утв":
        return утв.format(a=a, b=b)
    if форма == "утв_воп":
        # УТВЕРЖДЕНИЕ И ЕГО ВОПРОС ОДНОЙ СТРАНИЦЕЙ — тот же ход, каким дом
        # счётных единиц связал парадигму с вопросом о ней: род, чей факт
        # спрошен рядом, не нем, и мера широты вопроса читает его вопросным.
        return f"{утв.format(a=a, b=b)} {воп.format(a=a, b=b)}"
    if форма == "воп":
        return воп.format(a=a, b=b)
    if форма == "вывод":
        вопрос, следствие = ВЫВОД[язык]
        return f"{утв.format(a=a, b=b)} {вопрос} {следствие.format(a=a, b=b)}"
    # СИММЕТРИЯ СПРОШЕНА ОБЕИМИ СТОРОНАМИ: отношение взаимно, и корпус,
    # показавший одну сторону, учил бы половине закона.
    return воп.format(a=b, b=a)


def _все_показы():
    вон = {}
    for язык in ПАРЫ:
        for форма in ФОРМЫ:
            for i in range(len(ПАРЫ[язык])):
                вон[показ(язык, форма, i)] = (язык, форма)
    return вон


ПОКАЗЫ = _все_показы()


# РАМКА С ЧУЖИМ СЛОВОМ — ЛОЖЬ, А НЕ МОЛЧАНИЕ: рамка объявлена домом, и пара в
# ней объявлена тоже; строка той же рамки с иной парой отрицает объявленное.
#
# ОБРАЗЕЦ СОБРАН ИЗ ОБЪЯВЛЕННОГО ЦЕЛИКОМ, а не по началу строки (М-180-f2):
# рамка экранируется вся, а дыры связываются СПИСКОМ СЛОВ ЭТОГО ЯЗЫКА. Суд по
# началу претендовал бы на всякую строку, начавшуюся тем же словом; суд по
# полному образцу с занятыми дырами читает ровно рамку этого дома и молчит на
# чужой строке — а это и есть цена права называть строку ложной.
def _образцы():
    вон = []
    for язык, (утв, воп, кав) in РАМКИ.items():
        слова_узор = "|".join(sorted((re.escape(w) for п in ПАРЫ[язык] for w in п),
                                     key=lambda с: (-len(с), с)))
        вопрос, следствие = ВЫВОД[язык]
        for рамка in (утв, воп, кав, f"{утв} {вопрос} {следствие}", f"{утв} {воп}"):
            # ДЫРЫ ИМЕНУЮТСЯ, ЧТОБЫ ИХ МОЖНО БЫЛО СПРОСИТЬ (21.09). Прежде обе дыры были
            # безымянны, и образец умел сказать лишь «строка этой рамки» — а истиной дом
            # звал одно лишь членство в наборе показов.
            #
            #     СУД, У КОТОРОГО ИСТИНА ЕСТЬ ОДНО ЛИШЬ ЧЛЕНСТВО В НАБОРЕ, НЕ ЗАКОН, А
            #     СПИСОК: он не умеет сказать, ПОЧЕМУ страница верна, и потому не заметит
            #     подмены, вписанной в список.
            #
            # Дом объявляет ПАРЫ противоположностей — ими он и судит: слова из дыр обязаны
            # составить объявленную пару, в любом её порядке (обратный порядок есть свой
            # род показа, «воп_обратно», и он столь же верен).
            # ДЫРА ПОВТОРЯЕТСЯ — И КАЖДОЕ ЕЁ МЕСТО ПОЛУЧАЕТ СВОЁ ИМЯ, а не обратную
            # ссылку: строка с ОДНИМ подменённым вхождением должна быть ЛОЖЬЮ, а обратная
            # ссылка сделала бы её немой, отдав воротам то, что обязан ловить суд.
            узор, счёт = re.escape(рамка), {"a": 0, "b": 0}
            for имя in ("a", "b"):
                мета = re.escape("{%s}" % имя)
                while мета in узор:
                    счёт[имя] += 1
                    узор = узор.replace(мета, f"(?P<{имя}{счёт[имя]}>{слова_узор})", 1)
            вон.append((re.compile(узор), язык))
    return tuple(вон)


_ОБРАЗЦЫ = _образцы()



# ПАРЫ ЯЗЫКА КАК МНОЖЕСТВА — чтобы спрашивать их в любом порядке
_ПАРЫ_ЯЗЫКА = {язык: frozenset(frozenset(п) for п in пары) for язык, пары in ПАРЫ.items()}


def судить(строка):
    с = строка.strip()
    совпал = False
    for образец, язык in _ОБРАЗЦЫ:
        м = образец.fullmatch(с)
        if not м:
            continue
        совпал = True
        зн = м.groupdict()
        первые = {и: з for и, з in зн.items() if и.startswith("a")}
        вторые = {и: з for и, з in зн.items() if и.startswith("b")}
        if not первые or not вторые:
            continue                      # рамка без обеих дыр не судима этим законом
        # ВСЯКОЕ МЕСТО СЛОВА СВЕРЯЕТСЯ, А НЕ ПЕРВОЕ: одно слово во всех своих местах
        if len(set(первые.values())) != 1 or len(set(вторые.values())) != 1:
            continue
        a, b = next(iter(первые.values())), next(iter(вторые.values()))
        if a != b and frozenset((a, b)) in _ПАРЫ_ЯЗЫКА[язык]:
            return True, True
    return (True, False) if совпал else (False, False)


def _самопроверка():
    мутанты = 0
    for язык in ПАРЫ:
        for форма in ФОРМЫ:
            с = показ(язык, форма, 0)
            assert судить(с) == (True, True), (язык, форма, с)
        # МУТАНТ: пара, сцепленная накрест («что противоположно большому? короткий»)
        утв, воп, _кав = РАМКИ[язык]
        a = ПАРЫ[язык][0][0]
        чужой = ПАРЫ[язык][1][1]
        битая = воп.format(a=a, b=чужой)
        assert судить(битая) == (True, False), (язык, битая)
        мутанты += 1
        # МУТАНТ ВЫВОДА: основание верное, следствие о ЧУЖОЙ паре. Именно этот
        # мутант отличает дом, который учит выводу, от дома, который учит
        # заклинанию: строка звучит как вывод и не следует из своего основания.
        вопрос, следствие = ВЫВОД[язык]
        b = ПАРЫ[язык][0][1]
        подмена = f"{утв.format(a=a, b=b)} {вопрос} {следствие.format(a=a, b=чужой)}"
        assert судить(подмена) == (True, False), (язык, подмена)
        мутанты += 1
    for язык in ("ru", "en", "de", "pl"):
        print("  ", показ(язык, "воп", 0), "|", показ(язык, "воп_обратно", 0))
    for язык in ("ru", "de", "pl"):
        print("  ", показ(язык, "вывод", 4))
    print(f"  мутантов поймано: {мутанты}")
    print(f"  дом пишет показов: {len(ПОКАЗЫ)} (языков {len(ПАРЫ)}, пар {len(ПАРЫ['ru'])}, форм {len(ФОРМЫ)})")


if __name__ == "__main__":
    _самопроверка()
