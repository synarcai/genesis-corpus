...
def _судить_образцом(строка):
    ...
    #
    # Замерено: закон судит все 462 показов дома верно и без набора — выход был
    ...
    for образ, язык, форма, порядок in ОБРАЗЦЫ:
        ...
        if not м:
            continue
        # ВСЕ ОДНОИМЁННЫЕ ДЫРЫ ОБЯЗАНЫ НЕСТИ ОДНО, И ЭТО ПРОВЕРЯЕТСЯ ПРЕЖДЕ ВСЕГО (21.09).
        # Порядок дыр рамки «мера» есть ['A','Гп','ПР','n','ЕДn','Гп','A','ПР','n','ЕДn'] —
        # имена ПОВТОРЯЮТСЯ, ибо страница говорит факт и тут же отвечает им на вопрос. Суд
        # же складывал их в словарь, а словарь при повторе имени держит ПОСЛЕДНЕЕ: первое
        # вхождение — сам факт — терялось бесследно, и «der Frosch sprang 5 Zentimeter. wie
        # weit sprang der Frosch? 12 Zentimeter.» звалось ИСТИНОЙ.
        #
        #     ИМЯ, ПОВТОРЁННОЕ В СЛОВАРЕ, ТЕРЯЕТ ВСЁ, КРОМЕ ПОСЛЕДНЕГО. Суд, читающий дыры
        #     словарём, видит одно место из двух — и подмена в первом проходит бесследно.
        #
        # Нашёл это прибор подсадки, когда его порча научилась брать слово с того же места
        # СОСЕДНЕЙ страницы: рамка цела, число чужое — ровно то, чего словарь не видел.
        г = {}
        for имя, значение in zip([п for п in порядок if п not in ("ПР", "ДЕ")], м.groups()):
            if г.setdefault(имя, значение) != значение:
                return True, False
        if форма in ("мера", "сумма"):
        ...
        if (г["Нn"], г["Нk"], г["Нr"]) != (счётная(язык, ряд, n), счётная(язык, ряд, k), счётная(язык, ряд, r)):
            return True, False
        # ЗНАК ЛЕДЖЕРА ЕСТЬ ЗНАК ХОДА, И ОН СУДИТСЯ. Дыра «знак» стояла в образце с самого
        # начала, а вердикт считал по ИМЕНИ ФОРМЫ и напечатанный знак пропускал: «43 − 21 = 64»
        # звалось истиной. Дыра без проверки не слепа — она БЛАГОСЛОВЛЯЕТ порчу (М-489).
        if г.get("знак") != ("+" if форма == "прибыль" else "−"):
            return True, False
