...
# «(язык, свойство)», и пять свойств свои называл, — но указатель родов читает `РОДЫ`,
# и 200 строк мира числились безымянными при доме, знавшем имя каждой.
#
#     СЛОВАРЬ СТРАНИЦ ЕСТЬ ОТВЕТ, А ОБЪЯВЛЕНИЕ — ВОПРОС, НА КОТОРЫЙ ОН ОТВЕЧАЕТ.
#
# СВОЙСТВО И ЕСТЬ РОД: тяжесть сравнивается весом, жар — градусом, рост — ростом, и
# сравнительная степень у каждого своя.
РОД_В_ПОКАЗЕ = 1

РОДЫ = tuple(sorted({з[1] for з in ПОКАЗЫ.values()}))

ЗАЧЕМ_РОДА = {
    "тяжесть": "тяжелее/легче: сравнение по весу",
    "твёрдость": "твёрже/мягче: сравнение по твёрдости",
    "жар": "горячее/холоднее: сравнение по жару",
    "скорость": "быстрее/медленнее: сравнение по скорости",
    "рост": "выше/ниже: сравнение по росту",
}
