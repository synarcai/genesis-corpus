...
# РОД, ЖИВУЩИЙ ВНЕ СПИСКА, ВЫПАДАЕТ И ИЗ ОБЪЯВЛЕНИЯ, СОБРАННОГО ПО СПИСКУ (15.09). Строка
# выше собрана по `ФОРМЫ_ВЫ`, а `уверен_вы` рождается не оттуда: он идёт своей рукой через
# `ВЫ_УВЕРЕН`, и оттого объявление его не покрыло. Причина дыры у него ровно та же — «you»
# служит обоим регистрам, — но объявление, собранное обходом одного списка, покрывает
# только этот список.
#
#     ОБЪЯВЛЕНИЕ, СОБРАННОЕ ПО СПИСКУ, ЕСТЬ ОБЪЯВЛЕНИЕ О СПИСКЕ, А НЕ О ДОМЕ. Оно тем и
#     опасно, что выглядит полным: единственный род, заведённый в обход списка, молча
#     выпадает — и ни один читатель не отличит его от небрежности.
ОБЪЯВЛЕННЫЕ_ПРОПУСКИ["уверен_вы"] = БЕЗ_РАЗЛИЧИЯ_РЕГИСТРА
...
# МНОГИХ местах, и звать закон при каждом значило бы завести столько же читателей;
# набор показов есть ЕДИНСТВЕННОЕ место, через которое проходит всякая страница дома.
#
#     ЗАКОН, КАСАЮЩИЙСЯ ВСЯКОЙ СТРАНИЦЫ, ЗОВЁТСЯ ТАМ, ГДЕ ВСЯКАЯ СТРАНИЦА ПРОХОДИТ.
#
# Суд дома судит ПРИНАДЛЕЖНОСТЬЮ к этому набору, и потому страница и суд остаются
# в согласии сами собою.
...
# КЛЮЧ «счёт_да» ЕСТЬ ПОВЕРХНОСТЬ, А НЕ РОД: страницы его пишутся своим ходом и метятся
# именем «счёт_словами». ТАБЛИЦА РАМОК И СПИСОК РОДОВ СУТЬ РАЗНЫЕ ВЕЩИ.
РОДЫ = (tuple(ф for ф in ФОРМЫ if ф != "счёт_да")
        + tuple(ф + "_вы" for ф in ФОРМЫ_ВЫ)
        + ("уверен", "уверен_вы", "счёт_словами"))    # три рода вне обоих рядов
...
def _самопроверка():
    ...
    for язык in ЯЗЫКИ:
        for форма in ФОРМЫ:
            ...
            # ПРОВЕРКА ИДЁТ ТЕМ ЖЕ ПУТЁМ, ЧТО И ДЕЛО (09.09): показы мира проходят французскую
            # элизию (`ПОКАЗЫ` строится через `_fr.элизия`), а самопроверка звала рамку НАПРЯМУЮ
            # — и «parce que Anne» падало у неё, живя в мире как «parce qu'Anne».
            #
        ...
        целая = страница(язык, "почему", i0, t0, n0, k0)
        if язык == "fr":
            целая = _fr.элизия(целая)     # тем же путём, что и дело
        битая = re.sub(r"= (\d+)\.$", lambda м: f"= {int(м.group(1)) + 1}.", целая)
        # ПОРЧА ИТОГА — ЛОЖЬ ДОМА, А НЕ ЧУЖАЯ СТРОКА (05.09, закон цифрового скелета)
...
def судить(строка):
    ...
    if вердикт[0] is False and _зк.ложь_по_знаку(
        ...
        return True, False
    # РОД ПРОШЕДШЕГО — ТОТ ЖЕ ЗАКОН: «Анна отдал 2 шара» есть показ дома с испорченным родом
    if вердикт[0] is False and _зк.ложь_по_роду(строка, ПОКАЗЫ, _РОДЫ_ЛИЦ):
        return True, False
    return вердикт
