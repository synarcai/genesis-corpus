#!/usr/bin/env python3
"""GENESIS layer: HOW A MACHINE HOLDS A NUMBER.

    13 in binary is 1101.        13 in hex is d.
    13 and 6 is 4.               13 or 6 is 15.
    13 xor 6 is 11.              13 shifted left by 2 is 52.
    1101 in binary is 13 in decimal.
    a byte holds 8 bits and 256 values.

A programmer meets the same number in several dresses and must know
they are one number. Bases and bit operations are the most exactly
checkable knowledge there is — and the corpus had NOT ONE show of
either: no hex, no bitwise and/or/xor, no shifts.

WHAT IS SHOWN AND WHY EACH:
  · BASES two, eight and sixteen, in both directions — writing a
    number in a base and reading it back — because a base is a
    translation, and a translation shown one way teaches a table;
  · AND, OR, XOR as operations on the SAME pair, so their difference
    is met rather than described: the same 13 and 6 give 4, 15 and 11;
  · SHIFTS, with their arithmetic meaning stated beside them («shifted
    left by 2 is 52» beside «13 × 4 = 52»), because a shift that is
    not tied to multiplication is a trick instead of a fact;
  · WIDTH — what a byte holds — since every bound a programmer meets
    later stands on it.

NEGATIVE NUMBERS ARE ABSENT DELIBERATELY. Two's complement needs a
declared width, and the same bits mean different numbers at different
widths: shown without the width, it is not a fact but a coincidence.
It waits for a layer that declares width in every show.
"""

import bilang  # noqa: E402 — язык показа по азбуке
import pathlib
import sys

sys.path.insert(0, str(pathlib.Path(__file__).resolve().parent))
import json  # noqa: E402

from langpack import count_form_index  # noqa: E402
import rugram  # noqa: E402 — счётная форма объявлена пакетом
from layer import Сбор, emit  # noqa: E402
import mass  # noqa: E402
from plural import by_count  # noqa: E402

# ПУТЬ, СКАЗАННЫЙ ТОЛЬКО В ЗОВЕ, ЕСТЬ ПУТЬ, О КОТОРОМ НЕ ОБЪЯВЛЕНО (14.09): указатель
# читает объявление СТРОКОЙ ВЕРХНЕГО УРОВНЯ, и мир, названный лишь внутри `emit`,
# остаётся не связанным ни с одним домом.
ЦЕЛЬ = "datasets/genesis_machine.txt"

RU_PACK = json.loads(
    (pathlib.Path(__file__).resolve().parent
     / "langpacks/ru.json").read_text(encoding="utf-8"))
RU_RULE = {"forms": ["one", "few", "many"],
           "count_agreement": RU_PACK["count_agreement"]}
# ПРАВИЛО ЧИСЛА ЧИТАЕТСЯ ИЗ ПАКЕТА, а не переписывается: «4 бит» ложно,
# «4 бита» верно, и знает это описание русского языка.
#
# И ТАБЛИЦА, СТОЯВШАЯ ЗДЕСЬ ДО 12.09, БЫЛА ИМЕННО ПЕРЕПИСЫВАНИЕМ: она объявляла
# «бит» третьей формой там, где пакет говорит «битов», — и восемь строк этого мира
# расходились с тремястами двадцатью мира `compsci`, читавшего пакет честно.
#
#     ДОМ, НАПИСАВШИЙ В ЗАГОЛОВКЕ «ЧИТАЕТСЯ ИЗ ПАКЕТА» И ОБЪЯВИВШИЙ СВОЮ ТАБЛИЦУ,
#     ЧИТАЕТ НЕ ПАКЕТ, А СЕБЯ.


def ру(слово, k):
    return rugram.форма(слово, k)

# МАССА ОТ ПРАВИЛА (tools/mass.py, М-148): пара — из двух взаимно простых
# циклов шагом k; различных показов на рамку до 77 (было 10–16).
ЧИСЛА = [13, 6, 25, 40, 7, 100, 31, 64, 18, 255, 12, 9, 200, 33, 17, 48,
         5, 77, 128, 3, 150, 42, 99]
МЕНЬШИЕ = [6, 10, 3, 9, 15, 7, 16]
РАЗНОСТИ = [7, 2, 4, 16, 45, 1, 15, 64, 10, 42, 5]
СДВИГАЕМЫЕ = [13, 5, 7, 3, 9, 11, 6, 25, 2, 17, 4]
НА_СКОЛЬКО = [2, 3, 1, 4, 5]
ШИРИНА = 12
ШИРИНЫ = [(4, "nibble"), (8, "byte"), (16, "halfword"), (32, "word")]

# ИСКОМОЕ ОБЪЯВЛЯЕТ СВОЙ ВОПРОС ОДИН РАЗ, и вопрос берёт ТУ ЖЕ фразу
# предмета, какую берёт ответ. Замер вопросной поверхности назвал этот
# мир немым: 1200 строк, вопросов ноль — он сообщал, что 5 в двоичной
# записи это 101, и ни разу не спрашивал, чему равно 5 в двоичной.
# ИМЯ ЗАПИСИ В ОБРАТНОЙ РАМКЕ ОБЪЯВЛЕНО, А НЕ ВЫВЕДЕНО ИЗ ПРЯМОЙ. «в двоичной записи» и
# «двоичное 1101» суть разные падежи и разные роды: прилагательное согласуется с
# подразумеваемым словом («в записи» — женский предложный, «число» — средний именительный),
# и правило это есть грамматика, а не склейка строк.
ИМЯ_ЗАПИСИ = {"двоичной": "двоичное", "шестнадцатеричной": "шестнадцатеричное"}

СПРОСИТЬ = {
    "value": "what is {предмет}?",
    "значение": "чему равно {предмет}?",
}

# ФОРМУЛЫ РОДОВ — ЗАКОН ОТВЕТА ОТ ВЕЛИЧИН ВОПРОСА, объявлен при каждом вопросе
# (таблица родов declarations/GENERA.json — эталон суда охвата, holon 03.09).
ФОРМУЛЫ = {
    "value": "запись n в основании 2/8/16 | обратно в десятичную",
    "значение": "запись n в основании 2/16",
}
assert set(ФОРМУЛЫ) == set(СПРОСИТЬ), "формула у каждого вопроса"


def спросить(искомое, предмет, ответ):
    """Вопрос о предмете и ответ о нём же — одной строкой."""
    return f"{СПРОСИТЬ[искомое].format(предмет=предмет)} {ответ}"


# ПЯТЬ РОДОВ НАЗВАНЫ РАЗДЕЛАМИ «# ---» И НЕ ВЫШЛИ НАРУЖУ.
# ДОМ САМ НАЗВАЛ ДВА РОДА ТАМ, ГДЕ ПЕРВАЯ РЕДАКЦИЯ ДАЛА ОДИН («ДВА РОДА — ДВЕ РАМКИ», см.
# комментарий в теле): «100 in octal is 144» и «octal 144 is 100 in decimal» суть ДВА ДЕЛА —
# записать число в основании и прочесть запись обратно. А три битовые операции суть три
# разные функции: их показывают на ОДНОЙ паре, чтобы различие встретилось, но учатся они
# порознь, и род у каждой свой.
ЧИСЛО_В_ЗАПИСЬ = "число → запись в основании"
ЗАПИСЬ_В_ЧИСЛО = "запись в основании → десятичное число"
И = "побитовое И на паре"
ИЛИ = "побитовое ИЛИ на той же паре"
ИСКЛЮЧАЮЩЕЕ = "исключающее ИЛИ на той же паре"
СДВИГ_ВЛЕВО = "сдвиг влево, и рядом с ним его умножение"
СДВИГ_ВПРАВО = "сдвиг вправо: дело обратное"
РАВЕНСТВО_СДВИГА = "равенство сдвига и умножения: s × 2ᵏ, сказанное арифметикой"
ШИРИНА_СЛОВА = "ширина слова: сколько бит и сколько значений"
ОТКАЗ_ЦИФРЫ = "цифры от двух до девяти в двоичной не живут, и основание вычислимо"


def pass_shows(pass_i):
    out = Сбор()
    for i in range(ШИРИНА):
        k0 = mass.шаг(pass_i, i, ШИРИНА)
        n = ЧИСЛА[k0 % len(ЧИСЛА)]
        b, d = mass.пара(k0, МЕНЬШИЕ, РАЗНОСТИ)
        a = b + d
        s, k = mass.пара(k0, СДВИГАЕМЫЕ, НА_СКОЛЬКО)
        бит, имя = ШИРИНЫ[k0 % len(ШИРИНЫ)]
        # --- основания, в обе стороны
        for основание, запись in (("binary", f"{n:b}"),
                                  ("octal", f"{n:o}"),
                                  ("hex", f"{n:x}")):
            пред = f"{n} in {основание}"
            утв = f"{пред} is {запись}."
            out.род = ЧИСЛО_В_ЗАПИСЬ
            out.append(утв)
            out.append(спросить("value", пред, утв))
            # ДВА РОДА — ДВЕ РАМКИ (holon 03.09: «100 in octal is 144» и «37 in
            # octal is 31 in decimal» делили одно начало «# in octal is #», и
            # ни один исполнитель не воспроизводил обе строки). Обратное
            # направление открывается основанием: «octal 144 is 100 in decimal».
            обр = f"{основание} {запись}"
            утв_обр = f"{обр} is {n} in decimal."
            out.род = ЗАПИСЬ_В_ЧИСЛО
            out.append(утв_обр)
            # ВОПРОС БЕЗ ЧИСЛА НЕ СВЯЗАТЬ С ОТВЕТОМ, И ПОТОМУ ОН НЕ
            # ЗАДАЁТСЯ. Связь половин пары держится величинами (общий
            # дом `tools/asking.py`), а шестнадцатеричное «ff» цифр не
            # содержит вовсе: вопрос «что такое ff в шестнадцатеричной?»
            # не проверить ничем, и он был бы поверхностью лишь на вид.
            if any(з.isdigit() for з in запись):
                out.append(спросить("value", f"{обр} in decimal", утв_обр))
        for имя_ru, запись in (("двоичной", f"{n:b}"),
                               ("шестнадцатеричной", f"{n:x}")):
            out.род = ЧИСЛО_В_ЗАПИСЬ
            пред = f"{n} в {имя_ru} записи"
            утв = f"{пред} это {запись}."
            out.append(утв)
            out.append(спросить("значение", пред, утв))
            # ОБРАТНОЕ ДЕЛО ГОВОРИЛОСЬ ТОЛЬКО ПО-АНГЛИЙСКИ (15.09), и это был долг, а не
            # граница: русская рамка прямого дела стоя́ла строкою выше, и недоставало ей
            # зеркала. Направление здесь открывается ОСНОВАНИЕМ, как и у английской пары:
            # «двоичное 1101 это 13 в десятичной» начинается основанием, а не числом.
            out.род = ЗАПИСЬ_В_ЧИСЛО
            обр_ru = f"{ИМЯ_ЗАПИСИ[имя_ru]} {запись}"
            утв_обр_ru = f"{обр_ru} это {n} в десятичной."
            out.append(утв_обр_ru)
            if any(з.isdigit() for з in запись):
                out.append(спросить("значение", f"{обр_ru} в десятичной", утв_обр_ru))
        # --- три операции на ОДНОЙ паре, чтобы различие встретилось
        for слово, значение, род_оп in (("and", a & b, И), ("or", a | b, ИЛИ),
                                        ("xor", a ^ b, ИСКЛЮЧАЮЩЕЕ)):
            out.род = род_оп
            пред = f"{a} {слово} {b}"
            утв = f"{пред} is {значение}."
            out.append(утв)
            out.append(спросить("value", пред, утв))
        # ИСКЛЮЧАЮЩЕЕ ИЛИ ЗВУЧАЛО ТОЛЬКО ПО-АНГЛИЙСКИ, тогда как И с ИЛИ говорили обоими
        # языками: три операции показаны на ОДНОЙ паре ради различия, и различие это по-русски
        # было неполным — читатель видел два действия из трёх.
        for слово, значение, род_оп in (("и", a & b, И), ("или", a | b, ИЛИ),
                                        ("исключающее или", a ^ b, ИСКЛЮЧАЮЩЕЕ)):
            out.род = род_оп
            пред = f"{a} {слово} {b} побитово"
            утв = f"{пред} это {значение}."
            out.append(утв)
            out.append(спросить("значение", пред, утв))
        # --- сдвиг, всегда рядом со своим умножением
        сдв_л = f"{s} shifted left by {k}"
        сдв_п = f"{s << k} shifted right by {k}"
        сдв_ru = f"{s} сдвинутое влево на {k}"
        out.род = СДВИГ_ВЛЕВО
        out.append(f"{сдв_л} is {s << k}.")
        out.append(спросить("value", сдв_л, f"{сдв_л} is {s << k}."))
        out.род = РАВЕНСТВО_СДВИГА
        out.append(f"{s} × {2 ** k} = {s << k}.")
        out.род = СДВИГ_ВПРАВО
        out.append(f"{сдв_п} is {s}.")
        out.append(спросить("value", сдв_п, f"{сдв_п} is {s}."))
        out.род = СДВИГ_ВЛЕВО
        out.append(f"{сдв_ru} это {s << k}.")
        out.append(спросить("значение", сдв_ru, f"{сдв_ru} это {s << k}."))
        # СДВИГ ВПРАВО — ДЕЛО ОБРАТНОЕ, и по-русски оно молчало: дом показывал «сдвинутое
        # влево» на двух языках, а обратный ход — на одном.
        out.род = СДВИГ_ВПРАВО
        сдв_п_ru = f"{s << k} сдвинутое вправо на {k}"
        out.append(f"{сдв_п_ru} это {s}.")
        out.append(спросить("значение", сдв_п_ru, f"{сдв_п_ru} это {s}."))
        out.род = ШИРИНА_СЛОВА
        # --- ширина
        out.append(f"a {имя} holds {бит} {by_count(бит, 'bits')} and "
                   f"{2 ** бит} {by_count(2 ** бит, 'values')}.")
        out.append(f"with {бит} {by_count(бит, 'bits')} you can write {2 ** бит} "
                   f"{by_count(2 ** бит, 'numbers')}.")
        out.append(f"{имя} держит {бит} {ру('бит', бит)} и "
                   f"{2 ** бит} {ру('значение', 2 ** бит)}.")
    # ОТКАЗ С ОСНОВАНИЕМ: цифры от двух до девяти в двоичной записи не
    # живут. Вопрос «какое число записано цифрой 7 в двоичной?» имеет
    # честный ответ «никакое, и вот почему» — основание вычислимо и
    # судимо тем же перебором цифр.
    # ОБЕ ПОЛЯРНОСТИ ОДНОЙ РАМКОЙ (аудит покупок holon 03.09): цифры 0 и
    # 1 живут в двоичной записи и называют самих себя.
    out.род = ОТКАЗ_ЦИФРЫ
    for цифра in (0, 1):
        out.append(f"what number is written {цифра} in binary? {цифра}: "
                   f"the digit {цифра} in binary is the number {цифра}.")
        out.append(f"какое число записано цифрой {цифра} в двоичной? {цифра}: "
                   f"цифра {цифра} в двоичной записи есть число {цифра}.")
    for цифра in range(2, 10):
        out.append(f"what number is written {цифра} in binary? none: "
                   f"the digit {цифра} does not occur in binary.")
        out.append(f"какое число записано цифрой {цифра} в двоичной? "
                   f"такого нет: цифра {цифра} в двоичной записи не "
                   f"встречается.")
    return out


# --------------------------------------------------------------- ОБЪЯВЛЕНИЕ ДОМА

# ЯЗЫКОВОЙ ДОЛГ ДОМА НАЗВАН И ВЫПЛАЧЕН В ТОТ ЖЕ ДЕНЬ (15.09). Три рода звучали только
# по-английски — обратное чтение записи, исключающее ИЛИ и сдвиг вправо, — тогда как прямые
# их дела говорили обоими языками. Ничего, кроме рамки речи, тут и не разнилось: числа,
# основания и вердикты у обоих языков одни.
#
#     ДОЛГ, НАЗВАННЫЙ ДЕШЁВЫМ, ОБЯЗАН БЫТЬ ВЗЯТ ПЕРВЫМ. Иначе слово «дёшев» есть лишь
#     оправдание, записанное в доме.
РОДЫ = (ЧИСЛО_В_ЗАПИСЬ, ЗАПИСЬ_В_ЧИСЛО, И, ИЛИ, ИСКЛЮЧАЮЩЕЕ, СДВИГ_ВЛЕВО,
        СДВИГ_ВПРАВО, РАВЕНСТВО_СДВИГА, ШИРИНА_СЛОВА, ОТКАЗ_ЦИФРЫ)

ЗАЧЕМ_РОДА = {
    ЧИСЛО_В_ЗАПИСЬ: "«100 in octal is 144»: одно число тремя записями",
    ЗАПИСЬ_В_ЧИСЛО: "«octal 144 is 100 in decimal» — дело ОБРАТНОЕ, и рамка открывается "
                    "основанием, чтобы два рода не делили одного начала",
    И: "побитовое И — своя функция, свой род",
    ИЛИ: "побитовое ИЛИ на ТОЙ ЖЕ паре: различие встречается, а не предполагается",
    ИСКЛЮЧАЮЩЕЕ: "исключающее ИЛИ там же — третья функция той же пары",
    СДВИГ_ВЛЕВО: "сдвиг влево есть умножение на степень двойки, и оба показаны рядом",
    СДВИГ_ВПРАВО: "сдвиг вправо возвращает сдвинутое: дело обратное, и род иной",
    РАВЕНСТВО_СДВИГА: "«5 × 8 = 40» рядом со сдвигом: закон сказан АРИФМЕТИКОЙ, и это "
                      "другая страница, а не хвост той же",
    ШИРИНА_СЛОВА: "n бит держат 2ⁿ значений",
    ОТКАЗ_ЦИФРЫ: "«какое число записано цифрой 7 в двоичной?» — «никакое, и вот почему»; "
                 "обе полярности одной рамкой, ибо 0 и 1 в двоичной живут",
}


def страницы(pass_i):
    return pass_shows(pass_i)


def перебор_страниц(pass_i):
    return pass_shows(pass_i).парами


def группы(pass_i):
    return [страницы(pass_i)]


def _показы():
    from layer import PASSES                             # noqa: PLC0415
    вон = {}
    for шаг in range(len(PASSES)):
        for с, род in перебор_страниц(шаг):
            for строка in с.split("\n"):
                if строка.rstrip():
                    вон.setdefault(строка.rstrip(), (bilang.азбукой(строка.rstrip()), род))
    return вон


ПОКАЗЫ = _показы()

# ЯЗЫК ПОКАЗА НАЗВАН ПЕРВЫМ, РОД — ВТОРЫМ: так читает прибор щербатости.
РОД_В_ПОКАЗЕ = 1


def _самопроверка_дома():
    assert set(ЗАЧЕМ_РОДА) == set(РОДЫ), "глосса рода разошлась с объявлением"
    сбор = pass_shows(0)
    assert len(сбор) == len(сбор.роды), "показ остался без рода"
    пустые = set(РОДЫ) - {р for _я, р in ПОКАЗЫ.values()}
    assert not пустые, f"род объявлен и не кован: {sorted(пустые)}"


_самопроверка_дома()


def main():
    emit(ЦЕЛЬ, pass_shows)


if __name__ == "__main__":
    main()
