...
# ПАСПОРТ КОРНЯ (07.09): кто читает зачины разговора. Дом перевода берёт отсюда фразы —
# благодарность, привет, прощание — и спаривает их по месту; места здесь выровнены
# объявлением, а не длиной списка (шрам дома перевода того же дня).
ЧИТАЮТ = ("gen_genesis_dialogue", "translateforms", "dialogue_court")
...
# РОД, ЧЬЁ ЧИСЛО СТРАНИЦ ЕСТЬ ФАКТ О ПРЕДМЕТЕ (15.09, прибор перекоса назвал дом 73,6-кратным).
# Три рода здесь ограничены не выбором дома, а самим предметом:
#
#     «язык»  — «на каком языке я говорю?» имеет РОВНО ОДИН ответ на язык, и девять страниц
#               суть девять языков. Десятой быть неоткуда.
#     «языки» — «сколько языков ты знаешь?» — то же: один ответ на язык.
#     «ход»   — связок хода разговора две («это всё?», «у меня ещё вопрос»), и на девяти
#               языках их восемнадцать. Третью пришлось бы выдумать, а не найти.
#
#     РОД, ЧЬЁ ЧИСЛО СТРАНИЦ ЕСТЬ ФАКТ О ПРЕДМЕТЕ, НЕ ПОДЛЕЖИТ МЕРЕ РОВНОСТИ — НО ОБЯЗАН БЫТЬ
#     ОБЪЯВЛЕН ТАКОВЫМ, ИНАЧЕ ПОД ЭТИМ ИМЕНЕМ СПРЯЧЕТСЯ ВСЯКИЙ НЕДОПИСАННЫЙ.
#
# Прочие роды дома этим не оправдываются: «пара» (662), «малый» (324), «определение» (234)
# перебирают виды речи и темы, и число их страниц есть выбор, а не природа.
РОДЫ_ПО_ПРЕДМЕТУ = {
    "язык": "«на каком языке я говорю?» имеет ровно один ответ на язык: девять страниц суть "
            "девять языков, и десятой быть неоткуда",
    "языки": "«сколько языков ты знаешь?» — то же: один ответ на язык",
    "ход": "связок хода разговора две («это всё?», «у меня ещё вопрос»), и на девяти языках "
           "их восемнадцать; третью пришлось бы выдумать, а не найти",
}
...
ЯЗЫКИ = {
    ...
    "en": dict(
        ...
        ответы={БЛАГОДАРНОСТЬ: ("you are welcome", "not at all", "glad to help"),
                ИЗВИНЕНИЕ: ("it is all right", "no problem", "do not worry", 'never mind'),
    ...
    "de": dict(
        ...
        ответы={БЛАГОДАРНОСТЬ: ("bitte", "gern geschehen", "nichts zu danken"),
                ИЗВИНЕНИЕ: ("kein Problem", "schon gut", "mach dir nichts daraus", 'macht nichts'),
    ...
    "fr": dict(
        ...
        ответы={БЛАГОДАРНОСТЬ: ("de rien", "je t'en prie", "avec plaisir"),
                ИЗВИНЕНИЕ: ("ce n'est rien", "pas de problème", "ne t'inquiète pas", "ce n'est pas grave"),
    ...
    "pl": dict(
        ...
        ответы={БЛАГОДАРНОСТЬ: ("proszę", "nie ma za co", "cała przyjemność po mojej stronie"),
                ИЗВИНЕНИЕ: ("nic się nie stało", "nie ma problemu", "nie martw się", 'nic nie szkodzi'),
...
def страница(язык, форма, вид=None, i=0, j=0, вы=False):
    ...
    if форма in ("моё_имя", "бессмыслица", "о_себе", "ход"):
        ...
        # имеет смысла: цвет ничего не весит.» — отказ по категории, с основанием.
        в, о = я[форма][i % len(я[форма])]
        return f"{в} {о}"
...
РОДЫ = ФОРМЫ + tuple(ф + "_вы" for ф in ФОРМЫ_ВЫ)   # вежливая страница есть свой род
