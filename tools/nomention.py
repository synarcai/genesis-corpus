...
РЕЧЬ = {
    ...
    "es": dict(состояние="{М} hay {n} {Т}", вопрос="¿{КВ} {Тмн} hay {М}?",
               основание="no se dice nada {О}", двоеточие=": ", вопрос_знак="?"),
    "it": dict(состояние="{М} {ЕСТЬ} {n} {Т}", вопрос="{КВ} {Тмн} ci sono {М}?",
               основание="non è detto nulla {О}", двоеточие=": ", вопрос_знак="?"),
    "pt": dict(состояние="{М} há {n} {Т}", вопрос="{КВ} {Тмн} há {М}?",
...
def рамка(язык, форма, где, спрошено, Т=0, Тспр=0):
    ...
    # ВОПРОСНОЕ СЛОВО СОГЛАСУЕТСЯ С ТЕМ, О ЧЁМ СПРОСИЛИ, А НЕ С ТЕМ, О ЧЁМ РАССКАЗАЛИ:
    # у формы «вещь_чужая» это РАЗНЫЕ вещи, и род берётся у спрошенной (закон — у дома
    # орудий, `toolforms.квопрос`).
    вопрос = (р["вопрос"].replace("{М}", МЕСТА[язык][спрошено][0]).replace("{Тмн}", мн)
              .replace("{КВ}", T.квопрос(язык, мн)))
...
def _образец(язык, шаблон):
    # ЭЛИЗИЯ ПРИКЛАДЫВАЕТСЯ К ШАБЛОНУ, А НЕ К ОДНОМУ КУСКУ ПЕРЕД ДЫРОЙ (21.09). Страница
    # француза пишется через `_fr.элизия`, и «combien de enregistrements» становится на ней
    # «combien d'enregistrements». Образец же строился из СЫРОЙ рамки, а `_fr.в_образце`
    # берёт обе формы лишь тогда, когда служебное слово стои́т на КОНЦЕ куска — то есть
    # прямо перед дырой. Здесь имя товара литерально (оно взято из вопроса), элизия падает
    # в середину куска, и восемьдесят французских страниц не совпадали ни с чем.
    #
    #     ЗАКОН, ЧИТАЕМЫЙ ДВУМЯ, ДОЛЖЕН ЧИТАТЬСЯ ИМИ ОДИНАКОВО ДО КОНЦА: письмо применяло
    #     элизию ко всей странице, а чтение — к одному месту из двух.
    if язык == "fr":
        шаблон = _fr.элизия(шаблон)
    вещи = _альт(T.ФОРМЫ_ВЕЩЕЙ[язык])
    есть = _альт(T.ЕСТЬ[язык]) if язык in T.ЕСТЬ else None
    дыры = {"Тмн": вещи, "КВ": _альт(romgram.ПАРЫ[язык]) if romgram.гнётся(язык) else ""}
    for н in ("", "2"):
        дыры["n%s" % н] = r"\d+"
        дыры["Тn%s" % н] = вещи
...
def _все_рамки():
    ...
    for язык in ЯЗЫКИ:
        мест = len(МЕСТА[язык])
        видов = len(T.ВЕЩИ[язык])
        # ВИД ТОВАРА ОБХОДИТСЯ НАРАВНЕ С МЕСТОМ (21.09). Имя товара во множественном стои́т
        # в ВОПРОСЕ рамки литералом («сколько файлов в папке?», «сколько записей в ящике?»),
        # а рамки строились с `Т=0` по умолчанию — для одного первого вида. Дом же пишет
        # всеми: 1168 его страниц из 2160 не совпадали ни с одним образцом и были НЕМЫ.
        #
        #     РАМКА, ПОСТРОЕННАЯ ДЛЯ ОДНОГО ЗНАЧЕНИЯ ПАРАМЕТРА, ЧИТАЕТ ОДНО ЗНАЧЕНИЕ — А
        #     ПИШЕТ ДОМ ВСЕМИ. Умолчание в объявлении рамки есть удобство письма и ловушка
        #     чтения: пишущий передаёт свой вид, читающий забывает и берёт нулевой.
        for i in range(мест):
            for Т in range(видов):
                for спрошено in range(мест):
                    for форма in ("место_названо", "место_чужое"):
                        вон.append((_образец(язык, рамка(язык, форма, (i,), спрошено, Т=Т)),
                                    язык, форма, (i,), спрошено, None))
                for Тспр in range(видов):
                    вон.append((_образец(язык, рамка(язык, "вещь_чужая", (i,), i, Т=Т,
                                                     Тспр=Тспр)),
                ...
                for спрошено in range(мест):
                    ...
                    for форма in ("два_места", "третье_место"):
                        вон.append((_образец(язык, рамка(язык, форма, (i, i2), спрошено, Т=Т)),
...
def судить(строка):
    ...
    с = строка.strip()
    # ВЫХОД ПО НАБОРУ СНЯТ (21.09) — он укрывал 1168 страниц из 2160, которых закон не
    # узнавал вовсе: рамки строились для одного вида товара, а француз ещё и без
    # элизии. НАБОР ГОВОРИТ, ЧТО ДОМ СТРАНИЦУ НАПИСАЛ, А НЕ ЧТО ОНА ВЕРНА.
    ...
    for образ, язык, форма, где, спрошено, Тспр in ОБРАЗЦЫ:
        ...
        if зн is None:
            continue
        if _вердикт(язык, форма, где, спрошено, зн):
            return True, True
