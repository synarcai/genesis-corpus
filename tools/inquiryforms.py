...
# роду дома). Все семь родов этого дома суть ЛЕСТНИЦА ВОПРОСА над одним предметом:
# определение, случай с доводом, контрпример, вывод — и всякий род ведёт её по СВОЕМУ
# предмету (делимость, квадрат, простота, две переменные), а числа при нём материал.
#
# СЛУЧАЙ РЕДКИЙ, И ПОТОМУ НАЗВАН ВСЛУХ: «объявлено всё» есть ровно тот вид, какой принимает
# уклонение. Ровного самим собою рода здесь нет, ибо дом ВЕСЬ о том, как ОДНА И ТА ЖЕ
# лестница проходится по разным предметам; будь в нём род без чисел — он не был бы лестницей.
#
#     УПРАЖНЕНИЕ ТАМ, ГДЕ ЧИСЛА СУТЬ МАТЕРИАЛ, А МЫСЛЬ ОДНА.
ОЧЕРТАНИЕ_ПО_ПРИРОДЕ = ("divisibility", "square", "primality", "two-variables",
                        "conditional", "injectivity", "odd-sum")
...
def язык_строки(с):
    """Язык показа этого дома — по НАЧЕРТАНИЮ, и это объявление, а не догадка (15.09).

    Два языка мира пишутся РАЗНЫМИ азбуками, и пересечения у них нет: русская строка несёт
    кириллицу всегда (даже когда в ней стои́т латинская «k» переменной), английская — ни
    разу. Там, где алфавиты не пересекаются, письмо И ЕСТЬ объявление языка.

        ГДЕ ДВА ЯЗЫКА РАЗЛИЧЕНЫ АЗБУКОЙ, ЧИТАТЬ АЗБУКУ НЕ ЗНАЧИТ ГАДАТЬ. Там же, где они
        пишутся одной азбукой — немецкий с французским, польский с турецким, — язык берут у
        строителя (`bilang.Двое`), ибо письмо там молчит о половине строк.

    Мера этого закона сверена числом: на 2557 показах, где признак языка универсалий
    (`universals.язык`) говорит своё слово, расхождений с начертанием НОЛЬ.
    """
    return "ru" if КИРИЛЛИЦА.search(с) else "en"
...
def _показы():
    ...
    for шаг in range(len(PASSES)):
        for с, род in перебор(шаг):
            вон.setdefault(с, (язык_строки(с), род))
    return вон
...
def _самопроверка():
    ...
    # МЕТКА СВЕРЯЕТСЯ С ПРИЗНАКОМ УНИВЕРСАЛИЙ ТАМ, ГДЕ ОН ГОВОРИТ: два независимых чтения
    # одного и того же, и расхождение между ними есть находка, а не мелочь.
    сверено = 0
    for с, (я, _р) in ПОКАЗЫ.items():
        try:
            по_признаку = universals.язык(с, ЯЗЫКИ_МИРА)
        except ValueError:
            continue
        assert по_признаку == я, f"метка {я}, а признак {по_признаку}: «{с[:60]}»"
        сверено += 1
    assert сверено > 1000, f"сверено всего {сверено} строк — мало для пробы"
