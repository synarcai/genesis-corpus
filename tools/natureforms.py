...
import closedworld
import frgram as _фр  # noqa: E402 — французская элизия: закон поверх готовой страницы  # noqa: E402 — закон цифрового скелета замкнутого мира
...
def показ(язык, форма, i):
    ...
    строка = я[форма].format(**поля)
    if язык == "fr":
        # ЭЛИЗИЯ ЕСТЬ ЗАКОН ПИСЬМА, А НЕ ПРАВКА РАМКИ (09.09). Рамка пишет «combien de {ва}»,
        # и при имени на гласную выходит «combien de ailes» — три такие строки жили в мире, и
        # прибор [ЭЛИЗИЯ] держал их долгом. Рамку правкой не вылечить: имя приходит дырой, и
        # гласная в нём то есть, то нет. Закон стои́т ПОВЕРХ готовой страницы, и объявлен он
        # домом французской грамматики, а не переписан здесь.
        строка = _фр.элизия(строка)
    return строка
...
# РЯДЫ ДОМА — ЧАСТИ, СТОЯЩИЕ В ЕГО РАМКАХ НА ОДНОМ МЕСТЕ (21.09). Их объявляет ДОМ, ибо
# только он знает, что в его рамке взаимозаменимо по месту: носитель факта, его парная
# форма, названная часть, время года, день, месяц, вещество и глагол температуры.
#
#     ЧАСТИ, ОБЪЯВЛЕННЫЕ ВЗАИМОЗАМЕНИМЫМИ ПО МЕСТУ, НЕ ВЗАИМОЗАМЕНИМЫ ПО ИСТИНЕ: «у паука
#     8 ног» и «у насекомого 6 ног» суть две честные страницы, а «у паука 6 ног» — ложь,
#     собранная из честных половин.
def _столбец(ряд, i):
    """Столбец объявленной таблицы — ТОЛЬКО СЛОВА. Объявление неоднородно: у части языков
    клетка сама есть набор форм, и брать её целиком значило бы подать закону кортеж вместо
    слова. Неоднородность эта — свойство языков, а не порок объявления."""
    вон = set()
    for т in ряд:
        з = т[i] if isinstance(т, (tuple, list)) and len(т) > i else None
        if isinstance(з, str) and з:
            вон.add(з)
    return tuple(вон)
...
def _ряды():
    ...
    for я in ЯЗЫКИ.values():
        for имя, столбцы in (("факты", (0, 1, 2)), ("времена", (0, 1)), ("дни", (0, 1)),
                             ("месяцы", (0, 1)), ("температуры", (0, 1))):
            ряд = я.get(имя) or ()
            for i in столбцы:
                вон.append(_столбец(ряд, i))
    return tuple(р for р in вон if len(р) > 1)


РЯДЫ = _ряды()

# ДОМ ОБЪЯВЛЯЕТ СЕБЯ СУДЯЩИМ ОБЪЯВЛЕНИЕМ (21.09). Его страница есть утверждение из таблицы,
# а не вычисление: «у паука 8 ног» неоткуда вывести, кроме как из объявления, и набор
# показов ЕСТЬ это объявление. Прибор подсадки, вписывающий порчу в набор, вписывает её в
# объявление — и спрашивает дом о строке, которую сам же объявил верной.
#
# Объявление это не отговорка и проверяется прибором: принимается оно лишь от СТРАЖА — от
# дома, который ловит свою порчу без всякого вписывания. «у паука 6 ног» есть ложь этого
# дома, и он говорит это сам, пятым братом замкнутого мира.
СУДИТ_ОБЪЯВЛЕНИЕМ = True


def _судить_рамкой(строка):
    """(судимо, истинно): показ дома; строка, отличающаяся от показа ОДНИМ членом
    объявленного ряда, — его ложь, а не чужая строка.

    Прежде здесь стояло одно членство в наборе, и дом был стерегущим НАБОРОМ, а не
    законом: «у паука 6 ног» он звал чужой строкой, хотя обе её половины писал сам.
    Пересчитать в такой странице нечего — дом не считает, он объявляет; но объявление и
    есть его закон, и пятый брат замкнутого мира спрашивает ровно его.
    """
    с = строка.strip()
    if с in ПОКАЗЫ:
        return True, True
    if closedworld.ложь_по_ряду(с, ПОКАЗЫ, РЯДЫ):
        return True, False
    return False, False
