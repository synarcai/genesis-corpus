...
ЯЗЫКИ = {
    ...
    "de": dict(
        ...
        связки={"ein Werkzeug": "das", "ein Tier": "das", "eine Pflanze": "die", "ein Gefäß": "das",
                ...
                "ein Gebäude": "das", "eine Kleidung": "die"},
...
ФОРМЫ = ("имя", "род", "отличие", "обращение")
...
def вопрос_ответ(язык, форма, i, сдвиг=0):
    ...
    if форма == "род":
        return я["род"].format(В=В, Р=Р, Р2=_другой_род(язык, Р, сдвиг))
    if форма == "отличие":
...
def показ(язык, форма, i, сдвиг=0):
    ...
    return f"{утверждение(язык, i)} {вопрос_ответ(язык, форма, i, сдвиг)}"
