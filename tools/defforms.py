...
# СЛОВАРЬ СТРАНИЦ ДОМА ОПРЕДЕЛЕНИЙ (13.09, восьмой переезд того же вида). Перебор в семь
# строк жил у кузницы, и род его — переменная `форма`, взятая из `ФОРМЫ` ЭТОГО ЖЕ дома.
#
#     ДОМ, НЕ ЗНАЮЩИЙ СВОИХ СТРАНИЦ, НЕ МОЖЕТ НАЗВАТЬ ИХ РОДА.
#
# ЧИСЛО ПЕРЕЕХАЛО ВМЕСТЕ С ПЕРЕБОРОМ, А НЕ ПЕРЕПИСАНО ПО ПАМЯТИ: первая попытка поставила
# здесь двойку, у кузницы стои́т тройка, и мир вышел бы короче на треть чужих родов. ЧИСЛО,
# ПЕРЕПИСАННОЕ ПО ПАМЯТИ, ЕСТЬ ВТОРОЕ ЧИСЛО, И ОНО РАЗОЙДЁТСЯ.
ЧУЖИХ_РОДОВ = 3
def перебор(язык):
    """[(страница, род)] — всякая строка мира по порядку таблицы."""
    вон = []
    for i in range(len(ЯЗЫКИ[язык]["определения"])):
        for форма in ФОРМЫ:
            сдвиги = range(ЧУЖИХ_РОДОВ) if форма.startswith("род") else (0,)
            вон += [(показ(язык, форма, i, с), форма) for с in сдвиги]
    return вон


ПОКАЗЫ = {с: (язык, род) for язык in ЯЗЫКИ for с, род in перебор(язык) if с}
