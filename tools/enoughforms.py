...
def _образец(язык, форма, один=False):
...
# ОБЕ РАМКИ ЧИТАЮТСЯ, А НЕ ОДНА (21.09). Дом объявляет рамку единичного остатка и
# единичной нехватки — «1 dollar is missing», «1 Euro fehlt», «falta 1 euro» — и ПИШЕТ ею
# восемнадцать страниц на шести языках. Образцы же строились по одной множественной
# рамке, и суд этих восемнадцати не узнавал вовсе: они проходили лишь выходом по набору.
#
#     РАМКА, ОБЪЯВЛЕННАЯ ДЛЯ ПИСЬМА И НЕ ВЗЯТАЯ ДЛЯ ЧТЕНИЯ, ЕСТЬ ПОЛОВИНА РАМКИ: дом
#     пишет ею страницы, а суд их не узнаёт — и ловит не закон, а память набора.
#
# Родня закону «поле, объявленное и никем не читаемое, есть обещание без адресата», но
# тяжелее: там молчал ОДИН читатель, здесь — единственный, кто судит.
ОБРАЗЦЫ = [(_образец(язык, форма, один), язык, форма)
           for язык in ЯЗЫКИ for форма in ФОРМЫ for один in (False, True)]
...
def _судить_образцом(строка):
    ...
    с = строка.strip()
    # ВЫХОД ПО НАБОРУ СНЯТ (21.09) — вместе с уплатой рамки единичной нехватки
    # выше: пока суд не читал этой рамки, выход по набору был единственным, чем
    # восемнадцать её страниц держались. НАБОР ГОВОРИТ, ЧТО ДОМ СТРАНИЦУ НАПИСАЛ,
    # А НЕ ЧТО ОНА ВЕРНА. Замер: закон судит все 324 показа верно и без набора.
