...
ОПОРА = 8                      # LAW³: столько страниц причитается роду — и форме счёта
...
def _ровно(числа, сколько):
    """Не более `сколько` чисел, взятых по ряду РОВНО, а не кучей с его начала."""
    if len(числа) <= сколько:
        return list(числа)
    шаг = len(числа) / сколько
    return [числа[int(i * шаг)] for i in range(сколько)]
...
def _показы():
    ...
    for язык in ЯЗЫКИ:
        ...
        # ШОВ ФОРМЫ СЧЁТА (12.09). Второй диагонали довольно, чтобы выровнять ВЕЩИ, и её не
        # хватает, чтобы выровнять ФОРМЫ: диагональ знает вещь, а форма счёта есть свойство
        # ЧИСЛА. Русское «где 24 книги?» стоит на числах, кончающихся на 2, 3 и 4 (кроме 12—14);
        # их в ряду до сорока девять из тридцати девяти, и две диагонали приводят к каждой вещи
        # по десять чисел, из которых такими оказываются два. Прибор «ключ без опоры» назвал оба
        # русских рода этой рамки поимённо: «где книги» 5 страниц, «где яблока» 4 при законе
        # опоры LAW³ = 8.
        #
        #     ФОРМА СЧЁТА ЕСТЬ РОД, А НЕ ОТТЕНОК РОДА: ВОСЕМЬ СТРАНИЦ ЗАКОНА ПРИЧИТАЮТСЯ ЕЙ
        #     САМОЙ — А ДИАГОНАЛЬ, ЗНАЮЩАЯ ЛИШЬ ВЕЩЬ, ОТДАЁТ ИХ СЛУЧАЮ.
        #
        # Оттого ряд ходит здесь не диагональю, а ПАРОЙ «вещь и её написание»: каждая вещь берёт
        # по ОПОРА чисел КАЖДОГО написания, какое язык ей даёт в этом ряду. Языку с одной формой
        # множественного это добавляет ровности, языку с тремя (русскому, польскому) возвращает
        # третью. Где ряд до сорока держит меньше восьми таких чисел (русское «книга» живёт в нём
        # лишь при 21 и 31), дом берёт все и не выдумывает сорок первого.
        for Т in range(вещей):
            по_написанию = {}
            for n in range(2, ВЕРХ + 1):
                по_написанию.setdefault(A._вещь(язык, Т, n), []).append(n)
            for числа in по_написанию.values():
                # ГДЕ РЯД ЧИСЕЛ НЕ ДАЁТ ФОРМЕ ВОСЬМИ, ЕЁ ДОБИРАЕТ ДРУГАЯ ДЫРА РАМКИ. Русское
                # «где 21 книга?» стои́т на числах, кончающихся единицей, — их в ряду от двух
                # до сорока ровно два (21 и 31), и больше двух чисел этой форме взять негде.
                # Место же свободно: четыре места на два числа дают восемь РАЗНЫХ страниц, и
                # все они верны. Ряд идёт по парам «число и место», а не по числам.
                взятые = _ровно(числа, ОПОРА)
                пары = [(n, (i + м + Т) % мест)
                        for м in range(мест) for i, n in enumerate(взятые)]
                for n, М in пары[:ОПОРА]:
                    вон[страница(язык, "место_чего", 0, 1, Т, n, М=М)] = (язык, "место_чего")
    return вон
...
def судить(строка):
    ...
    #
    # Замерено: закон судит все 6777 показов дома верно и без набора — выход был
