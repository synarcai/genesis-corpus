...
def arith_lexicon(pack):
    ...
    # A NUMERAL BENDS BY GENDER, AND THE BENT FORM IS THE SAME NUMBER (22.09):
    # «одна», «две», «дві» are declared in `numeral_gender`.
    for k, forms in (pack.get("numeral_gender") or {}).items():
        if str(k).lstrip("-").isdigit():
            for v in forms.values():
                words.setdefault(str(v).lower(), int(k))
    ops = {
        "+": "+", "-": "-", "*": "*",
        "\u00d7": "*", "/": "/", "\u00f7": "/",
    }
...
def элизия(pack, строка):
    """Сокращение служебного слова перед гласной — ПО ОБЪЯВЛЕНИЮ ПАКЕТА, а не по правилу движка.

    ФРАНЦУЗСКОЕ «je» ПЕРЕД ГЛАСНОЙ ОБЯЗАНО СТАТЬ «j'», и это не украшение речи, а её закон:
    «je ouvre» есть не оттенок, а ошибка. Шаблон держит служебное слово ЦЕЛЫМ (иначе он не
    покрыл бы согласных), и потому сокращение делается ПОСЛЕ подстановки — над готовой строкой,
    когда слово за ним уже известно.

        СЛУЖЕБНОЕ СЛОВО, СТОЯЩЕЕ В ШАБЛОНЕ ЦЕЛЫМ, НЕ СЛЫШИТ ТОГО, ЧТО ЗА НИМ ПОДСТАВЯТ.

    Пакет объявляет пары и гласные буквы:

        "elision": {"pairs": {"je": "j'", "que": "qu'", "de": "d'", "ne": "n'"},
                    "vowels": "aeiouéèêàâîôû", "before_h": ["heure", "heures"]}

    «h» бывает придыхательным («le héros»), и потому слова с НЕМЫМ «h» названы поимённо, а не
    выведены буквой. Пакет, `elision` не объявивший, строку не трогает.
    """
    правило = pack.get("elision") or {}
    пары = правило.get("pairs") or {}
    if not пары:
        return строка
    гласные = правило.get("vowels", "")
    немое_h = set(правило.get("before_h", ()))
    вон = строка
    for целое, сокращённое in пары.items():
        образец = re.compile(r"(?<![-\w'])" + re.escape(целое) + r" (?=(\w+))")

        def ход(м):
            слово = м.group(1).lower()
            if слово[:1] in гласные:
                return сокращённое
            if слово[:1] == "h" and слово in немое_h:
                return сокращённое
            return м.group(0)

        вон = образец.sub(ход, вон)
    return вон
...
def род_имени(pack, слово):
    """Род имени — ПО ОБЪЯВЛЕНИЮ ПАКЕТА: поимённо, где письмо молчит, иначе окончанием.

    ОКОНЧАНИЕ ДАЁТ РОД НЕ ВСЕГДА, И ПАКЕТ НАЗЫВАЕТ, ГДЕ ОНО МОЛЧИТ:

        "noun_gender": {"endings": {"а": "ж", "о": "с", "е": "с"},
                        "undecidable": ["ь", "я"], "default": "м",
                        "declared": {"цибуля": "ж"}}

    Слово на молчащее окончание, не объявленное поимённо, есть ОШИБКА ЗОВА, а не догадка:
    дом не пишет того, чего не знает. Пакет, `noun_gender` не объявивший, рода не знает, и
    дверь отвечает `None` — тот, кто зовёт, решает, чем платить за незнание.
    """
    правило = pack.get("noun_gender") or {}
    if not правило:
        return None
    с = str(слово).strip().lower()
    объявлен = (правило.get("declared") or {}).get(с)
    if объявлен is not None:
        return объявлен
    for хвост in правило.get("undecidable", ()):
        if с.endswith(хвост):
            raise KeyError(f"окончание «{хвост}» рода не даёт, а слово не объявлено: {слово!r}")
    for хвост, род in sorted((правило.get("endings") or {}).items(), key=lambda п: -len(п[0])):
        if с.endswith(хвост):
            return род
    return правило.get("default")


def числительное_по_роду(pack, числительное, род):
    """Числительное, согласованное с родом считаемого — ПО ОБЪЯВЛЕНИЮ ПАКЕТА.

    РОД ЧИСЛИТЕЛЬНОГО ЕСТЬ РОД ИМЕНИ. «Один» и «два» гнутся по роду того, что считают:

        "numeral_gender": {"1": {"м": "один", "ж": "одна", "с": "одно"},
                           "2": {"м": "два", "ж": "две", "с": "два"}}

    Пакет объявлял число ОДНИМ словом, и одиннадцать миров, бравших его рукой или из
    пакета, писали «два книги», «один минута», «одна яблоко» — 143 строки на 22.09.

    СОСТАВНОЕ СОГЛАСУЕТСЯ ПОСЛЕДНИМ СЛОВОМ: «двадцать одна», «тридцать две». Слово узнаётся
    по ВСЕМ формам своей цифры, а не по одной, — дом, подавший «одна» при мужском имени,
    получит «один»: дверь согласует, а не дописывает. Пакет, `numeral_gender` не объявивший,
    и число, чьё последнее слово рода не несёт («три», «пять»), отдаются как есть.
    """
    таблица = pack.get("numeral_gender") or {}
    if not таблица or род is None:
        return числительное
    слова = str(числительное).split()
    if not слова:
        return числительное
    for формы in таблица.values():
        if слова[-1].lower() in {ф.lower() for ф in формы.values()}:
            # РОД, КОТОРОГО ЯЗЫК НЕ ИМЕЕТ, НЕ ГНЁТ НИЧЕГО: у португальского нет среднего, и
            # спрошенный о нём отдаёт слово как есть — дверь молчит, а не падает.
            if род not in формы:
                return числительное
            слова[-1] = формы[род]
            return " ".join(слова)
...
def счётная_форма(pack, cls, формы_лексемы, k):
    ...
    ОДИН ЗАКОН — ОДИН ЧИТАТЕЛЬ. Правило это читали двое: движок (кладя строку) и суд формы
    (сверяя её), — и в час, когда движок научился брать `count_*`, суд остался при основной
    ячейке и назвал ложью 280 верных строк немецкого слоя.

    Ячейка `count_<имя>`, если объявлена, есть ФОРМА ПРИ ЧИСЛЕ: у романских и нидерландского
    она голая, без артикля («livres» против «les livres»), у немецкого — с прописной, как
    пишется имя. Пакет, её не объявивший, берёт основную ячейку, как и брал.
    """
    имена = cls.get("forms") or []
    i = count_form_index(pack, cls, k)
    if i >= len(имена):
        return формы_лексемы[i] if i < len(формы_лексемы) else None
    счётная = f"count_{имена[i]}"
    j = имена.index(счётная) if счётная in имена else i
    return формы_лексемы[j] if j < len(формы_лексемы) else None
...
def chain_of(pack, seed):
    """Счётная цепь языка от объявленных числительных, или «{chain}».

    Язык, не объявивший всех числительных отрезка, цепи не говорит:
    слот остаётся неподставленным, и показ выбывает сам — тот же
    честный отказ, каким живёт весь пакет.
    """
    numerals = pack.get("numerals", {})
    начало = 1 + seed % 5
    длина = 8 + seed % 8
    слова = [numerals.get(str(k))
             for k in range(начало, начало + длина)]
    if any(w is None for w in слова):
        return "{chain}"
    # РАЗДЕЛИТЕЛЬ ПЕРЕЧНЯ ОБЪЯВЛЯЕТСЯ, А НЕ ВЫВОДИТСЯ ИЗ РЕЗКИ. Цепь
    # «一二三四五六七八九十», написанная слитно, ДВУСМЫСЛЕННА и по
    # существу: «八九十» есть и «восемь, девять, десять», и «восемь,
    # девяносто» — жадное чтение берёт второе, и оно НЕ НЕВЕРНО.
    # Китайский перечисляет через «、», и это факт языка, а не уловка
    # прибора: неоднозначность снимается тем же знаком, каким её
    # снимает сам язык.
    шов = pack.get("list_separator")
    if шов is None:
        шов = ("" if pack.get("segmentation") == "longest-match"
               else " ")
    return шов.join(слова)
...
def gen_kind(pack, kind_name, kind, pass_i):
    ...
    for ti, template in enumerate(
        ...
        # ЯДРО ДОСЛОВНЫХ ПОВТОРОВ ПОД РАНДОМИЗАЦИЕЙ. Форма чеканится
        # ПОВТОРНОСТЬЮ: русская школа говорит «три плюс два равно
        # пять» три тысячи раз, и потому пятёрка «□ плюс □ равно □»
        # складывается. Слой, у которого КАЖДЫЙ инстанс берёт новые
        # числа, не даёт ни одного дословного повтора — и дерево формы
        # не строится вовсе, при любой массе (замер: масса ×3.4 дала
        # те же нули). Первые `verbatim_core` инстансов берут семя БЕЗ
        # прохода — они одинаковы во всех проходах — и выходят дважды;
        # при пяти проходах это десять дословных повторов на
        # поверхность. Рандомизация остаётся ПОВЕРХ, для обобщения.
        ядро = kind.get("verbatim_core", min(3, сколько))
        # СЧЁТНЫЙ РЯД ЕСТЬ ХОД, А НЕ ЖРЕБИЙ. Числа показов берутся семенем,
        # и ряд «один плюс один — два, два плюс один — три …» складывался
        # тогда лишь случайно: у немецкого рынок насчитал одиннадцать шагов
        # подряд, у французского пять (замер holon 03.09, COUNT-CHAINS).
        # Род, объявивший «run», ведёт СВОЙ счёт: инстанс j берёт n = j + 1,
        # и ряд выходит полным ровно той длины, какую род объявил.
        рядом = bool(kind.get("run"))
        # ЕДИНИЧНЫЙ ОБХОД: КОЛЬЦО ОБЕЩАЕТ ВСТРЕЧУ, НО НЕ НАЗНАЧАЕТ ЧАСА (08.09).
        #
        # Седьмого числа лексема была отвязана от семени числа и пущена по кольцу — «за
        # `len(pool)` оборотов лексема встречает все объявленные ячейки». Обещание верно и
        # ныне, но оно есть обещание О ПРЕДЕЛЕ, а проходов у свода конечное число: восемь
        # английских счётных лексем из двадцати пяти не встретили ЕДИНИЦЫ ни разу — book,
        # egg, mile, message, ball, kopeck, minute, year.
        #
        #     ЕДИНИЦА СЛИШКОМ ВАЖНА, ЧТОБЫ ЖДАТЬ ЕЁ ОТ РАСПИСАНИЯ КОЛЬЦА. Форма при единице
        #     есть та, на какой язык виднее всего расходится с числом («1 kopeck», а не
        #     «1 kopecks»); её показывают НАЗНАЧЕНИЕМ, а не удачей оборота.
        #
        # Обход идёт ОДИН РАЗ (первым проходом) и лишь по тем образцам, что берут счётную
        # ячейку по числу: каждая объявленная лексема класса выходит при единице ровно раз.
        обход = []
        if pass_i == 0:
            for имя_кл in kind.get("sweep_one", ()):
                if f"lex:{имя_кл}:by_n" not in template:
                    continue
                обход += [(имя_кл, w) for w in sorted((mc.get(имя_кл) or {}).get("lexemes") or {})]
        for j in range(сколько + len(обход)):
            свой = обход[j - сколько] if j >= сколько else None
            # ОБХОД ИДЁТ ДОСЛОВНЫМ ЯДРОМ, И ЭТО НЕ УКРАШЕНИЕ, А ЗАКОН (08.09). Показ,
            # выходящий ОДИН раз, роняет ворота движка: слово, живущее однажды, есть слово
            # необъявленное на деле (LAW = 2), и романские пакеты тут же дали по десятку
            # таких — их счётные ячейки («livre» без артикля) прежде не жили в мире вовсе.
            # Ядро же выходит дважды по своему устройству, и единственная форма получает ту
            # же двойную опору, какой держится всякая другая.
            дословно = j < ядро or свой is not None
            seed = (ti * 11 + j * 7 if дословно
                    else pass_i * 37 + ti * 11 + j * 7)
            n = 1 if свой else (j + 1 if рядом else seed % 9 + 1)
            m = 1 if рядом else seed % 5 + 1
            ctx = {
                "n": n,
                "m": m,
                "sum": n + m,
                ...
                # СЧЁТНАЯ ЦЕПЬ ЕСТЬ УТВЕРЖДЕНИЕ О ПОРЯДКЕ,
                # и его не было НИ У ОДНОГО из тринадцати
                # языков: «one two three … twenty» жило
                # только в английском ядре. Границы цепи
                # ходят с проходом, чтобы порядок покупался
                # разными отрезками, а не одной заученной
                # строкой. Шов ВЫВОДИТСЯ из объявленной
                # резки: письмо без пробелов пишет цепь
                # слитно, и это то же объявление, каким
                # живёт сегментатор.
                "chain": chain_of(pack, seed),
                "prod": n * m,
                "num:prod": numerals.get(
                    str(n * m), str(n * m)
                ),
                # ШАГ РЯДА НАЗЫВАЕТСЯ СВОИМИ СЛОВАМИ: единица и следующее
                # число суть дыры того же рода, что «num:n» — язык говорит
                # их своими числительными, а не цифрой.
                "one": 1,
                "num:one": numerals.get("1", "1"),
                "next": n + 1,
                "num:next": numerals.get(
                    str(n + 1), str(n + 1)
                ),
            }
            for cls_name, cls in mc.items():
                ...
                # ЛЕКСЕМА ИДЁТ СВОИМ ШАГОМ, А НЕ СЕМЕНЕМ ЧИСЛА (07.09). Прежде
                # `pick` брался от `seed`, из которого выводится и число, — и пара
                # «лексема ↔ число» была жёстко связана: часть лексем не встречала
                # часть чисел никогда («минута» при 6…11 и ни разу при 2…4).
                #
                #     ДВА СЧЁТЧИКА ОТ ОДНОГО ЧИСЛА СВЯЗАНЫ, А НЕ НЕЗАВИСИМЫ.
                #
                # Ныне лексема шагает по кольцу единицей инстанса и прохода, число же
                # остаётся при своём семени: за оборот кольца лексема встречает разные
                # числа, за `len(pool)` оборотов — все объявленные ячейки.
                pick = (свой[1] if свой and свой[0] == cls_name
                        else pool[(j + pass_i * 3 + ti) % len(pool)])
                ...
                if cls.get(
                    ...
                    ctx[
                        f"lex:{cls_name}:by_n"
                    ] = счётная_форма(pack, cls, forms, n)
                    # ЧИСЛО ПРИ ЯЧЕЙКЕ, А НЕ ПРИ РАМКЕ (22.09) — тем же ходом, каким артикль
                    # встал при ячейке: «{num:n}» одно на всех имён давало «один карта» и
                    # «два марки» в тридцати двух строках русского слоя. Класс, объявивший
                    # свой род (`gender`), получает число этого рода; не объявивший — число
                    # как есть, и не платит ничем.
                    ctx[
                        f"num:{cls_name}:by_n"
                    ] = числительное_по_роду(
                        pack, numerals.get(str(n), str(n)), cls.get("gender")
                    )
                    ctx[
...
def main():
    ...
    if green and out:
        # ДВИЖОК ПАКЕТОВ ПИШЕТ ЧЕРЕЗ ТЕ ЖЕ ВОРОТА, ЧТО И ВСЯКИЙ ДОМ (23.09). Девять полей выше
        # судят ПАКЕТ; строку судит палата, и её здесь не спрашивали никогда: двадцать девять
        # языковых миров писались мимо ворот, и ложь пласта видел лишь полный набор раз в
        # тринадцать часов. Набор 22–23.09 назвал 33 лжи русского пласта («принесла одна
        # карта»), записанные движком ночью, — ворота, будь они здесь, не пустили бы ни одной.
        #
        #     ВОРОТА, МИМО КОТОРЫХ ПИШЕТ ОДИН ДВИЖОК, СТЕРЕГУТ ВСЕ ДОМА, КРОМЕ ДВАДЦАТИ ДЕВЯТИ
        #     МИРОВ. Выключатель прежний (`layer.ВОРОТА_ВЫКЛ`): им пользуется прибор
        #     воспроизводимости, собирающий миры в пустом дворе, где судов нет.
        import layer
        layer._ворота(out, body)
        with open(
