...
# СЛОВАРЬ СТРАНИЦ ДОМА ПОИСКА (13.09, пятый переезд того же вида). Перебор жил у кузницы, и
# тысяча двести восемьдесят строк мира `search_langs` не имели рода нигде, хотя род стои́т в
# самом переборе переменной `род`, взятой из `РОДЫ` этого дома.
#
#     ДОМ, НЕ ЗНАЮЩИЙ СВОИХ СТРАНИЦ, НЕ МОЖЕТ НАЗВАТЬ ИХ РОДА.
#
# Дверь здесь одна — единственный `вон.append`, — и правка легла в неё.
КАНДИДАТЫ = tuple(n for n in range(8, 113) if следующее_простое(n) - n <= 8)
...
def перебор(шаг, язык):
    ...
    for i in range(16):
        ...
        вон.append(((вопрос(язык, род, **з) if спросить else
                     утверждение(язык, род, **з)), род))
    return вон
