...
def показ_среднего(ряд):
    ...
    # ЗВЕНЬЯ ЦЕПИ (compose, e9): сумма слагаемыми, счёт слагаемых числом
    # леджера, деление звеном — делитель «3» становится числом ответа.
    сумма = " + ".join(str(x) for x in ряд)
    return [
        f"the average of {р} is {среднее(ряд)}: {сумма} = {sum(ряд)}, "
        f"there are {к} of them, {sum(ряд)} ÷ {к} = {среднее(ряд)}.",
        f"среднее чисел {р} — это {среднее(ряд)}: {сумма} = {sum(ряд)}, "
        f"их количество {к}, {sum(ряд)} ÷ {к} = {среднее(ряд)}.",
    ]
...
#
# ТАБЛИЦА КУЗНЕЦОВ ЗВАЛАСЬ `РОДЫ` И ТЕМ ЗАНИМАЛА ИМЯ ОБЪЯВЛЕНИЯ (14.09).
# Указатель родов читает `РОДЫ` как РЯД ИМЁН; получив ряд пар «(имя,
# функция)», он отвергал дом с причиной «объявление не кортеж имён, а
# таблица, совпавшая именем» — и 1 300 строк мира оставались безымянными
# при том, что имя рода стояло в этом файле первым словом каждой пары.
#
#     ИМЯ РОДА, ЗАПИСАННОЕ РЯДОМ СО СВОИМ КУЗНЕЦОМ, СКАЗАНО ДОМУ И НЕ
#     СКАЗАНО СВОДУ. Таблица зовётся теперь по делу своему, а объявление
#     берётся из неё ПЕРВЫМИ СТОЛБЦАМИ — двум спискам разойтись негде.
КУЗНЕЦЫ = (
...
РОДЫ = tuple(имя for имя, _кузнец in КУЗНЕЦЫ)

ЗАЧЕМ_РОДА = {
    "average": "среднее ВМЕСТЕ с суммой и счётом, из которых оно частное",
    "question": "та же истина, спрошенная и отвеченная с тем же основанием",
    "sum": "сумма ряда отдельной строкой — первая половина частного",
    "count": "счёт ряда отдельной строкой — вторая половина частного",
    "largest": "наибольшее из ряда: величина, нужная закону",
    "smallest": "наименьшее из ряда: вторая граница",
    "law": "среднее лежит МЕЖДУ границами — первое, что не есть определение",
    "counterexample": "«среднее всегда одно из чисел» — ложь, убитая одним свидетелем",
}


def перебор_страниц(шаг):
    """[(строка, род)] прохода — ТЕМ ЖЕ ОБХОДОМ, каким кузница строит группы."""
    ряд_прохода = ряды(шаг)
    вон = []
    for имя, показать in КУЗНЕЦЫ:
        for ряд in ряд_прохода:
            вон += [(с, имя) for с in показать(ряд)]
    return вон


def группы(шаг):
    """Дом отдаёт кузнице то же, что и прежде: группа на род."""
...
def _самопроверка_дома():
    ...
    # ПЕРЕБОР И ГРУППЫ ОБЯЗАНЫ БЫТЬ ОДНИМ ОБХОДОМ, А НЕ ДВУМЯ ПОХОЖИМИ. Сложенные
    # группы прохода суть тот же ряд строк, что и перебор без меток, — иначе
    # указатель называл бы род строкам, которых в мире нет.
