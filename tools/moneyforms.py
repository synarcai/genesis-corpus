...
# РОД-УПРАЖНЕНИЕ ОБЪЯВЛЕН ВСЛУХ (14.09, прибор очертаний). «Сумма двух размеров» есть ОДНО ДЕЛО НА МНОГИХ ЧИСЛАХ: два размера, их сумма и
# проверка в мелкой монете. Триста шестьдесят страниц на девять очертаний суть девять
# валют, каждая на сорока парах чисел, — и всякая пара считается заново.
#
# Род «курс, сказанный предложением» (9 страниц на 9 очертаний) упражнением НЕ объявлен:
# там девять валют и девять разных курсов, и число очертаний равно числу страниц само.
#
#     ОБЪЯВИТЬ РОД УПРАЖНЕНИЕМ — ЗНАЧИТ ВЗЯТЬ НА СЕБЯ, А НЕ УКЛОНИТЬСЯ.
# ПРОЧТЁН ВЕСЬ СВОД РОДОВ, А НЕ ДВЕ СТРОКИ ВЕРДИКТА (14.09). Рядом с объявленным родом
# стоял ЕГО БЛИЗНЕЦ — «обратный ход», те же девять валют на сорока парах чисел, — и первая
# редакция его пропустила: прибор называет крайних, а не всех.
#
# Четыре рода размена суть упражнения: девять валют, каждая на многих парах чисел, и всякая
# пара считается заново. Роды курса («1 Euro = 100 Cent», «2 Euro = 200 Cent») НЕ объявлены:
# там число очертаний равно числу валют само собою, и повторять нечего.
ОЧЕРТАНИЕ_ПО_ПРИРОДЕ = ("сумма двух размеров", "обратный ход: из крупной в мелкую",
                        "мелкая единица, писанная крупной через запятую, с леджером",
                        "вопрос о том же размене, отвечаемый тем же утверждением",
                        "курс на целом числе")
...
def _проверить_предметные():
    ...
    есть = {}
    # ПОКАЗ НЕСЁТ ПАРУ «(ЯЗЫК, РОД)» С 15.09: род — вторая её половина.
    for _с, (_язык, род) in ПОКАЗЫ.items():
