...
РЕЧЬ = {
    "ru": dict(польз="пользователь", орг="организм", файл="файл {f}", нет_файла="файла {f}",
               ...
               пуст="{НФ} нет — акт не совершён", двоеточие=": "),
    "en": dict(польз="user", орг="organism", файл="the file {f}", нет_файла="the file {f}",
               ...
               пуст="{НФ} is not there — the act is not performed", двоеточие=": "),
    "de": dict(польз="Benutzer", орг="Organismus", файл="die Datei {f}", нет_файла="die Datei {f}",
               ...
               пуст="{НФ} gibt es nicht — die Handlung wird nicht ausgeführt", двоеточие=": "),
    "fr": dict(польз="utilisateur", орг="organisme", файл="le fichier {f}", нет_файла="le fichier {f}",
               ...
               пуст="{НФ} n'existe pas — l'acte n'est pas exécuté", двоеточие=" : "),
    "es": dict(польз="usuario", орг="organismo", файл="el archivo {f}", нет_файла="el archivo {f}",
               ...
               пуст="{НФ} no existe — el acto no se realiza", двоеточие=": "),
    "it": dict(польз="utente", орг="organismo", файл="il file {f}", нет_файла="il file {f}",
               ...
               пуст="{НФ} non esiste — l'atto non viene eseguito", двоеточие=": "),
    "pt": dict(польз="utilizador", орг="organismo", файл="o ficheiro {f}", нет_файла="o ficheiro {f}",
               ...
               пуст="{НФ} não existe — o ato não é realizado", двоеточие=": "),
    "nl": dict(польз="gebruiker", орг="organisme", файл="het bestand {f}", нет_файла="het bestand {f}",
               ...
               пуст="{НФ} bestaat niet — de handeling wordt niet uitgevoerd", двоеточие=": "),
    "pl": dict(польз="użytkownik", орг="organizm", файл="plik {f}", нет_файла="pliku {f}",
               ...
               пуст="{НФ} nie ma — akt nie zostaje wykonany", двоеточие=": "),
}
ФОРМЫ = ("ход", "отказ", "занято", "пусто", "без_счёта")
...
def страница(язык, форма, акт, было, файл=ФАЙЛЫ[0]):
    ...
    хвост = р["папка"].format(N=файлов(язык, стало)) + р["двоеточие"] + счёт + "."
    if форма in ("занято", "пусто"):
        # АКТ НЕВОЗМОЖЕН: организм говорит ПОЧЕМУ и не двигает счёта
        причина = (р["есть"] if форма == "занято" else р["пуст"]).format(Ф=Ф, НФ=НФ)
        return приказ + " " + _реплика(язык, "орг", причина + ". " + хвост)
...
def _показы():
    ...
    for язык in ЯЗЫКИ:
        for акт in АКТЫ:
            for было in СОСТОЯНИЯ:
                for форма in ФОРМЫ:
                    if _годно(форма, акт, было):
                        вон[страница(язык, форма, акт, было)] = (язык, форма)
    return вон
...
def _дыры(язык):
    ...
    return {"f": файлы, "f2": файлы, "f3": файлы, "f4": файлы, "N": счёт,
...
def _самопроверка():
    ...
    for язык in ЯЗЫКИ:
        ...
        # (4) ЧТЕНИЕ ПРИБАВИЛО ФАЙЛ
        битая = б.replace("3 = 3", "3 + 1 = 4").replace(файлов(язык, 3), файлов(язык, 4))
        assert судить(битая) == (True, False), битая
