...
def _образцы():
    ...
    for язык, (утв, воп) in РАМКИ.items():
        ...
        свои = sorted({о for пары in ОПРЕДЕЛЕНИЯ[язык].values() for _, о in пары}, key=len, reverse=True)
        чужое = "(?:" + "|".join(re.escape(о) for о in свои) + ")"
        темы = sorted({т for пары in ОПРЕДЕЛЕНИЯ[язык].values() for т, _ in пары}, key=len, reverse=True)
