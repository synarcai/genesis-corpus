...
import units  # noqa: E402 — отношения календаря: дверь единиц, а не рука дома
...
ОПРЕДЕЛЕНИЯ = {
    ...
    "nl": {
        ...
        "время": (("een week", "zeven dagen achter elkaar"), ("een jaar", "twaalf maanden achter elkaar"),
                  ("een uur", "zestig minuten"), ("een dag", "vierentwintig uur"),
...
# РОД МЕРЫ ТАМ, ГДЕ ВОПРОСНОЕ СЛОВО ГНЁТСЯ (12.09). «¿cuántos horas tiene un día?» стояло в
# своде: рамка держала вопросное слово ЦЕЛЫМ и не слышала, что за ним подставят. Меры этого
# дома — те же, что у дома единиц времени, и беда была та же; род объявлен ПОИМЁННО и целиком.
#
#     СЛУЖЕБНОЕ СЛОВО, СТОЯЩЕЕ В РАМКЕ ЦЕЛЫМ, НЕ СЛЫШИТ ТОГО, ЧТО ЗА НИМ ПОДСТАВЯТ.
РОД_МЕРЫ = {
    "es": {"días": "m", "meses": "m", "horas": "f", "minutos": "m", "segundos": "m"},
    "it": {"giorni": "m", "mesi": "m", "ore": "f", "minuti": "m", "secondi": "m"},
    "pt": {"dias": "m", "meses": "m", "horas": "f", "minutos": "m", "segundos": "m"},
}
def квопрос(язык, мв):
    """Вопросное слово, согласованное с мерой: «cuántas horas», но «cuántos minutos»."""
    return romgram.по_слову(язык, мв, РОД_МЕРЫ.get(язык, {}))
...
СЧЁТ = {
    ...
    "es": dict(в="{б} tiene {n} {м}.", воп="{б} tiene {n} {м}. ¿{КВ} {мв} tiene {б}? {n} {м}.",
    ...
    "it": dict(в="{б} ha {n} {м}.", воп="{б} ha {n} {м}. {КВ} {мв} ha {б}? {n} {м}.",
    ...
    "pt": dict(в="{б} tem {n} {м}.", воп="{б} tem {n} {м}. {КВ} {мв} tem {б}? {n} {м}.",
    ...
    "nl": dict(в="{б} heeft {n} {м}.", воп="{б} heeft {n} {м}. hoeveel {мв} heeft {б}? {n} {м}.",
               пары=((("een week", "dagen", "dagen"), 7), (("een jaar", "maanden", "maanden"), 12), (("een dag", "uur", "uur"), 24),
...
# ЧИСЛА КАЛЕНДАРЯ ПРИВЯЗАНЫ К ДВЕРИ УТВЕРЖДЕНИЕМ (22.09). Дом писал 7, 12, 24, 60, 60 рукой на
# девяти языках, а дверь `units` держит те же отношения рёбрами графа. Копия, привязанная
# утверждением к первой, не разойдётся молча — она падёт на ввозе.
ЕДИНИЦЫ_ПАР = (("week", "day"), ("year", "month"), ("day", "hour"), ("hour", "minute"),
               ("minute", "second"))
for _язык, _с in СЧЁТ.items():
    for (_, _n), (_б, _м) in zip(_с["пары"], ЕДИНИЦЫ_ПАР):
        assert units.отношение(_б, _м) == _n, (_язык, _б, _м, _n, "рука разошлась с дверью")

# ЗНАНИЕ СЧЁТА БЕЗ ФАКТА НА СТРАНИЦЕ (22.09, заказ руки agent по полосе беседы). Дом показывал
# вопрос ЛИШЬ ПОСЛЕ факта: «в неделе 7 дней. сколько дней в неделе? 7 дней.» — ответ списывался
# со страницы, и один показ на язык стоял ниже закона повтора. Полоса беседы спрашивает БЕЗ
# факта: «сколько дней в неделе?» — и организм, видевший ответ лишь рядом с его основанием,
# молчал, а вычисляющий судья звал молчание ложью (ждал 7).
#
#     ФАКТ, ПОКАЗАННЫЙ ЛИШЬ ПЕРЕД СВОИМ ВОПРОСОМ, УЧИТ ЧИТАТЬ СТРАНИЦУ, А НЕ ЗНАТЬ.
#
# Форма знания: вопрос — ДВЕ поверхности языка, ответ — голова одна («7 дней.») и голова с
# основанием («7 дней: в неделе 7 дней.»): 2 × 2 = 4 показа на факт и язык. Голова одна
# показана — по закону «голова доказана ответом» того же дня.
ВТОРОЙ_ВОПРОС = {
    "de": ("wie viele {мв} gibt es in {в_б}?",
           ("einer Woche", "einem Jahr", "einem Tag", "einer Stunde", "einer Minute")),
    "es": ("¿{КВ} {мв} hay en {б}?", None),
    "it": ("{КВ} {мв} ci sono in {б}?", None),
    "pt": ("{КВ} {мв} há {в_б}?", ("numa semana", "num ano", "num dia", "numa hora", "num minuto")),
    "nl": ("hoeveel {мв} zitten er in {б}?", None),
    "pl": ("ile {мв} jest w {в_б}?", ("tygodniu", "roku", "dobie", "godzinie", "minucie")),
}
ОСНОВАНИЕ = {"fr": "{n} {м} : {факт}"}   # прочие языки: «{n} {м}: {факт}»


def _вопросы(язык, с, i, б, мв):
    """Поверхности вопроса о факте: вопросные части объявленных рамок и вторая, где нужна."""
    вон = []
    for ключ in ("воп", "воп2"):
        if ключ in с:
            рамка = с[ключ]
            вопрос = рамка[рамка.index(". ") + 2:рамка.rindex("? ") + 1] if "? " in рамка else None
            if вопрос is None:   # французская рамка кончает вопрос « ?»
                вопрос = рамка[рамка.index(". ") + 2:рамка.rindex(" ? ") + 2]
            вон.append(вопрос.format(б=б, мв=мв, КВ=квопрос(язык, мв)))
    if язык in ВТОРОЙ_ВОПРОС:
        рамка, падеж = ВТОРОЙ_ВОПРОС[язык]
        вон.append(рамка.format(б=б, мв=мв, КВ=квопрос(язык, мв),
                                в_б=(падеж[i] if падеж else б)))
    return вон


# ЦЕПЬ КАЛЕНДАРЯ: две единицы через третью, и кузница показывает оба шага
# ЦЕПЬ ТРЕБУЕТ СТЫКА: выход одной пары обязан быть входом другой (сутки→часы, часы→минуты).
# В объявленных парах счёта стыков ДВА, и род «цепь счёта» имеет 18 страниц против 477 у
# определения — перекос дома 26,5×.
#
# ДОЛГ НАЗВАН, А НЕ ОБЪЯВЛЕН ПРИРОДОЙ (15.09). Цепей мало не оттого, что их мало в мире, а
# оттого, что в `СЧЁТ` не объявлены пары «день → часы» и «месяц → дни». Стоит их добавить — и
# цепей станет пять вместо двух.
#
...
#     недоделку.
#
# Работа отложена и названа: пары счёта объявлены ОТДЕЛЬНО НА ДЕВЯТИ ЯЗЫКАХ, и дописывать их
# надо на всех девяти разом — иначе дом станет двуязычным в одном роде и девятиязычным в
# прочих. Спешить тут значило бы чинить перекос родов, наживая перекос языков.
...
ЦЕПЬ_РАМКА = {
    ...
    "es": "{б1} tiene {n1} {м1}, {б2} tiene {n2} {м2}. ¿{КВ2} {мв2} tiene {б1}? {r} {м2}: {n1} × {n2} = {r}.",
    "it": "{б1} ha {n1} {м1}, {б2} ha {n2} {м2}. {КВ2} {мв2} ha {б1}? {r} {м2}: {n1} × {n2} = {r}.",
    "pt": "{б1} tem {n1} {м1}, {б2} tem {n2} {м2}. {КВ2} {мв2} tem {б1}? {r} {м2}: {n1} × {n2} = {r}.",
...
def _показы():
    ...
    for язык, с in СЧЁТ.items():
        for (б, м, мв), n in с["пары"]:
            ...
            вон[с["воп"].format(б=б, м=м, мв=мв, n=n, КВ=квопрос(язык, мв))] = (язык, "вопрос_счёта")
            if "воп2" in с:   # вторая вопросная рамка там, где язык её имеет
                вон[с["воп2"].format(б=б, м=м, мв=мв, n=n, КВ=квопрос(язык, мв))] = (язык, "вопрос_счёта")
        for i, ((б, м, мв), n) in enumerate(с["пары"]):
            факт = с["в"].format(б=б, м=м, мв=мв, n=n)
            голова = f"{n} {м}"
            ответы = (голова + ".", ОСНОВАНИЕ.get(язык, "{n} {м}: {факт}").format(n=n, м=м, факт=факт))
            for вопрос in _вопросы(язык, с, i, б, мв):
                for ответ in ответы:
                    вон[f"{вопрос} {ответ}"] = (язык, "знание_счёта")
        for i, j in ЦЕПИ:
            (б1, м1, мв1), n1 = с["пары"][i]
            (б2, м2, мв2), n2 = с["пары"][j]
            вон[ЦЕПЬ_РАМКА[язык].format(б1=б1, м1=м1, мв1=мв1, n1=n1, КВ2=квопрос(язык, мв2),
...
ПОКАЗЫ = {(_fr.элизия(с) if (v[0] if isinstance(v, tuple) else v) == "fr" else с): v
          for с, v in _показы().items()}
ФОРМЫ = ("определение", "вопрос_определения", "счёт", "вопрос_счёта", "знание_счёта", "цепь_счёта",
         "граница")
ЯЗЫКИ = СЧЁТ   # девять языков: счёт держат все, определения — пять


# ОБРАЗЦЫ СОБРАНЫ ЦЕЛИКОМ, А НЕ ПО НАЧАЛУ СТРОКИ (М-180-f2, и здесь цена этому
# названа делом). Суд по началу читал «{т} есть …» и звал ложью ВСЯКУЮ строку,
# начавшуюся объявленной темой, — а тема «snow» стоит первым словом и у соседа:
# «snow is cold. why? because snow is frozen water.» есть честный факт мира,
# который дом тем назвал ложью в тот час, как он появился.
#
# ЛОВУШКА НАЧАЛА ВЗВЕДЕНА НА БУДУЩЕЕ И ПОТОМУ НЕ ЛОВИТСЯ ПРИБОРАМИ СЕГОДНЯ:
# прибор ЧУЖОЙ РАМКИ сличает дома по СУЩЕСТВУЮЩИМ показам и нашёл 0 захватов —
# честно, ибо ни один нынешний показ соседа не начинался темой этого дома. Суд
# по началу молчит ровно до дня, когда сосед напишет свою первую такую строку.
#
# ДЫРА ОПРЕДЕЛЕНИЯ НЕ ПЕРЕХОДИТ ГРАНИЦЫ ПРЕДЛОЖЕНИЯ, и в этом вся разница.
# Всякое определение есть ОДНА клауза; дыра, связанная как «[^.?!]+», берёт
# любое определение — и объявленное, и выдуманное, — но обрывается на первой
# точке. Оттого дом по-прежнему зовёт ложью подмену определения («вода — это
# металл.»), а к строке соседа, где после точки идёт своя речь, не прикасается.
# Список объявленных определений связал бы дыру уже предмета: подмена, которой
# дом не предвидел, ушла бы в молчание.
def _длиннее(с):
    """ПОРЯДОК ВЕТВЕЙ ЕСТЬ ПОЛНЫЙ ПОРЯДОК: длиннее вперёд, равные — по самому слову.

    Сортировка одной лишь длиной оставляет равные слова в порядке МНОЖЕСТВА, а он у строк
    свой в каждом процессе (соль хеша). Вердикт от этого не менялся — равные по длине ветви
    суть разные литералы, и в одном месте строки может совпасть лишь одна, — но ТЕКСТ образца
    плясал от запуска к запуску. Кэш собранных образцов сделал это видимым числом: палата
    строит 5 164 образца, и около 3 100 из них при следующем запуске оказывались другими
    строками. Порядок ветвей обязан быть выводим из содержания (закон детерминизма, ранг
    содержания вместо порядка множества).
    """
    return (-len(с), с)


def _образцы():
    # ОДИН ОБРАЗЕЦ НА ЯЗЫК И РАМКУ, А НЕ ОДИН НА ТЕМУ (05.09, мандат ускорения: построение палаты
    # есть цена каждого рабочего ворот). Прежде дом компилировал 954 образца — по одному на тему и
    # рамку, каждый с полным перечислением 53 определений языка, — и на это уходило 6 с из 22 с
    # холодной палаты. Тема есть такая же дыра, как определение: перечисление тем в дыре темы
    # даёт ровно то же множество строк, что объединение образцов по темам; вопросная рамка
    # называет тему ДВАЖДЫ — второе место обязано повторить первое (обратная ссылка), как в
    # каждом прежнем образце тема стояла одна и та же. Тождество вердиктов — самопроверка ниже.
    вон = []
    for язык, (утв, воп) in РАМКИ.items():
        # ПОДМЕНА ЕСТЬ ОБЪЯВЛЕННОЕ ОПРЕДЕЛЕНИЕ ДРУГОЙ ТЕМЫ, А НЕ ЛЮБОЙ ПРЕДИКАТ. Дыра «{о}» брала
        # всё до точки, и суд звал ложью честные строки соседних миров о своём слове: «год — это
        # мера.» (единицы), «work is measured in joules.» (физика) — граница берётся у предмета
        # (М-180-f2): чужое сказуемое суду не подсудно, чужое определение — ложь
        свои = sorted({о for пары in ОПРЕДЕЛЕНИЯ[язык].values() for _, о in пары}, key=_длиннее)
        чужое = "(?:" + "|".join(re.escape(о) for о in свои) + ")"
        темы = sorted({т for пары in ОПРЕДЕЛЕНИЯ[язык].values() for т, _ in пары}, key=_длиннее)
        тема = "(?P<т>" + "|".join(re.escape(т) for т in темы) + ")"
        for ш in (утв, воп):
            узор = re.escape(ш).replace(re.escape("{т}"), тема, 1)
            узор = узор.replace(re.escape("{т}"), "(?P=т)")
            узор = узор.replace(re.escape("{о}"), чужое)
...
def _самопроверка():
    ...
    for язык in ОПРЕДЕЛЕНИЯ:
        ...
        мутанты += 1
    # ТОЖДЕСТВО ОБРАЗЦА ПО ЯЗЫКУ С ОБРАЗЦАМИ ПО ТЕМАМ: всякая рамка с объявленной темой и любым
    # объявленным определением ловится; чужая тема, чужое определение и вопрос, называющий две
    # разные темы, — нет (ровно то, что ловили и не ловили 954 прежних образца)
    for язык in ОПРЕДЕЛЕНИЯ:
        утв, воп = РАМКИ[язык]
        темы = [т for пары in ОПРЕДЕЛЕНИЯ[язык].values() for т, _ in пары]
        свои = [о for пары in ОПРЕДЕЛЕНИЯ[язык].values() for _, о in пары]
        for т in темы:
            for о in свои:
                for ш in (утв, воп):
                    assert any(об.fullmatch(ш.format(т=т, о=о)) for об in _ОБРАЗЦЫ), (язык, т, о)
        т1, т2 = темы[0], темы[1]
        for с in (утв.format(т="xyzzy", о=свои[0]), утв.format(т=т1, о="xyzzy"),
                  воп.replace("{т}", т1, 1).replace("{т}", т2).format(о=свои[0])):
            assert not any(об.fullmatch(с) for об in _ОБРАЗЦЫ), (язык, с)
            мутанты += 1
    for язык in СЧЁТ:
        (б, м, мв), n = СЧЁТ[язык]["пары"][0]
        с = СЧЁТ[язык]["воп"].format(б=б, м=м, мв=мв, n=n, КВ=квопрос(язык, мв))
    ...
    for язык in ("ru", "en", "de"):
        ...
        print("  ", СЧЁТ[язык]["воп"].format(б=б, м=м, мв=мв, n=n, КВ=квопрос(язык, мв))[:112])
    i, j = ЦЕПИ[0]
...
# ДОМ ОБЪЯВЛЯЕТ СЕБЯ СУДЯЩИМ ОБЪЯВЛЕНИЕМ (21.09). Его страница есть утверждение из
# объявленной таблицы, а не вычисление: вывести её неоткуда, кроме как из объявления, и
# набор показов ЕСТЬ это объявление. Прибор подсадки, вписывающий порчу в набор, вписывает
# её в объявление — и спрашивает дом о строке, которую сам же объявил верной.
#
# Объявление это не отговорка и проверяется ДВАЖДЫ: принимается оно лишь от СТРАЖА, ловящего
# свою порчу без всякого вписывания, и лишь у того, кто без набора не признаёт НИ ОДНОЙ
# своей страницы, — то есть кому и вправду нечего вычислять.
