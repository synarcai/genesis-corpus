...
# родов). Семнадцать родов этого дома суть счёт на числовом ряду: одно дело — сравнить, назвать
# соседа, поделить пополам, продолжить ряд — и числа при нём материал. Роды «словом» (180 на
# 180) и «шагом» (144 на 144) НЕ объявлены: там перебирается САМА ЗАПИСЬ числа словами и сам
# шаг счёта, и род ровен собою.
#
#     УПРАЖНЕНИЕ ТАМ, ГДЕ ЧИСЛА СУТЬ МАТЕРИАЛ, А МЫСЛЬ ОДНА.
ОЧЕРТАНИЕ_ПО_ПРИРОДЕ = ("больше",
                        "больше_ли",
                        "больше_ли_нет",
                        "не_больше_ли",
                        "не_больше_ли_да",
...
# РЯД ВОЗРАСТОВ ДОВЕДЁН ДО ОПОРЫ (08.09). Прибор «ключ без опоры» держал два рода вопросов
# ключа — русский «сколько мне будет через k лет?» и польский «ile będę mieć lat?» — на ШЕСТИ
# страницах при законе опоры LAW³ = 8. Шесть пар давали ровно шесть страниц на язык: рамка
# ходит по объявленному ряду и ничего сверх него не выдумывает.
#
#     РОД ВОПРОСА СТОИ́Т НА СТОЛЬКИХ СТРАНИЦАХ, СКОЛЬКО ПАР ОБЪЯВЛЕНО, — И НИ НА ОДНОЙ БОЛЬШЕ.
#
# Четыре пары приписаны, и выбраны они не наугад: 21 + 1 = 22 ведёт русскую форму через «год —
# года» на переходе десятка, 14 + 7 = 21 — через «лет — год», 2 + 2 = 4 держит малый счёт,
# 11 + 3 = 14 — счёт подросткового десятка, где русское «лет» стои́т при всех числах.
...
# ТАБЛИЦА РАМОК И СПИСОК РОДОВ СУТЬ РАЗНЫЕ ВЕЩИ, ХОТЯ ЧАЩЕ ВСЕГО СОВПАДАЮТ (13.09). Ключ рамки
# есть ПОВЕРХНОСТЬ; род есть ЗАКОН. Где у закона две поверхности — повеление и его вопросный
# близнец, — ключей два, а род ОДИН, и страницы обоих метятся именем повеления. Указатель
# родов, прочитавший `ФОРМЫ` как список родов, насчитал два рода без единой страницы.
#
#     РОД, ОБЪЯВЛЕННЫЙ И НЕ КОВАННЫЙ, ЕСТЬ ОБЕЩАНИЕ, А НЕ РОД, — но здесь и обещания не было:
#     была таблица рамок, прочитанная как список родов.
РОДЫ = tuple(ф for ф in ФОРМЫ if ф not in БЛИЗНЕЦЫ.values())
...
def _судить_образцом(строка):
    """(судимо, истинно) по образцу — СЫРОЙ суд, без закона знака."""
    с = строка.strip()
...
import closedworld  # noqa: E402 — закон замкнутого мира читается после сборки показов
_СКЕЛЕТЫ_ЗНАКА = closedworld.скелеты_знака(ПОКАЗЫ)


def судить(строка):
    """(судимо, истинно): the line is a page of a frame, and its numbers AND SIGN hold.

    ЗНАК ДЕЙСТВИЯ СУДИТСЯ ЗАКОНОМ ЗАМКНУТОГО МИРА, А НЕ ДЫРОЙ В КАЖДОЙ ИЗ ТРИДЦАТИ РАМОК:
    строка, становящаяся ИСТИННОЙ при замене одного знака, есть строка ЭТОГО дома с испорченным
    знаком. Прежде «11 + 1 = 10» вместо «11 − 1 = 10» получало НЕМОТУ, и палата брала истину у
    суда арифметики — у соседа (М-131).
    """
    вердикт = _судить_образцом(строка)
    if вердикт[0] is False and closedworld.ложь_по_знаку(
