...
def показы_меченые(языки, шаг, сколько=12):
    """То же, но при каждой строке — её язык: он взят у цикла кладки, а не у письма."""
    свои = [я for я in языки if я in ФОРМЫ]
    порядок = [я for _ in range(сколько) for я in свои]
    return list(zip(показы(языки, шаг, сколько), порядок))
