    ...
    for язык, я in ЯЗЫКИ.items():
        ...
        for форма in ФОРМЫ:
            ...
            for м in re.finditer(r"\{(\w+)\}", ш):
                ...
                дыра = м.group(1)
                куски.append(альт(имена if дыра in ("a", "b", "c") else
...
def судить(строка):
    """Показ дома истинен; строка ЕГО ЖЕ рамки с перевёрнутым порядком — ложь,
    ибо ряд объявлен и места в нём считаны. Чужая строка дому не подсудна."""
    с = строка.strip()
    if с in ПОКАЗЫ:
        return True, True
