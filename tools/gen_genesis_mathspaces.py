...
def показы_точек(шаг):
    ...
    for i in range(14):
        ...
        if i % 4 == 0:
            ...
            # ПОЛЯРНОСТЬ ОТВЯЗАНА ОТ ЯЗЫКА (М-136, ширина вопроса 03.09): чётность
            # (шаг + i) выбирает язык, и та же чётность делала русские повороты
            # одними отрицаниями — рамка без второй полярности.
            if (шаг + i) // 2 % 2:
...
# ---------- вопросные поверхности ----------
# ВОПРОС ВЫВОДИТСЯ ИЗ УТВЕРЖДЕНИЯ ОБЪЯВЛЕННОЙ ТАБЛИЦЕЙ (ширина вопроса 03.09:
# 41 род пространств без единого вопроса). Ответ есть само утверждение,
# и судит его тот же образец через дом пары; отрицания вопроса не получают
# — они полярность, а не предмет. Зачин каждого вопроса объявлен домом
# пары; величины вопроса открывают ответ в его порядке.
ВОПРОСЫ = (
    (r"^(the distance between \(.*?\) and \(.*?\)) is (?!not )", "what is {1}?"),
    (r"^(расстояние между \(.*?\) и \(.*?\)) — (?!не )", "каково {1}?"),
    (r"^(the midpoint of \(.*?\) and \(.*?\)) is (?!not )", "what is {1}?"),
    (r"^(середина отрезка \(.*?\) и \(.*?\)) — (?!не )", "какова {1}?"),
    (r"^(\(.*?\) shifted by \(.*?\)) is (?!not )", "what is {1}?"),
    (r"^(\(.*?\) после сдвига на \(.*?\)) — (?!не )", "куда переходит {1}?"),
    (r"^(\(.*?\) reflected in the [xy]-axis) is (?!not )", "what is {1}?"),
    (r"^(\(.*?\) после отражения относительно оси [xy]) — (?!не )", "куда переходит {1}?"),
    (r"^(\(.*?\) rotated \d+° counterclockwise about the origin) is (?!not )", "what is {1}?"),
    (r"^(\(.*?\) после поворота на \d+° против часовой стрелки вокруг начала координат) — (?!не )", "куда переходит {1}?"),
    (r"^(the degree of \d+ in graph .*?) is (?!not )", "what is {1}?"),
    (r"^(степень вершины \d+ в графе .*?) — (?!не )", "какова {1}?"),
    (r"^graph (.*?) has (\d+) (vertices|edges)\.$", "how many {3} does graph {1} have?"),
    (r"^в графе (.*?) (\d+) (вершин\S*|р[её]б\S*)\.$", "сколько {3:вершин|рёбер} в графе {1}?"),
    (r"^graph (.*?) has a path from (\d+) to (\d+) of length", "in graph {1}, is there a path from {2} to {3}?"),
    (r"^в графе (.*?) есть путь от (\d+) до (\d+) длины", "есть ли в графе {1} путь от {2} до {3}?"),
    (r"^there is no path from (\d+) to (\d+) in graph (.*?):", "in graph {3}, is there a path from {1} to {2}?"),
    (r"^пути от (\d+) до (\d+) в графе (.*?) нет:", "есть ли в графе {3} путь от {1} до {2}?"),
    (r"^graph (.*?) is (?:not )?connected:", "is graph {1} connected?"),
    (r"^граф (.*?) (?:не )?связен:", "связен ли граф {1}?"),
    (r"^graph (.*?) (?:has|does not have) the cycle ([\d-]+):", "does graph {1} have the cycle {2}?"),
    (r"^в графе (.*?) (?:нет|есть) цикла? ([\d-]+):", "есть ли в графе {1} цикл {2}?"),
    (r"^Fano plane (.*?): points (\d) and (\d) lie on exactly one line", "which line of Fano plane {1} carries points {2} and {3}?"),
    (r"^плоскость Фано (.*?): точки (\d) и (\d) лежат ровно на одной линии", "какая линия плоскости Фано {1} несёт точки {2} и {3}?"),
    (r"^Fano plane (.*?): lines (\d\d\d) and (\d\d\d) meet", "which point of Fano plane {1} do lines {2} and {3} share?"),
    (r"^плоскость Фано (.*?): линии (\d\d\d) и (\d\d\d) пересекаются", "какую точку плоскости Фано {1} делят линии {2} и {3}?"),
    (r"^Fano plane (.*?): point (\d) lies on exactly", "how many lines of Fano plane {1} pass through point {2}?"),
    (r"^плоскость Фано (.*?): точка (\d) лежит ровно", "сколько линий плоскости Фано {1} проходит через точку {2}?"),
    (r"^(matrix \[.*?\] transposed) is (?!not )", "what is {1}?"),
    (r"^(матрица \[.*?\] транспонированная) — (?!не )", "какова {1}?"),
    (r"^(\[.*?\] \+ \[.*?\]) = ", "what is {1}?"),
    (r"^(\[.*?\] · \(.*?\)) = ", "what is {1}?"),
    (r"^(the trace of \[.*?\]) is (?!not )", "what is {1}?"),
    (r"^(след матрицы \[.*?\]) — (?!не )", "каков {1}?"),
    (r"^(the determinant of \[.*?\]) is (?!not )", "what is {1}?"),
    (r"^(определитель матрицы \[.*?\]) — (?!не )", "каков {1}?"),
    (r"^((?:row|column) \d of \[.*?\]) is ", "what is {1}?"),
    (r"^(строка \d матрицы \[.*?\]) — ", "какова {1}?"),
    (r"^(столбец \d матрицы \[.*?\]) — ", "каков {1}?"),
)
_ВОПРОСЫ = tuple((re.compile(о), ш) for о, ш in ВОПРОСЫ)


def вопрос_к(утверждение):
    """Вопросная строка к утверждению по таблице, или None (отрицание, чужой род)."""
    for о, ш in _ВОПРОСЫ:
        м = о.match(утверждение)
        if not м:
            continue
        группы = list(м.groups())
        if "{3:вершин|рёбер}" in ш:
            ш = ш.replace("{3:вершин|рёбер}", "вершин" if группы[2].startswith("верш") else "рёбер")
        текст = ш
        for k, г in enumerate(группы, 1):
            текст = текст.replace("{" + str(k) + "}", г or "")
        return f"{текст} {утверждение}"
    return None


def с_вопросами(строки, шаг):
    """Каждое третье утверждение получает вопросного двойника рядом."""
    вон = []
    for k, с in enumerate(строки):
        вон.append(с)
        if (k + шаг) % 3 == 0:
            в = вопрос_к(с)
            if в:
...
def pass_groups(шаг):
    return [с_вопросами(г, шаг) for г in (показы_точек(шаг), показы_графов(шаг), показы_фано(шаг), показы_матриц(шаг))] + [laws.ступень("mathspaces")]
