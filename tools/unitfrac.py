...
def леджер(единица, дробь):
    ...
    ЗНАМЕНАТЕЛЬ И ЗНАК ДЕЙСТВИЯ СТОЯТ ДЫРАМИ, А НЕ БУКВАМИ. Держать «÷ 2» буквой образца значит ослепнуть ровно
    ...
    ТО ЖЕ ВЕРНО ДЛЯ САМОГО ЗНАКА (М-489, прибор scripts/sign_traps.py): «60 × 2 = 30» вместо
    «60 ÷ 2 = 30» — половина, посчитанная умножением, — тоже не совпадала ни с одним образцом
    и тоже молчала. Дом брал знак буквой, и палата ловила подмену судом арифметики, то есть
    брала истину у соседа (М-131). Ныне знак есть дыра; вердикт получается сам, ибо суд дома
    ПЕРЕСОБИРАЕТ страницу и сличает строкой.
    """
    if дробь in ("половина", "четверть"):
        return "{C} {зн1} {z} = {n}"
    if дробь == "три_четверти":
        return "{C} {зн1} {z} {зн2} {ч} = {n}"
    return "{C} {зн1} {H} = {n}"
...
def рамка(язык, единица, дробь, форма):
    ...
    return р["сумма"] + р["двоеточие"] + "{n} {зн3} {k} = {s}."
...
def страница(язык, единица, дробь, форма, прибавка=0):
    ...
    зн = dict(Ф=_слово(язык, единица, дробь), N=_счётно(язык, единица, n), C=C, H=C // 2, n=n,
              z=з, ч=ч, зн1=("+" if дробь == "полтора" else "÷"), зн2="×", зн3="+")
    if форма == "сумма":
...
def _дыры(язык, единица):
    ...
    return {"Ф": фразы, "N": счётные, "K": счётные, "S": счётные,
            ...
            "z": r"\d+", "ч": r"\d+",
            # ЗНАК ДЕЙСТВИЯ — ДЫРА: подменённый знак обязан быть ЛОЖЬЮ, а не немотой (М-489)
            "зн1": r"[+−×÷]", "зн2": r"[+−×÷]", "зн3": r"[+−×÷]"}
...
def судить(строка):
    ...
    #
    # Замерено: закон судит все 198 показов дома верно и без набора — выход был
