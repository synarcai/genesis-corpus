...
def леджер(единица, дробь):
    ...
    C, n = ОСНОВА[единица], величина(единица, дробь)
    if дробь == "половина":
        return "{C} ÷ 2 = {n}"
    if дробь == "четверть":
        return "{C} ÷ 4 = {n}"
...
def рамка(язык, единица, дробь, форма):
    ...
    if форма == "вверх":
        # ЧИСЛО ПРОЧТЕНО ОБРАТНО ИМЕНЕМ — РАВЕНСТВО ОБРАТИМО
        return р["связка"] + р["двоеточие"] + леджер(единица, дробь) + "."
