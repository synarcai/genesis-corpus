...
РЕЧЬ = {
    ...
    "en": dict(утром="in the morning it was {t} {Гt}.", теплее="it became {d} {Гd} warmer.",
               ...
               двоеточие=": ", вопрос="?"),
    "de": dict(утром="am Morgen {БЫЛО} {t} {Гt}.", теплее="es wurde um {d} {Гd} wärmer.",
    ...
    "es": dict(утром="por la mañana hacía {t} {Гt}.", теплее="subió {d} {Гd}.",
               ...
               двоеточие=": ", вопрос="?"),
    "it": dict(утром="la mattina {БЫЛО} {t} {Гt}.", теплее="è salito di {d} {Гd}.",
               ...
               двоеточие=": ", вопрос="?"),
    "pt": dict(утром="de manhã {БЫЛО} {t} {Гt}.", теплее="subiu {d} {Гd}.",
    ...
    "nl": dict(утром="'s ochtends was het {t} {Гt}.", теплее="het werd {d} {Гd} warmer.",
               ...
               двоеточие=": ", вопрос="?"),
    "pl": dict(утром="rano {БЫЛО} {t} {Гt}.", теплее="zrobiło się cieplej o {d} {Гd}.",
...
# СВЯЗКА УТРА ГНЁТСЯ СВОИМ ЧИСЛОМ В ЧЕТЫРЁХ ЯЗЫКАХ ИЗ ДЕВЯТИ, И ЭТО ЗАМЕР, А НЕ ДОГАДКА
# (08.09). У английского, французского, испанского, нидерландского и русского связка утра одна
# на все числа («it was», «il faisait», «hacía», «was het», «было») — им дыры не нужно. У
# немецкого, итальянского и португальского она гнётся ЕДИНИЦЕЙ, и рамки писали множественное
# буквой: двенадцать страниц свода несли «am Morgen waren es 1 Grad», «la mattina c'erano 1
# grado», «de manhã estavam 1 grau». У польского она гнётся ПОЛОСОЙ СЧЁТА и родом вещи.
#
#     ГРАНИЦА ЗАКОНА: СВЯЗКА ВОПРОСА НЕ ГНЁТСЯ ОТВЕТОМ, ИБО ВОПРОС ЕГО НЕ ЗНАЕТ. «wie viel
#     Grad sind es jetzt?» стои́т при ответе «1 Grad» законно — спрашивающий числа не видел,
#     и множественное вопроса есть множественное НЕИЗВЕСТНОГО, а не ложь о единице.
#
# ГНЁТСЯ ПО МОДУЛЮ, КАК И СЧЁТНАЯ ФОРМА РЯДОМ: «minus ein Grad», «meno un grado», «menos um
# grau» — единственное; «meno 3 gradi» — множественное. Дом уже брал модуль для существительного
# (`_счётная`), и связка берёт тот же.
БЫЛО_ЯЗЫКИ = ("pl", "de", "it", "pt")
# (единственное при модуле единицы, множественное при прочих)
УТРО_СВЯЗКА = {"de": ("war es", "waren es"), "it": ("c'era", "c'erano"),
               "pt": ("estava", "estavam")}
# ГРАДУС ЕСТЬ «stopień», И ОН МУЖСКОГО РОДА — род берётся у ДОМА ЯЗЫКА по лемме, а не пишется
# здесь: «był 1 stopień», «były 3 stopnie», «było 6 stopni».
_ЛЕММА_ГРАДУСА = {"pl": "stopień"}


def _было(язык, t):
    """Связка утра при подписанном числе градусов.

    ОТРИЦАТЕЛЬНОЕ БЕРЁТ СРЕДНЕЕ ЕДИНСТВЕННОЕ, И ЭТО СКАЗАНО, А НЕ УМОЛЧАНО: «−3 stopnie»
    читается «minus trzy stopnie», и подлежащим тут стоит вся величина с «minus», а не
    существительное, — польский ставит при ней «było» при всяком числе («było minus 3
    stopnie», «było minus 5 stopni»). Полоса счёта правит лишь НЕОТРИЦАТЕЛЬНЫМ утром.

        ЗАКОН, ПРИЛОЖЕННЫЙ ЗА СВОЕЙ ГРАНИЦЕЙ, ЕСТЬ ТА ЖЕ БУКВА, ЧТО И ЕГО ОТСУТСТВИЕ.
    """
    if язык in УТРО_СВЯЗКА:
        return УТРО_СВЯЗКА[язык][0 if abs(t) == 1 else 1]
    if t < 0:
        return _PL.СВЯЗКА["прош"][2]
    return _PL.связка_вещи("прош", _ЛЕММА_ГРАДУСА[язык], t)
...
def страница(язык, форма, t=0, d=0, a=0, b=0):
    ...
    if язык in БЫЛО_ЯЗЫКИ:
        поля["БЫЛО"] = _было(язык, t)
    return рамка(язык, форма).format(**поля)
...
def _образец(язык, шаблон):
    ...
    дыры = {"t": подписанное, "t0": в_скобках, "v": подписанное, "a": подписанное, "b": подписанное,
            ...
            "Эd": _альт(ЭТАЖ[язык])}
    if язык in УТРО_СВЯЗКА:
        дыры["БЫЛО"] = _альт(УТРО_СВЯЗКА[язык])
    elif язык in БЫЛО_ЯЗЫКИ:
        дыры["БЫЛО"] = _альт(_PL.ГРУППЫ_СВЯЗКИ[1])
    счёт, куски = {}, []
...
def _вердикт(язык, форма, зн):
    ...
    if "БЫЛО" in зн and зн["БЫЛО"] != _было(язык, t):
        return False              # связка утра есть связка ЕГО числа, а не буква рамки
...
def судить(строка):
    ...
    #
    # Замерено: закон судит все 1296 показов дома верно и без набора — выход был
