    ...
    for яз, я in РАМКИ.items():
        for форма in ФОРМЫ:
            for поверхность in ("вопрос", "вопрос2"):
                ...
                узор = re.escape(рамка).replace("\x00", "(?P<у>[^.?!]+)")
                вон.append((re.compile(узор), яз, форма))
        # РАМКА ПАРЫ ЧИТАЕТСЯ НАРАВНЕ С ПРОЧИМИ (21.09). Дом пишет ею 54 страницы — факт и
        # мнение рядом при одном вопросе, — а `_хвосты` её не строил вовсе, и суд этих
        # страниц не узнавал: они держались одним лишь членством в наборе.
        воп, отв = ПАРА[яз]
        рамка = воп + " " + отв.replace("{ф}", "\x00").replace("{м}", "\x01")
        узор = (re.escape(рамка).replace("\x00", "(?P<ф>[^.?!]+)")
                .replace("\x01", "(?P<м>[^.?!]+)"))
        вон.append((re.compile(узор), яз, "пара"))
    return tuple(вон)


_РАМКИ_ЦЕЛИКОМ = _хвосты()

# УТВЕРЖДЕНИЕ НАЗЫВАЕТ СВОЙ РОД, И РОД ЭТОТ ОБЪЯВЛЕН (21.09). Прежде истиной дома было одно
# лишь членство в наборе показов: `return True, с in ПОКАЗЫ`.
#
#     СУД, У КОТОРОГО ИСТИНА ЕСТЬ ОДНО ЛИШЬ ЧЛЕНСТВО В НАБОРЕ, НЕ ЗАКОН, А СПИСОК. Здесь
#     это особенно видно: дом не вычисляет ничего — он ОБЪЯВЛЯЕТ, какое утверждение есть
#     факт (взято у `worldfacts`), а какое мнение (свой список), — и потому закон его есть
#     ровно это объявление, а не набор готовых строк.
#
# Разница между объявлением и набором видна на подмене, склеившей честную голову с честным
# хвостом из РАЗНЫХ строк: «мне нравится эта книга — это факт или мнение? факт: это можно
# проверить». Обе половины дом писал; вместе они — ложь.
_РОД_УТВЕРЖДЕНИЯ = {}
for _яз in ЯЗЫКИ:
    for _i in range(ФАКТОВ):
        _РОД_УТВЕРЖДЕНИЯ[(_яз, утверждение(_яз, "факт", _i))] = "факт"
    for _i in range(len(МНЕНИЯ[_яз])):
        _РОД_УТВЕРЖДЕНИЯ[(_яз, утверждение(_яз, "мнение", _i))] = "мнение"
...
def судить(строка):
    ...
    if not с:
        return False, False
    совпал = False
    for образец, яз, форма in _РАМКИ_ЦЕЛИКОМ:
        м = образец.fullmatch(с)
        if not м:
            continue
        совпал = True
        if форма == "пара":
            если_факт = _РОД_УТВЕРЖДЕНИЯ.get((яз, м.group("ф")))
            если_мнение = _РОД_УТВЕРЖДЕНИЯ.get((яз, м.group("м")))
            if (если_факт, если_мнение) == ("факт", "мнение"):
                return True, True
        elif _РОД_УТВЕРЖДЕНИЯ.get((яз, м.group("у"))) == форма:
            return True, True
