...
# в своде дыру, — чтобы дом не был построен второй раз. Роды здесь заданы МЕСТОМ в паре рамок,
# а не ключом, и потому названы явно; `assert` держит имена при таблице.
#
# ДВА ВОПРОСА ОБ ОДНОМ ПЕРЕВОДЕ — НЕ ИЗБЫТОК: один спрашивает о СЛОВЕ («как будет «вода»
# по-французски?»), другой о ДЕЙСТВИИ говорящего («как сказать «вода» по-французски?»), и всякий
# язык несёт свою пару оборотов («what is …» / «how do you say …»).
...
РОДЫ = ("слово", "фраза")
ЗАЧЕМ_РОДА = {"слово": "перевод слова, вопрос о слове",
              "фраза": "перевод слова, вопрос о действии говорящего"}
assert all(len(п) == len(РОДЫ) for п in РАМКИ.values()), "рамок в паре не столько же, сколько родов"
