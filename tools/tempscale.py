...
def страница(язык, форма, c):
    ...
    return р["опора"].format(C=градусы(язык, c), F=градусы(язык, f), Ш1=Ш1, Ш2=Ш2,
                             что=_вода(язык, что)) + "."
...
def рамка(язык, форма, скрещено=False):
    ...
    return р["опора"].format(C="{C}", F="{F}", Ш1="{Ш1}", Ш2="{Ш2}", что="{что}") + "."
...
def _самопроверка():
    ...
    for язык in ЯЗЫКИ:
        ...
        # (5) ОПОРА НАЗВАЛА ЧУЖОЙ ФАКТ: при нуле вода не кипит
        битая = о.replace(_вода(язык, "замерзает"), _вода(язык, "кипит"))
        assert судить(битая) == (True, False), битая
        мутанты += 5
