...
# ЦЕЛЬСИИ ПОДОБРАНЫ ТАК, ЧТОБЫ СЧЁТНАЯ ФОРМА ПРИ ДВУХ-ЧЕТЫРЁХ ЖИЛА НЕ МЕНЬШЕ ЗАКОНА
# КОПИИ В КУБЕ (12.09). Прибор КЛЮЧ БЕЗ ОПОРЫ нашёл два рода, держащих вопросы удержанного
# ключа при массе два: русское «N градуса по» и польское «N stopnie Fahrenheita». Оба
# возникают, когда ФАРЕНГЕЙТ кончается на 2, 3 или 4 (кроме второго десятка), — а прежних
# десяти цельсиев такими были лишь два: −5 (23 °F) и 40 (104 °F).
#
...
#
# Прибавлены −30 (−22 °F), −20 (−4 °F), 0 (32 °F), 50 (122 °F), 90 (194 °F), 95 (203 °F):
# масса обоих родов 2 → 8, и ключ обрёл опору по закону LAW³.
# ДВА ЧИСЛА ОТДАНЫ УДЕРЖАННОМУ КЛЮЧУ (12.09, вечером того же дня). Прибавляя цельсии ради
# массы двух родов, я взял −20 и 50 — а они лежат в ТАЙНОМ СПИСКЕ `scripts/holdout_key.py`,
# которым мерится посадка читателя на числах, каких он не мог запомнить. Прибор объявил это
# в тот же час.
#
...
#
# Замена искалась тем же счётом: цельсии, кратные пяти, чей фаренгейт кончается на 2, 3 или
# 4 (кроме второго десятка), — и вне удержанного: −70 (−94 °F) и 150 (302 °F). Оба
# физически честны: −70 °C бывает в Антарктиде, 150 °C есть жар печи.
ЦЕЛЬСИИ = (-70, -30, -10, -5, 0, 5, 10, 15, 20, 25, 30, 35, 40, 90, 95, 150)
