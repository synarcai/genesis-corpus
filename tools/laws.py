...
def ступень_по_законам(мир):
    """[(строка, номер закона)] — ступень, прочтённая ВМЕСТЕ С НОМЕРОМ своего закона.

    ОДИН ОБХОД — ДВА ЧТЕНИЯ (14.09). Дому, объявляющему роды, нужно знать, ИЗ КАКОГО
    закона вышла строка: определение принадлежит тому делу, которое оно делает возможным,
    и разные законы ступени идут в разные роды. Второй обход, строящий те же строки ради
    их номеров, разошёлся бы с первым молча; оттого строит только этот, а `ступень`
    читает его без номеров.
    """
    вон = []
    for k, (в_en, в_ru, з_en, з_ru) in enumerate(ЗАКОНЫ[мир]):
        вон += [(з_en, k), (з_ru, k), (f"{в_en} {з_en}", k), (f"{в_ru} {з_ru}", k)]
    return вон


def ступень_с_языком(мир):
    """[(строка, номер закона, ЯЗЫК)] — тот же обход, но язык назван вслух.

    ЯЗЫК ЗДЕСЬ БЫЛ ИЗВЕСТЕН ВСЕГДА И НЕ БЫЛ СКАЗАН (15.09). Всякий закон объявлен
    четвёркою «(вопрос en, вопрос ru, ответ en, ответ ru)», и ступень раскладывает её в
    четыре строки твёрдым порядком — английская, русская, английская, русская. Порядок этот
    есть ОБЪЯВЛЕНИЕ, а не привычка: он стои́т в одной строке тела и виден целиком.

        СТУПЕНЬ ЗАКОНОВ ХОДИТ ПО ДЕСЯТКАМ ДОМОВ, И ЯЗЫК, ПОТЕРЯННЫЙ ЗДЕСЬ, ТЕРЯЕТСЯ У ВСЕХ
        РАЗОМ. Оттого он назван в общем помощнике, а не восстанавливается в каждом доме
        порознь — двадцать домов, гадающих об одном, дадут двадцать разных ответов.
    """
    вон = []
    for k, (в_en, в_ru, з_en, з_ru) in enumerate(ЗАКОНЫ[мир]):
        вон += [(з_en, k, "en"), (з_ru, k, "ru"),
                (f"{в_en} {з_en}", k, "en"), (f"{в_ru} {з_ru}", k, "ru")]
    return вон


def ступень(мир):
    """Строки ступени определений мира: утверждение и ответ на вопрос, оба языка."""
    return [с for с, _k in ступень_по_законам(мир)]
