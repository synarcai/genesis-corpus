...
class Сбор(list):
    """Ряд показов, помнящий РОД каждого: ОДИН ОБХОД — ДВА ЧТЕНИЯ (14.09).

    Кузница, строящая показы плоским рядом `out.append(...)`, знает род каждого
    показа ТОЧНО — он стоит в ветви, из которой показ вышел, — и не говорит его
    никому. Свод получает строки без имён, и мера зовёт мир безымянным при доме,
    который назвать его мог бы даром.

    Напрашивается второй обход, перечисляющий те же показы ради их имён. Он и есть
    ошибка:

        ДВЕ МЕРЫ, СУДЯЩИЕ ОБ ОДНОМ ПО РАЗНЫМ ПРАВИЛАМ, ДАЮТ ДВА ЧИСЛА, И ОБА БУДУТ
        НАЗВАНЫ ПРАВДОЙ. Второй обход разойдётся с первым в тот день, когда строку
        прибавят лишь в один из них, — и разойдётся молча.

    Оттого род ставится ПОЛЕМ СБОРА перед ветвью, а `append` кладёт его рядом с
    показом сам. Кузница читает сбор как список (ничего не заметив), указатель —
    как `парами`. Показ, добавленный прежде, чем назван род, падает на месте:
    прибавить ветвь — значит назвать её род, и это условие сборки, а не обычай.
    """

    def __init__(self, род=None, язык=None):
        super().__init__()
        self.род = род
        self.язык = язык
        self.роды = []
    ...
    def append(self, с):
        if not self.род:
            raise AssertionError("показ добавлен прежде, чем назван его род")
        super().append(с)
        self.роды.append(self.род)
        self.языки.append(self.язык)
    ...
    @property
    def парами(self):
        return list(zip(self, self.роды))

    @property
    def тройками(self):
        """[(показ, род, ЯЗЫК)] — ОДИН ОБХОД, ТРИ ЧТЕНИЯ (15.09).

        Язык ставится тем же способом, что и род: полем сбора перед ветвью. Дом, его не
        назвавший, отдаёт None — и остаётся для меры щербатости нечитаемым, но не падает:

            ОРУДИЕ, ТРЕБУЮЩЕЕ ОБЪЯВЛЕНИЯ ОТ ВСЕХ РАЗОМ, НЕ ПРИНИМАЕТСЯ НИКЕМ. Лестница
            кладётся ступенями: сперва орудие научается нести язык, потом дома — его
            называть, и всякий дом всходит своим днём.

        Тридцать пять домов корпуса строят показы этим сбором, и язык, названный здесь,
        доходит до словаря показов у каждого, кто его поставит.
        """
        return list(zip(self, self.роды, self.языки))
