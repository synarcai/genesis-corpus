...
# в своде дыру, — чтобы дом не был построен второй раз. Таблица `ЯЗЫКИ` здесь НЕ СМЕШАНА: все
# четыре ключа несут род, и потому имена выводятся ИЗ неё целиком. Список, живущий отдельно,
# разошёлся бы с делом на первой правке и смолчал; этот на новом ключе без имени валит дом
# ПРЯМО НА ВВОЗЕ.
_ИМЯ_РОДА = {"ск": "скорость с леджером", "в_ск": "вопрос о скорости",
             "да": "давление с леджером", "в_да": "вопрос о давлении"}
...
def язык_группа(шаг, язык):
    ...
    for i in range(6):
        t = 2 + (шаг * 3 + i) % 9
        v = 2 + (шаг * 5 + i * 3) % 11
        вон.append(утверждение(язык, "ск", v * t, t) if i % 2 == 0 else вопрос(язык, "ск", v * t, t))
        A = 2 + (шаг * 2 + i * 5) % 7
        p = 2 + (шаг * 7 + i) % 9
        вон.append(вопрос(язык, "да", p * A, A) if i % 2 == 0 else утверждение(язык, "да", p * A, A))
    return вон

def _меченая(шаг, язык):
    """[(страница, род)] — скорость и давление, утверждением и вопросом."""
    ск_утв, ск_воп, да_утв, да_воп = РОДЫ
    вон = []
    for i in range(6):
        t = 2 + (шаг * 3 + i) % 9
        v = 2 + (шаг * 5 + i * 3) % 11
        вон.append((утверждение(язык, "ск", v * t, t), ск_утв) if i % 2 == 0
                   else (вопрос(язык, "ск", v * t, t), ск_воп))
        A = 2 + (шаг * 2 + i * 5) % 7
        p = 2 + (шаг * 7 + i) % 9
        вон.append((вопрос(язык, "да", p * A, A), да_воп) if i % 2 == 0
                   else (утверждение(язык, "да", p * A, A), да_утв))
    return вон
