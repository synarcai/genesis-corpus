    ...
    for форма in ("больше", "обратно"):
        for i in range(12):
            ...
            if форма == "больше":
                ...
                if знак == "−" and k * n - d < 2:
                    n += d
                вон.append(F.страница(язык, "больше", X=лица[a][0], Y=лица[b][0], k=k, знак=знак, d=d, вещь=(i + шаг * 2) % 6, n=n, хвост=хвост))
            else:
                r = 3 + (шаг * 5 + i * 7 + j) % 28
                if знак == "−" and k * r - d < 2:
                    r += d
                вон.append(F.страница(язык, "обратно", X=лица[a][0], Y=лица[b][0], k=k, знак=знак, d=d, вещь=(i + шаг * 2 + 3) % 6, r=r, хвост=хвост))
            j += 1
