...
def перефразы_меченые(пары, языки, роды):
    """То же, но с языком при каждой строке — и язык этот СВЕРЯЕТСЯ, а не переписывается.

    Перефраза рождается ПРАВИЛОМ, а правило принадлежит языку пакета; строитель же назвал
    язык исходной строки сам. Два имени должны совпасть, и дом, у которого они разошлись,
    узнаёт об этом здесь, а не через сайдкар месяц спустя.
    """
    правила = []
    for язык in языки:
        for род in роды:
            ф = формы(язык, род)
            if len(ф) > 1:
                правила.append((язык, re.compile("^" + образец(ф[0]) + " (.*)$"), ф[1:]))
    вон = []
    for с, я in пары:
        вон.append((с, я))
        for язык, первая, другие in правила:
            m = первая.match(с)
            if m:
                assert язык == я, f"перефраза {язык}, а строитель назвал {я}: «{с[:50]}»"
                числа, ответ = m.groups()[:-1], m.groups()[-1]
                for форма in другие:
                    вон.append((форма.format(*числа) + " " + ответ, язык))
                break
    return вон
