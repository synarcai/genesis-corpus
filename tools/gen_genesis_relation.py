...
def вопросы(шаг):
    ...
    for имя, ру_имя, _п in сдвиг(о.ОТНОШЕНИЯ, шаг)[:3]:
        ...
        вон.append(f"транзитивно ли отношение «{ру_имя}» на множестве "
                   f"{S}? {ру_хвост}")
    for имя, ру_имя, _п in сдвиг(о.ОТНОШЕНИЯ, шаг):
        новые = sorted(о.замыкание(имя) - set(о.пары(имя)))
        if not новые:
            continue
        a, b = новые[шаг % len(новые)]
        звенья = " - ".join(str(э) for э in о.цепь(имя, a, b))
        вон.append(f"is the pair ({a}, {b}) in the transitive closure of "
                   f"the relation «{имя}»? the pair ({a}, {b}) is not in "
                   f"the relation «{имя}», but is in its transitive "
                   f"closure: the chain {звенья}.")
        вон.append(f"стоит ли пара ({a}, {b}) в транзитивном замыкании "
                   f"отношения «{ру_имя}»? пара ({a}, {b}) не стоит в "
    ...
    узлы = сдвиг(о.УЗЛЫ, шаг)
    a, b = узлы[0], узлы[1 % len(узлы)]
    d = о.расстояние(a, b)
    вон.append(f"what is the distance between {a} and {b} in the space "
               f"of the declared graph? in the space of the declared "
               f"graph the distance between {a} and {b} is {d}.")
    вон.append(f"каково расстояние между {a} и {b} в пространстве "
               f"объявленного графа? в пространстве объявленного графа "
               f"расстояние между {a} и {b} равно {d}.")
    return вон
