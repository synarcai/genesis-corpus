#!/usr/bin/env python3
"""ДОМ МАТЕМАТИЧЕСКИХ ПРОСТРАНСТВ — точка, граф, плоскость Фано, матрица.

Пять родов объявлены не здесь: ЧЕТЫРЕ назвала своя кузница именами строителей, прямо в
проходе, а пятый стои́т ПОДЛЕ них слагаемым — `laws.ступень("mathspaces")`.

    ГРУППА, ПРИБАВЛЕННАЯ К ПРОХОДУ МИМО КОРТЕЖА, ЕСТЬ РОД, О КОТОРОМ ОБЪЯВЛЕНИЕ МОЛЧИТ.
    Здесь кортежа нет вовсе — строители перечислены прямо в скобках прохода, — и оттого
    объявление собрано ЗДЕСЬ из тех же имён, а не выдумано.

КАЖДОЕ ТРЕТЬЕ УТВЕРЖДЕНИЕ ПОЛУЧАЕТ ВОПРОСНОГО ДВОЙНИКА РЯДОМ (`с_вопросами`), и двойник этот
принадлежит ТОМУ ЖЕ роду, что утверждение: вопрос о точке есть показ рода «точки», а не
особый род вопроса.

    ДВЕ ПОВЕРХНОСТИ ОДНОГО ПРЕДМЕТА СУТЬ ОДИН РОД, ПОКУДА ДОМ НЕ ОБЪЯВИЛ ИНОГО.
"""
...
ВОПРОСЫ = (
    ...
    # ВОПРОС ГОВОРИТ НА ЯЗЫКЕ СВОЕГО УТВЕРЖДЕНИЯ, А НЕ НА ЯЗЫКЕ ТАБЛИЦЫ (15.09). Строка
    # «[0 0 / −1 2] · (2, 1) = (0, 0): 0 × 2 + 0 × 1 = 0 и −1 × 2 + 2 × 1 = 0.» несёт русский
    # союз в основании — и получала АНГЛИЙСКИЙ вопрос, отчего выходила страница на двух
    # языках разом. Ложью она не была ни на йоту: числа сходятся, дело верно. Но читателю
    # одного языка половина её нечитаема, а прибор языка не знает, что с нею делать.
    #
    #     СТРАНИЦА НА ДВУХ ЯЗЫКАХ РАЗОМ НЕ ПРИНАДЛЕЖИТ НИ ОДНОМУ ИЗ НИХ.
    #
    # Союз основания и есть объявление языка: по нему вопрос и берётся.
    (r"^(\[.*?\] · \(.*?\)) = .*? и ", "какова {1}?"),
    (r"^(\[.*?\] · \(.*?\)) = .*? and ", "what is {1}?"),
    # СУММА МАТРИЦ ЖЕ ВНЕЯЗЫЧНА ЦЕЛИКОМ — «[a] + [b] = [c].» не несёт ни одного слова, — и
    # языка своего вопросу не диктует. Такой строке объявляется ПАРА зачинов, и берётся из
    # неё тот, на чей черёд пришёлся проход: две двери в одну внеязычную запись.
    #
    #     ГДЕ УТВЕРЖДЕНИЕ МОЛЧИТ О ЯЗЫКЕ, ЯЗЫК ВОПРОСА ВЫБИРАЕТ ПРОХОД, А НЕ ТАБЛИЦА.
    (r"^(\[.*?\] \+ \[.*?\]) = ", ("what is {1}?", "каково {1}?")),
...
def вопрос_к(утверждение, шаг=0):
    """Вопросная строка к утверждению по таблице, или None (отрицание, чужой род).

    ПАРА ЗАЧИНОВ ОБЪЯВЛЯЕТСЯ ЛИШЬ ДЛЯ ВНЕЯЗЫЧНОГО УТВЕРЖДЕНИЯ, и выбор между ними делает
    проход: строка, сама назвавшая язык, получает свой зачин одним образцом.
    """
    for о, ш in _ВОПРОСЫ:
        м = о.match(утверждение)
        if not м:
            continue
        if isinstance(ш, tuple):
            ш = ш[шаг % len(ш)]
...
# ОБЪЯВЛЕНИЕ СТОИ́Т ПОСЛЕ СТРОИТЕЛЕЙ, А НЕ ПЕРЕД НИМИ: `СОСТАВ` называет их ПО ССЫЛКЕ, и
# первая редакция этого дома положила его в голову — до того, как строители написаны. Дом
# пал на `NameError`, мир не записался, и подпись сказала «НЕ ПЕРЕКОВАН».
#
#     ОБЪЯВЛЕНИЕ, ССЫЛАЮЩЕЕСЯ НА СТРОИТЕЛЯ, СТОИТ ПОСЛЕ НЕГО. Объявление ИМЕНАМИ может стоять
#     где угодно; объявление ССЫЛКАМИ — только ниже того, на что ссылается.
СОСТАВ = (("точки", показы_точек), ("графы", показы_графов),
          ("фано", показы_фано), ("матрицы", показы_матриц))
РОДЫ = tuple(и for и, _ in СОСТАВ) + ("законы",)

ЗАЧЕМ_РОДА = {
    "точки": "точка и расстояние между точками",
    "графы": "граф: вершины, рёбра и путь",
    "фано": "плоскость Фано — семь точек и семь прямых",
    "матрицы": "матрица как действие над вектором",
    "законы": "ступень определений фразами рода",
}

# РОД-УПРАЖНЕНИЕ ОБЪЯВЛЕН ВСЛУХ (14.09, ключ `--свод`). Четыре рода этого дома суть счёт в объявленном пространстве: плоскость Фано, граф,
# матрица, точки на плоскости — и числа при них материал.
#
# Род «законы» (40 страниц на 40 очертаний) РОВЕН СОБОЮ: там всякая страница говорит СВОЙ
# закон словами — «вершина графа есть одна из его точек», — и чисел в ней нет.
#
#     РОД РОВЕН САМ СОБОЮ, КОГДА ЕГО МАТЕРИАЛ МНОГОМЕРЕН.
ОЧЕРТАНИЕ_ПО_ПРИРОДЕ = ("фано",
                        "графы",
                        "матрицы",
                        "точки",)
РОВНЫ_СОБОЮ = ("законы",)
...
def группы(шаг):
    """[[страница]] — четыре строителя с вопросными двойниками и ступень законов."""
    return [с_вопросами(сделать(шаг), шаг) for _и, сделать in СОСТАВ] + [laws.ступень(МИР)]
...
def _самопроверка():
    """Число групп сверяется с числом родов: пятая группа стои́т подле кортежа."""
    assert all(isinstance(р, str) for р in РОДЫ), "РОДЫ обязаны быть рядом ИМЁН"
    сколько = len(группы(0))
    assert сколько == len(РОДЫ), (
        f"проход даёт {сколько} групп при {len(РОДЫ)} родах — "
