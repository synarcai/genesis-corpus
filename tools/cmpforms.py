...
#
# «ФАКТ» НЕ ЕСТЬ РОД, ХОТЯ И ЛЕЖИТ В ОДНОЙ ТАБЛИЦЕ С РОДАМИ: это общий зачин, подставляемый
# внутрь «больше» и «кратно» ({факт} в их рамках), и отдельной страницей он не выходит
# никогда. Взять `ФРАЗЫ` целиком значило бы объявить родом кусок чужой рамки.
_РОДЫ_ВО_ФРАЗАХ = ("больше", "в_больше", "кратно", "в_кратно")
_ИМЯ_РОДА = {"больше": "разность двух счётов, с леджером вычитания",
             "в_больше": "вопрос о разности",
             "кратно": "кратность двух счётов, с леджером деления",
             "в_кратно": "вопрос о кратности"}
assert set(_РОДЫ_ВО_ФРАЗАХ) <= set(next(iter(ФРАЗЫ.values()))), "род ушёл из таблицы ФРАЗЫ"
РОДЫ = tuple(_ИМЯ_РОДА[к] for к in _РОДЫ_ВО_ФРАЗАХ)
...
def язык_группа(шаг, язык):
    ...
    for i in range(6):
        ...
        d = 1 + (шаг * 2 + i * 5) % 8
        x = y + d
        вон.append(больше(язык, A, B, x, y, в) if i % 2 == 0 else вопрос_больше(язык, A, B, x, y, в))
        k = 2 + (шаг + i) % 4
        y2 = 2 + (шаг * 7 + i) % 6
        вон.append(вопрос_кратно(язык, A, B, y2 * k, y2, в) if i % 2 == 0 else кратно(язык, A, B, y2 * k, y2, в))
    return вон

def _меченая(шаг, язык):
    """[(страница, род)] — разность и кратность, утверждением и вопросом."""
    разн, разн_воп, крат, крат_воп = РОДЫ
    вон = []
    имена = ИМЕНА[язык]
    for i in range(6):
        A = имена[(шаг * 3 + i * 2) % len(имена)]
        ...
        d = 1 + (шаг * 2 + i * 5) % 8
        x = y + d
        вон.append((больше(язык, A, B, x, y, в), разн) if i % 2 == 0
                   else (вопрос_больше(язык, A, B, x, y, в), разн_воп))
        k = 2 + (шаг + i) % 4
        y2 = 2 + (шаг * 7 + i) % 6
        вон.append((вопрос_кратно(язык, A, B, y2 * k, y2, в), крат_воп) if i % 2 == 0
                   else (кратно(язык, A, B, y2 * k, y2, в), крат))
    return вон



# ЯЗЫКИ ЭТОГО ДОМА — КЛЮЧИ ЕГО ТАБЛИЦЫ ФРАЗ, и берутся они ИМЕННО ОТТУДА, ибо тем же рядом
# шла кузница. Первая проба взяла ключи таблицы ВЕЩЕЙ: ряды совпадают нынче до языка, но
# совпадение это — удача, а не закон, и мир мог сдвинуться молча.
#
#     РЯД, ВЫВОДИМЫЙ ИЗ ТАБЛИЦЫ, НЕ ОБЪЯВЛЯЕТСЯ ВТОРОЙ РАЗ — но выводить его надо ИЗ ТОЙ
#     ТАБЛИЦЫ, ПО КОТОРОЙ ШЁЛ ПРЕЖНИЙ ОБХОД, а не из соседней с тем же составом.
ЯЗЫКИ = tuple(ФРАЗЫ)
