...
def _инфикс_пакета(язык):
    """Слово оператора берётся у ПАКЕТА языка (show_kinds.arithmetic.ops), а не у дома:
    это то же объявление, которым суд поспешности читает словесное уравнение, и связь слова
    со знаком проверяется здесь же. Из нескольких слов одного глифа берётся самое длинное —
    оно и есть полная фраза языка («divided by», «podzielone przez», «разделить на»)."""
    оп = ((json.loads((_ПАКЕТЫ / f"{язык}.json").read_text(encoding="utf-8"))
           .get("show_kinds") or {}).get("arithmetic") or {}).get("ops") or {}
    вон = {}
    for глиф in ("+", "-", "*", "/"):
        свои = sorted((с for с, г in оп.items() if г == глиф), key=lambda с: (-len(с), с))
        assert свои, (язык, глиф, "the pack declares no word for this sign")
        вон[глиф] = свои[0]
    return вон


ИНФИКС = {язык: _инфикс_пакета(язык) for язык in ЯЗЫКИ}
ГЛИФ_ДЕЙСТВИЯ = {"сложи": "+", "вычти": "-", "умножь": "*", "раздели": "/", "поровну": "/"}
...
ФОРМЫ = ("повеление", "повеление_леджер", "повеление_инфикс", "повеление_равенством", "отказ")
...
ПАРЫ = {
    # СЛОЖЕНИЙ ВДВОЕ БОЛЬШЕ, ЧЕМ ПРОЧИХ ДЕЙСТВИЙ, И ЭТО КУПЛЕНО ЗАМЕРОМ (12.09).
    # Прибор ДОЛЯ ОПЕРАТОРА нашёл три слова, стоящие оператором реже пяти процентов
    # своих вхождений: «más» (es) 3,8 %, «mais» (pt) и «più» (it) по 4,5 %. Читатель,
    # встречая «más» три тысячи раз как «более» и сто как «плюс», учит первое; ядро
    # так и прочло «¿cuánto es 23 más 19?» — и ответило 4, то есть 23 − 19.
    #
    #     ДОЛЯ, ПОДНЯТАЯ НОВЫМИ ПОКАЗАМИ, ЕСТЬ УПЛАТА; ДОЛЯ, ПОДНЯТАЯ СДВИГОМ
    #     РУБЕЖА, ЕСТЬ ОТКАЗ ОТ УПЛАТЫ.
    #
    # Одиннадцать пар прибавлено к десяти: они дают ЭТИМ ТРЁМ словам по сорок четыре
    # новых стояния оператором, и доля каждого переходит пять процентов.
    #
    # ШЕСТЬ ПАР ПРИБАВЛЕНО К ДВАДЦАТИ ОДНОЙ (13.09). Полный набор — первый в этой сессии,
    # дошедший до конца, — застал «más» на 4,97 %: доля вернулась под порог, хотя свод с
    # часа уплаты не тронут ни одной строкой со словом «más».
    #
    #     ДОЛЯ ЕСТЬ ОТНОШЕНИЕ, И ОНА ПАДАЕТ САМА, КОГДА РАСТЁТ ЗНАМЕНАТЕЛЬ. Уплата, ставшая
    #     впритык к порогу, есть уплата НА ОДИН ДЕНЬ: 5,1 % и 4,97 % — одно и то же
    #     состояние свода, померенное до и после того, как рядом выросло чужое.
    #
    # Оттого шесть пар, а не одна: запас берётся не ради красоты числа, а чтобы следующий
    # чужой рост не возвращал долга.
    "сложи": ((3, 4), (5, 6), (7, 8), (9, 12), (11, 15), (13, 6), (17, 4), (21, 9), (25, 14), (30, 7),
              (2, 9), (6, 11), (8, 13), (10, 16), (14, 5), (19, 8), (23, 6), (26, 12), (28, 3), (31, 14),
              (33, 8), (4, 7), (12, 9), (16, 5), (18, 11), (24, 13), (29, 6)),
    ...
    # ДВЕ РАМКИ ОДНОГО ДЕЙСТВИЯ ХОДЯТ ОДНИМ РЯДОМ ЧИСЕЛ (12.09, ночь, заявка holon-f9).
    # «раздели 14 на 2» и «раздели 14 на 2 поровну» суть ОДНО действие (знак ÷ у обоих), а
    # ряды чисел у них были РАЗНЫЕ и не пересекались НИ ОДНОЙ парой: первая рамка знала
    # (14,2),(18,3),(24,4)…, вторая (12,3),(20,4),(30,6)… Живое ядро (проба omega-ad по
    # девяти языкам) отвечало деление в первой рамке и МОЛЧАЛО во второй — и молчало не
    # оттого, что не знало слова «поровну», а оттого, что не встречало его С ЭТИМ ЧИСЛОМ.
    #
    #     ДВЕ РАМКИ ОДНОГО ДЕЙСТВИЯ, ХОДЯЩИЕ ПО РАЗНЫМ РЯДАМ ЧИСЕЛ, УЧАТ ДВУМ ДЕЙСТВИЯМ:
    #     встретивший число в одной рамке и слово в другой не встретил их ВМЕСТЕ ни разу.
    #
    # Ряд «поровну» есть теперь ОБЪЕДИНЕНИЕ обоих: прежние десять пар сохранены (показ не
    # отнимается), и к ним добавлены десять пар первой рамки. Пересечение стало полным.
    "поровну": ((12, 3), (20, 4), (30, 6), (16, 2), (27, 3), (42, 7), (25, 5), (32, 4), (54, 9), (21, 7),
                (14, 2), (18, 3), (24, 4), (35, 5), (36, 6), (28, 7), (40, 8), (45, 9), (30, 5), (48, 6)),
}
...
def рамка(язык, форма, действие, в):
    ...
    if форма == "повеление":
        ...
        слово = ИНФИКС[язык][ГЛИФ_ДЕЙСТВИЯ[действие]]
        ответ = ("{r}" + ДВОЕТОЧИЕ[язык] + "{a} " + слово + " {b} " + СВЯЗКА_СЧЁТА[язык]
                 + " {r}, " + "{a} {зн} {b} = {r}.")
    elif форма == "повеление_равенством":
        # СЛОВО ОПЕРАТОРА В УРАВНЕНИИ СО ЗНАКОМ РАВЕНСТВА — рынок слов-операторов читателя
        # покупает слово только отсюда (просьба holon 05.09); глиф стоит рядом тем же числом,
        # и суд поспешности отныне считает ОБА (закон слова-знака, courts/indu_court.py)
        слово = ИНФИКС[язык][ГЛИФ_ДЕЙСТВИЯ[действие]]
        ответ = ("{r}" + ДВОЕТОЧИЕ[язык] + "{a} " + слово + " {b} = {r}, "
                 + "{a} {зн} {b} = {r}.")
    else:
        # СЛОВО ПОРЯДКА И ЗНАК РАВЕНСТВА НА ОДНОЙ СТРАНИЦЕ — это и есть покупка
        ответ = "{r}" + ДВОЕТОЧИЕ[язык] + "{a} {зн} {b} = {r}."
    return приказ + " " + вопрос + " " + ответ
        ...
        for действие in ДЕЙСТВИЯ:
            for i, (a, b) in enumerate(ПАРЫ[действие]):
                ...
                for в in (0, 1):
                    ...
                    вон[страница(язык, "повеление_равенством", действие, в, a, b)] = (язык, "повеление_равенством")
...
def _вердикт(форма, действие, зн):
    ...
    # ГЛИФ ЕСТЬ ЗНАЧЕНИЕ СЛОВА ДЕЙСТВИЯ: «сложи» пишется «+», и никак иначе
    if "зн" in зн and зн["зн"] != ЗНАКИ[действие]:
        return False
...
def _самопроверка():
    ...
    for язык in ЯЗЫКИ:
        for действие in ДЕЙСТВИЯ:
            ...
            assert судить(битая) == (True, False), битая
            мутанты += 1
            # (3c) УРАВНЕНИЕ СЛОВОМ СО ЗНАКОМ РАВЕНСТВА: слово и глиф дают одно число
            у = страница(язык, "повеление_равенством", действие, 1, a, b)
            assert судить(у) == (True, True), у
            битая = у[:у.rindex("= ")] + "= %d." % (верно + 4)
            assert судить(битая) == (True, False), битая
            мутанты += 1
    ...
    for язык, действие in (("pl", "раздели"), ("ru", "умножь"), ("en", "раздели"), ("de", "поровну")):
        print("  ", страница(язык, "повеление_равенством", действие, 1, *ПАРЫ[действие][2]))
