...
# «UGLY» ВНЕСЕНО 16.09, И ВНЕСЕНО ПО СОБСТВЕННОМУ ОБЪЯВЛЕНИЮ ЭТОГО ЖЕ МЕСТА: граница
# «слово, не названное ни в одном списке, судится ПЕРВОЙ БУКВОЙ» была объявлена вместе с
# оговоркой — «ошибка возможна лишь на слове, которого дом ещё не писал». Дом порядка
# прилагательных написал такое слово: «u» здесь звучит /ʌ/, а не /juː/, и верно «an ugly».
#
#     ГРАНИЦА, ОБЪЯВЛЕННАЯ ВСЛУХ, НЕ ЕСТЬ ДОЛГ — ОНА ЕСТЬ ЧЕСТНОЕ МЕСТО, ГДЕ МЕРА ЖДЁТ
#     ПОПОЛНЕНИЯ. Пополняется она словом, а не правилом: правило по букве дало бы «an unit».
ГЛАСНЫЙ_ЗВУК = frozenset(("hour", "hours", "honest", "honestly", "honour", "heir", "ugly"))
