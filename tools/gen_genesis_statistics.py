...
def pass_shows(pass_i):
    ...
    for i in range(8):
        ...
        # --- середина набора
        out.append(f"the mean of {spaced(набор)} is {среднее}.")
        out.append(f"среднее {spaced(набор)} равно {среднее}.")
        out.append(f"the median of {spaced(нечёт)} is {медиана}.")
        out.append(f"медиана {spaced(нечёт)} равна {медиана}.")
        out.append(f"the range of {spaced(нечёт)} is {размах}.")
        out.append(f"размах {spaced(нечёт)} равен {размах}.")
        # --- счёт способов
