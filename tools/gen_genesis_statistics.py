...
def рассуждения(pass_i, i, набор, среднее, нечёт, медиана):
    """[(страница, ЯЗЫК)] — среднее и медиана рассуждением: счёт-свидетель, связка, вывод той
    же формы, что утверждение мира, закон рода; «почему» — вторым зачином.

    ЯЗЫК ЗДЕСЬ СТОИ́Т ПЕРВЫМ ДОВОДОМ `discourse` — он назван при каждом зове и отдаётся вместе
    со страницей, чтобы сбор поставил его в словарь показов.
    """
    вон = []
    ...
    if (pass_i + i) % 2 == 0:
        вон.append((discourse.рассуждение_величины("en", f"what is the mean of {п}", св_en, выв_en, ЗАКОН_СРЕДНЕЕ["en"], k=pass_i + i), "en"))
        вон.append((discourse.рассуждение_величины("ru", f"чему равно среднее {п}", св_ru, выв_ru, ЗАКОН_СРЕДНЕЕ["ru"], k=pass_i + i), "ru"))
    else:
        вон.append((discourse.почему("en", f"why is the mean of {п} equal to {среднее}", св_en, выв_en, ЗАКОН_СРЕДНЕЕ["en"], k=pass_i + i), "en"))
        вон.append((discourse.почему("ru", f"почему среднее {п} равно {среднее}", св_ru, выв_ru, ЗАКОН_СРЕДНЕЕ["ru"], k=pass_i + i), "ru"))
    ...
    if (pass_i + i) % 2 == 1:
        вон.append((discourse.рассуждение_величины("en", f"what is the median of {п2}", св_en, выв_en, ЗАКОН_МЕДИАНА["en"], k=pass_i + i), "en"))
        вон.append((discourse.рассуждение_величины("ru", f"чему равна медиана {п2}", св_ru, выв_ru, ЗАКОН_МЕДИАНА["ru"], k=pass_i + i), "ru"))
    else:
        вон.append((discourse.почему("en", f"why is the median of {п2} equal to {медиана}", св_en, выв_en, ЗАКОН_МЕДИАНА["en"], k=pass_i + i), "en"))
        вон.append((discourse.почему("ru", f"почему медиана {п2} равна {медиана}", св_ru, выв_ru, ЗАКОН_МЕДИАНА["ru"], k=pass_i + i), "ru"))
    return вон
...
def pass_shows(pass_i):
    ...
    for i in range(ШИРИНА):
        ...
        for с, яз in рассуждения(pass_i, i, набор, среднее, нечёт, медиана):
            out.язык = яз
            out.append(с)
        if i == 0:
            ...
            # ЯЗЫК СТОИ́Т КЛЮЧОМ СЛОВАРЯ ЗАКОНА — и вписан третьим полем, чтобы дойти до
            # словаря показов, а не остаться при чтении.
            for строка, род, яз in ((ЗАКОН_СРЕДНЕЕ["en"], "mean", "en"), (ЗАКОН_СРЕДНЕЕ["ru"], "mean", "ru"),
                                    (ЗАКОН_МЕДИАНА["en"], "median", "en"), (ЗАКОН_МЕДИАНА["ru"], "median", "ru"),
                                    (f"what is the mean of a list? {ЗАКОН_СРЕДНЕЕ['en']}", "mean", "en"),
                                    (f"что такое среднее списка? {ЗАКОН_СРЕДНЕЕ['ru']}", "mean", "ru"),
                                    (f"what is the median of a list? {ЗАКОН_МЕДИАНА['en']}", "median", "en"),
                                    (f"что такое медиана списка? {ЗАКОН_МЕДИАНА['ru']}", "median", "ru")):
        ...
        # РУССКАЯ СТОРОНА РОДА ДОПИСАНА ПО СЛОВУ МЕРЫ (15.09): щербатость назвала эту клетку
        # пустой в тот час, когда дом объявил язык при странице. Два счёта поставлены рядом и
        # различены числом — по-русски это говорится не хуже.
        out.язык = "ru"
        out.append(f"выбор не есть расстановка: {k} из {n} даёт "
                   f"{сочетаний}, а расставить {k} из них — "
                   f"{сочетаний * math.factorial(k)}.")
...
def перебор_с_языком(pass_i):
    """[(строка, род, ЯЗЫК)] — ТОТ ЖЕ ОБХОД, прочтённый тремя столбцами (15.09).

    Язык ставится полем сбора перед ветвью — тем же способом, что и род, — и оттого доходит до
    словаря показов без второго обхода.
    """
