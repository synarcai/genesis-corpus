#!/usr/bin/env python3
"""ДОМ ОТНОШЕНИЙ — иерархия, свойства, замыкания, порядки, пространство.
...
ОРАКУЛ ОСТАЛСЯ В КУЗНИЦЕ, И ЭТО НАРОЧНО. Кузница спрашивает `relation.оракул()` ПРЕЖДЕ ковки и
отказывается писать мир, если встречные ходы разошлись. Это её дело — ворота перед письмом, а
не закон страницы; дом же отвечает за то, ЧТО на странице стои́т, а не за то, писать ли её.

    ВОРОТА ПЕРЕД ПИСЬМОМ И ЗАКОН СТРАНИЦЫ — РАЗНЫЕ ОБЯЗАННОСТИ, И ЖИВУТ ОНИ ПОРОЗНЬ.

ПЕРЕЕЗЖАЕТ НЕ ФУНКЦИЯ, А ЗАМЫКАНИЕ ЦЕЛИКОМ, и граница его взята СРАЗУ ЗА ВВОЗАМИ.
"""
import pathlib
import sys
...
РОДЫ = ('иерархия', 'свойства', 'замыкания', 'эквивалентности', 'соответствия', 'порядки', 'связь', 'пространство', 'вопросы')

ЗАЧЕМ_РОДА = {
    "иерархия": "род и вид: что под чем стоит",
    "свойства": "рефлексивность, симметрия, транзитивность — названные и проверенные",
    "замыкания": "что прибавится, если требовать свойства",
    "эквивалентности": "разбиение на классы равного",
    "соответствия": "отображение и его обратимость",
    "порядки": "строгий и нестрогий, полный и частичный",
    "связь": "есть ли путь и сколько шагов",
    "пространство": "расстояние и соседство",
    "вопросы": "тот же предмет, спрошенный вопросом",
}

# РОД-УПРАЖНЕНИЕ ОБЪЯВЛЕН ВСЛУХ (14.09, ключ `--свод`: решено по КАЖДОМУ роду
# дома). Шесть родов этого дома суть счёт над отношением на объявленном ряду: эквивалентность,
# порядок, свойства, замыкание — числа при них материал.
#
# Три рода РОВНЫ СОБОЮ: «иерархия» (490 на 490) перебирает САМИ УЗЛЫ дерева родов, «связь»
# (218 на 218) — рёбра объявленного графа, «пространство» (44 на 44) — пары его вершин. Там
# меняется не число при неизменной записи, а сама вещь, о какой речь.
#
#     РОД РОВЕН САМ СОБОЮ, КОГДА ЕГО МАТЕРИАЛ МНОГОМЕРЕН.
ОЧЕРТАНИЕ_ПО_ПРИРОДЕ = ("эквивалентности",
                        "порядки",
                        "свойства",
                        "замыкания",
                        "соответствия",
                        "вопросы",)
РОВНЫ_СОБОЮ = ("иерархия",
                "связь",
                "пространство",)
...
def иерархия(шаг):
    ...
    for узел in узлы:
        ...
        if чужие:
            ...
            вон.append((f"in the tree of kinds the node {узел} does not "
                       ...
                       f"in the node {где}.", "en"))
            вон.append((f"в дереве родов узел {узел} не содержит узел "
                       f"{чужой}: узел {чужой} лежит в узле {где}.", "ru"))
    ...
    вон.append((f"the tree of kinds is a structure of {у} levels: from "
               f"level 0 to level {у - 1}.", "en"))
    вон.append((f"дерево родов есть структура из {у} "
               f"{о.ру('уровень', у)}: от уровня 0 до уровня {у - 1}.", "ru"))
    return вон
...
def замыкания(шаг):
    ...
    for имя, ру_имя, _п in сдвиг(о.ОТНОШЕНИЯ, шаг):
        ...
        вон.append((f"пара ({a}, {b}) не стоит в отношении «{ру_имя}», но "
                   f"стоит в его транзитивном замыкании: цепь {звенья}.", "ru"))
    return вон
    ...
    for имя, ру_имя, ан_обл, ру_обл, _п in сдвиг(о.СООТВЕТСТВИЯ, шаг):
        ...
        if о.однозначно(имя):
            ...
            вон.append((f"the correspondence «{имя}» on {ан_обл} is not "
                       ...
                       f"and the same value {з}.", "en"))
            вон.append((f"соответствие «{ру_имя}» на {ру_обл} не "
                       f"взаимно-однозначно: элементы {a} и {b} "
                       f"переходят в одно и то же значение {з}.", "ru"))
            э = ключи[шаг % n]
            вон.append((f"in the correspondence «{имя}» the element {э} "
                       f"goes to the value {туда[э]}.", "en"))
            вон.append((f"в соответствии «{ру_имя}» элемент {э} переходит "
                       f"в значение {туда[э]}.", "ru"))
    return вон
    ...
    for имя, множество in сдвиг(о.ПОРЯДКИ, шаг):
        ...
        for э in сдвиг(о.максимальные(имя, множество), шаг):
            вон.append((f"{зачин} the element {э} is maximal: no element "
                       f"stands above it.", "en"))
            вон.append((f"{ру_зачин} элемент {э} максимален: ни один "
                       f"элемент не стоит выше него.", "ru"))
    return вон
...
def пространство(шаг):
    ...
    вон.append((f"узлы объявленного графа с этим расстоянием образуют "
               ...
               f"превышает суммы через третий узел.", "ru"))
    return вон
...
def вопросы(шаг):
    ...
    if вниз:
        ...
        вон.append((f"содержит ли узел {узел} узел {низ}? в дереве родов "
                   f"узел {узел} содержит узел {низ}.", "ru"))
    ...
    for откуда in сдвиг(о.УЗЛЫ, шаг)[:3]:
        ...
        вон.append((f"какова длина кратчайшего пути из {откуда} в {куда} "
                   ...
                   f"путь из {откуда} в {куда} имеет длину {шагов}.", "ru"))
    узлы = сдвиг(о.УЗЛЫ, шаг)
...
def перебор_с_языком(шаг):
    ...
    Дом отношений пишет о дереве родов, о свойствах отношения, о замыканиях и порядках —
    и всякую мысль дважды, русской рамкой и английской. Язык был известен рамке и терялся на
    пороге словаря показов; дом становился для меры щербатости НЕЧИТАЕМ.
    """
    вон = []
