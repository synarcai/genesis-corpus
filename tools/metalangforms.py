#!/usr/bin/env python3
"""ДОМ МЕТАЯЗЫКА — определение, аксиома, лемма, теорема, доказательство.

Роды объявлены не здесь: кузница держит их кортежем `СТУПЕНИ` — парами «имя, строитель», —
и одна группа прохода есть одна ступень. Ступени не мешаются друг с другом: границей между
ними стои́т шов, и читатель корпуса режет по нему честно.

    РОД, РЕШЁННЫЙ ПОСТРОЕНИЕМ, УЖЕ ОБЪЯВЛЕН — ОСТАЁТСЯ ЛИШЬ ЗАПИСАТЬ ЕГО ТАМ, ГДЕ ЕГО ИЩУТ.

ЧИТАТЕЛЬ ШЕСТОГО РОДА, И ОН НАЙДЕН ТОЛЬКО ЗДЕСЬ: `scripts/broken_law.py` держит БЕЛЫЙ СПИСОК
пар «путь файла → фраза закона», и кузница метаязыка стои́т в нём с фразой «ЯЗЫК, НА КОТОРОМ
НАПИСАНЫ ВСЕ ОСТАЛЬНЫЕ.». Фраза эта живёт в ЗАГОЛОВКЕ кузницы (строка 17), а заголовок при
переезде остаётся на месте — значит связь цела. Проверено прежде переезда, а не после.

    БЕЛЫЙ СПИСОК, КЛЮЧЁННЫЙ ПУТЁМ ФАЙЛА, ЕСТЬ ШЕСТОЙ РОД ЧИТАТЕЛЯ: он не ввозит, не
    разбирает и не зовёт — он ПОМНИТ. И рвётся молча, когда помнимое уезжает.
...
РОДЫ = tuple(и for и, _ in СТУПЕНИ)

ЗАЧЕМ_РОДА = {и: f"ступень «{и}» метаязыка: показ своего рода" for и, _ in СТУПЕНИ}

# РОД-УПРАЖНЕНИЕ ОБЪЯВЛЕН ВСЛУХ (14.09, прибор очертаний; прочтён весь свод тринадцати
# родов). Два рода суть одна форма речи на многих числах: открытая гипотеза («всякое чётное
# больше 2 есть сумма двух простых») и аксиома деления с остатком.
#
# Прочие одиннадцать НЕ объявлены: определения, теоремы, их условия и отказы перебирают САМИ
# УТВЕРЖДЕНИЯ, а не числа при них, и материал их многомерен сам собою.
#
#     РОД РОВЕН САМ СОБОЮ, КОГДА ЕГО МАТЕРИАЛ МНОГОМЕРЕН; ОБЪЯВЛЕНИЕ НУЖНО ЛИШЬ ТАМ, ГДЕ
#     МЕРНОСТЬ ОДНА: ЧИСЛА ПРИ НЕИЗМЕННОЙ ЗАПИСИ.
ОЧЕРТАНИЕ_ПО_ПРИРОДЕ = ("hypothesis-open", "axiom")
...
def _самопроверка():
    """Ряд имён сверяется со ступенями, а число групп — с делом прохода."""
