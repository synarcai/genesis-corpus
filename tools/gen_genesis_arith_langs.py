...
def язык_группа(шаг, язык):
    ...
    # ЯЗЫК ИЗВЕСТЕН ЗДЕСЬ ТОЧНО — ОН ЕСТЬ ДОВОД ЭТОЙ САМОЙ ФУНКЦИИ (16.09), и терялся он
    # на пороге словаря показов. Сбор помнит его полем, как помнит род.
    вон = Сбор(язык=язык)
...
# ПЕРЕКОС ЭТОГО ДОМА ЕСТЬ ОПИСЬ ПАКЕТОВ, А НЕ КРИВИЗНА ДОМА (16.09). Ширина языка здесь
# равна восьми показам на КАЖДУЮ объявленную форму вопроса, и форм этих у языков разное
# число: en 8, ru 4, de/fr/es/it/pl/pt/nl по 3, tr 2. Оттого английских страниц вчетверо
# больше турецких — и это ровно то, ради чего дом построен: ВСЯКАЯ ФОРМА ВСТРЕЧАЕТ ВСЯКОЕ
# ДЕЙСТВИЕ, а сколько форм объявлено, столько и звучит.
#
#     ДОМ, ПОКАЗЫВАЮЩИЙ ВСЕ ОБЪЯВЛЕННЫЕ ФОРМЫ, РОВЕН РОВНО НАСТОЛЬКО, НАСКОЛЬКО РОВНЫ
#     ПАКЕТЫ. Выровнять его можно лишь двумя путями: дописать формы бедным языкам (работа
#     языковая, не домовая) либо ПОКАЗЫВАТЬ НЕ ВСЕ объявленные — то есть солгать о пакете.
#
# Долг здесь потому НЕ у дома, а у пакетов: турецкому и португальскому недостаёт форм
# вопроса о значении, какие у них в языке есть. Названо числом, чтобы работа была видна.
ОБЪЯВЛЕННЫЕ_ПРОПУСКИ = {}          # щербатости нет: всякий род звучит на всех десяти языках
