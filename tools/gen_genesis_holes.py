...
# ПЕРЕБОР ЖИВЁТ В ДОМЕ, А НЕ ЗДЕСЬ (13.09): дом обязан знать свои страницы, чтобы назвать их
# род, и потому `роли`, `ЧИСЛА`, `ШИРИНА` и `ШИРИНА_ЗАПРОСОВ` объявлены там, а кузница их
# ЗОВЁТ. ДВА ПЕРЕБОРА ОДНОГО ПРОСТРАНСТВА РАЗОЙДУТСЯ НА ПЕРВОЙ ЖЕ ПРАВКЕ.
ШИРИНА = holes.ШИРИНА
_роли = holes.роли
...
def язык_группа(шаг, язык):
    ...
    for i in range(ШИРИНА):
        ...
        вон.append(факт)
        # СРОК НА ВТОРОМ МЕСТЕ — тот же факт, иная позиция (08.09): класс срока обязан
        # покупаться МЕСТОМ В РАМКЕ, а не тем, что он всегда первый.
        второй = holes.факт_вторым_местом(язык, *роли)
        if второй:
            вон.append(второй)
...
ШИРИНА_ЗАПРОСОВ = holes.ШИРИНА_ЗАПРОСОВ
