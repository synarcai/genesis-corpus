...
# ДОЛГ РУССКОЙ СТОРОНЫ ДВУХ РОДОВ НАЗВАН УТРОМ И ВЫПЛАЧЕН ВЕЧЕРОМ (15.09). Едва дом объявил
# язык при странице, мера щербатости показала: «целое из двух счётов» и «та же доля, но об
# ушедшем» сказаны только по-английски, а весь дом писал 252 английские страницы против 49
# русских — впятеро.
#
...
#
# Ныне обе стороны сказаны своими словами, и перекос дома сошёл 5,1× → 1,7×. Английская
# сторона остаётся шире на вопросных поверхностях доли, объявленных двумя рамками («1/2» и
# «a half»), — у русского вторая рамка одна («половина»), и это свойство языка, а не долг.
...
def pass_shows(pass_i):
    ...
    for i in range(ШИРИНА):
        ...
        # ДВА ОСТАВШИХСЯ РОДА ПО-РУССКИ (15.09). Долг назван был верно, и причина его —
        # английские списки вещей и имён — снята тем же способом, каким снята у остатка:
        # русские вещи и имена объявлены домом СВОИМИ (`ВЕЩИ_RU`, `ИМЕНА_RU`), со счётными
        # формами из пакета.
        #
        #     ПОЛЯРНОСТЬ ВОПРОСА ЕСТЬ ЧАСТЬ УРОКА: «сколько осталось» и «сколько ушло» суть
        #     два разных вопроса об одном мире, и язык, знающий лишь первый, знает половину.
        out.род = УШЕДШЕЕ
        out.язык = "ru"
        out.append(
            f"у {имя_ru} было {всего} {ру(формы, всего)}. "
            f"{доля_ru} {родит} ушла. сколько {родит} ушло? "
            f"ушло {ушло} {ру(формы, ушло)}{кузн_ушло}."
        )
        # ИМЕНА ОБЪЯВЛЕНЫ В РОДИТЕЛЬНОМ («у иды»), И РАМКА ВЗЯТА ПОД НИХ: «иды насчитал»
        # было бы не русской речью, а именительным, которого дом не объявлял.
        #
        #     РАМКА ВЫБИРАЕТСЯ ПОД ОБЪЯВЛЕННЫЕ ФОРМЫ, А НЕ ФОРМЫ ПОД КРАСИВУЮ РАМКУ.
        out.род = ЦЕЛОЕ_ИЗ_ДВУХ
        out.язык = "ru"
        out.append(
            f"у {имя_ru} было {a} {ру(формы, a)}, а потом ещё {b} {ру(формы, b)}. "
            f"{доля_ru} {родит} ушла. сколько {родит} осталось? "
            f"осталось {осталось} {ру(формы, осталось)}{кузн2}."
        )
