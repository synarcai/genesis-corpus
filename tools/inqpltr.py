#!/usr/bin/env python3
"""ДОМ ВОПРОШАНИЯ (inquiry_pl_tr) — шесть ступеней определения на двух языках.

Мир `inquiry_pl_tr` стоял без объявленного рода: шесть родов лежали в кузнице кортежем
...
def исп_делимость(шаг):
    ...
    """
    вон = []
    # ОБЕ ПОЛЯРНОСТИ НА КАЖДЫЙ ДЕЛИТЕЛЬ (holon, аудит покупок 03.09): гармония
    # гласных дробит турецкую рамку по дательному делителя («6'ya», «3'e»,
    # «10'a»), и в дроблёной рамке «'ya» оставалась одна полярность — нет
    # фальсификатора, рынок такую рамку не покупает. Потому к каждому
    # делителю идут делимое кратное и делимое с остатком.
    ход = []
    for i in range(5):
        a0, b = 100 + шаг * 11 + i * 9, 3 + (шаг + i) % 8
        ход.append((a0 - a0 % b, b))
        ход.append((a0 - a0 % b + 1 + i % (b - 1), b))
...
def группы(шаг):
    ...
    вон.append(coprime.показы(ЯЗЫКИ_МИРА, шаг))
    return вон
