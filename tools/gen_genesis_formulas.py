...
GRAPHS = [
    ...
    # ГРАФ ОБ ОДНОМ РЕБРЕ (08.09): шесть графов ряда имели от двух рёбер и трёх узлов, и дом,
    # всюду зовущий закон счёта, не показал ни «1 edge», ни «1 ребро», ни «eine kante». Два
    # узла и одно ребро суть наименьший связный граф — не вырождение, а начало ряда.
    ("graph TD; A-->B;", 2, 1, "A", "B", 1),
]
СЛОВОМ = {
    1: ("one", "одно", "eine", "一"),
...
ПО_РОДУ = {"ж": "равна", "м": "равен", "с": "равно"}


def _равно(предмет):
    """«равно / равна / равен» по роду ГЛАВНОГО слова предмета.

    СВЯЗКА СОГЛАСУЕТСЯ С ПОДЛЕЖАЩИМ, А НЕ С ПРИВЫЧКОЙ ОБРАЗЦА (08.09). Образец вопроса стоял
    средним родом на всех предметах разом, и сорок страниц свода спрашивали «чему равно сумма
    первых 4 чисел?» — при том что ОТВЕТ на той же строке говорил «сумма … равна». Дом знал
    род и в ответе, и не знал в вопросе.

    Род берётся законом дома счёта (`rugram.род`, выводящий его из окончания), а не списком.
    ГРАНИЦА ЗАКОНА НАЗВАНА ЗДЕСЬ ЖЕ: окончание не различает рода имён на мягкий знак —
    «площадь» тот же закон зовёт мужским. Оттого площадь в этом доме имеет СВОЙ объявленный
    образец вопроса, и общий закон её не касается; предмет, начинающийся не буквой (формула,
    «2 в степени 5»), берёт средний род.
    """
    первое = re.match(r"[^\W\d_]+", предмет)
    return ПО_РОДУ.get(rugram.род(первое.group(0)) if первое else "с", "равно")
...
def pass_shows(pass_i):
    ...
    for i in range(WIDTH):
        ...
        # ВОПРОС О ТОЖДЕСТВЕ СПРАШИВАЛСЯ ЛИШЬ ПО-РУССКИ (15.09), и оттого весь род бинома
        # стоя́л русским при внеязычных выкладках: сами тождества слов не несут, а речь при
        # них была одна. Английский зачин у дома объявлен («value») и стои́т рядом — недоставало
        # его позвать.
        out.append(спросить("значение", квадрат,
                            f"{квадрат} = {(p + q) ** 2}."))
        out.append(спросить("value", квадрат, f"{квадрат} = {(p + q) ** 2}."))
        ...
        out.append(текст)
        # ОДНО УСЛОВИЕ ПРАВИЛО ДВУМЯ СЧЁТАМИ — И ПО-АНГЛИЙСКИ ТОЖЕ (08.09). Русская сторона
        # была вылечена законом дома (`rugram.форма` порознь для узла и ребра), а английская
        # осталась двумя литералами; они верны лишь потому, что в ряду графов нет ни одного
        # с единственным узлом или ребром. Половинчатый закон живёт ровно в этом зазоре.
        out.append(f"{текст} this graph has {узлов} {by_count(узлов, 'nodes')} and "
                   f"{рёбер} {by_count(рёбер, 'edges')}.")
        ...
        out.append(f"{текст} in this graph {откуда} reaches {куда} in "
                   f"{шагов} {by_count(шагов, 'steps')}.")
        en, ru, de, zh = СЛОВОМ.get(рёбер, (str(рёбер),) * 4)
        out.append(f"{текст} the diagram has {en} {by_count(рёбер, 'edges')}.")
        # РУЧНОЕ ПРАВИЛО «< 5» ЗАМЕНЕНО ЗАКОНОМ: оно верно на нынешнем ряду {2, 3, 4} и лжёт
        # на единице («одно ребра») и на двадцати одном («21 рёбер»); `rugram.форма` знает и
        # то, и другое. Слово числа берётся из таблицы, ФОРМА ИМЕНИ — из дома счёта.
        out.append(f"{текст} диаграмма имеет {ru} {rugram.форма('ребро', рёбер)}.")
        # НЕМЕЦКОЕ ИМЯ ПРИ ЧИСЛЕ ОБЪЯВЛЕНО ЗДЕСЬ ПАРОЙ, А НЕ ЗАКОНОМ: пакет немецкого «kante»
        # не объявляет, и звать закон счёта неоткуда. Долг назван, а не скрыт молчанием.
        out.append(f"{текст} das diagramm hat {de} "
