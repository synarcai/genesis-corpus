...
ЯЗЫКИ = {
    ...
    "de": dict(большие={"час": dict(many="Stunden"), "минута": dict(many="Minuten"), "неделя": dict(many="Wochen"), "сутки": dict(many="Tagen")},
    ...
    "es": dict(большие={"час": dict(many="horas"), "минута": dict(many="minutos"), "неделя": dict(many="semanas"), "сутки": dict(many="días")},
    ...
    "it": dict(большие={"час": dict(many="ore"), "минута": dict(many="minuti"), "неделя": dict(many="settimane"), "сутки": dict(many="giorni")},
    ...
    "pt": dict(большие={"час": dict(many="horas"), "минута": dict(many="minutos"), "неделя": dict(many="semanas"), "сутки": dict(many="dias")},
               малые={"минута": dict(вопрос="minutos", one="minuto", many="minutos"), "секунда": dict(вопрос="segundos", one="segundo", many="segundos"),
    ...
    "nl": dict(большие={"час": dict(many="uur"), "минута": dict(many="minuten"), "неделя": dict(many="weken"), "сутки": dict(many="dagen")},
    ...
    "pl": dict(большие={"час": dict(few="godzinach", many="godzinach"), "минута": dict(few="minutach", many="minutach"),
...
def страница(язык, i, k, словом=True):
    """The k of the larger unit in words (2..4) or in digits."""
    я = ЯЗЫКИ[язык]; б, м, f = ПЕРЕВОДЫ[i]
    v = k * f
    K = я["словом"][k] if словом and k in я["словом"] else str(k)
    п = dict(М=я["малые"][м]["вопрос"], k=K, Б=форма(язык, я["большие"][б], k), v=v, k_=k, f=f)
    воп, отв = я["рамка"]
...
def _показы():
    ...
    for язык in ЯЗЫКИ:
        for i in range(len(ПЕРЕВОДЫ)):
            for k in СЧЁТ:
                вон[страница(язык, i, k, True)] = (язык, "единицы")
                вон[страница(язык, i, k, False)] = (язык, "единицы")
    return вон
...
def _образцы():
    ...
    for язык, я in ЯЗЫКИ.items():
        дыры = {"М": "(?P<М>" + alt(м["вопрос"] for м in я["малые"].values())[1:],
                "k": r"(?P<k>\d+|" + alt(я["словом"].values())[1:],
...
def судить(строка):
    ...
    for образ, язык in ОБРАЗЦЫ:
        ...
        if пара is None:
            return True, False
        б = пара[0]
        слово_k = я["словом"].get(k)
        if not (г["k"] == str(k) or г["k"] == слово_k):
            return True, False
        return True, v == k * f and г["Б"] == форма(язык, я["большие"][б], k)
    return False, False
