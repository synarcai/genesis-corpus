...
import unitforms  # noqa: E402 — дверь форм единиц: род имени в восьми языках
import re
...
# КЛЮЧ ДОМА И ИМЯ ДВЕРИ — ОДНА ЕДИНИЦА, НАЗВАННАЯ ДВАЖДЫ: дом зовёт единицы по-русски, дверь
# форм (`unitforms`) — канонически; связь объявлена здесь, а не угадана по написанию.
ЕДИНИЦА_ДВЕРИ = {"час": "hour", "минута": "minute", "секунда": "second",
                 "день": "day", "сутки": "day", "неделя": "week"}


def _род_единицы(язык, б):
    """Род единицы по объявлению двери форм; язык или единица не объявлены — рода нет."""
    имя = ЕДИНИЦА_ДВЕРИ.get(б)
    формы = (unitforms.ЕДИНИЦЫ.get(язык) or {}).get(имя) if имя else None
    return формы[-1] if формы and формы[-1] in ("m", "f", "n") else None


def _слово(язык, б, k, рамка):
    """Число словом; где язык гнёт его РОДОМ, род берётся у считаемой единицы (22.09).

    Дом писал «duas» одним словом на все имена и дал «duas minutos», «duas dias» — четыре
    строки лжи: по-португальски «duas horas», но «dois minutos». Русский и польский держат
    число ПО ЕДИНИЦАМ (`словом2`) и потому лгать не могли; португальский держал одно слово.
    Ныне слово согласуется дверью пакета (`numeral_gender`) по роду двери форм — а язык,
    рода числительного не объявивший, получает слово как есть и не платит ничем.
    """
    я = ЯЗЫКИ[язык]
    if рамка == "рамка2" and "словом2" in я:
        return я["словом2"][б].get(k)
    слово = я["словом"].get(k)
    if слово is None:
        return слово
    return langpack.числительное_по_роду(_пакет(язык), слово, _род_единицы(язык, б))
...
# ------------------------------------------------------------- ОБЪЯВЛЕНИЕ РОДОВ
#
# СЛОВАРЬ СТРАНИЦ БЫЛ, А ОБЪЯВЛЕНИЯ НЕ БЫЛО (14.09): дом метил каждую страницу парой
# «(язык, род)», а `РОДЫ` не объявлял — и 420 строк мира числились безымянными.
#
# РОД ЗДЕСЬ ОДИН, И ЭТО НЕ ЛЕНЬ ИМЕНОВАНИЯ. Все страницы делают одно: переводят k большей
# единицы времени в v меньшей через множитель f. Две рамки («сколько минут в двух часах»
# и конструкция обладания с именительным при большей единице) и две записи числа (словом
# и цифрой) суть ЧЕТЫРЕ ПОВЕРХНОСТИ одного дела, и развести их родами значило бы назвать
# родом способ речи.
РОД_В_ПОКАЗЕ = 1
...
ЗАЧЕМ_РОДА = {
    "единицы": "k больших единиц времени суть k × f малых — на девяти языках, двумя "
               "рамками и двумя записями числа",
}
...
def _образцы():
    ...
    for язык, я in ЯЗЫКИ.items():
        # ОБРАЗЕЦ ЗНАЕТ ЧИСЛО ВО ВСЕХ ЕГО РОДАХ, ИНАЧЕ СУД НЕМЕЕТ НА ВЕРНОЙ СТРОКЕ: «dois
        # minutos» есть та же двойка, что «duas horas», и слово берётся у той же двери, какой
        # его пишет дом. Заодно суд получает поимку чужого рода даром: строка «duas minutos»
        # теперь ОБРАЗЦУ подходит, а сверке с `_слово` — нет, и потому зовётся ложью.
        слова = list(я["словом"].values()) + [с for т in я.get("словом2", {}).values() for с in т.values()]
        слова += [langpack.числительное_по_роду(_пакет(язык), с, р)
                  for с in list(слова) for р in ("m", "f", "n")]
