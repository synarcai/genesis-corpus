...
# ХОДЫ ДОКАЗАТЕЛЬСТВА БЫЛИ НАЗВАНЫ КОММЕНТАРИЯМИ «# ---» И НЕ ВЫШЛИ НАРУЖУ. Род здесь —
# ХОД, которым доказывают, и вопрос о шаге индукции есть индукция, спрошенная, а не род
# «вопрос».
ИНДУКЦИЯ = "индукция: основание и шаг, оба числами"
РАЗБОР_СЛУЧАЕВ = "полный разбор случаев: третьего не остаётся"
КОНТРПРИМЕР = "контрпример: одно свидетельство убивает всеобщее"
ПРЯМОЕ = "прямое доказательство: посылка выполнена, следствие проверено"
ИСЧЕРПАНИЕ = "отказ исчерпанием: свидетеля нет на всём отрезке"
ПОЛОЖИТЕЛЬНОСТЬ = "знак числа: больше ли нуля"
...
def pass_shows(pass_i):
    ...
    for i in range(8):
        ...
        # КВАДРАТЫ, и рынок вердикта видел двух исполнителей, согласных на всех
        # строках — «квадрат числа» и «сдвиг чётности», — и честно отказывал
        # как двусмысленной. Второе чётное берётся из того же объявленного ряда
        # и квадратом НЕ является; свидетель — разложение на два множителя.
        друг = ЧЁТНЫЕ[(pass_i * 3 + i * 5 + 3) % len(ЧЁТНЫЕ)]
        if друг == e:
            друг = ЧЁТНЫЕ[(pass_i * 3 + i * 5 + 4) % len(ЧЁТНЫЕ)]
...
РОДЫ = (ИНДУКЦИЯ, РАЗБОР_СЛУЧАЕВ, КОНТРПРИМЕР, ПРЯМОЕ, ИСЧЕРПАНИЕ, ПОЛОЖИТЕЛЬНОСТЬ)

ЗАЧЕМ_РОДА = {
    ИНДУКЦИЯ: "сумма первых n чисел: основание n = 1 и шаг n → n + 1, оба показаны числами",
    РАЗБОР_СЛУЧАЕВ: "«всякое целое чётно или нечётно» — полнота названа на странице, ибо "
                    "ею разбор и доказывает",
    КОНТРПРИМЕР: "всеобщее утверждение убито одним свидетелем, и свидетель назван",
    ПРЯМОЕ: "«если n чётно, то n² чётно» с выполненной посылкой и проверенным следствием, "
            "и обе полярности той же рамкой",
    ИСЧЕРПАНИЕ: "«чётного простого между 3 и N нет» — контрпример наизнанку: утверждение "
                "обо ВСЁМ отрезке, проверяемое проходом",
    ПОЛОЖИТЕЛЬНОСТЬ: "«больше ли нуля» двумя рамками, каждая над своим рядом знаковых чисел",
}
