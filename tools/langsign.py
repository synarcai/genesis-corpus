...
# ПРИМЕЧАНИЕ ЕСТЬ РЕЧЬ О ПАКЕТЕ, А НЕ РЕЧЬ ПАКЕТА (16.09). Поля-примечания писаны ПО-РУССКИ
# внутри пакетов всех языков, и слова их шли в словарь того языка, чей это пакет: русское «но»
# из примечания к английскому пакету стало ЕДИНОЛИЧНЫМ АНГЛИЙСКИМ знаком, и русская страница
# мира оснований отошла английской стороне (прибор `scripts/name_crossed.py`).
#
#     КОММЕНТАРИЙ, ПИСАННЫЙ НА ОДНОМ ЯЗЫКЕ ВНУТРИ ПАКЕТА ДРУГОГО, СТАНОВИТСЯ СЛОВАРЁМ ЭТОГО
#     ДРУГОГО, ЕСЛИ МЕРА НЕ ОТЛИЧАЕТ РЕЧЬ ОТ РЕЧИ О РЕЧИ.
#
# Прежде поля назывались СПИСКОМ, и в списке стояло три из пятнадцати: двенадцать примечаний
# всё это время шли в словарь — 592 слова, из них 280 стали единоличными знаками языка.
#
#     СПИСОК, КОТОРЫЙ НАДО ПОПОЛНЯТЬ РУКОЙ, РАЗОЙДЁТСЯ С ЖИВУЩИМИ; вопрос лишь в том,
#     назовёт ли кто-нибудь день расхождения. Оттого ныне здесь ПРАВИЛО, а не список: всякое
#     поле, чьё имя есть «comment» либо кончается на «_comment», есть примечание.
НЕ_СЛОВАРНЫЕ = frozenset(("comment", "lexeme_source"))


def _примечание(поле):
    """Поле-примечание: речь О пакете, а не речь пакета."""
    return поле in НЕ_СЛОВАРНЫЕ or поле.endswith("_comment")
...
# ОБРАТНЫЙ УКАЗАТЕЛЬ: слово → его единственный язык. Тот же закон, читаемый с другого конца, —
# и цена его чтения падает с «двадцать девять множеств на слово» до одного взгляда. Знак
# спрашивается всякой строкой всякого суда, и лишняя доля миллисекунды здесь есть минуты точки.
ЕДИНОЛИЧНЫЙ_ЯЗЫК = {слово: язык for язык, слова in ЕДИНОЛИЧНЫЕ.items() for слово in слова}
...
def счёт(строка):
    ...
    # ОДНО СЛУЖЕБНОЕ СЛОВО НЕ ЕСТЬ ЗНАК, ЕСЛИ ОНО ГОМОГРАФ (08.09, вечер; прибор «знак не тот»).
    # Из 173 ошибок знака 118 дало ОДНО слово — «on»: английский предлог, финская связка («kauppa
    # on tässä»), польское местоимение («on mieszka»), турецкое «десять» («on iki»). Строка, где
    # английский набрал ЕДИНИЦУ этим словом, а финский — ноль, отходила английскому суду, хотя
    # два других её слова объявлены только финским.
    #
    #     ЗНАК В ОДНО СЛОВО СЛИШКОМ ТОНОК, ЧТОБЫ РЕШАТЬ СПОР В ОДИНОЧКУ. При таком знаке к нему
    #     ПРИБАВЛЯЮТСЯ единоличные слова — те, что объявлены ровно одним пакетом; при знаке в два
    #     слова и более спор решён и без них.
    if max(вон.values(), default=0) > 1:
        return вон
    свои = collections.Counter()
    for w in слова:
        язык = ЕДИНОЛИЧНЫЙ_ЯЗЫК.get(w)
        if язык is not None:
            свои[язык] += 1
    if свои:
        языки = set(вон) | set(свои)
        return {язык: вон.get(язык, 0) + свои.get(язык, 0) for язык in языки}
    if any(вон.values()):
        return вон
    # ЗНАК ПОСЛЕДНЕЙ НАДЕЖДЫ ЗОВЁТСЯ ДЛЯ ВСЕХ ПАКЕТОВ, А НЕ ДЛЯ ТЕХ ЛИШЬ, ЧТО ОБЪЯВИЛИ СЛУЖЕБНЫЕ
    # СЛОВА (08.09, вечер). `СЛОВА` строится по полю `function_words`, и его нет у тринадцати
    # пакетов из двадцати девяти — амхарского, арабского, греческого, японского и прочих. Оттого
    # запасной счёт шёл лишь по шестнадцати языкам, и строка, чьё единоличное слово принадлежит
    # одному из тринадцати, оставалась беззнаковой при полном указателе.
    #
    #     ЯЗЫК, НЕ ОБЪЯВИВШИЙ СЛУЖЕБНЫХ СЛОВ, НЕ ЕСТЬ ЯЗЫК БЕЗ СЛОВ ВОВСЕ. Первый список — о
    #     том, чем язык различается ЧАЩЕ; второй — о том, чем он различается ВООБЩЕ.
    return {язык: свои.get(язык, 0) for язык in set(СЛОВА) | set(свои)}
