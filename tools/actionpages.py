...
import romgram  # noqa: E402 — пары вопросного слова: один дом закона
...
def _лица(язык):
    ...
    for имя in п.get("person_names", ()):
        ...
        if not ф or ф.get("gender") not in ("m", "f"):
            continue
        # РЕГИСТР ИМЕНИ ОБЪЯВЛЯЕТ ПАКЕТ (05.09). Прежде заглавная делалась здесь
        # и только русскому; английский и русский пакеты хранили имена строчными,
        # и свод два месяца писал «ann has 5 apples» 29 045 раз при семи тысячах
        # «Ann» из прозы полки — а русский расщепился по домам («у Анны» через
        # этот реестр, «у тома» у тех, кто читал пакет напрямую). Имя лица
        # пишется с заглавной во всяком языке, имеющем регистр букв; знает это
        # пакет, а не один из его читателей.
        показ = имя
        род = ф.get("gen") or показ
...
ЯЗЫКИ = {
    "en": dict(
        ...
        вместе_имена="{X} has {n} {Тn}. {Y} has {m} {Тm}. {X} and {Y} have {r} {Тr}: {л}",
    ...
    "ru": dict(
        ...
        вместе_имена="у {Xр} {n} {Тn}. у {Yр} {m} {Тm}. у {Xр} и {Yр} вместе {r} {Тr}: {л}",
        # ГЛАГОЛЬНАЯ ПОВЕРХНОСТЬ ПЛЮРАЛЬНОГО ФАКТА — ТОЛЬКО РУССКОМУ (04.09,
        # число 32). Русский плюральный факт этого дома БЕЗГЛАГОЛЕН («у ани и
        # бори вместе 21 яблоко»), и рамка эта законна и хороша; но строитель
        # квадов ищет факт как «агент ГЛАГОЛ число товар», и безглагольная
        # посессивная рамка ему не видна — русский не давал квадов даже в
        # одиночку, тогда как у прочих шести языков глагол в этой позиции
        # стоит сам («have», «haben», «ont», «tienen», «hanno», «têm»).
        # Безглагольная и местоименная поверхности НЕ убираются: они и есть
        # материал для будущего закона, который научит читать безглагольный
        # плюральный факт. Глагольная встаёт РЯДОМ третьей.
        вместе_глагол="у {Xр} {n} {Тn}. у {Yр} {m} {Тm}. {X} и {Y} имеют {r} {Тr}: {л}",
        ...
        воп_курс="сколько {Т2м} стоят {n} {Тn}?", воп_курс_обр="сколько {Т2м} стоит {О1}?",
    ...
    "de": dict(
        ...
        вместе_имена="{X} hat {n} {Тn}. {Y} hat {m} {Тm}. {X} und {Y} haben {r} {Тr}: {л}",
        ...
        воп_курс="wie viele {Т2} kosten {n} {Тn}?", воп_курс_обр="wie viele {Т2} kostet {О1}?",
    ...
    "es": dict(
        ...
        вместе_имена="{X} tiene {n} {Тn}. {Y} tiene {m} {Тm}. {X} y {Y} tienen {r} {Тr}: {л}",
        ...
        # ДОЛЯ ОТ ОПРЕДЕЛЁННОГО МНОЖЕСТВА (15.09): род долей разблокирован ковкой ДОМА ДОЛЕЙ —
        # `fracforms` знал три языка, и потому здешняя `доля` молчала на четырёх романских.
        ушло="{Ч} {ТИХ} {Т} {У}.", ушли=("desapareció", "desaparecieron"),
        их={"m": "de ellos", "f": "de ellas"}, от_вещи={"m": "de los", "f": "de las"},
        # ЦЕЛОЕ, НАЗВАННОЕ МЕСТОМ, И ЧАСТЬ ЕГО — РОМАНСКАЯ КОВКА 15.09 (заказ прибора щербатости).
        # Порядок слов взят у своего же `место_п`: испанский ставит глагол впереди места,
        # прочие три — место впереди глагола. СВОЙСТВА ВЗЯТЫ НЕИЗМЕНЯЕМЫЕ ПО РОДУ, и это не
        # уловка: дом не объявлял рода прилагательного, и писать «rojas» при монетах и «rojos»
        # при мячах ему нечем. Романские прилагательные на -e (и на согласный) множественного
        # числа рода не различают — «verdes», «grandes», «rouges», «verdi», «azuis», — и дом
        # берёт ровно их, как дом метки роли взял рамки без местоимений по той же причине.
        #
        #     ЯЗЫК ДАЁТ СРЕДСТВО СКАЗАТЬ ТО, ЧЕГО ДОМ НЕ ОБЪЯВЛЯЛ, И ЭТИМ СРЕДСТВОМ НАДО
        #     ПОЛЬЗОВАТЬСЯ, А НЕ ОБХОДИТЬ ЕГО МОЛЧАНИЕМ.
        место="hay {n} {Тn} {П}.",
        свойства=("verdes", "azules", "grandes", "grises", "brillantes"),
        часть_их="{k} {ИХ} son {С}.", часть_их_п="{k} {ИХ} eran {С}.",
        часть_товар="{k} {Тk} son {С}.", часть_товар_п="{k} {Тk} eran {С}.",
        воп_не="¿{КВ} no son {С}?", воп_не_п="¿{КВ} no eran {С}?",
        воп_не_товар="¿{КВ} {Т} no son {С}?", воп_не_товар_п="¿{КВ} {Т} no eran {С}?",
        воп_часть="¿{КВ} {ИХ} son {С}?",
        остальные={"m": "los demás", "f": "las demás"},
        две_части="{k} {ИХ} son {С}, {k2} son {С2}, y {ОСТ} son {С3}.",
        две_части_п="{k} {ИХ} eran {С}, {k2} eran {С2}, y {ОСТ} eran {С3}.",
        воп_остаток="¿{КВ} son {С3}?", воп_остаток_п="¿{КВ} eran {С3}?",
        # ЖИВОЕ И ВМЕСТИЛИЩЕ — ВТОРАЯ ПОЛОВИНА РОМАНСКОЙ КОВКИ 15.09.
        # Живые взяты ОДНОГО РОДА нарочно: рамка ухода одна на всех, а «se fue» / «se fueron»
        # согласуется с числом, но не с родом, и смешение внесло бы ошибку, какой здешний суд
        # не поймает. Вместилище объявлено ИМЕНИТЕЛЬНОЙ формой рядом с местной: «la caja» при
        # «en la caja», — ибо список говорит о вместилище как о подлежащем, а не о месте.
        живые=(("niño", "niños"), ("pájaro", "pájaros"), ("gato", "gatos"),
               ("pato", "patos"), ("invitado", "invitados")),
        место_п_живые="había {n} {Жn} {П}.",
        ушло_живое="{k} {Жk} se fueron.", ушло_живое_один="{k} {Жk} se fue.",
        воп_живые_теперь="¿cuántos {Ж} hay ahora?",
        вместилища=("la caja", "la bolsa", "el estante", "la cesta", "el cajón"),
        список_числом="{П1} tiene {Чс} {ВЕЩ}: {О1}, {О2} y {О3} — 1, 2, 3.",
        список_итогом="en {П1} hay {О1}, {О2} y {О3} — {Чс} {ВЕЩ}.",
    ...
    "fr": dict(
        ...
        вместе_имена="{X} a {n} {Тn}. {Y} a {m} {Тm}. {X} et {Y} ont {r} {Тr} : {л}",
        ...
        # РОД ВЕЩИ ОБЪЯВЛЕН И ФРАНЦУЗСКОМУ (15.09): вопросное слово он не согласует («combien»
        # одно на оба рода), и оттого род его вещей молчал — а «из них» согласования требует.
        род_вещи=("f", "m", "m", "f", "f", "m", "m", "f"),
        ушло="{Ч} {ТИХ} {Т} {У}.", ушли=("a disparu", "ont disparu"),
        их={"m": "d'entre eux", "f": "d'entre elles"}, от_вещи={"m": "des", "f": "des"},
        место="{П} il y a {n} {Тn}.",
        свойства=("rouges", "jaunes", "roses", "mauves", "beiges"),
        часть_их="{k} {ИХ} sont {С}.", часть_их_п="{k} {ИХ} étaient {С}.",
        часть_товар="{k} {Тk} sont {С}.", часть_товар_п="{k} {Тk} étaient {С}.",
        воп_не="combien ne sont pas {С} ?", воп_не_п="combien n'étaient pas {С} ?",
        воп_не_товар="combien {ДЕ}{Т} ne sont pas {С} ?",
        воп_не_товар_п="combien {ДЕ}{Т} n'étaient pas {С} ?",
        воп_часть="combien {ИХ} sont {С} ?",
        остальные={"m": "les autres", "f": "les autres"},
        две_части="{k} {ИХ} sont {С}, {k2} sont {С2}, et {ОСТ} sont {С3}.",
        две_части_п="{k} {ИХ} étaient {С}, {k2} étaient {С2}, et {ОСТ} étaient {С3}.",
        воп_остаток="combien sont {С3} ?", воп_остаток_п="combien étaient {С3} ?",
        # ФРАНЦУЗСКИЕ ЖИВЫЕ ВЗЯТЫ НА СОГЛАСНУЮ: вопрос «combien de {Ж}» элизии не делает, а
        # слот её не несёт («combien d'enfants» потребовал бы второго слота ради пяти слов).
        живые=(("chat", "chats"), ("canard", "canards"), ("garçon", "garçons"),
               ("voisin", "voisins"), ("danseur", "danseurs")),
        место_п_живые="{П} il y avait {n} {Жn}.",
        ушло_живое="{k} {Жk} sont partis.", ушло_живое_один="{k} {Жk} est parti.",
        воп_живые_теперь="combien de {Ж} y a-t-il maintenant ?",
        вместилища=("la boîte", "le sac", "le panier", "le tiroir", "la valise"),
        список_числом="{П1} contient {Чс} {ВЕЩ} : {О1}, {О2} et {О3} — 1, 2, 3.",
        список_итогом="dans {П1} il y a {О1}, {О2} et {О3} — {Чс} {ВЕЩ}.",
    ...
    "it": dict(
        ...
        вместе_имена="{X} ha {n} {Тn}. {Y} ha {m} {Тm}. {X} e {Y} hanno {r} {Тr}: {л}",
        ...
        ушло="{Ч} {ТИХ} {Т} {У}.", ушли=("mancava", "mancavano"),
        # КРАТНОЕ ПО-ИТАЛЬЯНСКИ БЕРЁТ ТОТ ЖЕ СЛОТ РОДА, ЧТО И ДОЛЯ: «il doppio DELLE monete».
        # Четвёртая кратность не объявлена нарочно: «quattro volte» слитного оборота с артиклем
        # не даёт, а писать её иначе значило бы завести вторую рамку ради одного числа.
        кратно="{Y} ha {K} {ТИХ} {Т} di {X}.", кратные={2: "il doppio", 3: "il triplo"},
        место="{П} ci sono {n} {Тn}.",
        свойства=("verdi", "grandi", "celesti", "lucenti", "pesanti"),
        часть_их="{k} {ИХ} sono {С}.", часть_их_п="{k} {ИХ} erano {С}.",
        часть_товар="{k} {Тk} sono {С}.", часть_товар_п="{k} {Тk} erano {С}.",
        воп_не="{КВ} non sono {С}?", воп_не_п="{КВ} non erano {С}?",
        воп_не_товар="{КВ} {Т} non sono {С}?", воп_не_товар_п="{КВ} {Т} non erano {С}?",
        воп_часть="{КВ} {ИХ} sono {С}?",
        остальные={"m": "gli altri", "f": "le altre"},
        две_части="{k} {ИХ} sono {С}, {k2} sono {С2}, e {ОСТ} sono {С3}.",
        две_части_п="{k} {ИХ} erano {С}, {k2} erano {С2}, e {ОСТ} erano {С3}.",
        воп_остаток="{КВ} sono {С3}?", воп_остаток_п="{КВ} erano {С3}?",
        живые=(("bambino", "bambini"), ("uccello", "uccelli"), ("gatto", "gatti"),
               ("ospite", "ospiti"), ("ragazzo", "ragazzi")),
        место_п_живые="{П} c'erano {n} {Жn}.",
        ушло_живое="{k} {Жk} sono andati via.", ушло_живое_один="{k} {Жk} è andato via.",
        воп_живые_теперь="quanti {Ж} ci sono adesso?",
        # ИТАЛЬЯНСКИЙ И ПОРТУГАЛЬСКИЙ СЛИВАЮТ ПРЕДЛОГ С АРТИКЛЕМ («in la scatola» не
        # существует — «nella scatola»), и потому список у них говорит о вместилище как о
        # ПОДЛЕЖАЩЕМ («la scatola contiene»), а не о месте. Испанский и французский предлога
        # не сливают и держат прежний оборот.
        вместилища=("la scatola", "la borsa", "lo scaffale", "il cesto", "il cassetto"),
        список_числом="{П1} ha {Чс} {ВЕЩ}: {О1}, {О2} e {О3} — 1, 2, 3.",
        список_итогом="{П1} contiene {О1}, {О2} e {О3} — {Чс} {ВЕЩ}.",
        их={"m": "di essi", "f": "di esse"}, от_вещи={"m": "dei", "f": "delle"},
    ...
    "pt": dict(
        ...
        вместе_имена="{X} tem {n} {Тn}. {Y} tem {m} {Тm}. {X} e {Y} têm {r} {Тr}: {л}",
        ...
        ушло="{Ч} {ТИХ} {Т} {У}.", ушли=("desapareceu", "desapareceram"),
        кратно="{Y} tem {K} {Т} que {X}.",
        кратные={2: "o dobro de", 3: "o triplo de", 4: "quatro vezes mais"},
        место="{П} há {n} {Тn}.",
        свойства=("verdes", "grandes", "azuis", "simples", "fortes"),
        часть_их="{k} {ИХ} são {С}.", часть_их_п="{k} {ИХ} eram {С}.",
        часть_товар="{k} {Тk} são {С}.", часть_товар_п="{k} {Тk} eram {С}.",
        воп_не="{КВ} não são {С}?", воп_не_п="{КВ} não eram {С}?",
        воп_не_товар="{КВ} {Т} não são {С}?", воп_не_товар_п="{КВ} {Т} não eram {С}?",
        воп_часть="{КВ} {ИХ} são {С}?",
        остальные={"m": "os outros", "f": "as outras"},
        две_части="{k} {ИХ} são {С}, {k2} são {С2}, e {ОСТ} são {С3}.",
        две_части_п="{k} {ИХ} eram {С}, {k2} eram {С2}, e {ОСТ} eram {С3}.",
        воп_остаток="{КВ} são {С3}?", воп_остаток_п="{КВ} eram {С3}?",
        живые=(("menino", "meninos"), ("pássaro", "pássaros"), ("gato", "gatos"),
               ("pato", "patos"), ("convidado", "convidados")),
        место_п_живые="{П} havia {n} {Жn}.",
        ушло_живое="{k} {Жk} foram embora.", ушло_живое_один="{k} {Жk} foi embora.",
        воп_живые_теперь="quantos {Ж} há agora?",
        вместилища=("a caixa", "a bolsa", "a prateleira", "o cesto", "a gaveta"),
        список_числом="{П1} tem {Чс} {ВЕЩ}: {О1}, {О2} e {О3} — 1, 2, 3.",
        список_итогом="{П1} contém {О1}, {О2} e {О3} — {Чс} {ВЕЩ}.",
        их={"m": "deles", "f": "delas"}, от_вещи={"m": "dos", "f": "das"},
    ...
    "pl": dict(
        вещи=("moneta", "piłka", "książka", "jabłko", "jajko", "kwiat", "karta", "długopis"),
        предметы=("rzecz", "rzeczy", "rzeczy"),
    ),
    "nl": dict(
        вещи=(("munt", "munten"), ("bal", "ballen"), ("boek", "boeken"), ("appel", "appels"),
              ("ei", "eieren"), ("bloem", "bloemen"), ("kaart", "kaarten"), ("pen", "pennen")),
        предметы=("ding", "dingen"),
    ),
}
ЛИЦА = {л: _лица(л) for л in ЯЗЫКИ}

# СОЮЗ ИМЁН СТОИТ В ШАБЛОНЕ СЛОВОМ, И ЭТО ЗАКОННО РОВНО ПОКА ИМЕНА ЕГО НЕ
# МЕНЯЮТ. Два языка из семи меняют союз по звуку СЛЕДУЮЩЕГО слова: испанское
# «y» становится «e» перед i-/hi- («Ana e Isabel»), итальянское «e» — «ed»
# перед e- («Marco ed Elena»). Ни одно объявленное имя такого звука не имеет,
# и потому союз написан буквой; но правило не выведено из нынешнего списка, а
# ОБЪЯВЛЕНО вместе с ним — и день, когда в пакет войдёт «Isabel», кончится
# здесь громким «нет», а не тихой ошибкой в трёхстах показах.
СОЮЗ_МЕНЯЕТСЯ = {"es": ("i", "hi"), "it": ("e",)}
for _яз, _начала in СОЮЗ_МЕНЯЕТСЯ.items():
    _беда = [и for и, _, _ in ЛИЦА[_яз] if и.lower().startswith(_начала)]
    assert not _беда, f"{_яз}: союз имён меняется перед {_беда} — объявить форму, а не букву"
...
# ЯЗЫК ВПРАВЕ ОБЪЯВИТЬ ЧАСТЬ ДОМА (04.09). Голландский и польский пришли сюда
# за одним: за ВЕЩАМИ с формами счёта и словом предмета — их читают дом
# сочленений и дом связок, и второго словаря заводить нельзя (два дома, один
# словарь). Страниц действия они не пишут: рамок действия у них нет, и
# генератор их не спросит. Это не ущерб, а объявление: дом отвечает ровно тем,
# что язык объявил.
# ПЛЮРАЛЬНЫЙ ФАКТ БЕЗГЛАГОЛЕН НЕ ВЕЗДЕ (объявление 15.09 тому, что сборка знает с первого дня).
# Род `вместе_глагол` заведён ради языка, чей плюральный факт обходится без глагола: «у Ани 5,
# у Бори 3» — и тогда нужен отдельный показ, где глагол назван вслух («Аня и Боря имеют 8»).
# Там, где глагол стои́т в самом факте («Ann has 5»), этот род был бы повтором рода `вместе_имена`,
# и сборка его не заводит — строка `if "вместе_глагол" in ЯЗЫКИ[язык]` в теле дня.
#
#     ОБЪЯВЛЕНИЕ ЕСТЬ ПЕРЕВОД УСЛОВИЯ СБОРКИ НА ЯЗЫК ПРИБОРА. Условие стои́т в теле и верно;
#     прибор же ходит по трёмстам домам и в тело не заглядывает — он читает словарь.
ОБЪЯВЛЕННЫЕ_ПРОПУСКИ = {"вместе_глагол": frozenset(я for я in ЯЗЫКИ
                                                   if "вместе_глагол" not in ЯЗЫКИ[я])}

ФОРМЫ = ("остаток", "прибавка", "доля", "вместе", "вместе_имена", "вместе_глагол", "кратное", "место", "место_эхо", "место_две", "место_п", "место_две_п", "место_товар", "место_товар_п", "ставка", "вместе_они", "курс", "курс_обр", "список_числом", "список_итогом", "список_чисел", "убыль", "место_убыло", "место_ушли",
...
def _вещь(язык, i, n):
    в = ЯЗЫКИ[язык]["вещи"][i]
    # ЯЗЫК СО СВОИМ ДОМОМ СЧЁТА БЕРЁТ ФОРМУ ОТТУДА, а не из пары «один/много»:
    # у русского и польского ТРИ формы при числе, и пара сказала бы о них ложь
    # при всяком числе от двух до четырёх (дом tools/plgram.py, 04.09).
    if язык == "ru":
...
# ПАРЫ ВОПРОСНОГО СЛОВА СВЕРЯЮТСЯ С ДОМОМ ЯЗЫКА (12.09). Таблица языка называет их при себе —
# так она читается целиком, — а дом романского вопросного слова (`tools/romgram.py`) объявляет
# их для всех, кто их пишет. Два списка, разойдясь молча, дали бы своду два разных «cuántas»:
# сверка стои́т ЗДЕСЬ и роняет дом при несогласии, а не откладывает беду до ворот.
#
#     ДВА ОБЪЯВЛЕНИЯ ОДНОГО ЗАКОНА ЖИВУТ, ПОКА ИХ СВЕРЯЮТ; НЕСВЕРЯЕМЫЕ — РАСХОДЯТСЯ.
for _язык, (_м, _ж) in romgram.ПАРЫ.items():
    _своё = ЯЗЫКИ.get(_язык, {}).get("вопросное")
    assert _своё == {"m": _м, "f": _ж}, (_язык, _своё, (_м, _ж))
...
def _их(язык, i):
    """«из них» — и у романских языков оно СОГЛАСОВАНО В РОДЕ С ВЕЩЬЮ (15.09).

    Русское «из них», немецкое «davon», английское «of them» рода не знают, и потому четыре
    старших языка дома писали часть-от-целого одной рамкой. Романские так не умеют:
    местоимение третьего лица множественного числа у них имеет род, и «5 de ellos son verdes»
    при монетах — ошибка согласования, какой не поймал бы здешний суд.

        БЕЗРОДОВОГО «ИЗ НИХ» В РОМАНСКИХ ЯЗЫКАХ НЕТ, И ЭТО НЕ ПОМЕХА, А УСЛОВИЕ. Род вещи
        объявлен при каждой вещи (`род_вещи`) — тот же словарь, что кормит вопросное слово;
        значит и «из них» берётся оттуда же, и ковка романской части-от-целого стои́т ровно
        одного нового слота, а не нового устройства.

    Язык, не объявивший `их`, получает пустую строку: его рамки этого слота не держат.
    """
    я = ЯЗЫКИ[язык]
    роды, слова = я.get("род_вещи"), я.get("их")
    if not роды or not слова:
        return ""
    return слова.get(роды[i], "")
...
def _от_вещи(язык, i):
    """«de las monedas», «dei palloni» — предлог с артиклем при ВЕЩИ, по её роду.

    Доля берётся ОТ ОПРЕДЕЛЁННОГО множества, и романский артикль при нём обязателен:
    «la mitad de las monedas», а не «de monedas». Русское «половина монет» и немецкое «der
    Münzen» обходятся падежом, английское «of the coins» — одним артиклем на оба рода.

        ОДИН ЯЗЫК ДЕЛАЕТ ОДНИМ СЛОВОМ ТО, ЧТО ДРУГОЙ РАЗЛИЧАЕТ ПО РОДУ, И РАМКА ОБЩЕЙ НЕ
        БУДЕТ. Французское «des» одинаково при обоих родах — и объявлено всё-таки словарём
        из двух равных строк, а не пустотой: равенство здесь ФАКТ ЯЗЫКА, и молчание о нём
        неотличимо от недоделки.
    """
    я = ЯЗЫКИ[язык]
    роды, слова = я.get("род_вещи"), я.get("от_вещи")
    if not роды or not слова:
        return ""
    return слова.get(роды[i], "")


def _остальные(язык, i):
    """«los demás», «le altre» — остаток множества, и он тоже по роду вещи.

    Первая редакция романских рамок писала «el resto son verdes», «il resto sono verdi»,
    «le reste sont roses» — и это ошибка согласования у всех троих: «resto» есть
    существительное единственного числа, и глагол при нём единственный. Согласование по
    смыслу («ad sensum») испанский и португальский терпят, французский — нет.

        ГДЕ ОДИН ЯЗЫК ТЕРПИТ СОГЛАСОВАНИЕ ПО СМЫСЛУ, А ДРУГОЙ ЕГО ЗАПРЕЩАЕТ, ОБЩАЯ РАМКА
        ПИШЕТСЯ ПО СТРОГОМУ. Оборот «остальные» множествен во всех четырёх и спора не
        знает; ценою ему — третий слот рода, и это дешевле, чем четыре разных рамки.
    """
    я = ЯЗЫКИ[язык]
    роды, слова = я.get("род_вещи"), я.get("остальные")
    if not роды or not слова:
        return ""
    return слова.get(роды[i], "")
...
def _а(язык, пол):
    """Прошедшее склоняется по роду подлежащего: «отдал» / «отдала», «miał» /
    «miała». Оба славянских языка дома делают это одной буквой, но буквы РАЗНЫЕ
    (кириллическая «а» и латинская «a»), и объявлены они порознь: совпадение
    правил не есть тождество письма."""
    if пол != "f":
        return ""
    return {"ru": "а", "pl": "a"}.get(язык, "")
...
def страница(язык, форма, X, Т=0, n=0, k=0, m=0, Y=None, доля=None, кратность=2,
    ...
    if Y is not None:
        имяY, пол_Y, род_Y = _лицо(язык, Y)
        з.update(Y=имяY, Yр=род_Y)
    def вещь(сколько):
    ...
    общ = dict(з, Т=вещь(2), Тn=вещь(n), Тk=вещь(k), Тm=вещь(m), Тм=вещь(5), Твk=вин,
               ...
               ИХ=_их(язык, Т), ТИХ=_от_вещи(язык, Т), ОСТ=_остальные(язык, Т))
    if форма in ("вместе_они", "вместе_имена", "вместе_глагол") and Y is None:
    ...
    if форма == "остаток":
        ...
        С = я["свойства"][свойство]
        факт = я["место"].format(**общ, n=n, П=я["места"][место]) + " " + я["часть_их"].format(**общ, k=k, С=С)
        воп = я["воп_часть"].format(**общ, С=С)
        ...
        if форма.startswith("место_товар"):
            ...
            воп = я["воп_не_п"].format(**общ, С=С, Ст=Ст)
        леджер = f"{n} − {k} = {r}."
        ...
        воп = я["воп_остаток_п"].format(**общ, С3=С3, С3р=С3р)
        ...
        С3р = (я.get("свойства_род") or я["свойства"])[свойство3]
        воп = я["воп_остаток"].format(**общ, С3=С3, С3р=С3р)
        леджер = f"{n} − {k} = {r1}, {r1} − {k2} = {r}."
        return f"{факт} {воп} {леджер} {я['ответ'].format(r=r)}"
        ...
        # «ОДНО X» БЕРЁТСЯ ОДНОЙ ДВЕРЬЮ ВО ВСЕХ ЯЗЫКАХ (22.09, суд рода числительного): романские
        # рамки звали `{О1}` — объявленное «одно X» с верным родом, — а русская вшила «одна» и
        # писала «одна яблоко», «одна шар», немецкая вшила «ein» и писала «ein Münze», «ein
        # Karte». Таблица `один` знала род верно; рамка его не спрашивала.
        if форма == "курс":
            r = n * k
        ...
        # ПЛЮРАЛЬНЫЙ ФАКТ ДВУМЯ ПОВЕРХНОСТЯМИ: местоимением («together they
        # have») и ИМЕНАМИ («greg and anna have»). Вторая объявлена 04.09 по
        # числу 32: глагол множественного числа стоял в этом мире 240 раз, но
        # НИ РАЗУ в утверждении с числом и НАЗВАННЫМ носителем — 50 раз при
        # местоимении (один «агент» для закона четырёхместности) и всё
        # прочее внутри вопросов, где числа нет. Рынок, покупающий глагол
        # законом [агент глагол ЧИСЛО товар] с ≥LAW разными агентами, не мог
        # купить «have» ни при каком числе страниц — а плюральный факт
        # агрегата ОБЯЗАН стоять во множественном по согласованию. Мир учил
        # форме, которую сам же и не показывал в покупаемом виде.
        r = n + m
        л = f"{n} + {m} = {r}."
        ключ = форма if форма in ("вместе_имена", "вместе_глагол") else "вместе_они"
        return я[ключ].format(**общ, n=n, m=m, r=r, Тr=вещь(r), л=л) + " " + я["ответ"].format(r=r)
    return f"{факт} {воп} {леджер} {я['ответ'].format(r=r)}"
...
def _дыры(язык):
    ...
    return {"X": _альт(л[0] for л in лица), "Y": _альт(л[0] for л in лица),
            ...
            # РОМАНСКИЕ СЛОТЫ РОДА: «из них» при местоимении и «от вещей» при вещи (15.09).
            "ИХ": _альт((я.get("их") or {}).values()),
            "ТИХ": _альт((я.get("от_вещи") or {}).values()),
            "ОСТ": _альт((я.get("остальные") or {}).values()),
            "n": ч, "k": ч, "m": ч, "r": ч}
...
def _шаблоны(язык):
    ...
    """
    я = ЯЗЫКИ[язык]
    # ЯЗЫК БЕЗ РАМОК ДЕЙСТВИЯ НЕ ПИШЕТ СТРАНИЦ И НЕ СУДИТ ИХ: он пришёл в дом
    # за словарём (вещи и слово предмета), и молчание его законно.
    if "ответ" not in я:
        return []
    л = "{л}"
    хвост = "{л} " + я["ответ"]
    состав = (
        ...
        ("вместе_имена", ("вместе_имена",), True),
        ("вместе_глагол", ("вместе_глагол",), True),
    ...
    for форма, ключи, с_хвостом in состав:
        ...
        if форма == "место_эхо":
            continue
        шаблон = " ".join(куски)
        if с_хвостом and форма not in ("вместе_они", "вместе_имена", "вместе_глагол"):
            шаблон = f"{шаблон} {хвост}"
        elif форма in ("вместе_они", "вместе_имена", "вместе_глагол"):
...
# СЛОВАРЬ СТРАНИЦ ДОМА СТРАНИЦ ДЕЙСТВИЯ (13.09, четвёртый переезд того же вида и самый
# крупный: 4 921 строка, тридцать один род). Перебор в двести сорок три строки жил у кузницы,
# и дом не знал ни одной своей страницы.
#
#     ДОМ, НЕ ЗНАЮЩИЙ СВОИХ СТРАНИЦ, НЕ МОЖЕТ НАЗВАТЬ ИХ РОДА, И РОД ЕГО ЖИВЁТ ОБЪЯВЛЕНИЕМ
#     БЕЗ АДРЕСА.
#
# ПЕРЕНОСЧИК НЕ ТРОНУЛ НИ ОДНОГО ИЗ ДВАДЦАТИ ШЕСТИ ВЫЗОВОВ. Род у них стои́т ТРЕТЬИМ ДОВОДОМ
# — иногда буквой («остаток»), иногда переменной (`форма`), — и оттого правка легла в ОДНУ
# функцию `_добавить`, которая теперь кладёт пару (страница, род). Двадцать шесть текстовых
# правок были бы двадцатью шестью случаями ошибиться.
#
#     ГДЕ ВСЕ ВЫЗОВЫ ИДУТ ЧЕРЕЗ ОДНУ ДВЕРЬ, ПРАВКА КЛАДЁТСЯ В ДВЕРЬ, А НЕ В КАЖДОГО ВХОДЯЩЕГО.
def _добавить(вон, язык, форма, *арг, **кв):
    """Страница, чьей формы язык не объявил, не пишется — и пара её не кладётся.

    ДВА РОДА НЕОБЪЯВЛЕННОГО, И ОБА МОЛЧАТ ОДИНАКОВО: форма ВЕЩИ, которой нет в парадигме
    (ValueError дома), и целый РОД, слов которого язык не назвал (KeyError таблицы).
    """
    try:
        вон.append((страница(язык, форма, *арг, **кв), форма))
...
def перебор(шаг, язык):
    лица = ЛИЦА[язык]
    вещей = len(ЯЗЫКИ[язык]["вещи"])
    кратности = sorted(ЯЗЫКИ[язык].get("кратные") or (2,))
    вон = []
    j = шаг * 17
    for i in range(5):
    ...
    вещей_мест = len(ЯЗЫКИ[язык].get("места") or (0,))
    for i in range(5):
        ...
        _добавить(вон, язык, "место", X=лица[0][0], Т=(шаг + i) % вещей, n=n, k=k,
                  место=(шаг + i) % вещей_мест, свойство=(шаг * 2 + i) % len(ЯЗЫКИ[язык].get("свойства") or (0,)))
        j += 1
    ...
    for i in range(5):
        ...
        _добавить(вон, язык, "место_эхо", X=лица[0][0], Т=(шаг + i + 1) % вещей, n=n, k=k,
                  место=(шаг + i + 1) % вещей_мест, свойство=(шаг + i) % len(ЯЗЫКИ[язык].get("свойства") or (0,)))
        j += 1
    for i in range(5):
        n = 10 * (3 + (шаг * 3 + i * 7 + j) % 12)
        k = 2 + (шаг * 5 + i * 3 + j) % max(2, n // 4)
        k2 = 2 + (шаг * 7 + i * 5 + j) % max(2, n // 4)
        свойств = len(ЯЗЫКИ[язык].get("свойства") or (0,))
    ...
    for i in range(5):
        ...
        _добавить(вон, язык, "место_п", X=лица[0][0], Т=(шаг + i + 3) % вещей, n=n, k=k,
                  место=(шаг + i + 3) % вещей_мест, свойство=(шаг + i + 1) % len(ЯЗЫКИ[язык].get("свойства") or (0,)))
        j += 1
    for i in range(5):
    ...
    for i in range(10):
        ...
        _добавить(вон, язык, форма, X=лица[0][0], Т=(шаг + i + 5) % вещей, n=n, k=k,
                  место=(шаг + i + 5) % вещей_мест, свойство=(шаг + i + 2) % len(ЯЗЫКИ[язык].get("свойства") or (0,)))
        j += 1
    ...
    вещей_мест = len(ЯЗЫКИ[язык].get("вместилища") or ЯЗЫКИ[язык].get("вместилища_где") or (0,))
    for i in range(7):
    ...
    for i in range(8):
        ...
        _добавить(вон, язык, "ставка", X=X, Т=(шаг + i + 5) % вещей, n=n, k=k, m=n * k,
                  глагол_ставки=(шаг + i) % len(ЯЗЫКИ[язык].get("ставки") or (0,)), дней=за)
        j += 1
    ...
    съедобные = (ЯЗЫКИ[язык].get("съедобные") or (0,))
    убылей = len(ЯЗЫКИ[язык].get("убыли") or (0,))
    ...
    живых = len(ЯЗЫКИ[язык].get("живые") or ())
    for i in range(живых):
        n = 8 + (шаг * 7 + i * 5 + j) % 40
        k = 1 + (шаг * 3 + i * 7 + j) % (n - 2)
        _добавить(вон, язык, "место_ушли", X=лица[0][0], Т=i, n=n, k=k,
                  место=МЕСТА_ЖИВЫХ[(шаг * 2 + i) % len(МЕСТА_ЖИВЫХ)])
        j += 1
    ...
    мест = len(ЯЗЫКИ[язык].get("места") or (0,))
    for i in range(мест):
        n = 7 + (шаг * 11 + i * 7 + j) % 70
        k = 1 + (шаг * 3 + i * 5 + j) % (n - 2)
    ...
    вместилищ = len(ЯЗЫКИ[язык].get("вместилища_где") or ЯЗЫКИ[язык].get("вместилища") or (0,))
    for i in range(7):
        ...
        j += 1
    # СОЧЛЕНЕНИЕ ДВУХ КОЛИЧЕСТВ ОДНОГО АКТА (заказ 32 04.09): двенадцать
    # страниц на проход на язык — рынок агрегатов покупает слово-список с
    # ПАРЫ, и пар нужно много, чтобы слово стало словом, а не случаем
    for i in range(12):
        т1 = (шаг * 5 + i) % вещей
        т2 = (т1 + 1 + (шаг + i) % (вещей - 1)) % вещей
        n1 = 5 + (шаг * 11 + i * 7 + j) % 200
        n2 = 3 + (шаг * 7 + i * 13 + j) % 150
        _добавить(вон, язык, "список_воп2", X=лица[(шаг + i) % len(лица)][0],
                  Т=т1, Т2=т2, n=n1, m=n2)
        j += 1
    ...
    for i in range(5):
        ...
        n = 3 + (шаг * 11 + i * 7 + j) % 60
        m = 3 + (шаг * 5 + i * 13 + j) % 60
        # ОБЕ ПОВЕРХНОСТИ ПЛЮРАЛЬНОГО ФАКТА, А НЕ ОДНА: местоимение («together
        # they have») и ИМЕНА («greg and anna have»). Местоименная поверхность
        # даёт закону четырёхместности ОДНОГО агента на все показы, и глагол
        # множественного числа не покупается ею никогда; именованная даёт
        # столько разных агентов, сколько пар выпало пассу.
        формы = ["вместе_они", "вместе_имена"]
        if "вместе_глагол" in ЯЗЫКИ[язык]:
            формы.append("вместе_глагол")   # язык, чей плюральный факт безглаголен
        for форма in формы:
            _добавить(вон, язык, форма, X=лица[a][0], Y=лица[b][0],
                      Т=(шаг + i + 6) % вещей, n=n, m=m)
        j += 1
    return вон
