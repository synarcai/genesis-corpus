...
ФОРМЫ = ("остаток", "прибавка", "доля", "вместе", "кратное", "место", "место_эхо", "место_две", "место_п", "место_две_п", "место_товар", "место_товар_п", "ставка", "вместе_они", "курс", "курс_обр", "список_числом", "список_итогом", "список_чисел")
...
def _дыры(язык):
    ...
    return {"X": _альт(л[0] for л in лица), "Y": _альт(л[0] for л in лица),
            ...
            "П1": _альт(я["вместилища"]), "n1": r"(\d+)", "n2": r"(\d+)", "n3": r"(\d+)",
...
def разобрать(язык, строка):
    ...
    for образец, имена, форма in ОБРАЗЦЫ[язык]:
        ...
        if X is None and not (форма.startswith("место") or форма.startswith("курс")
                              or форма in ("список_числом", "список_итогом")):
            continue
