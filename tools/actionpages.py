...
ФОРМЫ = ("остаток", "прибавка", "доля", "вместе", "кратное", "место", "место_эхо", "место_две", "ставка", "вместе_они")
...
def страница(язык, форма, X, Т=0, n=0, k=0, m=0, Y=None, доля=None, кратность=2,
             место=0, свойство=0, свойство2=1, свойство3=2, k2=0, глагол_ставки=0, дней=True):
    я = ЯЗЫКИ[язык]
...
def _дыры(язык):
    ...
    return {"X": _альт(л[0] for л in лица), "Y": _альт(л[0] for л in лица),
            ...
            "С3р": _альт(я.get("свойства_род", я["свойства"])),
...
def разобрать(язык, строка):
    ...
    for образец, имена, форма in ОБРАЗЦЫ[язык]:
        ...
        if плохо:
            continue
        X = _имя(з, язык, "X")
        if X is None and not форма.startswith("место"):
