...
ФОРМЫ = ("остаток", "прибавка", "доля", "вместе", "вместе_имена", "кратное", "место", "место_эхо", "место_две", "место_п", "место_две_п", "место_товар", "место_товар_п", "ставка", "вместе_они", "курс", "курс_обр", "список_числом", "список_итогом", "список_чисел", "убыль", "место_убыло", "место_ушли",
...
def страница(язык, форма, X, Т=0, n=0, k=0, m=0, Y=None, доля=None, кратность=2,
    ...
    if форма in ("вместе_они", "вместе_имена") and Y is None:
    ...
    if форма == "остаток":
        ...
        r = n + m
        л = f"{n} + {m} = {r}."
        ключ = "вместе_имена" if форма == "вместе_имена" else "вместе_они"
...
def _шаблоны(язык):
    ...
    for форма, ключи, с_хвостом in состав:
        ...
        if форма == "место_эхо":
            continue
        шаблон = " ".join(куски)
        if с_хвостом and форма not in ("вместе_они", "вместе_имена"):
            шаблон = f"{шаблон} {хвост}"
        elif форма in ("вместе_они", "вместе_имена"):
