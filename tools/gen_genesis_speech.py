...
def вывод(шаг):
    ...
    for i in range(24):
        ...
        # СЛОВО ВЕРДИКТА ОТКРЫВАЕТ ОТВЕТ (М-147): «да» перед посылкой и
        # выводом, как «нет» перед остатком — одна форма рассказа на полярность.
        вон.append(f"делится ли {n} на {d}? да: {утв_ru}")
        вон.append(f"is {n} divisible by {d}? yes: {утв_en}")
        if n % 2 == 0:
            чёт_ru = f"{n} делится на 2. значит {n} чётно."
            чёт_en = f"{n} is divisible by 2. therefore {n} is even."
            вон.append(чёт_ru)
            вон.append(чёт_en)
            вон.append(f"делится ли {n} на 2? да: {чёт_ru}")
            вон.append(f"is {n} divisible by 2? yes: {чёт_en}")
