...
def pass_shows(pass_i):
    ...
    for i, (base, s3, past, done, its) in enumerate(VERBS):
        ...
        # КУЗНИЦА СТОИТ ПРИ КАЖДОМ ВЫЧИСЛЕННОМ ОТВЕТЕ (09.09). Чередование, поставленное
        # против «310 вычисленных ответов без единого шага», уплатило половину долга и
        # оставило половину: суд родов насчитал 60 строк, чью ВЕЛИЧИНУ не пересчитывает
        # никто — их читали лишь суд памяти и суд счёта на языках, а те подписывают слова.
        #
        #     УТВЕРЖДЕНИЕ О ВЕЛИЧИНЕ, КОТОРОЕ НЕКОМУ ПЕРЕСЧИТАТЬ, ЕСТЬ УТВЕРЖДЕНИЕ БЕЗ
        #     ПОРУЧИТЕЛЯ. Две поверхности одного факта хороши, покуда обе поручены суду;
...
#
# Проверено счётом по ВСЕМУ словарю показов: 620 страниц, кириллицы 0, диакритики 0
ЯЗЫК = "en"
