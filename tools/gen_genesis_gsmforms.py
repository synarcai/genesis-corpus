...
def гл(имя, прошедшее):
    ...
    return прошедшее + "а" if имя in ЖЕНСКИЕ_RU else прошедшее
...
def население(шаг, i):
    ...
    return f"if the town has {всего} people and {слово} of the whole population lives in the centre, how many people live in the centre? {всего} ÷ {доля} = {часть}."
...
def скидка(шаг, i):
    ...
    return f"if each pack costs {ц} dollars and there is a discount of {с} dollars on each pack, how much do you have to pay for each pack? {ц} − {с} = {п_} dollars."
