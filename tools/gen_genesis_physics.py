...
def приставочные():
    ...
    # …и ТОТ ЖЕ ДОВОД ПРИЛОЖЕН ВТОРОЙ РАЗ (08.09). Вылечив сантиметр, ряд оставил в том же
    # долгу МИЛЛИМЕТР: он был мельчайшей объявленной мерой длины и стоял лишь меньшей
    # стороной. Дому мер приписана четвёртая приставка (микро), и звено цепи продлилось;
    # строка того же рода, тот же вопрос, новых родов не заводится.
    for большая, меньшая in (("centimetre", "millimetre"),
                             ("millimetre", "micrometre"),
                             ("litre", "millilitre"),
                             # ЛИТР ДОЛЖЕН СТОЯТЬ И МЕНЬШЕЙ СТОРОНОЙ: показав «1 litre = 1000
                             # millilitres», я показал единственное — а множественное осталось
                             # тем же долгом, каким было. Мельчайшая мера цепи не бывает при
                             # единице, крупнейшая — при многих; литру нужны обе стороны.
                             ("kilolitre", "litre")):
        сколько = int(units.отношение(большая, меньшая))
        пары.append((f"1 {units.англ(большая, False, ПИСЬМО)} = "
                     f"{сколько} {units.англ(меньшая, True, ПИСЬМО)}.",
...
def pass_shows(pass_i):
    ...
    for i in range(ШИРИНА):
        ...
        out.род = "скорость из пути и времени"
        out.append(ск_en)
        out.append(ск_ru)
        ...
        out.append(спросить("law", закон_ск, закон="speed = distance / time",
                            x=s, y=t))
        out.род = "сила из массы и ускорения"
        ...
        out.append(спросить("закон", раб_ru, закон="работа = сила × путь",
                            x=f, y=d))
        out.род = "мощность из работы и времени"
        ...
        out.род = "плотность из массы и объёма"
        ...
        out.append(f"плотность = масса / объём; "
                   ...
                   f"на кубометр.")
        out.род = "напряжение из тока и сопротивления"
        ...
        out.род = "кинетическая энергия из массы и скорости"
        ...
        out.род = "делится ли путь на время нацело"
...
# ПУТЬ, СКАЗАННЫЙ ТОЛЬКО В ЗОВЕ, ЕСТЬ ПУТЬ, О КОТОРОМ НЕ ОБЪЯВЛЕНО: указатель читает
# объявление строкой верхнего уровня, а не разбирает вызов.
ЦЕЛЬ = "datasets/genesis_physics.txt"

# РОД ЗДЕСЬ — ЗАКОН, ПО КОТОРОМУ СЧИТАНО ЧИСЛО, А НЕ ПОВЕРХНОСТЬ РЕЧИ. Утверждение,
# вопрос о нём и запись самого закона суть ТРИ ЛИЦА ОДНОГО ДЕЛА: делить путь на время.
# Свести их в род «вопрос» значило бы собрать великана по форме речи — тот самый, кого
# корпус уже разбирал в разметке 13.09.
РОДЫ = (
    "скорость из пути и времени",
    "сила из массы и ускорения",
    "работа из силы и пути",
    "мощность из работы и времени",
    "плотность из массы и объёма",
    "напряжение из тока и сопротивления",
    "кинетическая энергия из массы и скорости",
    "приставка при единице",
    "единица величины",
    "чем измеряется величина",
    "делится ли путь на время нацело",
)

ЗАЧЕМ_РОДА = {
    "скорость из пути и времени": "v = s / t — сказано, спрошено и записано законом",
    "сила из массы и ускорения": "F = m × a, и оба множителя названы своими единицами",
    "работа из силы и пути": "A = F × s — закон, стоящий на предыдущем",
    "мощность из работы и времени": "N = A / t: работа, отнесённая ко времени",
    "плотность из массы и объёма": "ρ = m / V — частное двух величин разных родов",
    "напряжение из тока и сопротивления": "U = I × R, закон Ома в прямую сторону",
    "кинетическая энергия из массы и скорости": "E = m v² / 2 — квадрат в законе",
    "приставка при единице": "кило-, милли- и прочие: приставка множит единицу",
    "единица величины": "имя единицы и то, чего она единица",
    "чем измеряется величина": "величина названа и названа её единица",
    "делится ли путь на время нацело": "целая скорость есть не всегда, и «нет» сказано "
