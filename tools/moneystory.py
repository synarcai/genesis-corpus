...
СЕМЕЙСТВА = ("акт", "покупка", "имеет", "ставка", "покупка1", "покупка2")
...
def страница(язык, сем, имя, держ=None, k=0, A=None, B=None, глагол=None, вопрос="теперь", вещь=0, п_C=None, п_n1=None, п_n2=None, вещь2=0):
...
def _дыры(язык):
    ...
    return {"X": "(" + _альт([л[0] for л in лица]) + ")", "Xр": "(" + _альт([л[2] for л in лица]) + ")",
            ...
            "Т1": "(" + _альт(товары) + ")", "Т2": "(" + _альт(товары) + ")", "P1": "(" + _СУММА[язык] + ")", "P2": "(" + _СУММА[язык] + ")",
            "л": r"((?:\d+ [×+] \d+ = \d+(?:, )?)+)"}
