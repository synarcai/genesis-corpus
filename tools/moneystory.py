...
ЯЗЫКИ = {
    ...
    "ru": dict(
        держит={"у": "у {X_р} {A}.", "имеет": "{X} имеет {A}."},
        ...
        теперь="сколько денег {у_него} теперь?", осталось="сколько денег осталось?", имеет="сколько денег у {X_р}?",
        леджер="{a} {S} {b} = {r} {м}.", мост="{r} {м} — это {R}.",
        он={"m": "он", "f": "она"}, у_него={"m": "у него", "f": "у неё"}, вещи=("ручку", "книгу", "яблоко", "карандаш", "чашку"),
        ставка="у {X_р} {A}. {он} зарабатывает {B} в день. {Ц} стоит {C}. сколько дней {ему} нужно?",
        ему={"m": "ему", "f": "ей"},
    ...
    "de": dict(
        ...
        леджер="{a} {S} {b} = {r} {м}.", мост="{r} {м} sind {R}.",
...
# МЕЛКАЯ МЕРА НА ТРЁХ ЯЗЫКАХ ОДНИМ ЗАКОНОМ (08.09). Русская сторона держала форму законом
# («копейка / копейки / копеек» по счёту), а английская и немецкая — ЛИТЕРАЛОМ в образце; и
# правота английской держалась не законом, а тем, что дом не пишет одноцентовых остатков.
# Немецкая мера при числе не меняется — и это ОБЪЯВЛЕНО здесь, а не подразумевается молчанием.
#
#     ДОМ, ПИШУЩИЙ ОДНУ СТОРОНУ ЗАКОНОМ, А ДРУГУЮ БУКВОЙ, НЕ ЗАБЫЛ ЗАКОН — ОН ЕГО НЕ ПОЗВАЛ.
#
# Проверять форму особо не нужно: суд этого дома ПЕРЕПИСЫВАЕТ страницу из разобранных чисел и
# сличает с исходной, — и всякая несогласованная форма расходится сама.
def _мелкая(язык, n):
    """Слово мелкой меры при числе n на языке дома."""
    if язык == "ru":
        return _мелкая_ru(n)
    if язык == "en":
        return by_count(n, "cents")
    return "Cent"          # немецкая мера при числе неизменна
...
def страница(язык, сем, имя, держ=None, k=0, A=None, B=None, глагол=None, вопрос="теперь", вещь=0, п_C=None,
    ...
    X, пол, X_р = лицо
    он = я["он"][пол]
    з = dict(X=X, X_р=X_р, он=он, у_него=я.get("у_него", {}).get(пол, ""))
    if сем == "ставка":
...
def _дыры(язык):
    ...
    return {"X": "(" + _альт([л[0] for л in лица]) + ")", "X_р": "(" + _альт([л[2] for л in лица]) + ")",
...
def разобрать(язык, строка):
    ...
    for образец, имена, сем, держ, вопрос in ОБРАЗЦЫ[язык]:
        ...
        for имя, г in zip(имена, м.groups()):
            ...
            з[имя] = г
        if "X_р" in з and "X" not in з:
            з["X"] = next((л[0] for л in ЛИЦА[язык] if л[2] == з["X_р"]), None)
        if з.get("X") is None:
            return None
        if сем == "ставка":
