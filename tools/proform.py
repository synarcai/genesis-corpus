...
def _вердикт(язык, форма, i, зн):
    ...
    if зн["M"] != проформа(язык, i, m):
        return False
    # «СКОЛЬКО» СПРАШИВАЕТСЯ У ТОГО, КТО ЕГО СКЛОНЯЕТ, И ТОЛЬКО У НЕГО (21.09). Испанский,
    # итальянский и португальский гнут вопросительное слово по роду опущенного имени
    # («cuántos» / «cuántas»), и рамка их несёт дыру `{сколько}`. Французский не гнёт:
    # «combien» одно для обоих родов — и стои́т в рамке ЛИТЕРАЛОМ, а не дырой. Проверка же
    # спрашивала дыру у всех четырёх и у француза получала пустоту, отчего тридцать две
    # честные страницы звались ложью.
    #
    #     ПРОВЕРКА, ТРЕБУЮЩАЯ ТОГО, ЧЕГО РАМКА НЕ ДАЛА, ОТВЕРГАЕТ ЧЕСТНУЮ СТРАНИЦУ — И
    #     ОТВЕРГАЕТ ТЕМ ТИШЕ, ЧЕМ НАДЁЖНЕЕ НАБОР ЕЁ ПРИКРЫВАЕТ: покуда выход по набору
    #     стоял впереди закона, эти тридцать две проходили, и никто не знал, что закон их
    #     не признаёт.
    #
    # Ослабления нет: у гнущих языков дыра в образце есть, и строка без верного слова не
    # совпадёт с образцом вовсе; у француза же слово литерально, и подмена его ломает
    # совпадение тем же образцом.
    сказано = зн.get("сколько")
    if сказано is not None and сказано != СКОЛЬКО[язык][_род(язык, i)]:
        return False
    if форма == "два_шага":
        k = _число(зн["K"])
        if k >= n:
...
def судить(строка):
    ...
    с = строка.strip()
    # ВЫХОД ПО НАБОРУ СНЯТ (21.09) — он и прикрывал тридцать две страницы, которые
    # закон звал ложью (см. `_вердикт`). НАБОР ГОВОРИТ, ЧТО ДОМ СТРАНИЦУ НАПИСАЛ, А
    # НЕ ЧТО ОНА ВЕРНА. Замер: закон судит все 288 показов верно и без набора.
