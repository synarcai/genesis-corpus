...
# ≤ 9 — разность не уходит ниже нуля.
ЛЕВЫЕ = [12, 15, 20, 11, 14, 10, 18]
ПРАВЫЕ = [(3, "*"), (4, "+"), (5, "-"), (6, "*"), (3, "-"), (7, "+"),
          (4, "*"), (2, "-"), (9, "+"), (8, "-"), (5, "*")]
ЛЕВЫЕ_УСЛОВИЯ = [3, 7, 4, 9, 1, 6, 2, 10, 5]
ПРАВЫЕ_УСЛОВИЯ = [(5, "<"), (2, ">"), (4, "<"), (8, ">"), (6, ">"), (9, "<"),
                  (3, ">")]
НИЖНИЕ = [1, 2, 3]
ДЛИНЫ = [3, 4, 2, 5, 6, 1, 7]
ФУНКЦИИ = [("f", "n * 2"), ("g", "n + 7"), ("h", "n * n"), ("p", "n - 3"),
           ("q", "n * 3"), ("r", "n + n"), ("s", "n * 5"), ("t", "n - 1")]
АРГУМЕНТЫ = [5, 4, 6, 9, 7, 8, 3, 12, 10, 11, 13]
ФАКТОРИАЛ = list(range(3, 13))
ШИРИНА = 8


# РОДЫ ЭТОГО ДОМА БЫЛИ НАЗВАНЫ КОММЕНТАРИЯМИ «# ---» НАД КАЖДОЙ ВЕТВЬЮ И НЕ ВЫШЛИ НАРУЖУ.
# Вопросная поверхность идёт В РОД СВОЕГО ДЕЛА, а не в род «вопрос»: «чему равна сумма от
# 3 до 7» есть та же сумма, спрошенная. Накопление разделено на два: сложение и умножение
# по отрезку — РАЗНЫЕ действия, и дом показывает их порознь, а не одной строкой.
СВЯЗЫВАНИЕ = "связывание: имя значению и счёт над именами"
ВЕТВЛЕНИЕ = "ветвление: невзятая ветвь ничего не меняет"
СУММА = "ограниченный цикл: сумма по отрезку"
ПРОИЗВЕДЕНИЕ = "ограниченный цикл: произведение по отрезку"
ФУНКЦИЯ = "функция как имя вычисления"
РЕКУРСИЯ = "рекурсия, всегда со своим основанием"
ПУСТОЙ_ОТРЕЗОК = "пустой отрезок: цикл, не делающий ни шага"
...
def pass_shows(pass_i):
    ...
    for i in range(ШИРИНА):
        ...
        a, (b, op) = mass.пара(k0, ЛЕВЫЕ, ПРАВЫЕ)
        x, (y, знак) = mass.пара(k0, ЛЕВЫЕ_УСЛОВИЯ, ПРАВЫЕ_УСЛОВИЯ)
        lo, длина = mass.пара(k0, НИЖНИЕ, ДЛИНЫ)
        hi = lo + длина
        имя, тело = ФУНКЦИИ[k0 % len(ФУНКЦИИ)]
        арг = АРГУМЕНТЫ[k0 % len(АРГУМЕНТЫ)]
        n = ФАКТОРИАЛ[k0 % len(ФАКТОРИАЛ)]
        ...
        # --- ВОПРОСНАЯ ПОВЕРХНОСТЬ: обе половины строит одна величина.
        # Вопрос не пишется рядом с ответом, а ПОРОЖДАЕТСЯ из него, и
        # закон пары (`tools/asking.py`) требует, чтобы числа вопроса
        # были начальным отрезком чисел ответа: порча любой величины
        # вопроса рвёт пару, и суд зовёт её ложью.
        out.род = СУММА
        out.append(f"what is the sum for i from {lo} to {hi}? sum for i "
                   f"from {lo} to {hi} is {сумма}.")
        out.append(f"чему равна сумма для i от {lo} до {hi}? сумма для i "
                   f"от {lo} до {hi} равна {сумма}.")
        out.род = ПРОИЗВЕДЕНИЕ
        out.append(f"what is the product for i from {lo} to {hi}? product "
                   f"for i from {lo} to {hi} is {произв}.")
        out.append(f"чему равно произведение для i от {lo} до {hi}? "
                   f"произведение для i от {lo} до {hi} равно {произв}.")
        out.род = ФУНКЦИЯ
        out.append(f"if {имя}(n) = {тело}, what is {имя}({арг})? function "
                   f"{имя}(n) = {тело}; {имя}({арг}) = {итог}.")
        out.append(f"если {имя}(n) = {тело}, чему равно {имя}({арг})? "
                   f"функция {имя}(n) = {тело}; {имя}({арг}) = {итог}.")
        out.род = ВЕТВЛЕНИЕ
        out.append(f"if {x} {знак} {y}, what is y? if {x} {знак} {y} then "
                   f"y = 1 else y = 2; y = {1 if верно else 2}.")
        # --- ОТКАЗ С ОСНОВАНИЕМ: цикл, не делающий ни шага. Слой
        # показывал только идущие циклы и учил, что цикл идёт всегда;
        # пустой отрезок есть первое, обо что спотыкается пишущий
        # программы. Границы взяты СВОИ же, переставленные местами.
        out.род = ПУСТОЙ_ОТРЕЗОК
        out.append(f"for i from {hi} to {lo} there is no step at all: "
                   f"{hi} is already greater than {lo}.")
        out.append(f"для i от {hi} до {lo} нет ни одного шага: {hi} уже "
                   f"больше {lo}.")
    return out
...
РОДЫ = (СВЯЗЫВАНИЕ, ВЕТВЛЕНИЕ, СУММА, ПРОИЗВЕДЕНИЕ, ФУНКЦИЯ, РЕКУРСИЯ, ПУСТОЙ_ОТРЕЗОК)

ЗАЧЕМ_РОДА = {
    СВЯЗЫВАНИЕ: "let a = …; let b = …; c = a ⊙ b — имя получает значение, и над именами "
                "считают",
    ВЕТВЛЕНИЕ: "обе ветви названы, а взята одна, и невзятая не выглядит взятой",
    СУММА: "накопление сложением по отрезку — сказано и спрошено",
    ПРОИЗВЕДЕНИЕ: "накопление умножением по тому же отрезку: другое действие, другой род",
    ФУНКЦИЯ: "имя вычисления и его применение к доводу",
    РЕКУРСИЯ: "factorial(n) = n × factorial(n − 1) ВСЕГДА со своим основанием: "
              "рекурсия без основания есть зависание, а не программа",
    ПУСТОЙ_ОТРЕЗОК: "цикл от 7 до 3 не делает ни шага, и причина названа числами",
}
