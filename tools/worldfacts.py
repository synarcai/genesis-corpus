...
def судить(строка):
    """(судимо, истинно): показ дома, или ложь. Мир ЗАМКНУТ — и замкнут ПО ЦИФРАМ ТОЖЕ.
    ...
    if not any(о.match(с) for о in ОБРАЗЦЫ):
        return (True, False) if closedworld.ложь_по_цифре(с, _СКЕЛЕТЫ, ПОКАЗЫ) else (False, False)
