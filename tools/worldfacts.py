...
КЛАССЫ = {
    "ru": (("паук", "животное"), ("дуб", "дерево"), ("железо", "металл"),
           ("молоко", "жидкость"), ("солнце", "звезда"), ("роза", "цветок")),
    "en": (("a spider", "an animal"), ("an oak", "a tree"), ("iron", "a metal"),
           ("milk", "a liquid"), ("the sun", "a star"), ("a rose", "a flower")),
    "de": (("eine Spinne", "ein Tier"), ("eine Eiche", "ein Baum"), ("Eisen", "ein Metall"),
           ("Milch", "eine Flüssigkeit"), ("die Sonne", "ein Stern"), ("eine Rose", "eine Blume")),
    "fr": (("une araignée", "un animal"), ("un chêne", "un arbre"), ("le fer", "un métal"),
           ("le lait", "un liquide"), ("le soleil", "une étoile"), ("une rose", "une fleur")),
    "es": (("una araña", "un animal"), ("un roble", "un árbol"), ("el hierro", "un metal"),
           ("la leche", "un líquido"), ("el sol", "una estrella"), ("una rosa", "una flor")),
}
...
ЯЗЫКИ = tuple(ФАКТЫ)
ФОРМЫ = ("факт", "основание", "следствие", "принадлежность", "правда", "согласен", "согласен_что", "следствие2")

for _яз in ЯЗЫКИ:
    assert set(РАМКИ[_яз]) == set(ФОРМЫ) - {"факт"}, _яз
...
def страница(язык, форма, i):
    ...
    р = РАМКИ[язык]
    if форма == "принадлежность":
        ч, к = КЛАССЫ[язык][i % len(КЛАССЫ[язык])]
        return р["принадлежность"].format(ч=ч, к=к)
    ряд = ФАКТЫ[язык]
...
def _образцы():
    ...
    for язык in ЯЗЫКИ:
        ...
        for форма in ФОРМЫ:
            рамка = "{ф}." if форма == "факт" else РАМКИ[язык][форма]
