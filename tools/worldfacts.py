...
# СИЛЛОГИЗМ ВОПРОСОМ «ДА/НЕТ» (девятая полоса, 06.09): «если всякая рыба — животное, а акула —
...
ЯЗЫКИ = tuple(ФАКТЫ)
ФОРМЫ = ("основание", "следствие", "принадлежность", "правда", "согласен", "согласен_что", "согласен_что_вы", "следствие2", "принадлежность_вопрос", "силлогизм", "почему_прямо", "общего", "лишнее", "силлогизм_да", "силлогизм_все", "силлогизм_имя")

# ДОЛГ ПЕРЕКОСА НАЗВАН, А НЕ ОБЪЯВЛЕН ПРИРОДОЙ (15.09, прибор `scripts/house_skew.py` держит
# дом 72-кратным: «лишнее» 1296 страниц против «силлогизм_имя» 18).
#
# ОБЕ КРАЙНОСТИ СУТЬ ВЫБОР ДОМА, А НЕ ФАКТ О ПРЕДМЕТЕ, и потому освобождать их нельзя:
#
#     «лишнее» перебирает ВСЕ 144 тройки на язык — два члена одного класса и один чужой. Это
#     законное упражнение, но материала в нём вчетверо больше, чем у соседей: довольно было бы
#     трёх троек на пару классов, а не всех.
#
#     «силлогизм_имя» держит ДВУХ питомцев на язык (Мурка и Шарик), и восемнадцать страниц
#     суть девять языков на две клички. Третий питомец есть в мире, и его отсутствие — моя
#     недоделка, а не природа силлогизма.
#
# ПОЧИНКА ОТЛОЖЕНА И НАЗВАНА: обе правки суть КОВКА НА ДЕВЯТИ ЯЗЫКАХ — тройки лишнего надо
# сузить во всех девяти разом, питомцев дописать во всех девяти разом, и польскому сверх того
# нужен творительный падеж клички. Спешить тут значило бы чинить перекос родов, наживая
# перекос языков.
#
#     РОД, БЕДНЫЙ ИЛИ БОГАТЫЙ ОТ ВЫБОРА ДОМА, НЕ ЕСТЬ ФАКТ О ПРЕДМЕТЕ. Долг называют в доме и
#     платят ковкой; объявить его природой значило бы солгать о себе.
...
def _страница_сырая(язык, форма, i):
...
def страница(язык, форма, i):
    ...
    """
    готовая = _страница_сырая(язык, форма, i)
    ...
    for язык in ЯЗЫКИ:
        for форма in ФОРМЫ:
            ...
            if форма == "лишнее":
                ...
                continue
            if форма == "силлогизм_имя":
                for i in range(len(ПИТОМЦЫ[язык])):
                    вон[страница(язык, форма, i)] = (язык, форма)
                continue
    ...
    for язык in ЯЗЫКИ:
        ...
        for форма in ФОРМЫ:
            ...
            for м in re.finditer(r"\{(\w+)\}", рамка):
                # ОБРАЗЕЦ ЗНАЕТ ОБЕ ФОРМЫ СЛУЖЕБНОГО СЛОВА (12.09). Рамка несёт «que» целым,
                # а страница после элизии — «qu'»; образец, построенный из сырой рамки, своей
                # же страницы не узнавал, и мир, ЗАМКНУТЫЙ на себя, звал её ложью.
                #
                #     СТРАНИЦА, ПРОШЕДШАЯ ПРЕОБРАЗОВАНИЕ ПОСЛЕ ПОДСТАНОВКИ, ТРЕБУЕТ ОБРАЗЦА,
                #     ЗНАЮЩЕГО ЭТО ПРЕОБРАЗОВАНИЕ, — ИНАЧЕ ДОМ ОТВЕРГАЕТ СОБСТВЕННЫЙ ПОКАЗ.
                #
                # Закон обеих форм живёт в доме языка (`frgram.в_образце`), а не здесь.
                куски.append(_fr.в_образце(рамка[конец:м.start()]) if язык == "fr"
                             else re.escape(рамка[конец:м.start()]))
                ...
                конец = м.end()
            if годно:
                куски.append(_fr.в_образце(рамка[конец:]) if язык == "fr"
                             else re.escape(рамка[конец:]))
...
# РЯДЫ ДОМА — ЧАСТИ, СТОЯЩИЕ В ЕГО РАМКАХ НА ОДНОМ МЕСТЕ (21.09): целые половины (явление,
# причина, следствие) и ПРЕДМЕТЫ внутри них — первое слово каждой половины.
#
# ПОДМЕНА БЫВАЕТ ЦЕЛОЙ ПОЛОВИНОЙ И БЫВАЕТ СЛОВОМ ВНУТРИ НЕЁ, И ЛОВЯТСЯ ОНИ РАЗНЫМИ РЯДАМИ.
# «потому что камень тяжелее воды» вместо «потому что лёд легче воды» есть подмена
# половины; «потому что камень легче воды» — подмена одного слова, и такой причины дом не
# объявлял вовсе, отчего рамка его строку не узнавала и суд МОЛЧАЛ.
#
#     СУД, МОЛЧАЩИЙ НА ПОДМЕНЕ ОДНОГО СЛОВА В СВОЕЙ ЖЕ СТРАНИЦЕ, ОТДАЁТ ЕЁ ВОРОТАМ — а
#     ворот порча, внесённая в свод после ковки, уже не встретит.
def _ряды():
    вон = []
    for язык in ЯЗЫКИ:
        for таблица, ширина in ((ФАКТЫ.get(язык) or (), 4), (ИЕРАРХИЯ.get(язык) or (), 3)):
            for i in range(ширина):
                клетки = tuple({т[i] for т in таблица
                                if isinstance(т, (tuple, list)) and len(т) > i
                                and isinstance(т[i], str) and т[i]})
                вон.append(клетки)
                # РЯД СЛОВ КОЛОНКИ — ВСЕ ЕЁ СЛОВА, А НЕ ПЕРВЫЕ (21.09). Ряд из первых слов
                # ловил подмену предмета («лёд» → «камень») и молчал на подмене глагола
                # («плавает» → «тонет»): место порчи не выбирают.
                #
                # Ложных срабатываний это не даёт по устройству закона: он требует, чтобы
                # замена дала В ТОЧНОСТИ объявленный показ, — а случайное слово такого не
                # даёт.
                вон.append(tuple({с for к in клетки for с in к.split() if len(с) > 2}))
...
# ДОМ ОБЪЯВЛЯЕТ СЕБЯ СУДЯЩИМ ОБЪЯВЛЕНИЕМ: «лёд плавает в воде» неоткуда вывести, кроме как
# из объявления, и набор показов ЕСТЬ это объявление. Объявление проверяется прибором и
# принимается лишь от СТРАЖА — от дома, который ловит свою порчу без всякого вписывания.
СУДИТ_ОБЪЯВЛЕНИЕМ = True


def судить(строка):
    """(судимо, истинно): показ дома, or ложь. Мир ЗАМКНУТ — и замкнут ПО ЦИФРАМ ТОЖЕ.

    Порча одного числа («вода кипит при 100 градусах» → «при 101 градусах») выводила строку
    из объявленного, образцы её не узнавали, и суд МОЛЧАЛ вместо приговора (прибор мутантов
    05.09). Строка, отличающаяся от показа лишь цифрами, есть строка этого дома и его ложь.
    """
    с = строка.strip()
    if not с:
        return False, False
    if not any(о.match(с) for о in ОБРАЗЦЫ):
        if closedworld.ложь_по_цифре(с, _СКЕЛЕТЫ, ПОКАЗЫ):
            return True, False
        # ПЯТЫЙ БРАТ ЗАМКНУТОГО МИРА: строка, становящаяся показом при замене ОДНОГО члена
        # объявленного ряда, есть показ ЭТОГО дома с испорченной частью — и она ложна, а не
        # чужая. Так ловится подмена слова ВНУТРИ половины, какой рамка не узнаёт.
        return (True, False) if closedworld.ложь_по_ряду(с, ПОКАЗЫ, РЯДЫ) else (False, False)
    # СТРОКА СВОЕЙ РАМКИ, НЕ СТОЯЩАЯ В ОБЪЯВЛЕНИИ, ЕСТЬ ЛОЖЬ: образец выше уже признал её
    # страницей этого дома, а объявление её не содержит. Так ловится и склейка честных
    # половин из РАЗНЫХ строк — «лёд плавает в воде. почему? потому что камень тяжелее
    # воды»: обе половины дом писал, вместе они ложь.
    if с in ПОКАЗЫ:
        return True, True
    return True, False
