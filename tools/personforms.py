...
# родов). Четыре рода этого дома суть один и тот же счёт под разными ЛИЧНОСТЯМИ отвечающего:
# краткая, подробная, осторожная — и числа при них материал. Род «происхождение» (108 страниц
# на 108 очертаний) НЕ объявлен: там всякая страница говорит СВОЁ о том, откуда взялась
# черта, и чисел в ней нет.
#
#     РОД РОВЕН САМ СОБОЮ, КОГДА ЕГО МАТЕРИАЛ МНОГОМЕРЕН; ОБЪЯВЛЕНИЕ НУЖНО ЛИШЬ
#     ТАМ, ГДЕ МЕРНОСТЬ ОДНА: ЧИСЛА ПРИ НЕИЗМЕННОЙ ЗАПИСИ.
ОЧЕРТАНИЕ_ПО_ПРИРОДЕ = ("кратко",
                        "подробно",
                        "два_ответа",
                        "выбор_при_равных",)
...
def страница(язык, форма, Т, n, m, М=None, черта=None, источник=None, выбор=None):
    ...
    return _fr.элизия(рамка(язык, форма, М=М).format(**поля)) if язык == "fr" else рамка(язык, форма, М=М).format(**поля)
...
def _судить_образцом(строка):
    ...
    #
    # Замерено: закон судит все 3240 показов дома верно и без набора — выход был
