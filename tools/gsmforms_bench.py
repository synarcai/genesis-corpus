...
def свободен(текст):
    ...
    if len(вопрос) < 20:
        return True
    числа = set(__import__("re").findall(r"\d+", низ))
    for свод in _ТЕКСТ:
        if вопрос not in свод:
            continue
        for строка in свод.split("\n"):
            if вопрос in строка and числа and числа <= set(__import__("re").findall(r"\d+", строка)):
                return False
    return True
    ...
    for имя, п_ in семьи.items():
        for k in range(СЕМЯН):
            ...
            for сдвиг_семени in range(0, 400):
                ...
                # СВОБОДЕН ОБЯЗАН БЫТЬ КАЖДЫЙ ИЗ ТРЁХ ТАКТОВ СЕМЕНИ, А НЕ ОДИН (07.09,
                # второй заход). Семя рождает три такта: value, refuse (факт удалён) и
                # perturb (число сдвинуто), и у каждого СВОЙ набор чисел. Затвор,
                # проверявший лишь value, оставил течь в refuse: удаление факта
                # уменьшает набор чисел, и вопрос совпадает со сводом там, где полный
                # такт не совпадал.
                #
                #     ЗАТВОР, ПРОВЕРЯЮЩИЙ ОДИН ТАКТ ИЗ ТРЁХ, ЗАКРЫТ НА ТРЕТЬ. Свободно
                #     то семя, все три такта которого свободны.
                _ф, _в, _пр, _д, _н = факты_и_вопрос(имя, п)
                _имя_ф = next((n for n, _ in _ф if n in _д), None)
                _без = (". ".join(т_ for n, т_ in _ф if n != _имя_ф) + ". " + _в) if _имя_ф else текст
                if свободен(текст) and свободен(_без):
                    break
            else:
                п = п_(шаг, i)
                факты, вопрос, программа, данные, нужды = факты_и_вопрос(имя, п)
