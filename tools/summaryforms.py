...
def страница_фактов(язык, форма, Т, P, выкинуто, сдвиг, спрошено=None, числа=None):
    """С «числа» страница строится на ЧУЖИХ числах — так удержанный ключ спрашивает дом о
    том, чего дом не показывал, и меряет форму, а не память."""
    числа = tuple(числа)[:P] if числа else _числа(P, сдвиг)
    поля = {}
    for i in range(P):
        поля["n%d" % i] = числа[i]
...
def судить(строка):
    ...
    #
    # Замерено: закон судит все 6102 показов дома верно и без набора — выход был
