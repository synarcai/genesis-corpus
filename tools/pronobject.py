...
def судить(строка):
    ...
    с = строка.strip()
    # ВЫХОД ПО НАБОРУ СНЯТ (21.09) — он прикрывал 544 из 864 страниц, которые закон
    # звал ложью, судя их мерой чужого случая (см. обход ниже). НАБОР ГОВОРИТ, ЧТО ДОМ
    # СТРАНИЦУ НАПИСАЛ, А НЕ ЧТО ОНА ВЕРНА. Замер: закон судит все 864 верно.
    ...
    for образ, язык, форма, i, лицо in ОБРАЗЦЫ:
        ...
        if зн is None:
            continue
        if _вердикт(язык, форма, i, лицо, зн):
            return True, True
