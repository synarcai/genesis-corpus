...
ПРЕДИКАТЫ = {
    ...
    # ДВЕ ПЕРЕМЕННЫЕ: граница считается ПО ВСЕМУ КВАДРАТУ ПАР (32, 03.09).
    # Первая редакция брала пару по КОРНЮ из границы — дёшево, но притязание
    # при этом говорило «нет контрпримера среди 1..1000», а проверено было
    # 1..31: ФОРМА СКАЗАЛА БОЛЬШЕ, ЧЕМ ПРИБОР ПРОВЕРИЛ, и это ложь в нашем
    # смысле. Миллион пар считается один раз за прогон и запоминается — цена
    # честности здесь есть доля секунды.
    "_общ_перестановка": lambda N: all(
        a + b == b + a for a in range(1, N + 1) for b in range(1, N + 1)),
    "_общ_две_чётных": lambda N: all(
        (a + b) % 2 == 0 for a in range(2, N + 1, 2)
...
def с_вопросами_меченые(пары, кандидаты):
    """То же, но при КАЖДОЙ строке стои́т её язык, взятый у строителя (15.09).

    ВОПРОС ГОВОРИТ НА ЯЗЫКЕ СВОЕГО УТВЕРЖДЕНИЯ, и метка его не требует нового разбора:
    вопрос выведен из утверждения одним законом дома и той же таблицей языка.
    """
    вон = []
    for с, я in пары:
        вон.append((с, я))
        вон.append((вопрос(с, кандидаты), я))
    return вон
