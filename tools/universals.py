...
ПРЕДИКАТЫ = {
    ...
    # ДВЕ ПЕРЕМЕННЫЕ: граница считается ПО ПАРАМ, а не по числам (заказ 32
    # 03.09: «поиск контрпримера идёт по произведению»). Квадрат границы дорог,
    # и потому пара берётся по корню из неё — этого довольно, чтобы притязание
    # проверялось делом, а не словом; число проверенных пар названо в имени.
    "_общ_перестановка": lambda N: all(
        a + b == b + a for a in range(1, int(N ** 0.5) + 1)
        for b in range(1, int(N ** 0.5) + 1)),
    "_общ_две_чётных": lambda N: all(
        (a + b) % 2 == 0 for a in range(2, int(N ** 0.5) + 1, 2)
