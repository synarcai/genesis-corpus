...
def _сборка_вторым_местом(язык, день, имя, гл, слот, место):
    """ТОТ ЖЕ ФАКТ, ГДЕ СРОК СТОИ́Т НА ДРУГОМ МЕСТЕ (08.09, просьба holon по переписи Р-1).

        КЛАСС, СТОЯЩИЙ ВСЕГДА НА ОДНОМ МЕСТЕ, ПОКУПАЕТСЯ КАК ПОЗИЦИЯ, А НЕ КАК КЛАСС.
        Срок открывал КАЖДУЮ страницу этого дома (164 из 164), и рынок рамок читал его не
        как срок, а как «то, что стои́т первым».

    Второе место выбрано ПО ЯЗЫКУ, а не одно на всех: у большинства это конец фразы
    («Ann put 5 cups on the shelf on monday»), у немецкого — СЕРЕДИНА, ибо немецкий ставит
    время перед местом («Anna stellte am Montag 5 Tassen ins Regal»), и конец фразы там
    занят предложным оборотом. Турецкий пропущен: его срок и так стои́т не первым, а глагол
    держит конец.

        ВТОРОЕ МЕСТО НЕ ВЫВОДИТСЯ ИЗ ПЕРВОГО ПЕРЕСТАНОВКОЙ. Порядок слов объявляется —
        это тот же закон, что у условия в доме плана.
    """
    if язык == "tr":
        return None
    if язык in ("de", "nl"):
        return f"{имя} {гл} {день} {слот} {место}."
    return f"{имя} {гл} {слот} {место} {день}."
...
def факт_вторым_местом(язык, день, имя, род, k, n, вещь, место):
    """Тот же факт, где срок стои́т НЕ ПЕРВЫМ; None — язык второго места не имеет."""
    р = РАМКИ[язык][k]
    return _сборка_вторым_местом(язык, день, имя, _глагол(язык, р, род),
                                 f"{n} {_вещь(язык, вещь, n)}", место)
...
_ЛАТ_СЛОВО = "[^\\W\\d_]+"
ФАКТ = {
    # имя лица с заглавной — как в русском образце ниже (пакет объявляет письмо, 05.09)
    "en": re.compile(rf"^(?P<день>{_alt(ДНИ['en'])}) (?P<имя>[A-Z][a-z]+) (?P<гл>{_alt(_глаголы('en'))}) (?P<n>\d+) "
...
# ОБРАЗЕЦ ВТОРОГО МЕСТА СРОКА — по одному на язык, и порядок в нём ОБЪЯВЛЕН, а не выведен
# перестановкой первого (08.09). У большинства языков срок уходит в конец фразы, у немецкого и
# нидерландского — в середину, перед количеством; турецкий второго места не имеет.
#
#     СУД, ЧИТАЮЩИЙ ОДИН ПОРЯДОК, ЗОВЁТ ЛОЖЬЮ ВТОРОЙ. Мир этого дома ЗАМКНУТ: строка, которой
#     суд не признал, есть ложь, а не молчание, — и потому всякое новое место срока обязано
#     войти В СУД ОДНОВРЕМЕННО С ДОМОМ, а не следом.
ФАКТ_2 = {}
for _я in ("en", "ru", "es", "it", "fr", "pt", "pl"):
    # ИМЯ ПЕРЕМЕННОЙ НЕ ЗАНИМАЕТ ЧУЖОГО МЕСТА: «_вещь» в этом доме есть ФУНКЦИЯ, и первая
    # правка её затёрла строкой — дом упал на «'str' object is not callable». Та же ошибка,
    # что с «h» в доме записи числа, и ловится она так же — первым запуском.
    _имя_обр = "[A-Z][a-z]+" if _я == "en" else ("[А-ЯЁ][а-яё]+" if _я == "ru" else _ЛАТ)
    _вещь_обр = "[a-z]+" if _я == "en" else ("[а-яё]+" if _я == "ru" else _ЛАТ_СЛОВО)
    ФАКТ_2[_я] = re.compile(
        rf"^(?P<имя>{_имя_обр}) (?P<гл>{_alt(_глаголы(_я))}) (?P<n>\d+) "
        rf"(?P<вещь>{_вещь_обр}) (?P<место>{_alt(м for р in РАМКИ[_я] for м in р['места'])}) "
        rf"(?P<день>{_alt(ДНИ[_я])})\.$")
for _я in ("de", "nl"):
    _вещь_обр2 = _ЛАТ if _я == "de" else _ЛАТ_СЛОВО
    ФАКТ_2[_я] = re.compile(
        rf"^(?P<имя>{_ЛАТ}) (?P<гл>{_alt(_глаголы(_я))}) (?P<день>{_alt(ДНИ[_я])}) (?P<n>\d+) "
        rf"(?P<вещь>{_вещь_обр2}) (?P<место>{_alt(м for р in РАМКИ[_я] for м in р['места'])})\.$")
...
def разобрать(строка):
    ...
    if not м:
        return None
    # ОБА МЕСТА СРОКА ЧИТАЮТСЯ ОДНИМ РАЗБОРОМ: сперва первое, затем второе. Порядок проб
    # безразличен — образцы не пересекаются, ибо срок в них стои́т в разных местах.
    for язык, образец in list(ФАКТ.items()) + list(ФАКТ_2.items()):
        ф = образец.match(м["факт"])
        if not ф:
            continue
        n = int(ф["n"])
...
# СЛОВАРЬ СТРАНИЦ ДОМА ДЫР (13.09). Прежде дом кузнице только ОТВЕЧАЛ: она держала перебор
# ролей у себя, а дом не знал ни одной своей страницы. Оттого шесть с половиной тысяч строк
# мира `holes` не имели рода НИГДЕ — ни в указателе, ни в сайдкаре свода, — хотя род их
# объявлен здесь же таблицей `РОЛИ` и назван на каждой странице вопросным словом.
#
#     ДОМ, НЕ ЗНАЮЩИЙ СВОИХ СТРАНИЦ, НЕ МОЖЕТ НАЗВАТЬ ИХ РОДА, И РОД ЕГО ЖИВЁТ ОБЪЯВЛЕНИЕМ
#     БЕЗ АДРЕСА: он есть в списке и его нет ни у одной строки свода.
#
# ПЕРЕБОР ПЕРЕЕХАЛ СЮДА ЦЕЛИКОМ, А НЕ СКОПИРОВАН: кузница зовёт `роли`, `ЧИСЛА`, `ШИРИНА` и
# `ШИРИНА_ЗАПРОСОВ` отсюда, и потому мир выходит БАЙТ В БАЙТ прежним. ДВА ПЕРЕБОРА ОДНОГО
# ПРОСТРАНСТВА РАЗОЙДУТСЯ НА ПЕРВОЙ ЖЕ ПРАВКЕ — оттого он один.
ЧИСЛА = (5, 3, 8, 2, 7, 4, 9, 6, 12, 10, 11)   # counts ≥ 2: the frame's thing is plural
ШИРИНА = 12            # 5 проходов × 12 — двенадцать показов на (глагол, дыра, язык)
ШИРИНА_ЗАПРОСОВ = 10   # 5 проходов × 10 × 5 ролей — десять на (глагол, роль, язык)

# ПРОЗА СКАЗАЛА, СЛОВАРЬ ПРОМОЛЧАЛ (15.09). Сборка «вторым местом» отказывает турецкому
# первой же строкой тела и объясняет отказ восемью словами в своей же справке: срок там и
# так стои́т не первым, а конец фразы держит глагол. Прибор щербатости, впервые прочтя этот
# дом, назвал клетку пустой — и был прав по-своему: он читает объявление, а не справку.
ОБЪЯВЛЕННЫЕ_ПРОПУСКИ = {"срок вторым местом": frozenset({"tr"})}

РОДЫ = (("факт", "срок вторым местом") + РОЛИ
        + tuple("запрос " + р for р in РОЛИ))


def роли(шаг, i, язык):
    """Роли одного показа — по шагу прохода и месту в нём. Один перебор на дом и кузницу."""
    k = (шаг + i) % len(РАМКИ[язык])
    р = РАМКИ[язык][k]
    место = р["места"][(шаг * 2 + i) % len(р["места"])]
    вещь = р["вещи"][(шаг * 3 + i) % len(р["вещи"])]
    n = ЧИСЛА[(шаг * 7 + i) % len(ЧИСЛА)]
    день = ДНИ[язык][(шаг * 3 + i) % 7]
    имена = ИМЕНА[язык]
...
def _показы():
    """{страница: (язык, род)} — тем же перебором, каким кует кузница."""
    from layer import PASSES                              # noqa: PLC0415
    вон = {}
    for шаг in range(len(PASSES)):
        for язык in ЯЗЫКИ:
            for i in range(ШИРИНА):
                свои = роли(шаг, i, язык)
                ф = факт(язык, *свои)
                вон.setdefault(ф, (язык, "факт"))
                второй = факт_вторым_местом(язык, *свои)
                if второй:
                    вон.setdefault(второй, (язык, "срок вторым местом"))
                for j, (вопрос, ответ) in enumerate(дыры(язык, *свои)):
                    вон.setdefault(f"{ф} {вопрос} {ответ}", (язык, РОЛИ[j]))
            for i in range(ШИРИНА_ЗАПРОСОВ):
                свои = роли(шаг * 3 + 1, i + 5, язык)
                for j, (ф, вопрос) in enumerate(запросы(язык, *свои)):
                    вон.setdefault(f"{ф} {вопрос}", (язык, "запрос " + РОЛИ[j]))
    return вон
