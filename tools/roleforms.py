...
_НАЧАЛА = tuple((язык, тип, я[f"{тип}_воп"].split("{о}")[0])
                for язык, я in ЯЗЫКИ.items() for тип in ТИПЫ)
...
def судить(строка):
    ...
    if с in ПОКАЗЫ:
        return True, True
    for язык, тип, рамка in _НАЧАЛА:
        for д, _ in ЯЗЫКИ[язык][тип]:
            начало = рамка.format(д=д)
