...
#
# ТАБЛИЦА `ЯЗЫКИ` ЗДЕСЬ СМЕШАНА: часть ключей несёт РОД (рамку страницы), часть — списки пар «дело — обстоятельство» («орудие», «место», «вещество»).
...
_РОДЫ_В_ЯЗЫКЕ = ("орудие_утв", "орудие_воп", "место_утв", "место_воп", "вещество_утв", "вещество_воп")
_ИМЯ_РОДА = {"орудие_утв": "орудие действия, утверждение",
              "орудие_воп": "вопрос об орудии",
              "место_утв": "место действия, утверждение",
              "место_воп": "вопрос о месте",
              "вещество_утв": "вещество вещи, утверждение",
              "вещество_воп": "вопрос о веществе"}
assert set(_РОДЫ_В_ЯЗЫКЕ) <= set(next(iter(ЯЗЫКИ.values()))), "род ушёл из таблицы ЯЗЫКИ"
# РОДОВ ТРИ, А НЕ ШЕСТЬ: страница несёт ФАКТ И ТУТ ЖЕ ВОПРОС одной строкой, и «утверждение»
# с «вопросом» суть рамки ВНУТРИ страницы, а не роды страниц. Объявление, деля страницу надвое,
# называло шесть родов там, где мир держит три, и у всех шести стояло «страниц 0».
#
#     ОБЪЯВЛЕНИЕ МЕЛЬЧЕ СТРАНИЦЫ НАЗЫВАЕТ РОДАМИ ЕЁ ЧАСТИ — и ни одна часть не есть страница.
РОДЫ = ТИПЫ
ЗАЧЕМ_РОДА = {т: f"{_ИМЯ_РОДА[т + '_утв']}; тут же — {_ИМЯ_РОДА[т + '_воп']}" for т in ТИПЫ}
...
import re as _re  # noqa: E402 — образцы собираются после рамок


def _образец_ряда(язык, тип, ряд):
    """Образец рамки, где КАЖДОЕ место ответа — своя дыра, а не ссылка на первую.

    Ответ стои́т на странице дважды — в утверждении и в ответе на вопрос. Связать их
    обратной ссылкой было бы короче, но тогда страница с ОДНИМ подменённым вхождением не
    совпадёт с образцом вовсе и получит НЕМОТУ вместо лжи.

        СУД, ОТСТУПАЮЩИЙ ПЕРЕД ПОРЧЕЙ В МОЛЧАНИЕ, ОТДАЁТ ЕЁ ВОРОТАМ. Ворота поймают — они
        не пишут мира с несудимой строкой, — но порча, внесённая в свод ПОСЛЕ ковки, ворот
        уже не встретит, и тогда за неё не отвечает никто.

    Потому дыры читаются порознь, а равенство их — дело вердикта, а не образца.
    """
    куски = [_re.escape(к) for к in _скелет(язык, тип, ряд).split("\x00")]
    узор = куски[0]
    for i, к in enumerate(куски[1:]):
        узор += f"(?P<о{i}>[^.?!]+)" + к
    return _re.compile(узор)


# ЗАКОН ВМЕСТО НАБОРА (21.09). Здесь стоял суд, у которого истины не было вовсе: всякая
# строка рамки звалась ЛОЖЬЮ, а истиной — лишь то, что нашлось в наборе показов.
#
...
#     список.
#
# Ныне дом судит тем самым, что объявляет: ответ на странице обязан быть ответом, который
# объявлен при этом деянии, и обязан стоять одним и тем же в утверждении и в ответе.
_ОБРАЗЦЫ = tuple(
    (_образец_ряда(язык, тип, ряд), ряд[1])
...
def судить(строка):
    с = строка.strip()
    совпал = False
    for образец, ответ in _ОБРАЗЦЫ:
        м = образец.fullmatch(с)
        if not м:
            continue
        совпал = True
        # ВСЕ МЕСТА ОТВЕТА СВЕРЯЮТСЯ С ОБЪЯВЛЕННЫМ, И ТЕМ САМЫМ — ДРУГ С ДРУГОМ
        if all(з == ответ for з in м.groupdict().values()):
            return True, True
