...
# в своде дыру, — чтобы дом не был построен второй раз. Таблицы `ЯЗЫКИ` у этого дома нет вовсе:
# единицы он держит у `tools/units.py`, а рамки — парой БЕЗ ИМЁН, где род задан МЕСТОМ в паре.
# Потому имена написаны здесь, а `assert` держит их при паре: изменится длина — дом упадёт на
# ввозе, а не смолчит.
РОДЫ = ("перевод единиц, утверждение с леджером умножения",
        "вопрос о том же переводе, отвечаемый тем же утверждением")
assert all(len(п) == len(РОДЫ) for п in ФРАЗЫ.values()), "рамок в паре не столько же, сколько родов"

СКОЛЬКО = {"es": ("cuántos", "cuántas"), "it": ("quanti", "quante"), "pt": ("quantos", "quantas")}


# ФОРМЫ ЕДИНИЦЫ НА ЛЮБОМ ИЗ ДЕВЯТИ ЯЗЫКОВ — ОДНОЙ ДВЕРЬЮ (22.09). Дома брали их порознь: у
# `units` русские и английские, здесь — прочие семь, и всякий дом писал свою выборку. Выборка
# эта и есть понятие «формы единицы в языке», и дверь ей — одна.
#
#     ВЫБОР ДВЕРИ ПО ЯЗЫКУ ЕСТЬ ТОЖЕ ПОНЯТИЕ: дом, писавший его сам, писал вторую дверь.
#
# ПИСЬМО НАЗЫВАЕТСЯ ЗОВУЩИМ: у метра «metre» британское, у грамма «gram» американское, и дом
# знает, каким пером пишет он сам (`письмо`), — дверь за него не гадает.
def формы_единицы(язык, имя, письмо="amer"):
    """Счётные формы единицы `имя` на языке `язык` — без рода."""
    if язык == "ru":
        return tuple(units.ФОРМЫ_ВСЕХ[имя][1][:3])
    if язык == "en":
        пары = units.ФОРМЫ_ВСЕХ[имя][0]
        return tuple(пары.get(письмо) or next(iter(пары.values())))
    формы = ЕДИНИЦЫ[язык][имя]
...
def язык_группа(шаг, язык):
    вон = []
    for i, (б, м) in enumerate(ПАРЫ):
        n = ЧИСЛА[(шаг * 5 + i * 3) % len(ЧИСЛА)]
        вон.append(утверждение(язык, б, м, n))
        n2 = ЧИСЛА[(шаг * 7 + i * 5 + 1) % len(ЧИСЛА)]
        вон.append(вопрос(язык, б, м, n2))
    return вон

def _меченая(шаг, язык):
    """[(страница, род)] — перевод единиц утверждением и вопросом."""
    утв, воп = РОДЫ
    вон = []
    for i, (б, м) in enumerate(ПАРЫ):
        n = ЧИСЛА[(шаг * 5 + i * 3) % len(ЧИСЛА)]
        вон.append((утверждение(язык, б, м, n), утв))
        n2 = ЧИСЛА[(шаг * 7 + i * 5 + 1) % len(ЧИСЛА)]
        вон.append((вопрос(язык, б, м, n2), воп))
    return вон
...
def группы(шаг):
    ...
    return [язык_группа(шаг, язык) for язык in ЕДИНИЦЫ]
