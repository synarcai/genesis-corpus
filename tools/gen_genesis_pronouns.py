...
def страница_хвостов(шаг, i):
    ...
    return СРАВНЕНИЕ_АКТОВ, f"{имя} {сд} {a} {by_count(a, вещь)}. {имя} {пр} {b} {by_count(b, вещь)}. how many more {вещь} did {он} {сдq} than {прq}? {a} − {b} = {a - b}."
...
# ЯЗЫКОВОЙ ДОЛГ ДОМА НАЗВАН ЧИСЛОМ И ПРИЧИНОЙ (15.09). Четыре рода из шести звучат только
# по-английски: «состояние до актов», «хвост вопроса» («still have left», «altogether»),
# «другое держание той же пары» (holds, keeps вместо has) и «два акта одного носителя,
# сравненные». Дом весь построен на АНГЛИЙСКОМ местоимении «he/she» при опущенном имени, и
# русская сторона его — не перевод рамки, а другой урок: русский носителя местоимением
# держит иначе, и «он взял ещё столько-то» требует своих глаголов держания и своего хвоста
# вопроса, каких дом не объявляет.
#
#     ДОЛГ, У КОТОРОГО НАЗВАНА ПРИЧИНА, ОТЛИЧИМ ОТ НЕБРЕЖНОСТИ. Перекос дома 3,0× (en 360,
#     ru 120) и стои́т он на этих четырёх родах.
