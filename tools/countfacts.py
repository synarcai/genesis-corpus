...
def страница(вид, язык, форма, i):
    ...
    if форма == "пара":
        # ВТОРАЯ ВЕЩЬ БЕРЁТСЯ ШАГОМ, А ПАРА ОДНОГО ЦВЕТА НЕ ПИШЕТСЯ ВОВСЕ: противопоставление
        # одинаковых цветов не есть противопоставление.
        j = (i + ШАГ_ПАРЫ) % len(в["факты"])
        цвета = list(в["факты"].values())
        if цвета[i] == цвета[j]:
            return None
        п["Ф2"], п["С2"] = _имя(вид, язык, j)[0], _значение(вид, язык, j)
    воп, отв = я[форма]
    return f"{воп.format(**п)} {отв.format(**п)}"



# ПРОТИВОПОСТАВЛЕНИЕ ЦВЕТА — ВТОРАЯ ПОВЕРХНОСТЬ ТОГО ЖЕ ФАКТА (09.09).
#
#     ЦВЕТ, ПОКАЗАННЫЙ ОДИН РАЗ, ЕСТЬ ЦВЕТ НЕПОКАЗАННЫЙ: читателю не с чем его сличить.
#
# Прибор [СЛОВО ОДНАЖДЫ] держал тридцать одно слово из этого дома: зелёный, синий, чёрный,
# красный живут в своде РОВНО РАЗ на язык, ибо каждая вещь показана одной страницей. Белый и
# жёлтый долга не дают — их по две вещи (снег и молоко, солнце и лимон), и это же указывает
# лекарство: НЕ ВТОРАЯ ВЕЩЬ, А ВТОРАЯ ПОВЕРХНОСТЬ.
#
# ПОЧЕМУ ПРОТИВОПОСТАВЛЕНИЕ, А НЕ ПРОВЕРКА. Первым просилось «правда ли, что трава зелёная?» —
# но дом объявляет ОТВЕТНУЮ ФРАЗУ ЦЕЛИКОМ («Gras ist grün»), а немецкое придаточное требует
# глагола в конце («dass Gras grün ist»), и объявленная фраза туда не встанет.
#
#     ФОРМА, ТРЕБУЮЩАЯ ПЕРЕПИСАТЬ ОБЪЯВЛЕННОЕ, ЕСТЬ ФОРМА НЕ ЭТОГО ДОМА.
#
# Противопоставление соединяет две ГЛАВНЫЕ фразы, и порядок слов не меняется ни в одном из
# девяти языков. Пары берутся так, чтобы цвета РАЗНИЛИСЬ: шаг три по кругу восьми даёт восемь
# разноцветных пар, и всякая вещь стои́т и первой, и второй.
ПАРА_ЦВЕТА = {
    "ru": ("какого цвета {Ф} и {Ф2}?", "{С}, а {С2}."),
    "en": ("what colour are {Ф} and {Ф2}?", "{С}, but {С2}."),
    "de": ("welche Farbe haben {Ф} und {Ф2}?", "{С}, aber {С2}."),
    "fr": ("de quelle couleur sont {Ф} et {Ф2} ?", "{С}, mais {С2}."),
    "es": ("¿de qué color son {Ф} y {Ф2}?", "{С}, pero {С2}."),
    "it": ("di che colore sono {Ф} e {Ф2}?", "{С}, ma {С2}."),
    "pt": ("de que cor são {Ф} e {Ф2}?", "{С}, mas {С2}."),
    "nl": ("welke kleur hebben {Ф} en {Ф2}?", "{С}, maar {С2}."),
    "pl": ("jakiego koloru są {Ф} i {Ф2}?", "{С}, a {С2}."),
}
ШАГ_ПАРЫ = 3

# ПАРА ВСТАВЛЯЕТСЯ В ОБЪЯВЛЕНИЕ ЯЗЫКА, А НЕ ЖИВЁТ ОТДЕЛЬНО: сборщик образцов читает формы из
# `я[форма]`, и форма, стоящая в стороне, осталась бы без образца — то есть без суда.
for _яз, _пара in ПАРА_ЦВЕТА.items():
    ВИДЫ["цвет"]["языки"][_яз]["пара"] = _пара
        ...
        for язык, я in в["языки"].items():
            for форма in ФОРМЫ:
                ...
                for i in range(len(в["факты"])):
                    с = страница(вид, язык, форма, i)
                    if с is not None:
                        вон[с] = (язык, f"{вид}_{форма}")
    return вон
...
# родов читает `РОДЫ`, и всё, что кует дом, обязано быть в нём названо.
# РОД СТРАНИЦЫ ЗДЕСЬ СОСТАВЕН — вид и форма вместе: «сколько» о сторонах и «сколько»
# о ногах суть разные роды, ибо разные факты мира их поверяют.
РОДЫ = tuple(sorted({з[1] for з in ПОКАЗЫ.values()}))
...
def _образцы():
    ...
    for вид, в in ВИДЫ.items():
        for язык, я in в["языки"].items():
            ...
            # ВТОРАЯ ПОЛОВИНА ПРОТИВОПОСТАВЛЕНИЯ — те же дыры под своими именами: образец
            # обязан читать обе вещи и оба значения, иначе он прочтёт половину страницы.
            дыры["Ф2"], дыры["С2"] = дыры["Ф"], дыры["С"]
            for форма in ФОРМЫ:
                if форма not in я:
                    continue
                видены, куски = set(), []
...
def судить(строка):
    ...
    for образ, вид, язык, форма in ОБРАЗЦЫ:
        ...
        if г.get("Ф2"):
            # ПРОТИВОПОСТАВЛЕНИЕ СУДИТСЯ ОБЕИМИ ПОЛОВИНАМИ: страница, верная слева и ложная
            # справа, есть ложная страница. Суд, спросивший об одной, судит половину закона.
            имена2 = [_имя(вид, язык, i)[0] for i in range(len(в["факты"]))]
            for имя_к, знач_к in (("Ф", "С"), ("Ф2", "С2")):
                сущ2 = г[имя_к]
                if сущ2 not in имена2:
                    return True, False
                if _значение(вид, язык, имена2.index(сущ2)) != г[знач_к]:
                    return True, False
            return True, True
