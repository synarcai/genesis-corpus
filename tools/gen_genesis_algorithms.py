...
EXPONENTS = [5, 3, 8, 2, 4]
# ЕДИНИЦА В РЯДУ РАЗМЕРОВ: поиск по ОДНОМУ предмету есть честный крайний случай, и
# оговорка «если size > 1, иначе 1» здесь ВЕРНА (двоичный поиск по одному предмету
# делает одно сравнение), в отличие от той же по виду оговорки в доме кибернетики,
# где закон давал ноль. ОДИНАКОВЫЕ С ВИДУ ОГОВОРКИ НЕ ОДИНАКОВЫ ПО ЗАКОНУ.
SIZES = [1, 2, 4, 8, 16, 32, 64, 3, 5, 10, 20, 128, 256, 6, 7, 9, 12, 24, 48,
         100, 1000, 15, 50, 30]
WIDTH = 10
...
def pass_shows(pass_i):
    ...
    for i in range(WIDTH):
        ...
        # РУССКАЯ СТОРОНА ЗДЕСЬ ДЕРЖАЛА ЛИТЕРАЛ, А АНГЛИЙСКАЯ ШЛА ЗАКОНОМ — обратно тому,
        # что найдено в четырёх домах ночью (08.09). Предлог «по» правит дательным («по 1
        # элементу», «по 2 элементам»), оборот «не более» — родительным («не более 1 шага»,
        # «не более 2 шагов»); ни та, ни другая форма не есть счётная, и потому объявлены.
        #
        #     ПОЛОВИНЧАТЫЙ ЗАКОН НЕ ИМЕЕТ ЛЮБИМОГО ЯЗЫКА. Он есть привычка писать литералом
        #     там, где закон уже позван рядом, — и заводится в любой стороне страницы.
        дат = "элементу" if size == 1 else "элементам"
        род_шаг = "шага" if size == 1 else "шагов"
        out.append(
            f"линейный поиск по {size} {дат} требует не более "
            f"{size} {род_шаг}.")
        out.append(
