...
РЕЧЬ = {
    "ru": dict(ряд="{П}, {С}, {В} — три степени одного признака",
               ...
               вывод="значит, {A} {С}, чем {B}", вопрос_ряда="какие три степени у слова «{П}»?", вопрос_обратного="каков тогда {B}?", коротко="{С}, чем {A}", двоеточие=": ", и=", а "),
    "en": dict(ряд="{П}, {С}, {В} — three degrees of one property",
               ...
               вывод="so {A} is {С} than {B}", вопрос_ряда="what are the three degrees of «{П}»?", вопрос_обратного="what about {B} then?", коротко="{С} than {A}", двоеточие=": ", и=", and "),
    "de": dict(ряд="{П}, {С}, {В} — drei Stufen einer Eigenschaft",
               ...
               вывод="also ist {A} {С} als {B}", вопрос_ряда="welche drei Stufen hat «{П}»?", вопрос_обратного="wie ist dann {B}?", коротко="{С} als {A}", двоеточие=": ", и=", und "),
    "fr": dict(ряд="{П}, {С}, {В} — trois degrés d'une même qualité",
               ...
               вывод="donc {A} est {С} que {B}", вопрос_ряда="quels sont les trois degrés de «{П}» ?", вопрос_обратного="et {B} alors ?", коротко="{С} que {A}", двоеточие=" : ", и=", et "),
    "es": dict(ряд="{П}, {С}, {В} — tres grados de una misma cualidad",
               ...
               вывод="así que {A} es {С} que {B}", вопрос_ряда="¿cuáles son los tres grados de «{П}»?", вопрос_обратного="¿y {B} entonces?", коротко="{С} que {A}", двоеточие=": ", и=", y "),
    "it": dict(ряд="{П}, {С}, {В} — tre gradi di una stessa qualità",
               ...
               вывод="quindi {A} è {С} di {B}", вопрос_ряда="quali sono i tre gradi di «{П}»?", вопрос_обратного="e {B} allora?", коротко="{С} di {A}", двоеточие=": ", и=", e "),
    "pt": dict(ряд="{П}, {С}, {В} — três graus de uma mesma qualidade",
               ...
               вывод="portanto {A} é {С} do que {B}", вопрос_ряда="quais são os três graus de «{П}»?", вопрос_обратного="e {B} então?", коротко="{С} do que {A}", двоеточие=": ", и=", e "),
    "nl": dict(ряд="{П}, {С}, {В} — drie trappen van één eigenschap",
               ...
               вывод="dus is {A} {С} dan {B}", вопрос_ряда="welke drie trappen heeft «{П}»?", вопрос_обратного="hoe is {B} dan?", коротко="{С} dan {A}", двоеточие=": ", и=", en "),
    "pl": dict(ряд="{П}, {С}, {В} — trzy stopnie jednej cechy",
               ...
               вывод="więc {A} jest {С} niż {B}", вопрос_ряда="jakie są trzy stopnie słowa «{П}»?", вопрос_обратного="a jaki jest {B}?", коротко="{С} niż {A}", двоеточие=": ", и=", a "),
}
...
def страница(язык, форма, признак, сдвиг=0):
    ...
    if форма == "ряд":
        # РЯД СПРОШЕН, А НЕ ОБЪЯВЛЕН: читатель должен уметь спросить о степенях слова
        return (р["вопрос_ряда"].format(П=П) + " " + р["ряд"].format(П=П, С=С, В=В) + если + ".")
    имена_ = имена(язык)
    ...
    # ОТВЕТ НА ВОПРОС КОРОТОК: спрошенное подлежащее не повторяется, и «значит» после вопроса
    # лишне — вывод остался бы речью без вопроса, а здесь вопрос уже задан.
    return (р["сравнение"].format(A=A, С=С, B=B) + ". "
            + р["вопрос_обратного"].format(B=B) + " "
            + р["коротко"].format(С=Спр, A=A) + ".")
...
def рамка(язык, форма, неправильна):
    ...
    if форма == "ряд":
        return р["вопрос_ряда"] + " " + р["ряд"] + если + "."
    if форма == "цепь":
        первое = р["сравнение"].format(A="{A}", С="{С}", B="{B}")
...
def судить(строка):
    ...
    #
    # Замерено: закон судит все 270 показов дома верно и без набора — выход был
