...
def показы_меченые(языки, шаг, сколько=12):
    """То же, но при каждой строке — язык, на каком она сказана (15.09).

    ЯЗЫК ЗДЕСЬ И НЕ ТЕРЯЛСЯ: показ строится В ЦИКЛЕ ПО ЯЗЫКАМ, и метка берётся у того же
    цикла. Дома, что зовут этот показ, отдают его наружу вместе с родом — и до сего дня
    отдавали без языка, отчего прибор щербатости их не читал.
    """
    return [(с, я) for с, я in zip(показы(языки, шаг, сколько),
                                   _порядок(языки, шаг, сколько))]


def _порядок(языки, шаг, сколько):
    """Языки в том же порядке, в каком `показы` кладёт строки."""
    свои = [я for я in языки if я in ФОРМЫ]
    return [я for _ in range(сколько) for я in свои]
