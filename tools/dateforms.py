...
def рамка(язык, форма, месяц):
    ...
    if форма == "назад":
        return (р["дата"].replace("{D}", "{D2}") + " " + р["назад"] + " " + р["вопрос_назад"]
                # ЛОЖЬ, ПРОЖИТАЯ ДО 11.09: здесь стояло «{L} − {d2} = {r}», и это неверно
                # на КАЖДОЙ странице рода — «в апреле 30 дней, 30 − 7 = 5» при д=25. Длина
                # месяца убывает на день ИСХОДНОЙ даты (d), а не на день даты в следующем
                # месяце (d2); верная запись стои́т этажом ниже, в рамке «сколько дней».
                #
                #     ЛОЖЬ, КОТОРУЮ НЕ ЧИТАЕТ НИ ОДИН СУД, ЖИВЁТ СТОЛЬКО ЖЕ, СКОЛЬКО ДОЛГ,
                #     ЕЁ УКРЫВШИЙ. Сто шестьдесят строк лежали в доле «запятая: равенство
                #     рядом с неравенством», и нашлись в тот час, когда доля была уплачена.
                + " {D}" + р["двоеточие"] + длина + ", {L} − {d} = {r}, {k} − {d2} = {r2}.")
...
def _вердикт(язык, форма, месяц, зн):
    ...
    if форма == "вперёд":
        return int(зн["s"]) == d + k
    if форма == "назад":
        # ТА ЖЕ ЛОЖЬ, ЧТО ИСПРАВЛЕНА В РАМКЕ 11.09, ОСТАВАЛАСЬ ЗДЕСЬ ДО 21.09. В тот день
        # рамка «назад» была исправлена с «{L} − {d2} = {r}» на «{L} − {d} = {r}» — длина
        # месяца убывает на день ИСХОДНОЙ даты, а не на день даты следующего месяца, — и
        # рядом записан закон: ЛОЖЬ, КОТОРУЮ НЕ ЧИТАЕТ НИ ОДИН СУД, ЖИВЁТ СТОЛЬКО ЖЕ,
        # СКОЛЬКО ДОЛГ, ЕЁ УКРЫВШИЙ. Проверка же осталась при `L - d2` — и укрыта была тем
        # же самым долгом: выход по набору стоял впереди закона, и все 160 страниц рода
        # «назад» проходили, ни разу не спрошенные.
        #
        #     ПРАВКА, ВНЕСЁННАЯ В ПИСЬМО И НЕ ВНЕСЁННАЯ В ЧТЕНИЕ, ОСТАВЛЯЕТ ЛОЖЬ РОВНО ТАМ,
        #     ГДЕ ЕЁ ИСКАЛИ И СЧИТАЛИ НАЙДЕННОЙ.
        return int(зн["r"]) == L - d and int(зн["r2"]) == k - d2
...
def _судить_образцом(строка):
    ...
    с = строка.strip()
    # ВЫХОД ПО НАБОРУ СНЯТ (21.09) — он укрывал неверную проверку рода «назад», ту
    # самую ложь, что была найдена в рамке 11.09 и в законе НЕ исправлена. НАБОР
    # ГОВОРИТ, ЧТО ДОМ СТРАНИЦУ НАПИСАЛ, А НЕ ЧТО ОНА ВЕРНА. Замер: все 480 верны.
    ...
    for образ, язык, форма, месяц in ОБРАЗЦЫ:
        ...
        if зн is None:
            continue
        if _вердикт(язык, форма, месяц, зн):
            return True, True
