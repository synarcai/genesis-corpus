...
# дома). Четыре рода этого дома суть один и тот же опыт над собою при разных числах: организм
# предсказывает свой ответ, потом считает и сверяет — сбылось или нет. Числа здесь материал,
# а мысль одна: ПРЕДСКАЗАНИЕ О СЕБЕ НЕ МЕНЯЕТ ФАКТА.
#
# Род «предсказал_незнание» (81 страница на 81 очертание) РОВЕН СОБОЮ: там перебираются САМИ
# СЛУЧАИ незнания — чего именно не сказано в задаче, — и всякий говорит о своём.
#
#     РОД РОВЕН САМ СОБОЮ, КОГДА ЕГО МАТЕРИАЛ МНОГОМЕРЕН.
ОЧЕРТАНИЕ_ПО_ПРИРОДЕ = ("не_сбылось",
                        "ошибка",
                        "мысль_не_меняет_факт",
                        "сбылось",)
РОВНЫ_СОБОЮ = ("предсказал_незнание",)
...
def _судить_образцом(строка):
    ...
    #
    # Замерено: закон судит все 2097 показов дома верно и без набора — выход был
