...
ЯЗЫКИ = {
    ...
    "ru": dict(
        ...
        возраст_воп_x="{Yд} {n} {Л}. сколько лет {X_д}?", возраст_воп_сум="вместе им {N} {Л}. сколько лет {Yд}?",
        ...
        кратное_через="{X_д} {n} {Л}. {Y} {K} старше {X_р}. сколько лет будет {Yд} через {k} {Лk}?",
...
def страница(язык, форма, **п):
    ...
    if форма == "трое":
        ...
        X, X_р, X_д = _лицо(язык, п["X"]); Y, Y_р, Yд = _лицо(язык, п["Y"]); Z, Z_р, Zд = _лицо(язык, п["Z"])
    ...
    if форма in ("больше", "обратно"):
        X, X_р, X_д = _лицо(язык, п["X"]); Y, Y_р, Yд = _лицо(язык, п["Y"])
    ...
    if форма in ("через", "через_обратно", "кратное_через"):
        ...
        if форма == "через":
            ...
            Y, Y_р, Yд = _лицо(язык, п["Y"])
            n, kk = п["n"], п["лет"]
            a = k * n
            r = a + kk
            воп = я["кратное_через"].format(**з, Y=Y, Y_р=Y_р, Yд=Yд, n=n, k=kk, K=я["кратно"][k], Л=_лет(язык, n), Лk=_лет(язык, kk), Лдk=_лет(язык, kk, дат=True))
    ...
    if п["спрос"] == "x":
        n = п["n"]; r = k * n
        воп = я["возраст_воп_x"].format(Y=Y, Yд=Yд, X=X, X_д=X_д, n=n, Л=_лет(язык, n))
        леджер = f"{k} × {n} = {r}."
    else:
        x = п["x"]; N = x * (1 + k); r = x
...
def _дыры(язык):
    ...
    return {"X": _альт(л[0] for л in лица), "Y": _альт(л[0] for л in лица), "X_р": _альт(л[1] for л in лица), "Y_р": _альт(л[1] for л in лица),
            "X_д": _альт(л[2] for л in лица), "Yд": _альт(л[2] for л in лица),
