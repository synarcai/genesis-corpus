...
def _лица(язык):
    ...
    for имя in п.get("person_names", ()):
        ф = формы.get(имя) or формы_guess(формы, имя)
        if язык == "ru":
...
ЯЗЫКИ = {
    "en": dict(
        ...
        # ХВОСТ ВОПРОСА ЕСТЬ ВТОРАЯ ПОВЕРХНОСТЬ ТОГО ЖЕ ВОПРОСА (holon 03.09,
        # строка 208 атаки): миры-читатели спрашивают «does hugo HOLD NOW?», а
        # страницы с леджером спрашивали только «does hugo HAVE?» — и рынок,
        # купивший цепочку под один хвост, не узнавал её под другим. Оба хвоста
        # стоят теперь при одном факте, как знак стоит при слове.
        больше_теперь="{X} has {d} {З} than {K} as many {Т} as {Y}. {Y} has {n} {Т}. how many {Т} does {X} hold now?",
    ...
    "ru": dict(
        ...
        больше="у {X_р} на {d} {Тd} {З}, чем {У} число {Тм} у {Y_р}. у {Y_р} {n} {Тn}. сколько {Тм} у {X_р}?",
        обратно="у {X_р} на {d} {Тd} {З}, чем {У} число {Тм} у {Y_р}. у {X_р} {m} {Тm}. сколько {Тм} у {Y_р}?",
        больше_теперь="у {X_р} на {d} {Тd} {З}, чем {У} число {Тм} у {Y_р}. у {Y_р} {n} {Тn}. сколько {Тм} у {X_р} теперь?",
        обратно_теперь="у {X_р} на {d} {Тd} {З}, чем {У} число {Тм} у {Y_р}. у {X_р} {m} {Тm}. сколько {Тм} у {Y_р} теперь?",
        знаки={"−": "меньше", "+": "больше"},
        возраст="{X} {K} старше {Y_р}.",
        возраст_воп_x="{Y_д} {n} {Л}. сколько лет {X_д}?", возраст_воп_сум="вместе им {N} {Л}. сколько лет {Y_д}?",
        сумма_обр="{М} {x}. {Б} {K} больше, чем {М}. сколько всего {В}?",
        трое="вместе у {X_р}, {Y_р} и {Z_р} {N} {Тn}. у {X_р} {K} больше, чем у {Y_р}. у {Y_р} на {d} {Тd} {З}, чем у {Z_р}. сколько {Тм} у {W_р}?",
        через="{X_д} {n} {Л}. сколько лет будет {X_д} через {k} {Лk}?",
        через_обратно="через {k} {Лk} {X_д} будет {m} {Лm}. сколько лет {X_д} сейчас?",
        кратное_через="{X_д} {n} {Л}. {Y} {K} старше {X_р}. сколько лет будет {Y_д} через {k} {Лk}?",
    ...
    "de": dict(
        ...
        больше_теперь="{X} hat {d} {Т} {З} als {K} so viele wie {Y}. {Y} hat {n} {Т}. wie viele {Т} hat {X} jetzt?",
...
def страница(язык, форма, **п):
    ...
    if форма == "трое":
        ...
        X, X_р, X_д = _лицо(язык, п["X"]); Y, Y_р, Y_д = _лицо(язык, п["Y"]); Z, Z_р, Z_д = _лицо(язык, п["Z"])
        d, знак, i, a = п["d"], п["знак"], п["вещь"], п["a"]
        z = a + d if знак == "−" else a - d
        N = k * a + a + z
        W, W_р = {"X": (X, X_р), "Y": (Y, Y_р), "Z": (Z, Z_р)}[п["спрос"]]
        з = dict(X=X, X_р=X_р, Y=Y, Y_р=Y_р, Z=Z, Z_р=Z_р, W=W, W_р=W_р, N=N, K=я["кратно"][k], d=d, З=я["знаки"][знак],
    ...
    if форма in ("больше", "обратно"):
        X, X_р, X_д = _лицо(язык, п["X"]); Y, Y_р, Y_д = _лицо(язык, п["Y"])
        d, знак, i = п["d"], п["знак"], п["вещь"]
        З = я["знаки"][знак]
        if форма == "больше":
            n = п["n"]; kn = k * n; r = kn - d if знак == "−" else kn + d
            з = dict(X=X, X_р=X_р, Y=Y, Y_р=Y_р, d=d, З=З, K=я["кратно"][k], У=я.get("удвоенное", {}).get(k, ""), n=n,
                     Т=_вещь(язык, i, 2), Тd=_вещь(язык, i, d), Тn=_вещь(язык, i, n), Тм=_вещь(язык, i, 5))
            леджер = f"{k} × {n} = {kn}, {kn} {знак} {d} = {r}."
            ключ = "больше_теперь" if п.get("хвост") == "теперь" else "больше"
            return f"{я[ключ].format(**з)} {леджер} {я['ответ'].format(r=r)}"
        r = п["r"]; kr = k * r; m = kr + d if знак == "+" else kr - d
        обр = "−" if знак == "+" else "+"
        з = dict(X=X, X_р=X_р, Y=Y, Y_р=Y_р, d=d, З=З, K=(я["кратно"][k] + (" the number of" if язык == "en" else "")) if язык != "en" else я["кратно"][k],
        ...
        ключ = "обратно_теперь" if п.get("хвост") == "теперь" else "обратно"
        return f"{я[ключ].format(**з)} {леджер} {я['ответ'].format(r=r)}"
    if форма in ("через", "через_обратно", "кратное_через"):
        ...
        X, X_р, X_д = _лицо(язык, п["X"])
        з = dict(X=X, X_р=X_р, X_д=X_д)
        if форма == "через":
            n, kk = п["n"], п["k"]
            r = n + kk
            ...
            Y, Y_р, Y_д = _лицо(язык, п["Y"])
            n, kk = п["n"], п["лет"]
            a = k * n
            r = a + kk
            воп = я["кратное_через"].format(**з, Y=Y, Y_р=Y_р, Y_д=Y_д, n=n, k=kk, K=я["кратно"][k], Л=_лет(язык, n), Лk=_лет(язык, kk), Лдk=_лет(язык, kk, дат=True))
            леджер = f"{k} × {n} = {a}, {a} + {kk} = {r}."
        return f"{воп} {леджер} {я['ответ'].format(r=r)}"
    # ages
    X, X_р, X_д = _лицо(язык, п["X"]); Y, Y_р, Y_д = _лицо(язык, п["Y"])
    K = я["кратно"][k]
    утв = (я["возраст2"] if п.get("идиома") and "возраст2" in я else я["возраст"]).format(X=X, Y=Y, Y_р=Y_р, K=K)
    if п["спрос"] == "x":
        n = п["n"]; r = k * n
        воп = я["возраст_воп_x"].format(Y=Y, Y_д=Y_д, X=X, X_д=X_д, n=n, Л=_лет(язык, n))
        леджер = f"{k} × {n} = {r}."
    else:
        x = п["x"]; N = x * (1 + k); r = x
        воп = я["возраст_воп_сум"].format(Y=Y, Y_д=Y_д, N=N, Л=_лет(язык, N))
...
def _дыры(язык):
    ...
    ч = r"(\d+)"
    return {"X": _альт(л[0] for л in лица), "Y": _альт(л[0] for л in лица), "X_р": _альт(л[1] for л in лица), "Y_р": _альт(л[1] for л in лица),
            "X_д": _альт(л[2] for л in лица), "Y_д": _альт(л[2] for л in лица),
            "Z": _альт(л[0] for л in лица), "Z_р": _альт(л[1] for л in лица), "W": _альт(л[0] for л in лица), "W_р": _альт(л[1] for л in лица),
...
def _шаблоны(язык):
    ...
    вон = [("сумма", "м", f"{я['сумма_утв']} {я['сумма_воп_м']} {л} {я['ответ']}"), ("сумма", "б", f"{я['сумма_утв']} {я['сумма_воп_б']} {л} {я['ответ']}"),
           ...
           ("больше", "теперь", f"{я['больше_теперь']} {л} {я['ответ']}"), ("обратно", "теперь", f"{я['обратно_теперь']} {л} {я['ответ']}"),
...
def _параметры(язык, форма, вид, з):
    ...
    try:
        ...
        if Y is None:
            return None
        if форма == "трое":
            Z = имя_or(з, язык, "Z"); W = имя_or(з, язык, "W")
        ...
        if форма in ("больше", "обратно"):
            ...
            if k is None or знак is None:
                return None
            if форма == "больше":
                return dict(форма="больше", X=X, Y=Y, k=k, знак=знак, d=int(з["d"]), вещь=вещь, n=int(з["n"]), хвост=вид)
            m, d = int(з["m"]), int(з["d"])
            kr = m - d if знак == "+" else m + d
            if kr % k:
                return None
            return dict(форма="обратно", X=X, Y=Y, k=k, знак=знак, d=d, вещь=вещь, r=kr // k, хвост=вид)
        k = _обратно(я["кратно"], з["K"])
        if k is None:
            return None
...
def имя_or(з, язык, ключ):
    for к in (ключ, ключ + "_р", ключ + "_д"):
        if з.get(к):
...
# СЛОВАРЬ СТРАНИЦ ДОМА ОТНОШЕНИЙ (13.09, третий переезд того же вида). Перебор жил у кузницы,
# и дом не знал ни одной своей страницы: тысяча девятьсот восемьдесят девять строк мира
# `relations_story` не имели рода нигде, хотя род назван ВТОРЫМ ДОВОДОМ в каждом вызове.
#
...
# Переносчик читал имя рода из самого вызова и НЕ ВЫБИРАЛ его: ПЕРЕНОСЧИК, РЕШАЮЩИЙ ЗА ДОМ,
# К КАКОМУ РОДУ ОТНЕСТИ СТРАНИЦУ, ВЫДУМЫВАЕТ ОБЪЯВЛЕНИЕ.
def перебор(шаг, язык):
    ...
    for i in range(24):
        ...
        вон.append((страница(язык, "сумма", пара=(шаг * 5 + i * 7) % 6, k=k, долей=долей, спрос=("м", "б")[(i // 3 + шаг) % 2], x=5 + (шаг * 11 + i * 13 + j) % 56), "сумма"))
        j += 1
    # the whole asked from the small part (e9: «the same relation, another hole»): 12 per pass
    for i in range(12):
        k = 2 + (i + шаг) % 3
        вон.append((страница(язык, "сумма_обр", пара=(шаг * 7 + i * 5 + 2) % 6, k=k, x=6 + (шаг * 13 + i * 11 + j) % 60), "сумма_обр"))
        j += 1
    ...
    for i in range(36):
        ...
        if знак == "+" and база - d < 2:
            база += d
        вон.append((страница(язык, "трое", X=лица[a][0], Y=лица[b][0], Z=лица[c][0], k=k, знак=знак, d=d, вещь=(i + шаг * 3) % 6, a=база, спрос=спрос), "трое"))
        j += 1
    # a difference over a multiple, both signs, and the relation backwards
    # ОБА ХВОСТА ВОПРОСА ПРИ ОДНОМ ФАКТЕ (holon 03.09, строка 208 атаки):
    # миры-читатели спрашивают «does hugo HOLD NOW?», страницы с леджером
    # спрашивали только «does hugo HAVE?» — и рынок, купивший цепочку под один
    # хвост, не узнавал её под другим. Хвост чередуется по шагу случая.
    for форма in ("больше", "обратно"):
        for i in range(12):
            хвост = "теперь" if i % 2 else ""
            ...
            if форма == "больше":
                ...
                if знак == "−" and k * n - d < 2:
                    n += d
                вон.append((страница(язык, "больше", X=лица[a][0], Y=лица[b][0], k=k, знак=знак, d=d, вещь=(i + шаг * 2) % 6, n=n, хвост=хвост), "больше"))
            else:
                r = 3 + (шаг * 5 + i * 7 + j) % 28
                if знак == "−" and k * r - d < 2:
                    r += d
                вон.append((страница(язык, "обратно", X=лица[a][0], Y=лица[b][0], k=k, знак=знак, d=d, вещь=(i + шаг * 2 + 3) % 6, r=r, хвост=хвост), "обратно"))
            j += 1
    ...
    for i in range(24):
        ...
        if i % 2 == 0:
            вон.append((страница(язык, "возраст", X=лица[a][0], Y=лица[b][0], k=k, спрос="x", n=3 + (шаг * 3 + i * 5 + j) % 10, идиома=идиома), "возраст"))
        else:
            вон.append((страница(язык, "возраст", X=лица[a][0], Y=лица[b][0], k=k, спрос="сум", x=4 + (шаг * 5 + i * 3 + j) % 12, идиома=идиома), "возраст"))
        j += 1
    ...
    for i in range(7):
        ...
        n = 4 + (шаг * 7 + i * 5 + j) % 40
        k = 2 + (шаг * 3 + i * 7 + j) % 12
        вон.append((страница(язык, "через", X=X, n=n, k=k), "через"))
        j += 1
    for i in range(7):
        X = лица[(шаг * 7 + i * 5 + 1) % len(лица)][0]
        k = 2 + (шаг * 5 + i * 3 + j) % 12
        m = k + 3 + (шаг * 3 + i * 11 + j) % 40
        вон.append((страница(язык, "через_обратно", X=X, m=m, k=k), "через_обратно"))
        j += 1
    for i in range(7):
        a = (шаг * 3 + i * 7 + 5) % len(лица); b = (a + 2 + (шаг + i) % (len(лица) - 2)) % len(лица)
        n = 3 + (шаг * 5 + i * 3 + j) % 20
        лет = 2 + (шаг * 7 + i * 5 + j) % 10
        вон.append((страница(язык, "кратное_через", X=лица[a][0], Y=лица[b][0],
                              k=2 + (шаг + i) % 3, n=n, лет=лет), "кратное_через"))
        j += 1
    return вон
