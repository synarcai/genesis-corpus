...
# СОСТОЯНИЕ НАЗЫВАЕТ СВОЙ ПОВОД, А ПОВОД — СВОЙ СОВЕТ И СВОЙ ЗАКОН (21.09). Прежде истиной
# дома было одно лишь членство в наборе показов: `return True, с in ПОКАЗЫ`.
#
#     СУД, У КОТОРОГО ИСТИНА ЕСТЬ ОДНО ЛИШЬ ЧЛЕНСТВО В НАБОРЕ, НЕ ЗАКОН, А СПИСОК. Он не
#     умеет сказать, ПОЧЕМУ страница верна, — и потому не заметит подмены, вписанной в
#     список: ни чужого совета при своём состоянии, ни чужого закона при своём совете.
#
# Ныне дом судит тем самым, из чего страницу и сложил: состояние опознаёт повод, повод даёт
# совет (свой, объявленный) и закон (взятый у дома поведения — того же повода). Страница
# истинна, если её хвост есть в точности одна из двух объявленных рамок этого повода.
_ПО_СОСТОЯНИЮ = {}
for _язык, _я in ЯЗЫКИ_ДОМА.items():
    for _род, (_сост, _совет) in enumerate(_я["пары"]):
        _ПО_СОСТОЯНИЮ.setdefault(_сост, []).append((_язык, _род, _совет))


def _хвосты(язык, род, совет):
    """Оба объявленных хвоста повода: ответ с основанием и ответ с отдельным «почему».

    ГОЛОВА ВЫБИРАЕТСЯ ТЕМ ЖЕ ОБЪЯВЛЕНИЕМ, ЧТО И ПРИ ПИСЬМЕ: повод действия спрашивают «что
    мне делать?», повод состояния — «почему?». Первая редакция закона брала одну голову
    на все поводы, и 27 честных страниц — те, где повод не требует совета, — были названы
    ложью. Читающий обязан спросить `ДЕЙСТВИЕ` ровно там же, где спрашивает пишущий.
    """
    я = ЯЗЫКИ_ДОМА[язык]
    закон = B.страница(язык, "закон", род)
    дв = я.get("двоеточие", ": ")
    голова = я["что"] if род in ДЕЙСТВИЕ else я["почему"]
    return (f"{голова} {совет}{дв}{закон}", f"{совет}. {я['почему']} {закон}")
...
def судить(строка):
    ...
    if хвост.count("?") != 1 or хвост.rstrip().count(". ") > 1:
        return False, False
    for язык, род, совет in _ПО_СОСТОЯНИЮ.get(голова, ()):
        if хвост in _хвосты(язык, род, совет):
            return True, True
    return True, False
