...
for _яз, _я in ЯЗЫКИ_ДОМА.items():
    assert len(_я["пары"]) == len(B.ПОВОДЫ), _яз
    assert _яз in B.ЯЗЫКИ, _яз
    assert len(_я["сочувствие"]) == 3, _яз


def страница(язык, род, врозь=False):
    """«я устал. что мне делать? отдохни: когда человек устал, он ошибается чаще.»

    ВТОРАЯ ПОВЕРХНОСТЬ РАЗВОДИТ СОВЕТ И ЕГО ОСНОВАНИЕ (16.09, прибор
    `scripts/word_once.py`: 36 одиночек дома). Дом давал ОДНУ страницу на пару «язык ×
    повод», и всякое слово совета — «apresures», «affrettarti», «beeil» — стояло во всём
    своде ровно раз: девять языков, и на каждом свой глагол.

        РОД ОБ ОДНОМ ЯЗЫКЕ ДАЁТ ОДНУ СТРАНИЦУ НА ЯЗЫК, и всякое слово в ней сказано
        единожды. Долг платится не выдумкой десятого языка, а ВТОРЫМ ДОВОДОМ о том же.

    Врозь: «я устал. отдохни. почему? когда человек устал, он ошибается чаще.» Совет сказан
    ПРЯМО, а основание спрошено отдельно — и обе головы дома («что мне делать?», «почему?»)
    получают по странице на каждый повод, тогда как прежде голова выбиралась родом и вторая
    для этого повода не писалась НИКОГДА.

    Прибавки к объявлению нет ни на одно слово: та же пара, тот же закон, те же две головы.
    """
    я = ЯЗЫКИ_ДОМА[язык]
    ...
    if врозь:
        return f"{состояние}. {совет}. {я['почему']} {закон}"
    голова = я["что"] if род in ДЕЙСТВИЕ else я["почему"]
    return f"{состояние}. {голова} {совет}{двоеточие}{закон}"


def _показы():
    return {страница(язык, род, врозь): (язык, "отклик")
            for язык in ЯЗЫКИ for род in B.ПОВОДЫ for врозь in (False, True)}
...
def судить(строка):
    ...
    if not с:
        return False, False
    голова, точка, хвост = с.partition(". ")
    if not точка or голова not in _ЗАЧИНЫ:
        return False, False
    # ХВОСТ — ОДНА СТРАНИЦА, А НЕ СКОЛЬКО УГОДНО. Проба по одному лишь зачину
    # звала своей всякую строку, начатую объявленным состоянием, — и удвоенный
    # показ «я устал. … я устал. …» тоже (прибор ЛОВУШКИ НАЧАЛА, 40 из 40).
    # Совет с законом есть ОДНА фраза с одним вопросительным знаком; строка, где
    # их два, есть речь длиннее дома, и дому о ней сказать нечего (М-172).
    if хвост.count("?") != 1 or хвост.rstrip().count(". ") > 1:
        return False, False
...
def main():
    ...
    print(f"  дом пишет показов: {len(ПОКАЗЫ)} (языков {len(ЯЗЫКИ)}, поводов {len(B.ПОВОДЫ)}); "
